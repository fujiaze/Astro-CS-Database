...
## F00-12 · 我的「6 倍膨胀」口径订正（由 V5 实测推导触发）
- 我在 F00-08 及 M9-H-1 相关记述里写过「JSON 转义最坏 **6 倍**膨胀」（按标准 JSON：`"`→两字符、控制字符→`\u00XX` 六字符）。
- **V5 逐字读码纠正**：该实现把控制字符降为**单字节 `?` 占位** ⇒ **本实现的最坏膨胀是 2 倍**（仅引号与反斜杠 `need=2`）；且它是**逐字符写入前先判 `(end-p)` 与 `need`**，故**界严密性与膨胀系数无关**，越界结论与修复判定均不受影响。
- **性质**：不是撤条，是**我把"我以为的上界"当成了"代码的上界"**。教训：写边界推导须**先读实现自己的降格策略**再谈系数。此条与簇 10 的"自我否证"同族，计入审计自身可信度控制。