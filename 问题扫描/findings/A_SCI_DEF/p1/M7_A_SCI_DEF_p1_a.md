...
- related L08-001、L20-013（同一主题的枚举值数与保守向，另条）、M7-A-205、A-12；**独立性声明（父层归因更正令）**：本条是「zpf=300 三源三义」（登记册 A-05，后经 F00-08 认定为前台合成、M3 拒绝认领）在本仓唯一可复现的那半，按令**保持独立定稿、不与 M3-C-010（`docs/algorithms/PHOTOMETRIC_FIT.md:9` 声明输出 zero_point 字段而两结构体均无该成员）并档**——二者是同族不同事实，仅互为 related；本条锚一律写 `docs/algorithms/PHASE2_SAMPLER.md`（非 `docs/science/`，全量路径审计见 `_merge/M7.md::§9.1`）。判定 **仍成立**；置信度 中高（表形状按 ALG 文本，未读 MC 测试数据）
