# M2a 定稿 · A_SCI_DEF · P0（来源切片 L04+L10）

> 合并代理: M2a（第二层合并验证）· 域: 球面 Drizzle 重建与不确定度 + Gaia 星表客户端与数据/产物语义合同
> 复核时点: 本轮全部证据按**当前树**重新 read/grep 定位（不信叶子行号）；位置一律 path::符号。叶子引用的 commit 哈希一律不作为事实（本轮禁 git），凡涉及"代码是否已改"，本文件只写**代码现状 vs 文档记载**。
