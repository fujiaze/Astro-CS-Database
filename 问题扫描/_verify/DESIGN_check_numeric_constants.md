...
- 时点与漂移声明（实测）：本稿两次采样间隔约 1.5 小时，HEAD 从 521095b 前进到 d2493d55，**同一文件内漂移不等量**：gaia_client.c 缓存 #define 段 +14（QUERY_CACHE_CAPACITY :113→:127）、WL_COUNT :27→:28、MAX_STARS_RESULT :31→:32、bbox 守卫 :922/:923→:935/:936、AE_C45_FACTOR :956→:969、0.25 剪枝 :2054→:2114（+60）。sdet_api.cpp 与 snr 侧未漂。⇒ 本稿一律按**符号**定位、禁行号锚（此事实即 §4 末条与 S-4② 的直接依据）。
