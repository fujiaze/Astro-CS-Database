...
- **F-11｜收多分片轴的入账必须以「终报的 ID↔事实映射」再对一次账**：V20 的分片消息把 `--resource-detail` 给了 `N-04`、终报改称 `N-06`，我按分片号入账后既正确判掉一次重复、**又误以为 `N-04` 号位仍被占用而漏登终报真正的 `N-04`（`module_build_id` 恒空）**。 ⇒ 号位不等于事实，**逐条按事实比对**。（V20 的锚纪律可作正面模板：两轮机器复核 26/81 漂移→改正后 **88/89 命中**。）
