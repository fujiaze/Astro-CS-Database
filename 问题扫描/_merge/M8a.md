...
- 输入 → 定稿：**29 条源条目 → 28 条承接定稿 + 3 条 M8a 新立 = 31 条**（29 减 1 是因为 L25-001 与 L25-002 依前台指令并为一条三面/四面定稿 M8a-G-001）。
...
## 9. 移交表（跨域项，M8a 不定稿，只 related 指回）

| 事项 | 应由谁主落 | M8a 已提供的证据 | 本域承接条 |
|---|---|---|---|
| 追溯矩阵双头 / 行锚系统性失效 / 无据 VERIFIED 三类总述判词 | **M6b**（前台指定归属：E-001 双头、E-002 行锚、G-001 无据 VERIFIED） | `INDEX.yaml` 键集合与 4 处错锚、两页把不存在的 ARCH_CONTRACTS 标 VERIFIED、registry 26 vs descriptor 22 集合差 | M8a-E-001、E-004、G-006、G-007 全部 related 指回，本域不自写"系统性"判词 |
| 版本号复写簇 / ARCH_CONTRACTS.md 缺失 / CLI 假对象 / AST-API 恒真 / 追溯门 fail-open | **M5b**（G-01/G-02/G-03 为 P0 家族，G-04 版本簇） | 本域 P0 与版本簇事实不同源；ARCH_CONTRACTS 的 README 实例；三家 P0 用作 M8a-G-006/-007 的定级对比基准 | M8a-G-001（仅交叉引用 G-04）、M8a-E-004、M8a-G-006 |
| HEALPix_3.83（GPL-2+ 参考）移植声明与 `THIRD_PARTY_NOTICE.md:19/:22` 互斥 | **本域 M8a-G-001 面③续**（前台第二道令：由 M8a 主落，M2b 承诺不重复定稿；其原 M2b-G-04 与本面并档，汇总以本条为准，勿两计） | 措辞位置经放宽级 grep 订正（在源码注释不在 docs，见 §12-6）；可达性两段（`astrocs_p1_hips_writer` 直编 + `astrocs_aio`） | M8a-G-001、M8a-I-004 |
| 三套退出码互斥总述 | **L12-016 / M5b** | `ASTROCS_MODULE_MISSING` 在 `cli` 无实现对应物（三级复核 §12-5） | M8a-C-001 只落该 README 失实叙述与第二码表固化 |
| 同一算法两套实现（noise ↔ snr_estimator） | **L13-013** | 桥接层薄 README 完全不提生产真源归属 | M8a-I-002 |
| 注释硬编码线程数计票主落权 | **M5a（F00-03 / C-002⑤）** | `lib/astro_image_io/README.md:28` 与 `healpix_drizzle/README.md:58-59` 两个 README 面新实例 | M8a-C-003、M8a-C-004 |
| 编号标准悬空引用（注释/README 主条） | **L14-002** | 21/21 lib manifest + 2 份 conformance manifest 的字段值域挂在仓外标准上 | M8a-I-005（明写不重复计） |
| `entrypoint`/`module_status`/§0 状态词的跨源一致性判据；§8.4 义务范围目录写法；根 LICENSE 落地 | **负责人 / GOV**（宪章改动权 §1.2） | §6.2 的 C-07（已登记）+ R-01/R-02/R-03/R-04（新提）；§5.2 口径；M8a-I-006 的两义举证 | M8a-I-001、M8a-I-006、M8a-C-008、M8a-C-002 |
| 平台相关（oracle 写死 `.exe` 缺省名；msys2_mingw FORBIDDEN 与 tools README 的 PATH 注入相悖） | **M9（平台盲区）** | `rcr_oracle_compare.py:17-20`、`tools/README.md:76` vs `DEPENDENCIES.md:83` | M8a-C-009、M8a-I-003、M8a-F-001 |
| 符合性证据链三断点（前台指定归属 = M8） | **M8** | 依赖/许可/manifest 三面 0 门的实证（§2 R7） | M8a-G-003、-004 作 related 供给 |
| 不可达门 / 原 L21-L22 两轴 | **M7 / M8**（象限锁） | §7 已作废范围整包交回（L21 附录 A 矩阵与待复核 6 项；L22 A1-A12 判定表 + §8 三反证 + 六项正面标杆） | §7 |

---

## 10. 遗留跨域项与提请前台事项（未闭合项）

- **发布阻断面（P0 1 条四面）**：M8a-G-001 需负责人裁三事 —— 派生性质（Siril/HEALPix 两例）、copyleft 履行方式（随包 GPL 文本 + 源码提供承诺 / 兼容授权 / 替换求解器或改链接策略）、根 LICENSE 落地。本代理已把可裁事实分面列齐，未替负责人下法律结论。附带提醒：§17.10 要求 P0/P1=0，而 §17.11「白名单/哈希/版本/provenance 全过」在现状下**不可能为真**（登记面零覆盖 + 交付 NOTICE 为硬编码字面量）。
- **术语缺口（须先裁才能建门）**：`entrypoint` 至少两义、仓内无定义（M8a-I-006）。请前台在派发「manifest ↔ registry 一致性门」实现任务**之前**把该项送负责人裁决，否则新门必然对同一模块给出互斥结论。
- **建议条款 4 项待录入 SUMMARY**：R-01（模块 L2 文档义务与最少要素）、R-02（entrypoint 定义）、R-03（状态词唯一口径）、R-04（交付许可面由登记面派生）；C-07 已登记，本域提供其完整事实基础。
- **转述纠偏（须回收）**：前台转来的「M2b 的 **L15-002 许可证面**」为**源条错挂** —— L15-002 定身是 `hips_pixel_scale` 角秒写入度键（3600 倍，M2b-B-03），许可/派生面实际定稿是 L03-007 → M2b-G-04；且 Healpix_3.83 GPL-2+ 字样在生产源码注释而非 docs（§12-6）。已按第二道令回收为本域主落，汇总时请以 M8a-G-001 面③续引用，勿两计。
- **口径待钉**：目录单元分母 92 沿用 L27、本代理未重算（只做文件级 glob）。若 SUMMARY 要用 46.7% 这个覆盖率，建议先由前台用一次目录枚举钉死「应具文档单元」定义；本域给出的 43（42 标准命名 + 1 异形）只是 README 计数口径，且已确认不含 `问题扫描/**`。
- **本代理越权自检（请前台清掉）**：取证过程中因路径笔误在 `问题扫描/I_DOC_HYGIENE` 落过一个 **0 字节空文件**（非任何定稿）。本代理依"纯只读 + 只写 findings/_merge"的纪律未用 shell 删除，请前台顺手清理。定稿内容不受影响。
- **纪律声明**：全程只用 read / grep / glob（web 仅 3 次用于核对上游许可事实，均附 URL）；未修改任何被扫描文件；未跑构建/测试/git。凡涉运行期行为（GSL 静态或共享解析、zlib 缺失时是否编译失败、cfitsio 树是否漂移）一律标注为静态读入判断或"无法判定"。

---

## 11. 统计

- 输入：L25 12 条 + L27 17 条 = **29 条源条目**
- 定稿：**31 条** = P0 **1** / P1 **14** / P2 **16**
  - P0：M8a-G-001（一条四面，吸收 L25-001 + L25-002）
  - P1：M8a-G-003 ~ G-008（6）、M8a-C-001 ~ C-006（6）、M8a-E-001 ~ E-002（2）
  - P2：M8a-G-009/-010（2）、M8a-C-007 ~ C-009（3）、M8a-E-003/-004（2）、M8a-I-001 ~ I-007（7）、M8a-B-001（1）、M8a-F-001（1）
- 承接关系：29 源条目 → 28 条承接定稿（并为一条少 1 条）+ 3 条 M8a 新立（M8a-C-006 交付 NOTICE/SBOM、M8a-I-006 entrypoint 术语缺口、M8a-F-001 修复未固化）= **31 条**。
- 类别分布：G_GOV_GATE 9（P0 1 / P1 6 / P2 2）· C_DOC_CODE_GAP 9（P1 6 / P2 3）· I_DOC_HYGIENE 7（全 P2）· E_TRACE_BREAK 4（P1 2 / P2 2）· B_STD_MISMATCH 1（P2）· F_TEST_GAP 1（P2）；**A_SCI_DEF / D_COMMENT / H_NUMERIC 本域 0 条**（两轴无该型事实，未凑条）。
- 已修复（不进 findings）：**2 项**（L25-007 的 RCR 版本互斥、L25-009 的锁侧数据）；其中 1 项修复后反向造成文档失真 → 另立 M8a-I-007 + M8a-F-001。
- 断言被否证 / 剔除：**5 项**（L25-007 两条、L25-009 数字两点、L27-013 模板锚、L27-014「34 检查器」）。
- 降级：**2 条**（L25-004 → P2、L25-007 → P2）；不升 P0 的裁决：**2 条**（L27-002、L27-003，各附实质差别 + 条件升级条款）。
- 移交：**11 项**（§9），全部 related 指回、不重复登记。
- 复算台账 15 项（§2）+ 三级复核记录 12 项（§12）；其中外部 URL 核实 3 处，"无法判定" 2 处（受禁 git / 禁执行约束）。
- 定稿文件 10 份：`findings/G_GOV_GATE/p0|M8a_L25_L27.md`、`G_GOV_GATE/p1`、`G_GOV_GATE/p2`、`C_DOC_CODE_GAP/p1`、`C_DOC_CODE_GAP/p2`、`E_TRACE_BREAK/p1`、`E_TRACE_BREAK/p2`、`I_DOC_HYGIENE/p2`、`B_STD_MISMATCH/p2`、`F_TEST_GAP/p2`。
