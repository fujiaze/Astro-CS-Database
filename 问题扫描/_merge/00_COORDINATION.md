...
## 收档 M9（L24+L26 → 26 定稿：P0:4/P1:18/P2:4）—— 四条反证与一处前台遗漏
- **M9 纠正了三处数字口径**：①`ci/checks.json` 检查项 = **113**（三口径互证：python 解析顶层 checks 数组长度 113、全文件 "id" 串 113 且无嵌套、
  裸 `"id":` 行 113）→ **M5b/M8 的 112 与叶子的 111 都不成立**，我先前实测的 113 与之一致；②`windows-main` 声明 **68 项**（同口径）；
  ③gaia_cfg_parse 调用点实为 **4 处**（叶子写 3）、`aio_hips_read_snr_catalog` 调用方实为**六处**且叶子写的 `src/stage2.cpp` 路径不存在。
  → **全层统一按 113/68 订正**，并再次印证「裸数字必须带口径」通则。
- **撤回一条会被误升 P0 的声称（L26-005「CROTA2 符号相反」）**：M9 展开 **vendored cfitsio** `wcsutil.c:47-58` 得 CD1_1=·cos / CD1_2=−·sin /
  CD2_1=+·sin / CD2_2=·cos，与本仓 `fits_reader.cpp:365-368`、`hp_drizzle_api.cpp:566-571` **逐元一致**；`wcssub.c:250-254` 的两个 atan2 式对本仓构造**同角闭合**
  ⇒ 子主张撤回、**作废其"核实即升 P0"钩子**（定 P1 封顶），残余可自证事实只剩"双份复制 + 口径未固定 + 零 CROTA2≠0 测试 + 新站点 module_entry.cpp:711"。
  **这是本轮最有价值的撤条之一**：它挡掉的是一个会被写进发布阻断清单的假 P0。
- **加重了一条真 P0（M9-H-1）**：越界长度会被**回读进产品**（`:808 total=(hw−head)` → `:857 memcpy(big,head,hw−head)`），
  而 `:816-817` 作者自己的注释写明「严禁作为 total 长度源直接拷贝」**恰被该行违反**；且触发点前移到两阶段协议的**第一次调用**（:830-841 仅报尺寸分支也跑完整拼装）。
- **API_CONTRACTS.csv 的缺陷形态被订正**：units 列**确实存在**（叶子写"无该字段"不确），真相是 **102/102 数据行填同一句样板串 "ADU/pixel/deg per header"**
  （:124 与 :125 逐单元格相同）⇒ 等价于未声明，且**这是可直接机器断言的门**（同一列去重后唯一值即告警）。
- **M9 还纠正了 M7 的一条括注**：M7-A-112 写「300 档因子大于 600 档」与 `sampler.cpp:92-93` 表值方向相反（300″ 行 {1.2112,1.3925,1.4980} < 600″ 行 {2.3958,2.8971,3.2035}）；
  "反保守"结论不受影响，但括注会让负责人读反 ⇒ 我在 SUMMARY 与 40 文件按表值方向改写。
- **前台遗漏（我的错，不是代理的）**：L24 的 013/014/015 三条我**没代录**（分片 6 到达后被 M1a/L26/M5a 等事件打断），M9 如实指出"_cache/L24.md 自述 12 条、缺的正是那 2 条 P2"，
  并且**没有臆造补齐**、只按 related 指针 + 自己的独立取证重建了 L24-013。三条现已由我从分片 6 原文补录进档案并标注重建来源。
  → 教训入制度：**代录是异步流水，必须有"收齐判定"机制**；今后凡分片交付的轴，我在收到"收工片"后**立即跑一次条目数对账**（正文小节数 vs 头部计数 vs 代理自报数），三者不一致当场补。
- M9 的 R7–R14（需执行权限八项）与「三组应同批处置」清单（同一 IO-001 提交点三条 / 标定域与在册断言两条 / 写读同域自洽需配异源对拍）已并入 40 文件 B 组。