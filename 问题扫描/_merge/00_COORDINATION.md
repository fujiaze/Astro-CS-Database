...
## E4 收档（四项全交，边界自证最干净的一份：未跑任何形态 ctest、未跑 ci/run.py、未连远端）
- **B-08 三选一裁决 = 「没声明」**（"声明在但生成链未接"这一形态**不存在**：既无声明也无载体）。全仓真实源码 `longPathAware`/`activeCodePage`/`GetLongPathName` **0 命中**、`.manifest/.rc/.res` **0 文件**、`CMakeLists:591 add_executable(astrocs` 无 manifest/LINK_FLAGS（MSVC 只有 `/utf-8`、`/experimental:c11atomics`）、`make_windows_release.py` 无清单步骤、`ci_windows_driver::build_candidate` 无注入点、`install-tree.contract.json` 16 单元**无 `.exe.manifest`** ⇒ **要声明得先扩 BLD-003 白名单**（成本可静态定）。python3 扫 1634 源文件：`\?` 前缀 0、`MAX_PATH` 14（全不用于写出缓冲）。
- **由此牵出我一条方向性改判（已追加到 M9-G-4）**：`tmp==target` 阈值 = `strlen>=511`，而 Linux `PATH_MAX=4096`/`NAME_MAX=255` ⇒ **511~4095 路径在 Linux 合法且真能走到塌缩**；**未声明长路径的 Windows 反而在 >260 更早失败**。⇒「主要在 Windows 正式平台必现」**订正为「Linux 亦可复现，此半不必等实测」**。E4 还实测**双缺**：无任何路径长度用例；7 处 `write_fits` 全 `overwrite=1`（`:167` 硬编码）却无一例"目标已存在再写一次"；`fits_core_selftest.c` 唯一 `overwrite=1`(:464) 随后 `abort`(:472) **从不走 rename**，且**不属任何构建目标**。P1 维持。
- **B-05：必须 Windows 实测的半边从 3 条收敛为 2 条**，因为第 3 问已被**仓内既有真机日志**判定：`run/local/agent_ci_fix/windows_artifact.zip::run/ci/win-stage-test.log`（sha `87bb7b7f55a5`、`Test project D:/a/...`）里 **`18/57 Test #18: io_adapter … Passed`、`100% tests passed, 0 failed out of 57`**，被测用例正是 `io_adapter_test.cpp:204-206`「覆盖已有文件仍原子」（同一 `p` 连写两次、实现走 `fs::rename`），且 `git merge-base --is-ancestor` 退出码 0 证其属现行链。⇒ **MSVC STL 的 `fs::rename` 对已存在目标会替换 → `module_adapters.cpp:1091-1100` 的先删兜底在 MSVC STL 面是死代码 → S3-006「双灭窗口」的触发前提被削弱**。**不必实测**的那半边也定了：同仓已有两套正确写法（`hiss_stream_writer.cpp:171-200`+`:645` 明文禁止先删后 rename 并给 `MoveFileExW|WRITE_THROUGH`）⇒ 修复成本不受阻。**只剩 2 条待 Fatduck**：①裸 CRT `rename()` 对已存在目标；②`MoveFileExA(目录→已存在空目录, REPLACE_EXISTING)` 的错误码。
- 那张 10 站点表本身值得单独看一眼（同一语义七种写法）：`fits_core.c:1480`/`p3_output.cpp:354` 裸 `rename()`；`io_adapter.cpp:124-130` 注释称 STL 带 REPLACE_EXISTING，而**同模块头 `io_adapter.h:18-19` 却说禁止直接覆盖生产文件**（头源互斥）；`orchestrator/cpp/src/checkpoint.cpp:507-508` vs `:524` 自相矛盾（**且该份未编译**，构建里只有 `lib/core/src/checkpoint.cpp`）；全仓 `ERROR_NOT_SAME_DEVICE` **仅 1 处**（`aio_publish.cpp:364-367`），其余错误码全折叠进 STATE。
- **③ windows-main 算术（注册表 113→114）**：`total=114`、`windows-main=68`（platform any 64 + windows 4；waivable 3）、`linux-main=104`、`fast=63`、`linux-deep=7`。**`linux-main − windows-main = 40 项`**（含 `UT-CLI`、`UT-IO`、`UT-ABI`、`CTEST-LINUX-FULL` + 24 个 `CTEST-*`）⇒ **CLI/IO 合同族对 Windows 根本没声明，不只是恒零**；反向只有 4 项（WIN-BUILD/TEST/PACKAGE/CANDIDATE-VALIDATE）真读产物。静默族 3 项/4 站点：`API-DOCS`、`DUPLICATION`(`check_duplication.py:24,29` 无 else)、`ACR-DORMANT`(`check_legacy_exit.py:19,67,60` 无 else)。**锚点订正**：派发书里写的 `tools/quality/check_api_docs.py` 不存在，真身 `tools/check_api_docs.py:132-133`（行号对，目录错）。有效断言三档：项级 **63**、方法级 **716/728**（12 个被门控：monitoring 3× procfs、runtime 9× g++）、**产物级 4 声明 → 实测有效 0**。
- **④ UTF-8 合同三态精确化**：出向实现者**只存在于 `lib/astro_image_io/src/` 私有目录**（`aio_fopen_utf8` 消费者 `aio_fits.cpp:514,788,945,1090` 等 + 8 处平行私有转换器），而 `runtime/io/**` 与 `lib/hips/**` 的 `MultiByteToWideChar` **0 命中**；并发现**方向相反的适配器已在仓**：`cli/main.cpp:52 wmain` + `cli_common.h:44-53 utf8_from_wide_*` + ctest `cli_wideconv` ⇒ **入向有实现、出向缺**。ACP 误解释**已由 commit `da0cf578` 实证过一次**（真因是运行期窄字符 `CreateFileA` 按 ACP 解释 UTF-8 路径字节，`/utf-8` 早已全局存在）⇒ 不必再测。新登记：`runtime/io/hips_core.c` **现树不属任何构建目标** ⇒ 其 `:122/:168/:415` 属**不可达站点**（B-10 数据点）。
### 我对 E4 的一条移交做了定档：FD-F-003 = **P0**（本轮第 82 条 P0）
- E4 明确「不自开条目」并点名移交，我承接定档：**唯一跑 Windows C++ 单测的 `WIN-TEST-UNIT` 最近 10 次真机连续 PASS，而 stage 日志是 `ctest --preset win-rel` → `No tests were found!!!`、junit `tests="0"`、`<testcase>` 计数 0** ⇒ **Windows C++ 单测自 2026-09-07（57 用例全绿）之后一次都没跑过，账面全绿**。根因 `ci_windows_driver.py::stage_plan:344-356` 无"用例数>0"断言，而 `deep_ci_driver.py:248` 持相反说法（与真机 exit 0 冲突）。
- 两条同族制度化空转一并入账：`API-DOCS` PASS 且 **stdout+stderr 双空**（被 `ci/run.py:103-105 EMPTY_OUTPUT_SILENCE_EXEMPT` **写死为豁免**）；`WARNING-SUPPRESSION` 真机自宣 `QA-001_SKIP: 无可用构建树` **仍 PASS**。
- 我复验 `find run -name CI_RESULT.json -newermt 2026-09-01` = **169 份** ⇒ 留痕量充足、非孤例。**M9 §4 平台盲区是"没跑"，本条是它的记账面："显示跑过"——比没跑更糟。** 已开 **A-38** 三问请你拍。