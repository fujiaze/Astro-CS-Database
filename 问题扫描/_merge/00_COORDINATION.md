...
## M8 回执：又推翻我一处「已复验」（F00-02 部分撤回，第三次同类）+ 它自报一次写坏档案的事故
- 我给 M8 的订正行号（`io_ownership_test.cpp` :64/:77/:79 与 `:23/:27/:75`）**全部不成立**：该文件**只有 32 行**，
  `aio_write_bytes` 在真源 0 命中。M8 **没有采信**，自己 grep 出实况并指认我疑似与 `aio_abi_selfcheck.cpp:64/:69` 混线
  （那里是 `if (rc != 0 || cs.failures != 0) return 1;`，**正确的正对照**，它已升为合格线标杆 9）。
- M8-F-002 定档**不变且更强**：`::failures`（:9/:14）**从不参与退出码**、`:31` 无条件 `return 0` ⇒ 门能力为零。
- 已写入 **F00-02 部分撤回**，并加三条制度：①凡标「已复验」必须留当场命令与输出片段，否则降为线索；
  ②给下属的订正消息不得含未当场读取的行号；③下属发现前台锚点不成立时按 M8 做法「记不采 + 继续用自己的实测」，**不得因"前台已复验"放弃复核**。
- M8 自报：写台账时误用 `limit:1` 局部读取整文件写回，`_merge/M8.md` 一度截断为 160 行 → 已逐节重建（244 行）+ 自检定义=引用，
  并写下禁令「禁局部 read → 全量 write，一律整读整写或用 edit」。**这条与我自己的三次锚错一起进 SUMMARY 簇 10。**
- 两处口径请后续注意（M8 已标）：`ci/tests` 实测 **23** 个 py 文件（M5b-F-01 写 2/21）、`checks.json` 的 `"id"` M8 实测 **112**、
  M9 三口径互证 **113** —— 这是并发写入导致的漂移，**R 层一律写「审计当日实测 N（已漂移）」不判对错**；全局合计须由**各域定稿数相加**，任何域的「合计 26」都只是本域输出计数。