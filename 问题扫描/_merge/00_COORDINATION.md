...
## 归因更正（重要）：A-05「zpf=300」是**前台合成产物**，不是 M3 移交
- M3 用机器口径拒绝归属并给出可回溯证据，我已采信并写入 `_cache/F00_FRONT_SPOTCHECKS.md` 的 **F00-08**（公开记录，不问责、只改制度）。
- 与 F00-07a 的区别要说清：F00-07a 是**两条真事实被合并成一个不存在的锚**；A-05 是**整条无源**（连目录层都凭印象写成 `docs/science/PHASE2_SAMPLER.md`）。
- 三条真正成立的事实归位：**M3-C-010**（`PHOTOMETRIC_FIT.md:9` 声明 zero_point 为输出，而 `snr_estimator.h`/`photometric_calib.h` 两个结构体都无该字段，
  零点仅以未导出局部 `location` 存在：`star_matcher.cpp:479` 初值、`:528` `scale=pow(10.0,-location)`）；**M7-A-112**（`PHASE2_SAMPLER.md:189-193` 的
  300″/600″ 冻结表与 clamp `[0.5,1.0]×[300,600]`，"scale 未知→300 档"被称保守但值更小 ⇒ 反保守）；两者**不同事实，不并档**。
- **制度补丁三条**：①移交件必须带「路径 + 行 + 该句前 20 字符」的原句引用，无原句不进裁决表；②裁决表每条注明可回溯出处，前台自造标「前台合成」；
  ③负责人看到的每个待裁决项都要有第三方复核记录（本例三方一致才撤）。M3 亦承诺今后不再发出不能回溯到本域正文的移交项。