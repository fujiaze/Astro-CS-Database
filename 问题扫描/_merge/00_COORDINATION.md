...
## L28e 首批（三条 P1 + 八级阶梯）：它推翻 L28b 的分母，也纠正了我引用 79.9% 的方式
- **分母 563 不可复现**：L28e 试四种正则变体得 526/528/647/526，语料增删对账（加 .txt/.json = 0 变化、加 contracts/ = 0 变化、加根治理 8 文档 = +27 → 553）
  ⇒ **本域分母定稿 526**。注意它的做法是对的：**没有宣布 L28b 报错**，而是"在其自述正则下不可复现"+ 同口径重算（71.3%）；**L28b 原文不撤、只加口径注**（79.9% 在"只算生产注释"口径下仍是真值）。
- **八级阶梯已入 SUMMARY（簇 4 补二）**：79.9 → 71.3 → 68.1 → **66.3（主口径，注释+配置）** → 64.3 → 31.2 → 28.9 → **19.2%（真源任意字面量且扣范围锚后的死锚，101 枚）**。
  **采纳它两条判词**：① 第 5 级（+模块 README/module.yaml，跳到 31.2%）**不得当作"已引用"** —— README 与登记表同属登记面，**互指不构成代码侧溯源**；
  ② 汇报固定给两个数并带口径号（66.3% 与 19.2%）。⇒ 我此前说"79.9% 是下界"**方向对但幅度小**：补 tests 只净新增 17 枚（3.2%）、补配置共 26 枚。
- **新机制（L28e-E-001，我升为"无据 VERIFIED"的第七种）**：16 枚 TEST ID 标 VERIFIED 而字面量全零命中，因为 ID 是
  `tools/quality/v19r3_traceability.py:294/:311` **用 f-string 铸造出来的** ⇒ **注册表把生成器产出的字符串当作"测试已验证"的证据，任何引用检查必然通过，因为字符串本来就是它印的**。
  此前六例都是"引错对象"或"共源"，这例是"自造 ID 再自证"；总述归属改为 **M6b-G-001（锚指错）+ L28e-E-001（ID 铸造）并列，两机制不可互替**。附带 `TEST-UPMW-005` 的 `test_ids` 列填的是诊断号 `UPMW-005`，同表 `TEST-SNR-001` 倒是有头锚。
- **L28e-E-002 给出可直接写门的判词**：`DATA_ARTIFACTS.md` §1 表 15 枚合同 ID 真源**零承载**，同表 RAW/CAL 有 4/7 处、HIPS/TILE 有 15/5 处，
  且该文件 `:42-48` 自证"五处代码侧举证"纪律 ⇒ **纪律写在同一张表上方、行是空的**；唯一消费者 `check_data_artifacts.py`（`checks.json:332` 在册）
  只验表内唯一性/格式/列完整**不验承载 ⇒ 空挂行恒 PASS**。→ 并入 **C-06**：§12.3-9 之外必须加**"承载存在性"**（引用存在 ≠ 真承载该语义）。
- **四分类把"不该被引用"显式扣掉**（C 类 16 枚全排除、治理/评审/审计面 19 枚单列）⇒ 349 枚主口径孤悬里**缺陷候选只有 74 枚**（A5/B24/D28/E17），**没有虚报规模**，这正是我派发时要求的。
- 我给它的裁定：①采纳阶梯与两条判词并写进协议；②E-001 机制单列判词并改总述归属；③E-002 根因原样保留并入 C-06；④A/E 类不立条只 §3 移交**处理正确**，
  但 `ci/**+tools/**+scripts/**` 对 DISP- 字面量 0 命中、唯一消费者只验格式与闭包这句**留在 §3 别删**，R 层要拿它当"门只查词法不查语义"的实证；
  ⑤预告它下一步的 D-001（范围锚代逐 ID 锚）我预期最像 P0，但要求它按 M8a 三准则自检 —— **尤其第 2 条：§12.3-9 与 COMMENT_STANDARD 对"范围锚"到底有没有禁止性文字；**
  **若无，就只能立"标准与实践背离"，不能立违规**（这是我给过 M8a、现在同样用在自己派下的轴上，不偏袒）。