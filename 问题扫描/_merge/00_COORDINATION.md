...
## 收档 M3（43 定稿：P0:7/P1:26/P2:10）与 M6a（31 定稿：P0:2/P1:15/P2:14）—— 第一波十个域**全部合并完毕**
- **机械汇总（findings/**，按 finding 标题计数）＝ 333 条：P0 68 / P1 189 / P2 76**，域分布：
  M1a 44 · M2a 46 · M2b 29 · M3a 43 · M3b 22 · M4 31 · M5a 22 · M5b 43 · M6a 31 · M6b 22。INDEX.md 已按类别×优先级出矩阵。
- M3 与 M6a 是**执行两条方法论警示最规范的两个域**，其做法已作为范例固化进协议附则：
  ①先用正则检出证据面的 ≥1990 字符长行（M3 实测全证据面**只有 1 处** = `cli/CMakeLists.txt:201`，2043 字符），
  对其只用 grep 存在性判据；②结构化比对一律「键集合 + 逐列 grep + 计数」并**写明口径**（M3 用 16 列表头键集合重算 TRACEABILITY.csv：
  `^SCI-NOISE-0` 15 行、第 9/10/14 列 15/15 一致、SCI-CW 命中 0 行）；③他人转述数字（含我给的两处）就地降为线索。
- M3 **撤销叶子一条判据**（「Python oracle 从不运行」不实——UT-BACKEND 确实执行且 waivable:false；成立的是「§11/§15 承诺的
  NumPy rtol 1e-9 复算在仓库内没有实现」），M6a **主动修正自己两处过度断言**（DISP ID 的存在层；photometry_apply 编译归属
  ——复算后改为「仅 `tests/unit/CMakeLists.txt:744-755` 编入测试目标，生产库与根图 0 命中」）。**自我修正率已成为质量指标。**
- M6a 的两条 P0：`CON-COMMENTS` 注释门空壳（7 子事实全成立，另补 2 条：JUnit 只写一条名为 hygiene 的 testcase；
  被弃用的 `num_threads(16)` 正则本可命中 F00-03 的注释，但 `cli/` 不在扫描面）；`order_sel` 冻结语义**不参与执行**
  （会话与 IR 两通道全部后继只有 provenance/metadata，读路径恒按 `hips_order`，SCI §9a-5 的 `leaf_nside=2^(order_sel+9)` 未实现）
  → 后者是「**控制参数被计算被记录但不参与执行**」族的第 6 个成员，已提请 A-20 裁决。
- M6a 三要素抽查（20 个公共头）：算法来源缺 15%、输入来源缺 10%、**被消费者缺 55%**，且**抽样内 SCI/ALG ID 零悬空、
  失效集中在非 ID 锚**（归档标准/控制包章节/上游已删 C 源/截断哈希/已删 third_party 副本/ALG ID 交叉误标）
  → 这是 L28（注释溯源轴）的**先行量化基线**；若 L28 再不交档，SUMMARY 仍可用 M6a 数字，但覆盖面分母要如实标为抽样。
- 纪律偏差公开登记：M3 自报曾对自身产出用一次 bash 统计字符数（未碰真源、无构建/git），已记 D-01；无掩盖、不因此撤证据。