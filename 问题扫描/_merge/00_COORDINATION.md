...
## 前台合规自纠（读到 AGENTS.md 权威文本后必记两条）
### ① 我新建的 `问题扫描/` 是**未登记的根目录条目**，直接违反 AGENTS.md「目录规范（强制）」
- 条文原文：「仓库根目录只允许下述固定条目。**任何新产物必须落位到对应目录，禁止散落根目录**；
  **确需新增根目录条目，必须先在本节登记并获得项目负责人确认**。」
- 事实：负责人本次指令是「在根目录创建一个文件夹」→ 建 `问题扫描/` 有直接授权，但**"在本节登记"这一步我没做，也没请你确认**，
  而是把它当成待办 C-09 挂着。**这是我的程序性违规**，且正是本审计在报告的那一类（治理漂移 / 私造根条目）。
- 与已定稿条目的关系：M6a 报的「私造 logger 写源码树」（G_GOV_GATE/p1）与我这次是同族；**本审计不自外于自己的判据**。
- 处置选项（请你择一，我已写进 40 文件 C-09）：
  A. 授权我把 `问题扫描/` 登记进 AGENTS.md 目录规范为**常驻审计域**（与 reports/、engineering/ 并列，并写明只读纪律与免报区归属）；
  B. 扫描 + R 层结束后整体归位到 `engineering/audit-2026/` 或 `reports/`，并在 commit/memory.md 注明来源与去向（条文要求的处理方式）；
  C. 保持现状直到终报，由你决定归档时机（**当前默认，我不再自行扩面**）。
- 注：`问题扫描/**` 已在协议 §0.4 免报区与锚点核验器排除表内，且 M6b 已确立「我方自写文件不计入真源统计」，故**不会污染任何覆盖率数字**。

### ② AGENTS.md 权威文本**证实并细化了 M6b-G-002（P0）**
- AGENTS.md「状态机」要素自述：`REVIEW_PENDING` 系 V5 遗留状态，「当前台账验证器（`tools/quality/validate_task_ledger.py`）
  **已将其判为非法状态**，本条仅作历史映射、不得据此新增状态」。
- 而 M6b 实测：`tools/check_agents_gov.py:15` 却**强制 AGENTS.md 保留 REVIEW_PENDING 字样**（作为该要素的匹配证据），
  同时 `validate_task_ledger.py:42-43` 判该状态非法、**且该验证器未登记为 CI 检查项**。
- ⇒ 三方矛盾闭合：**治理门要求文档保留一个字面量，而另一个校验器宣布该字面量所代表的状态非法**；
  两者优先级无条文裁定（宪章 §14.5 的状态机口径与 AGENTS.md 映射表未同步）。**这条比我原先转述的「四份 RELEASE_STATUS 并存」更硬**，
  因为它是**同一份权威文件内部**的自相矛盾，且被机器门主动维护。
- 已作为 A-30 并入 40 文件（裁决点：以 §14.5 状态机为准删字面量，还是以 AGENTS.md 历史映射为准放行，并把 validate_task_ledger 注册进 CI）。