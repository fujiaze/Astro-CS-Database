...
## 收档 M2a（46 定稿：P0:6/P1:24/P2:16）—— 两处「比叶子更准」的重写，全层照此口径
1. **L04-001 定性整条重写**：不是「总是 FP32」，而是「**累加域由输入块 dtype 决定、precision_mode 完全不参与**」。
   请求 FP64 时 `p1_op_drizzle` 恒以 FLOAT32 提交（:2303）却写 PRECISION=请求值（:2346）、signal_dtype=请求值（engine:2022）、
   事后加宽（:2099）⇒ **binary32 累加 + FP64 声明**；只有 f64 manifest 的 DLL 通道能拿真 FP64（**通道级不对称**）。
   叶子原案「缺省静默降 FP32」半边**已被并发提交修掉**（缺失即拒 + p1001/inprocess 钉住）→ 归「已修复」不重复立条。
   → 教训：**「参数不参与执行」与「参数被忽略」是两种缺陷**，前者还要看是哪条通道。
2. **L04-002 升为「冻结合同自身缺陷」类**：复算 `S_p = B0/pf²` **与网格/nside/覆盖率/帧数无关**，pf=1 恰为 1（所以在册测试全隐形），
   生产默认 0.8 → 1.5625×，0.5 → 4×。故 §5+§7:82 与 §7:84、§11:117 **三者不可能同时为真** → 须负责人裁决归一分母。
3. **M2a-F-1 是第二个「全量共源」硬实例（P0）**：独立复算 **9003 例**，nside 64/128 分支 4563 例（≈50.7%）以生产
   `query_candidate_pixels` ∩ 生产 `compute_overlap_area_g` 充当真集合，**余下 4440 例真值集同样调用生产函数**
   ⇒ 全 9003 例均非 §13.1 要求的独立 Oracle；注册表 :162 仍以其作 CONFORMANT 证据且偏差列记「无」。
   → 「独立 Oracle 缺位」主题升至第 **六** 实例（前五：L02-001、L04-004、L07-006、L08-004→M4-F-02、L03-006+L15-013）。
4. **M2a-B-2 新模式（已转 M7 横向核）**：`D.catalog:186` 条款面含 J2016.0 历元维，`:193` 判定式**丢掉该维**仍打
   CONFORMANT/偏差「无」⇒「**注册表判定式 ⊂ 自身条款面**」。若多域同型，是可泛化的机器门判据。
5. 我方改判令回执：R-1 作废锚在 M2a 产出内 grep **0 命中**（并注明 512 截断站点归 L24-011）；R-2 已整条重写 L10-008
   并逐字引 `mode_negative:660-699`，补两证（header_len 全文件仅 :1036/:1037 读出即弃；fixture 只写正确值 ⇒ 字段撒谎族零覆盖）；
   R-3 与 L24 并档。**M2a 剔 `DATA_ARTIFACTS.md:150-152` 锚（现 103 行）与我的锚点核验报告 §二完全一致 → 工具已生效。**

## 前台待办（M2a 移交表登记）
- M2a-G-1（`check_standards_registry.py` 未进 ci/checks.json）与 L15-019、L23-004 并成
  「**符合性证据链三处断点**」：①判定器不在册 ②判定式丢维度 ③证据路径只验存在不验执行 → 由 M8 汇总主题，条目各自保留。
- pixfrac 双默认值单一事实源 → M5 域（M5a/M5b 均已收，前台并头）。
- 恒零天测列 / variance 单位维度 / support 钳制对 Phase2-3 消费方影响 → M3a/M4（M4 已定稿，前台在 SUMMARY 侧并链）。
- XPSD create 路径「只读区间模型」统一处置 → M9。
- 悬空锚一次性订正提交与「矩阵双头」归 M6；「状态声明无单一事实源」两实例（lib/drizzle 与 lib/gaia_xpsd_client **方向相反**，
  M2a 未擅自与 F00-04 自并）→ 由前台在 SUMMARY 主题化，**不得由单一代理合并**。