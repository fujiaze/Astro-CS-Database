...
## L28 重排：原轴中止 + 残余切成三轴（防卡死三条写进任务书）
- **死因诊断**：L28 不是没取证，是**想把全量一次写完**（其档案停在 29 行骨架）。对策不是催，是**把范围切小 + 把交付方式改成分片**。
- 原 `500d215d` 已发中止（interrupt 接受：true）；其 `_cache/L28.md` 骨架**保留不改**，在 SUMMARY 未覆盖面标注「未交付，由 L28b + L28c/d/e 承接」。
- **L28c 头文件外围（tier2）**：只查 lib/ 下不在 include/ 的 .h/.hpp（104+78）+ runtime/cli/providers 头；
  核心问题是"tier1 那 2.1%/1.2%/字段 2.5%/69 头零 ID 能不能外推"；纯缺失在 tier2 **不单独立条**，只进统计。
- **L28d 实现体注释可靠性**：只查 ≤25 个生产 .cpp/.c 的**注释撒谎**（R-6a 自报计数可复算 / R-6b 锚须有宿主 / 单位与公式断言），
  与 L28b 的"缺失"面严格区分；命中 0 也要报（那是正面结论）；明令禁止做全量普查与"注释太少"类判词（§12.2 零溯源义务已确证）。
- **L28e ID 引用侧对账**：补 `tests/**`+`modules/**`+`contracts/**`+`ci/**` 的注释引用面，重算 450/563 孤悬；
  再按四类判"该不该被引用"，**C 类（纯治理文档）必须显式统计为不该引用以免虚报规模**；只立"registry 记 VERIFIED 却无人引用"与"测试在树内但 ID 无人引用"两类。
- **三轴共同防卡死条款**：每 3 条 finding 分片回传一次；**上限 ≤12 条**；优先序=覆盖面口径>判据可机器化>实例数；上下文吃紧立刻交回并写未覆盖面；
  档案只准 edit 增量追加，禁"局部 read → 全量 write"（本会话已有两个代理因此写坏自己的档案：M8 截断到 160 行、M6b 也踩过）。
- **判据锁定**：三轴一律复用 L28b 的 R-1..R-6，**换判据会导致数字不可比**（M6a 的 55% 与 L28b 的 98.8% 之争正是判据宽严之争，不能再来一次）。
- 类别限死 D_COMMENT + E_TRACE_BREAK，其余"移交不立条"；免重报清单逐轴点名（含各自撞车即 related 的要求）。