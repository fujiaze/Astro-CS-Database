...
### L24 计数对账（M8 要求项，前台已闭合）
- `_cache/L24.md` §1 与 §2 曾不一致（§1 头写 P0:3/P1:9、§2 小结表写 P1:10，且 §5.1 仍留「分片 3-5 尚未收到」的过期话句）。
- 已订正：§1 终计 **15 条（P0:3/P1:10/P2:2）** 并加**形态分布**与**对账行**（与 §2 表 3+10+2=15 相符）；过期话句改为「分片 1-6 已全部收齐」。
- 同时告知 M8/M7：**分母从 12 改 15 不影响任何单条成立性**，凡引用 L24 覆盖面数字处（例如「12 条中 9 条落在外部输入解析路径」）
  由引用方**重新计数并写口径**，不得沿用旧分子；M8 已实测 `io_ownership_test.cpp` 仅 1 条可失败断言（`:64 aio_write_bytes(..,NULL,0)==AIO_OK`）、
  `:77`/`:79` 在 if 体内无条件 printf，该判据与分母无关。
- **M2b 的 6 件移交已全部改道**：L15-006/IO_003 合同侧 → M9（已接，出 M9-G-6 + 原子发布四层分布表）；L15-011 → M7（已并入其口径矩阵与门家族）；
  L15-018 → M8（已并入 G-003 基线恒真探针）；L25-001/002 许可证 → M8a（在跑）；L03-010 写出归属与 DISP-HIPS-004 路线 → 已入 40 文件 A-21/A-22；
  **L15-004（Gaia 历元）与 L15-007（D.fits 条款锚错位）经 M9 核证不属其切片，退回前台定归属 → 由 M7 承接（其已持有 D.catalog:193 与 registry 错位实例）**。
- M7 台账「**A-21/A-22/A-23 编号冲突**」：M7 与前台各自使用了同号不同内容。已在 40 文件以 `A-21b` 显式区分并保留 M7 的三项裁决，
  **后续代理引用裁决位时一律以 `40_OWNER_DECISIONS.md` 的行内容为准、不按编号简称转述**（编号只是索引，内容才是判据）。