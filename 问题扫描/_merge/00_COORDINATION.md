...
## 收档 M7（L19+L20+L21 = 73 输入 → 定稿 76 条：P0:4/P1:49/P2:23）—— 三个 agent 的数学链由它结案
### ① L19-004 被**推翻**（HEALPix 几何三方争议结案，结论回到最早的 L04）
- 链条：L04 猜「1.532 实为全对角、半对角 0.766 ⇒ 1.25 够用」→ L19 反驳「1.532=√(3π)/2 是**中心到角点**最大外接半径，1.25 不覆盖」→ **M7 重算**：
  赤道带每环 4·nside 颗 ⇒ φ 向全宽 /hp_res = **(π/2)/√(π/3) = 1.5350 = √(3π)/2**，即 **1.532 是全对角**、中心→角点是 **0.7675**；
  并与 `spherical_overlap.cpp:27-42` 的解析上界 **1.0068** 与穷举最坏 **1.043827@dec=−41.81°** 互证。
  ⇒ **L04 方向正确、L19 的核心结论撤回**，L19-004 由 P1 降 **P2**（只剩表述缺陷）。M7 并主动作废了自己先前发给 M2a 的「用 L19-004 替换 L04 猜测」请求。
- **前台核验**：我独立复算 √(3π)/2=1.53493、(π/2)/√(π/3)=1.53493、atan(π/2)=**57.5184°** ⇒ **M7 的数对、M1a 的 57.5194° 是第 4 位小数笔误**（A-08 已订正，级别不变）。
- 这是本轮方法论最强的正面样本：**同一个数学争议，三个层级各算一遍，最后由合并层结案并自我作废请求**，不是靠"谁说的权威"。
### ② A-05「zpf=300 三源三义」被 M7 退回 —— **我的第 3 次未复验转发**
- M7 实测 `(?i)\bzpf\b` 在 docs/ **0 命中**（全仓只有本 40 文件与 cfitsio 的 tzero）、`ap_corr/2.559` 在 docs/ 0 命中、
  被引的 ALG:52/:56 与 DATA:1253 **现文全是别的内容** ⇒ 我已在 40 文件把 A-05 划撤并写明根因：
  **M3 移交 → 我转发，两人都没复验锚点**。真正成立的是 `PHASE2_SAMPLER.md:189-193` 的 kcorr **300″/600″ 档"反保守"**
  （scale 未知落 300 档被文档称"保守"，但 300 档值小于 600 档值 ⇒ 反而偏小），已由 M7-A-112 定档。
- 已向 M3 复索原始术语（见回执）；光度零点若另有所指，其已定稿面是 M3-C-010。
- **规则升级（对全层，含 R 层）**：**移交件里的锚，接收方必须复验后才能立条或转第三方**；本会话已三次因此撤条（F00-07a 我自己、L10 的 modules/ 前缀、A-05）。
### ③ M7 的两条新增加成事实（读码所得，不是转述）
- `upm.cpp:376-413` 建**跨 tile 双向邻接边**，但 reload 路径 `:1207-1223` **只重建 tile 内 4 邻 ⇒ 载入后丢跨 tile 边**（校正场邻接在重启/续算后静默降级）。
- `p2_upm_geometry_hash:1351-1352` 只哈希 UPM **自家** grid/cell ⇒ "几何一致门"验的是自己的常数，与 `stage2_common.cpp:40-45`（只校 1..64）
  → `stage2.cpp:257`（原样透传）→ `sampler.cpp:594-595`（cell_side=512/G）三侧无一致性 rc ⇒ L20-002 前提**成立并维持 P0**（M7-C-001）。
  可达性旁证取 `reports/REAUDIT_V3/scripts/stage2_fatduck*.json:11` 的 G=4（**只作可达性，不作登记处**，符合协议 §0）。
### ④ 它把「不可达门家族」扩成两类并明确两扇不同的门不可互替
- 第 1 类不可达门 7 成员；第 2 类**「引用不存在的标准依据」3 例**（Górski 全文无 29 / FITS paper IV §2.1 不存在 / 宪章 §12.1 被 L15-002 错挂）。
- Oracle 缺位七实例中写明：M2b-F-01 是「Oracle **不存在**」→ 需 C-02 的在册门；M2a-F-1 是「Oracle 存在但**共源**」→ 需**依赖方向门**；**两扇门不同、不可互替**。
- STANDARDS_REGISTRY 六域横向核按我 4 号令**不另立条**，只作 M3-G-002 输入：CONFORMANT 行 24、偏差列以「无（」起手 17、被比行 198–265 字符（<2000 ⇒ 本域无截断风险），命中 hips:93（以「与读侧同一合同」自证 = **循环论证**）、drizzle:160/162、spherical-projection:64、catalog:193、healpix:135、fits:223。
### ⑤ 一条对**我的核验器**的重要修正
- M7 首轮 14 条锚报 NOT_FOUND，经查是**全角引号 / U+2212 减号 / 带撇上标 / 反引号包裹**造成的 grep 假阴；放宽 + 定点回读后全部命中。
- 已立「**整句 grep → 关键词 grep → 定点 read** 三级复核」为定稿纪律（M7 §9）。→ 前台据此提醒：**`ANCHOR_VERIFY_REPORT` 的 31 条符号未命中里可能含同类假阴，R 层不得据报告直接撤条**，必须先走三级复核。