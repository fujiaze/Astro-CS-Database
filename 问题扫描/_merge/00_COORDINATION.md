...
## 收档 L28b（注释溯源·接力，5 条 D_COMMENT：P1:4 / P2:1）+ INDEX v3
- **L28b 把"注释缺溯源"变成了带假阳率的可采纳判据**（本会话唯一给出 FP/漏报双向计量的域）：88 公共头 / 13,705 行 / 2,827 声明单元；
  有溯源仅 **2.1%**、被消费者形态 **1.2%**、**字段档最低 2.5%（字段正是单位与值域载体）**；88 头中 **69 个零 ID**（文件级 21.6% 含具体 ID）；
  该有 471（16.7%）中无溯源 **442（93.8%）**。**假阳率抽 22 判 7 = 32%**，加三白名单可压到约 15%；**不排除伪锚则朴素实现误通过 30.5%**
  ⇒ R-3 伪锚排除是必要条件。已交可直接采用的 §12.2 追加条文草案（档案 §1.5）+ 实测成本（strict 现网报 442 单元；R-6a/b/c 命中 3/8/1 处当天可入门）。
- **裁定一（口径冲突）**：M6a-D-010 的 55%/15% 是**人工宽松判读 + 抽样 20 符号**（把「用于 master 帧预生成」这类用途散文算命中），
  L28b 要求显式形态 ⇒ **分歧根因是判据宽严而非抽样误差**，混用会低估缺口一个数量级。全仓口径一律用 L28b；M6a 原文**不撤不改**、
  只加口径注。这是「同一事实一处定稿」的正当例外：**判据不同即事实不同，属两条结论**。
- **裁定二（承接移交件）**：三份注释产物零拦截 —— `check_comment_hygiene.py` 的 `main()` 在 **:116 无条件 return 0**（违例只写进
  `reports/v19r2`，ci 对它 0 引用、checks.json 内 comment 命中仅 :782），叠加 `check_comments.py` 现网 16 文件触发 / 0 报警；
  `COMMENT_STANDARD.md:44-45` 把标准唯一证据指针指向免报区 CSV ⇒ **标准证据不可由真源核验**；`aio_hips_reader.h:52-55` 与同 TU :253-259
  对**同一缓冲**给出两种行序口径（y*512+x vs (511-x)*512+y）⇒「口径无单一事实源」簇的**第 5 个成员**。
- **裁定三**：宪章 §12.2 逐字**零溯源义务**（只 L1/L2/L3 + :429 一句），义务在 §12.3-3 与 COMMENT_STANDARD ⇒ 你两项要求（每子库 README、
  注释写明来源/被消费者）**合理但仓内无条文**，已并列为 **A-15 / A-35** 同型两条，另新增 **C-11**（注释门合并改造 + 门必须能红入 §12.3）。
- **双向断链基线（须并列才成立）**：注释侧真悬空仅 **2.1%（distinct）/3.7%（实例）**，而文档侧 **450/563 枚（79.9%）在生产代码注释里从未被引用**
  ⇒ 两层各说各话。INDEX 主题表已加该行，SUMMARY 簇 4 与簇 10 引用。
- 流水线：**27 轴 + 14 域全部收工**，剩 R 层；L28b 另留一条只有执行权能做的收尾（442 条清单在只读纪律下无法落成 CSV）→ 并入 B-14。