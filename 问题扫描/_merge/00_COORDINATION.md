...
## E 层开工（负责人授权 B-14）—— 四路并行，边界写死在规程 §8
- **E1 在册与导出面**：p1noise tracked 终判（前台预告 `git ls-files | grep -c p1noise` = 8）、8 个孤儿 TU 的五列在册性表（tracked / 在构建图 /
  在 `ctest -N` 列表 / 在基线 / 被 registry 当证据）、checks.json 当场权威计数、snr_estimator 导出表 nm、L28b 那 442 单元清单重跑（CSV 落
  `run/审计执行层/E1/`，不写真源）、known_failures 现条目数。
- **E2 构建拓扑**：`build/cli/astrocs` 到底是不是交付物（回答负责人那句「被验收的是不是随包那个」）、三处构建孤儿（**区分"在图里没采集"与"不在图里"**）、
  `--changed-from`（**无 dry-run 就本地复刻逻辑并声明是复刻**）、CPU 分母取几个核、packaging 产物的 p1_session_run。
- **E3 既有运行期证据（不许新构建）**：树内已有 `build/asan/`（runtime/io/common/core/phase1_noise/phase1_session），故要求它
  在 run/、reports/、evidence/、logs/、工程控制/ 里检索既有 ASan/UBSan 记录，并判断那四条越界是否**已具备 ASan 构建条件但无运行记录**
  —— 这条直接决定负责人"下次要不要授权真跑"。
- **E4 平台盲区可静态部分**：longPathAware 是否声明**且生成链接得上**（接不上就不生效）、Windows rename 语义中「不必实测」的那半
  （同仓多写法对照表）、windows-main 有效断言数下界、UTF-8 合同实现者缺席确证。**禁止连 Fatduck。**
- 统一铁律：每条结论附「命令 + 输出摘录」，无输出写「未获证据」；与静态推演不符以实测为准并标「改判」；对已定稿 findings
  **只做末尾追加一行 `【E<n>-实测】`，不得删改原文一字**（E 层是第四态落地，不是新事实）。