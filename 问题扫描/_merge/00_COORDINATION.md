...
## 收档 M3（44 → 定稿 43：P0:7/P1:26/P2:10，16 个合并件）+ 建 40_OWNER_DECISIONS.md
- M3 是本会话里**执行两条方法论警示最完整**的域：它按 H-1 用正则检出 ≥1990 字符行（**全证据面仅 1 处** = cli/CMakeLists.txt:201，2043 字符），
  并声明对该行只用 grep 存在性判据；按 H-2 把 TRACEABILITY.csv 改用「表头 16 列键集合 + 逐列 grep」重算（SCI-NOISE 命中 15 行，
  第 9/10/14 列 15/15 全为 TEST-SNR-001 / noise_model_science_test.cpp / VERIFIED；SCI-CW 命中 0 行），并把转述数字（含我给的 dirty 17→8、
  L12 的 138 处）一律标为线索。**这两条警示从此有可复制的取证口径范例。**
- 它**撤销了叶子一条判据**（L07-006「Python oracle 从不运行」不实：UT-BACKEND 以 discover -s tests/backend 确实执行且 waivable:false；
  成立的是「§11/§15 承诺的 NumPy rtol 1e-9 复算在仓库内没有实现」），并按我的归口令把 M3-E-001/002 从「根因条目」改写为
  「本域实例清单」（14 组锚、13 组不命中，逐组标口径与失效形态：纯漂移/指向无关符号/数字与内部引用失效），删除全部「系统性」判词。
- **它自报一次纪律偏差**（对自身产出文件用过一次 bash 统计字符数）→ 我登记为 D-01，不改变判据但公开记录，并再次向全层重申边界。
- 新增「**第五种状态**」概念（M3 A.2 / M3-I-002）：两套测光实现**都未登记为任何检查项** —— 不属于仍成立/已修复/部分修复/无法判定任何一种，
  而是**无声明主体**。已在 40 文件 A-17 提请你认可为独立状态。
- **建立 40_OWNER_DECISIONS.md**：A 组 17 条口径裁决（Agent 无权自判，宪章 §1.2/§17.12）、B 组 13 条需执行/外网/数据权限
  （含**与禁执行令直接冲突**的 B-09 ASan 实跑）、C 组 9 条建议新增/订正规范条款、D 组 4 条纪律偏差自报。
  这是给负责人的单页决策入口，SUMMARY 会引用它而不重复展开。