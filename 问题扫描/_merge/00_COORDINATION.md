...
## 改派 L28b（L28 两次催办无进展，其 29 行骨架不采信）
- **L28 的处置**：前任不交档，其"覆盖面数字"我此前也拿不到 → **不作为发现、不留空槽**；其两项未覆盖面（全仓注释溯源矩阵、被消费者缺失全量）
  由 **L28b 接力**，并把范围**收窄成四件事**：①把 M6a 的定性结论变成**可机检判据 + 实测命中率/假阳率**；②「注释主动撒谎并驱动决策」的
  通用判据 + 3-8 条新实例（列明 7 处已知站点不得重报）；③注释侧 SCI/ALG ID 全仓计数（与 M6b 分界：M6b 主落文档侧断链，L28b 主落注释侧）；
  ④与 M6a 的双向分界声明。
- **优先级指令**：时间不足时**先交 ①判据与 ④分界**——判据比新增实例更有价值，因为它能直接变成宪章 §12.2 条文给负责人。
- 只允许写 D_COMMENT 一类，其余一律交 M6a，避免与已定稿的 31 条注释面重复。

## 里程碑：十四域合并全部有产出（机械计数 = 494 条目标题）
- 域分布：M1a 44 · M2a 46 · M2b 29 · M3 43 · M3b 22 · M4 31 · M5a 22 · M5b 43 · M6a 31 · M6b 22 · **M7 78** · M8 26 · M8a 31 · M9 26。
- 优先级：**P0 81 / P1 287 / P2 126**（按 `### Mxx-<类别>-<编号>` 标题机械计数；各域自报口径略有出入，以本表为导航、以 _merge 处置表为权威）。
- `_merge` 处置表 14 份共 3029 行；INDEX/SUMMARY/40_OWNER_DECISIONS 三份前台综合件已入库；本审计提交 40 次（全部限 问题扫描/、越界=0）。
- **注意**：机械计数含 M7 的 78（其自报 76 实条目 + 2 交叉指针）与 M8a 的 31（其 _merge 已 250 行，代理仍在收尾）；
  凡与域自报不符处，R 层不做判对错，只加「（审计当日实测 N，现已漂移）」注。