...
## 收档 M8（L22+L23 → 26 定稿：P0:4/P1:15/P2:7）＋ 一处前台实测再漂移
- 主落裁定已生效并转 M9：孤儿测试 TU 根因 = **M8-F-004**（61/61 逐 basename 零命中，弃用"4.5M 字符"这种不可复核口径），
  L24-003 降为实例面；「值口径本体 = M5a-G-001 / 测试面 = M8-F-009」。
- **M8 消费锚点核验报告的记账法升为全层模板**：本域 15 项异常（报告点名 8 + 自纠 7）→ 重锚 8、改述/改挂 5、撤旧锚 1、
  机械假阳注记 1、未闭合 1；未闭合项如实进 §7b，不因此下调置信度、也不假装闭合。
- 新形态两条并入簇 1/簇 5：**基线恒真探针**（`p2_seam_gate:71` 用被测函数自造通过输入 ×1.2 自证）；
  `ci/tests/test_ci001_failclosed.py:128/137/152` 用 `assertNotIn` **把"注册表零 --gate-required"钉成期望**（该文件本身不被 CI 采集）。
- 它**纠正了我一次订正的不精确**：我让它把 STD-REGISTRY 写成「挂 known_failures.json 账」，它实测该字面串全仓 0 命中、known_failures 仅 2 条
  ⇒ 既未注册也未挂账、只被 docs 引用。我的订正也被机器复核覆盖，记入 SUMMARY 簇 10。
- **再漂移一例（"裸数字禁止"的新证据）**：M5b 与 M8 均实测 `ci/checks.json` 的 "id" = 112，我刚才实测 = **113**；
  而 `grep -- "--gate-required" ci/checks.json` = **0**（此条稳，与各域"零应用点"一致）。
  → 通则补充：**计数类判据必须带当日时点与命令口径**。凡各域写「111/112 项」处，R 层不判对错，
    统一加注「（审计当日实测 N 项，现已漂移）」；我自己也照办。
- M8→M4 的三件补强（M4 已收工，前台承接）：`TestUpmParallel` 类名与 test_01 标题**超前于实现**；`OMP_LIB` 指向
  `build/linux-openmp-on/libphase2.a` 且**直链 .dll**（与 M4-F-01 out-of-topology 同族）；R6 待执行。前两条并入簇 9「声明超前」子清单。
- L28 第二次催办已发（限三行块回答：完成到哪一步 / §5 覆盖面数字 / 最重要 1 条压成 5 行），不回复则改派接力代理。