...
## M6b 二次复核回执（**本会话执行 H-1 警示的标准范例，已作为模板下发**）
- 它把 H-1 变成**可复核的口径**而不是口号：实测矩阵 CSV 有 **10 行 >2000 字符被截断**（第 7/8/9/10/16/17/20/26/28/29 行）、
  矩阵 JSON 有 6 行（且逐行核对**全部是 notes 行**）；关键判据是「**CSV 每行解析后仍得 24 个字段、异常行 0**」⇒ 截断只落在末列 notes 内部，
  前 23 列完整 ⇒ 「23 非 notes 列零差异」的结论**不是伪影**，L18-006 的剔除维持，但判据重写为
  「元素计数（行首 MOD- = 30 / "module_id": = 30）+ 列名与键集合双向零差 + 逐列比对仅取非截断列」。
- 旧表 `docs/TRACEABILITY.csv` 逐行扫描 **0 行超长** ⇒ E-001 的三项事实（API- 前缀 0 行、三例 ID 命中 0/0/0、可交比 8 / TEST 一致 0/8）**均不受影响**。
- 层取值全部改整行 grep 计数并复现；「未接线 16 行 / 其中七层 VERIFIED 14 行」改为**双判据**（整行 grep「未建」+ 状态短行判 VERIFIED）。
- **唯一因截断而错的数：`*-DESIGN-001` 9 → 11**（已改证据段与复算表）。
- 另**主动更正 2 处非截断类口径错误**：①E-004「138 处」混用了文档名集合与扫描面 → 改为三口径并列（真源 13 目录 **139 处/40 文件**；
  全仓 193 处中 `问题扫描/**` 占 35 处系我方自写，已剔除；八类不存在权威文件名 61 处/33 文件），并确认我转述的「15+ 处」在任何口径下仍是严重低估；
  ②**推翻自己一条判定**：原判 `docs/ARCHITECTURE.md:129`「SCI-/ALG-/DATA-/ENG- 全 VERIFIED」为不实，复算前缀分布
  （SCI 36/TEST 17/DATA 5/ALG 4/ENG 4/ACR 1）+ status 67/67 后确认**该句成立** ⇒ 该条事实从 3 项收敛为 2 项、改「部分成立（已收窄）」。
- **前台裁定**：这段回执的口径写法（先量截断面、再证结论落在未截断列、错的就改）作为 SUMMARY 簇 10 的正面样本；
  并把「我方自写文件不计入真源统计」升为全层通则（审计档案自身的引用不是产品缺陷证据）。