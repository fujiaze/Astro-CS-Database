...
| 分类 | 计数 | 明细 |
|---|---|---|
| **本任务新增退役工具** | **4** | `tools/assemble_audit.py`、`tools/make_capsule.py`、`tools/make_rev2_capsule.py`、`tools/pack_audit_package.py`（**仅打包入口**） |
| 其中「部分退役」（函数保留为活动依赖） | 1 | `tools/pack_audit_package.py`：`allowed()/denied()/EXCLUDE_EXT` 仍被 `CHK-SECRET-HYGIENE` 消费 |
| 已退役（本任务仅复核，**未改**） | 1 | `tools/check_traceability.py`（2026-09-16 按 GAP-032 退役，见 §2.1 退役记录） |
| 经实证**不退役**（保留原样） | 1 | `tools/quality/known_failures_baseline.py`（被 `CHK-KNOWN-FAILURES-BASELINE` 的 3 个 step 消费） |
| **注册项退役** | **0** | 4 个退役工具**均不在 `ci/checks.json` 注册**（无 command / changed_paths 引用）⇒ 无需注册项退役；CI-001 独占窗口内 `ci/checks.json` **一行未动** |
| 登记订正 | 2 处文件（3 项） | `ci/INVENTORY_REPORT.md`（1 条读数）、`ci/root_manifest.json`（7 条悬空登记）、`docs/ci/01_CHECKS.md`（新增 §2.2 工具层退役记录） |
...
- 退役前（`logs/01_grep_before.txt`，10 个文件）：
  `ci/checks.json`、`ci/tests/test_impact_map.py`、`tests/api/test_cli_protocol.py`、`tests/quality/test_docchk002_mutation.py`、
  `tools/assemble_audit.py`、`tools/check_traceability.py`、`tools/make_capsule.py`、`tools/make_rev2_capsule.py`、
  `tools/pack_audit_package.py`、`tools/quality/known_failures_baseline.py`
- 退役后（`logs/03_grep_after.txt`，11 个文件）：`diff` 仅多出 `ci/INVENTORY_REPORT.md`（本任务的订正注记）。

**结论：受影响工具集合前后完全一致（10 → 10，无增无减）**；新增文件不是工具。
退役抬头本身仍会命中该字符串 —— 那是**退役说明的文本**（与既有先例 `tools/check_traceability.py` 一致），
...
|---|---|---|---|---|---|---|
| 1 | `tools/assemble_audit.py` | 输入常量 `CP`/`TAB`，输出 `SRC`/`OUT` | **否**：`工程控制/RELEASE_V5/AstroCS_MAIN_RELEASE_CONTROL_V5_SINGLE_CLI_AMD64_20260828/`、`artifacts/prerelease_v5/tables`、`audit_src`、`AUDIT_REVIEW` 全不存在 | 否（`git grep assemble_audit` 仅自引用） | 无（发布打包门为 `CHK-PACKAGE`） | **退役** |
| 2 | `tools/make_capsule.py` | 输出目录 `artifacts/prerelease_v5/capsules` | **否**（该树随 `artifacts/` 删除） | 仅 `tests/cli/test_iso_acr_gpu_isolation.py::test_04` **静态扫描其路径**（扫描体为空操作）；不在注册表 | 无（逐提交复核由 git 历史承担） | **退役**（文件保留） |
| 3 | `tools/make_rev2_capsule.py` | 输入三表（`isfile` **静默跳过**）+ 输出目录 | **否**（tables/ 与 capsules/ 均不存在） | 否 | 无 | **退役** |
| 4 | `tools/pack_audit_package.py` | 输出 `OUT`、白名单前缀、排除前缀 | **否**（输出目录随 `artifacts/` 删除） | **是**：`tools/quality/check_secret_hygiene.py:53`（`DEFAULT_PACKER`）→ 注册项 `CHK-SECRET-HYGIENE`；`tests/quality/test_secret_hygiene.py` | 打包入口的替代 = `CHK-PACKAGE`；白名单函数**自身即活动替代** | **部分退役**：入口退役，白名单/排除函数保留 |
| 5 | `tools/check_traceability.py` | 退役抬头 + fail-closed 文案 | 否（同一个已删表） | 否（`TRACEABILITY-CODE` 已退役，§2.1） | 有：显式 CSV 入参仍按 R1-R7 全量校验 | 已退役（**本任务未改**，仅实跑复核 rc=2） |
| 6 | `tools/quality/known_failures_baseline.py` | 历史 finding `F-028` 的 repro/evidence **记录字符串** | 该字符串只是历史记录文本 | **是**：`CHK-KNOWN-FAILURES-BASELINE` 的 3 个 step（仍写 `artifacts/KNOWN_FAILURES_BASELINE.json`） | — | **保留（不退役）** |
| 7 | `ci/checks.json` | `dirty_ignore_prefixes` 2 处（`checks[12]`=UT-BACKEND 顶层与其 step） | 路径已被 ISA-* 旁路重建（**非原语义**） | 是（注册表自身） | — | **本任务不动**（CI-001 独占写窗口；登记为后续处置，见 §5） |
| 8 | `ci/tests/test_impact_map.py` | 注释（退役依据说明） | — | 是（`CI-BINDING-TESTS` step） | — | 保留（注释即登记） |
...
- **退役前实测**（`logs/10_before_assemble_audit.out`）：`rc=1`，未捕获
  ...
  （探针残留已由本任务清理，`artifacts/` 现状仅剩 ISA-001/002/003 与本任务外的 `ci/`、`KNOWN_FAILURES_BASELINE.json`）。
- **退役后实测**（`logs/20_after_assemble_audit.out`）：`rc=2`，输出 `ASSEMBLE_AUDIT_RETIRED: ...`，**无 traceback**。
- **复原方法**：`git show 01754fab8618:tools/assemble_audit.py > tools/assemble_audit.py`；复原输入见 §5 注（V5 控制包不在库）。

### 3.2 `tools/make_capsule.py`
...
- **复原方法**：`git show 01754fab8618:tools/make_capsule.py > tools/make_capsule.py`。

### 3.3 `tools/make_rev2_capsule.py`
...
- **复原方法**：`git show 01754fab8618:tools/make_rev2_capsule.py > tools/make_rev2_capsule.py`。

### 3.4 `tools/pack_audit_package.py`（**部分退役**）
...
- **复原方法**：`git show 01754fab8618:tools/pack_audit_package.py > tools/pack_audit_package.py`。

---

## 4. 登记不一致的订正

### 4.1 `ci/INVENTORY_REPORT.md`（`TRACEABILITY-CODE(PASS)` 陈旧读数）
- **问题**：§2「追溯类」表仍把 `TRACEABILITY-CODE` 记为 `PASS`，而该注册项已于 2026-09-16 按 GAP-032 **退役**
  （`docs/ci/01_CHECKS.md §2.1`、`ci/checks.json` 已移除该注册项），属「陈旧状态冒充活动状态」（ENGINEERING_SPEC §8）。
- **订正**：本报告是 base SHA `b4f923cc` 的**当时快照**，**不回改历史读数**；在报告头部加「订正记录（2026-09-16, RETIRE-001）」，
  并在原单元格就地加注 `~~PASS~~ **已于 2026-09-16 退役**`（含 `§2.1` 指针）。
- **复原方法**：`git show 01754fab8618:ci/INVENTORY_REPORT.md > ci/INVENTORY_REPORT.md`。

### 4.2 `ci/root_manifest.json`（`registered_local_retention` 7 条悬空登记）
...
- **复原方法**：
  ...
  - 整个文件：`git show 01754fab8618:ci/root_manifest.json > ci/root_manifest.json`。
...
- **关于 §5「执行器」**：**本任务未修改 §5**。`docs/ci/01_CHECKS.md §5` 与 `AGENTS.md:36` 属**新权威文档集**，
  其 `python3 ci/run_checks.py …` 是**规范要求**；仓库当前 tracked 的执行器是 `ci/run.py`，二者差异属 **GAP-016**
  登记的「规范要求、实现缺失」缺口，由 **CI-001** 交付 `ci/run_checks.py` 并一并处置 → **不改规范，登记为实现缺口**。
...
| 项 | 位置 | 为何不动 | 建议 |
|---|---|---|---|
| `dirty_ignore_prefixes: ["artifacts/prerelease_v5/"]`（2 处） | `ci/checks.json` `checks[12]`(UT-BACKEND) 顶层与 step | **CI-001 独占收敛窗口**（上级明令「一行都不要碰」）；且其为容错白名单前缀，不是命令/输入，不会静默坏 | CI-001 窗口结束后随 UT-BACKEND 收敛一并清理 |
| 三处注释引用 `artifacts/prerelease_v5` | `tests/api/test_cli_protocol.py`、`tests/quality/test_docchk002_mutation.py`、`ci/tests/test_impact_map.py` | 均为**已完成的断链修复说明**（夹具已重接线到 tracked 文件），且 `tests/**` 属 TEST-CLI-SYNC 活动域 | 无需动作；作为历史说明保留 |
| 其它提到 `工程控制/RELEASE_V5` 的工具 | `tools/make_linux_release.py:88`（许可证来源文案）、`tools/validate_cpu_profile.py:88`（注释） | 不在本任务 grep 判据内（不引用 `artifacts/prerelease_v5`），且 `make_linux_release.py` 属活动打包链路 | 另立任务按同一实证口径判定 |
...
| `30_secret_hygiene_pack.json`、`40_active_consumer_regression.out` | `CHK-SECRET-HYGIENE` 实跑 rc=0；`test_secret_hygiene.py` 22/22 OK；`ci/validate_registry.py --strict` `error_count=0` |
...
- [x] `ci/validate_registry.py --registry ci/checks.json --strict` → `error_count=0`（**只读**，未改 `ci/checks.json`）
- [x] 退役项实跑输出为**明确退役文案 + rc=2**（无未捕获 traceback）
- [x] 本台账覆盖全部退役项（4 = `docs/ci/01_CHECKS.md §2.2` 表行数）
- [x] 未越界改动：仅 `tools/{assemble_audit,make_capsule,make_rev2_capsule,pack_audit_package}.py`、`ci/INVENTORY_REPORT.md`、
      `ci/root_manifest.json`、`docs/ci/01_CHECKS.md`、本报告（+ `run/` 日志）；未触 `ci/checks.json`、`lib/**`、`cli/**`、
