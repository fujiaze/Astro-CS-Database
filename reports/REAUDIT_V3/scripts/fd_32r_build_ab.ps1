...
foreach ($a in $anchors) {
  ...
  if (-not (Test-Path -LiteralPath (Join-Path $wd '.git'))) {
    # C is the main worktree itself
    if ($a.name -ne 'C') { "SKIP_NO_WT $($a.name)"; continue }
    $wd = $root
  }
  ...
  Push-Location $wd
  try {
    & cmake -S . -B $bd -G 'MinGW Makefiles' -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DCMAKE_MAKE_PROGRAM=mingw32-make 2>&1 | Tee-Object -FilePath $log -Append
