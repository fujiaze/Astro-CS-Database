...
> **订正留痕（2026-09-20）**：上述两条**已被裁决覆盖**（旧文 =「`GAP_AUDIT.md §9.22` 已登记为待研究项」+「必须走 SCI 变更流程…把 `g_k` 纳入模型」）——
> 1. `g_k ≡ 1` **本期不启用**（§9.38 A2，`工程控制/RELEASE-02/GAP_AUDIT.md:1036-1043`）；「把 `g_k` 纳入模型」的路径**已被否决**（§9.50 定案 5「UPM = 纯加性，定案」，`:1550-1560`）。
> 2. 公共场（含 N 帧 ref 组合）按 **§9.30 ⑤**（(c) 排除自身 + 阻尼 + 拟合权重与叠加权重同源 + 末端扣残差场）执行；稀疏层按 §9.46（`:1360-1366`）默认消费。
> 3. §9.67 定案 1（`:2548-2555`）：加性施加量 = `raw_k − δ_k`（保留 `B_ref`），**多退少补到公共天光面**。
...
> **订正留痕（2026-09-20）**：**已裁决**（旧文 =「UNRESOLVED 需负责人裁决」）—— UPM = **纯加性**（§9.38 A2，`工程控制/RELEASE-02/GAP_AUDIT.md:1036-1043`）+ §9.40 C3（`:1122-1127`）⇒ `PHASE2_UPM.md:182` 的 UNRESOLVED **关闭**为「本期决议：纯加性」；`g_k ≡ 1` 本期不启用。⚠ 另注（不改变「纯加性」结论）：§9.67 定案 1（`:2548-2555`）订正的是**加性施加量** = `raw_k − δ_k`（保留公共天光面 `B_ref`），`raw − C_k` 不再是默认。
