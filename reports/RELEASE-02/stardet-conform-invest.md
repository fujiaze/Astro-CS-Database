...
> ⚠ **已取代横幅（2026-09-20 订正）**：本报告的**检测器切换建议**（§1.7「方案 A（推荐）：把 p1_op_star_psf 的检测源切到 sdet」/「方案 B（备选）」/「本分片建议 A」）**已被负责人 §9.49 定案 1 取代**：检测改**星表引导**（「根据 wcs 将 gaia 星表投影，能拟合的才是星点，拟合失败直接丢弃就好了」，工程控制/RELEASE-02/GAP_AUDIT.md:1464-1470）。本报告的 sdet 切换建议降级为「**星表引导实现后的可选评估项**」；§3 的 WCS 零点偏移旁支已按 §9.49 定案 8 立项 SDI-WCS-ZERO-POINT（:1477、:1537）。**本报告作为历史审计轨迹保留，不物理删除。**
...
> ⚠ **已被取代（2026-09-20）**：下述「方案 A（推荐）…切到 sdet」的**推荐地位已失效** —— §9.49 定案 1 定检测范式 = **星表引导**（`工程控制/RELEASE-02/GAP_AUDIT.md:1464-1470`）；sdet 切换降级为可选评估项。**原建议正文保留为历史留痕。**
...
> **但这是科学影响面变更，须负责人裁决**（AGENTS.md 第 9 节：涉及最高设计/文档集与现状冲突且需要改文档）。 **订正（2026-09-20）**：负责人已就检测范式裁决（**星表引导**，§9.49 定案 1，工程控制/RELEASE-02/GAP_AUDIT.md:1464-1470）⇒ 「sdet 切换」不再是负责人待决项，降级为「星表引导实现后的可选评估项」（旧文 =「须负责人裁决」）。
...
>
> **订正（2026-09-20）**：**已立项** —— §9.49 定案 8「（WCS 零点偏移）**应该仔细检查**」（工程控制/RELEASE-02/GAP_AUDIT.md:1477）⇒ SDI-WCS-ZERO-POINT 立项（:1537）；归因仍待该分片结论。旧文 =「本项未定案」。
...
上呈负责人另立调查项（例如 `SDI-WCS-ZERO-POINT`）：判据 = 用 Gaia 直接拟合零点的绝对残差门（当前链内无此门），并核对 IPV `IpvWcsResult.crpix` 的坐标系定义与 `sdet` 输入的换算是否恰好差 0.5 px。 订正（2026-09-20）：已立项 SDI-WCS-ZERO-POINT（§9.49 定案 8，GAP_AUDIT.md:1477/1537）—— 不再是「建议上呈」，判据与核对项照原文执行。
...
3. **未决**：
   - 3.1 链内 WCS 零点偏移的**归因**（IPV crpix 口径 / sdet 输入换算 / 其他）未定；本分片只给出量级与空间结构。 订正 2026-09-20：归因项已立项 SDI-WCS-ZERO-POINT（§9.49 定案 8）；归因结论仍待该分片。
