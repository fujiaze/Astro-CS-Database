...
1. 走变更 claim 把 PHASE2_UPM.md 重写为目标模型 y_k=g_k·s(x)+b_k(x)（WCS 联合参考天光面 B_ref + 逐帧梯度 δ_k + SNR 加权最小 RMS），或明确标注"现行实现口径，已被设计 §4.2/§4.4 取代"；FROZEN 状态需一并处理。 订正（2026-09-20）：本建议作废 —— §9.38 A2（工程控制/RELEASE-02/GAP_AUDIT.md:1036-1043）+ §9.40 C3（:1122-1127）：UPM = 纯加性，g_k ≡ 1 本期不启用，UNRESOLVED 关闭。保留第 2/3 条（k_corr 区间 1.2112–3.2035 / 「相对定标」措辞上呈）。
