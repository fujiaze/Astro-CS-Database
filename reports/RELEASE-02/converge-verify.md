...
| **U-4** | `smoothing_lambda` 默认三面不一致：`stage2_common.h`=0.1（`P2_SMOOTHING_LAMBDA_AUTO`）、`upm.h:75`=0.0、`module_adapters.cpp:5264` 缺键保持 0.0 | (b) 已登记 | `conform-fix-b` 注释自述「与负责人裁决 GAP_AUDIT §9.39 A5『λ 不能为 0』冲突已登记上呈」；`tools/fixtures/config_consistency_known_divergences.json` 仅登记 `underdetermined_n` 一条 | 生产 `p2_op_upm_fit` 缺键时 λ=0（关平滑）；~~**生产取值归 SMOOTH-LAMBDA 分片裁决**，未擅自改冻结面~~ ⇒ **判不了（默认值未冻结；订正 2026-09-20，V5 分片 3）**：两条裁决并列且**是否同指未判定** —— ① §9.55 S5（`GAP_AUDIT.md:1846-1859`）「λ 生产默认取 `0.1`（`P2_SMOOTHING_LAMBDA_AUTO=0.1`），`λ=0` 仅显式 opt-in」；② §9.67 定案 5（`:2584-2588`）「本期**不加**堆叠平滑项」。**缺什么证据**：§9.67 定案 5 原文未出现 `smoothing_lambda`／`P2_SMOOTHING_LAMBDA_AUTO` 字样，两条裁决的适用对象未在裁决文本中对齐（⑦ 批次 §4 判不了 1）⇒ **不写死默认值**；实现面（`upm.h:75`=0.0、缺键 0.0）尚未落地，仍如实登记 |
