...
## ② 默认保留 C 还是 δ：~~**默认保留 C（去掉 δ）**~~ ⇒ **已被 §9.67 定案 1 取代（订正 2026-09-20，V5 分片 3）**：生产默认 = `calibrated_k = raw_k − δ_k`（**保留 δ、去掉 C 全减**，保留公共天光面 `B_ref`）
...
5. **对 C 近过拟合（271555 DOF vs 277234 obs）的处置**：C 受 CG 平滑 + 弱零锚正则化；且 M0（Phase1 测光施加，另一分片）完成后，C 当前吸收的未归一化残余乘性将消失，过拟合压力下降。本实现保留配置开关，若负责人按 c-delta-ruling M1-a 裁决保留 δ，可无损切换（`additive_mode="delta"`），无需改代码。 **订正（2026-09-20，V5 分片 3）**：§9.67 定案 1 **正是**「保留 δ、去掉 C」⇒ 本假设句已成为**生产默认**（实现需把 `additive_mode` 默认由 `"c"` 改 `"delta"`；工作树 `module_adapters.cpp` 仍为 `"c"`，未落地）。

> 未改动的科学口径：输出仍保留公共面亮度（不扣整个背景、不除 g_k），与 **§9.67 定案 1**（`:2548-2555`：`calibrated_k = raw_k − δ_k`、**保留公共天光面 `B_ref`**、多退少补到公共天光面）语义一致；仅把**两次**扣除改为**一次**。**订正（2026-09-20，V5 分片 3）**：原文「与 FIX-GK/**方案 B** 的「多退少补」语义一致」中的方案 B 口径（`raw − δ_k`）已被 §9.67 定案 1 **采纳为生产默认**；而本节标题/正文主张的「**默认保留 C（去掉 δ）**、`raw−C`」= §9.54 前台裁决口径，**已被 §9.67 定案 1 取代**（冲突已登记；`raw − C_k` 全减**不再是默认**）。
...
- **登记为开放项**（**判不了/默认值未冻结**：§9.55 S5 与 §9.67 定案 5 是否同指未判定；订正 2026-09-20，V5 分片 3）：生产 `p2_op_upm_fit` 当前 `smoothing_lambda=0`（仅当 config 给值才非零）。~~是否把生产默认改为 >0 属科学行为变更，需负责人裁决；~~ §9.55 S5（`工程控制/RELEASE-02/GAP_AUDIT.md:1846-1859`）定「**λ 生产默认取 0.1**（`P2_SMOOTHING_LAMBDA_AUTO=0.1`）；`λ=0` 仅**显式 opt-in**」；ALG §13 冻结值走变更 claim。⚠ **判不了（缺什么证据）**：§9.67 定案 5（本期不加堆叠平滑项）与 S5 是否同指**未判定** —— §9.67 定案 5 原文未出现 `smoothing_lambda`／`P2_SMOOTHING_LAMBDA_AUTO` 字样，两条裁决的适用对象未在裁决文本中对齐（⑦ 批次 §4 判不了 1）⇒ **默认值未冻结**，本行不写死。本分片未擅自更改实现，只保证配置可达。
