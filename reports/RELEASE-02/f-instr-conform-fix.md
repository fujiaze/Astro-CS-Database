...
### 4.4 帧间门收紧（0.5 → 0.02 dex）的后果（**已裁决**：删除组间门、降级为报告字段；订正 2026-09-20，V5 分片 3）
...
- ⇒ **≤0.05 mag 判据只在"同仪器 + 同夜 + 同曝光"分组下可达**; 现 L4 分组（跨夜）下任何 F_instr 定义都不可达。
  ...
  ✅ **订正（2026-09-20，V5 分片 3）—— 已裁决，三选项全部作废**：§9.49 定案 2（`工程控制/RELEASE-02/GAP_AUDIT.md:1487-1495`「这玩意应该是帧间独立的，为啥要组间对比」「门只有一个：单帧标定是否可信，与其它帧无关」）+ §9.62（`:2258-2264`）：**删除组间 `k` 散度门**（含 fail-closed 分支），**降级为报告字段**（`photscale_spread_dex` / `photscale_spread_warn` / `photscale_spread_gate="none (owner ruling 9.49: frame-independent)"`）；**帧内判据保留**。本节实测的「12/12 板块被拒」正是该门被删的原因（负例：跨帧一致性门复活 ⇒ 必须判红）。
...
| 判据 | 目标 | 实测 | 判定 |
|---|---|---|---|
| 测光一致性（残差 MAD） | ~~≤0.03 mag~~（**已作废**：§9.67 定案 6 —— 阈值须从误差预算推导） | after 中位数 0.0617 / 最坏 0.1160 mag（before 最坏 1.8459） | **不是硬门**（§9.50 定案 3：判据 = 测光正确 + 拟合收敛；订正 2026-09-20，V5 分片 3） |
...
1. **PSF 域通量的方差代价（重要）**: PSF 域 flux 是 7 参数拟合的整平面外推（正比于 A·sx·sy）, 单星随机误差大于固定孔径盒和。
   ...
   ✅ **订正（2026-09-20，V5 分片 3）**：① **`≤0.03 mag` 不是硬门**（§9.50 定案 3，`GAP_AUDIT.md:1555`）⇒ 判据改为「**测光正确 + 拟合收敛**」；② 该阈值**作废**，新判据须**从误差预算推导**（光子噪声 + PSF 拟合不确定度 + 平场/天光残余 + 星等定标误差的合成，逐项数值 + 出处）并走变更 claim（**§9.67 定案 6**，`:2590-2594`）；③ D2 PSF 加权最优提取**已立项**（§9.54:1831-1833）⇒ 原文「请负责人裁决是否立项」作废。
