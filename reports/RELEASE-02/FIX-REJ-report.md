...
> **订正留痕（2026-09-20）**：本节「上呈（仅 1 项，需负责人裁决）」**已被裁决覆盖** —— SD-18（`工程控制/RELEASE-02/ACCEPTANCE.md:117`）：负责人 2026-09-18 裁决「抖动最外圈边缘可能出现 n=1,2,3，**这部分数据最终本身就是要丢弃的，直接走保守路径不排异，直接加权积分就行**」⇒ 裁决 =「**低 n 走保守（不排异+加权积分）**」；落地 = `astrocs_adaptive_pixel` 低 n 档 `n≤3→none`、移除生产路径逐像素先验计算、方法保留为显式 opt-in。**选项 A / 选项 B 的二选一不再需要**（旧文 =「n=2 档的 prior 来源选择…选项 A（推荐）…选项 B…」；原两项正文保留为历史留痕）。
