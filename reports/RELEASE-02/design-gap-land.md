...
| **DG-A-02 的 D-01 澄清点** | **已裁决**（§9.49 定案 2 / §9.62；订正 2026-09-20，V5 分片 3） | §3.5 已按 D-01 选项 A 口径落「语义目标、不作门禁」；claim 侧「帧间一致性」措辞订正 = §9.49 定案 2 已裁决（原「待裁决」作废），claim 侧按 §9.62 口径订正 |

## 3 doc 侧条目（本轮落地情况）

| 台账条目 | 状态 | 说明 |
|---|---|---|
| **DG-B-S3-024**（`UNIFIED_SCIENCE_MODEL.md:122` UNRESOLVED 同步关闭） | **已裁决**（§9.40 C3 / §9.55 S7；订正 2026-09-20，V5 分片 3） | 以「**已关闭**」为准（§9.55 S7：两份 FROZEN 文档打架 ⇒ 以「已关闭」为准、订正另一份）；剩余 = **纯文档落地**（改法：按 `FIX-SCI-SNR-CANON-001` §3.1 改写为「已关闭 + 纯加性」并登记 `OPEN-P2S-02` 仅剩数据面/schema）。原状态「**未落地**…收口令下达时尚未执行」保留为历史留痕 |
...
| **doc 侧 20 条 `needs_claim=true`** | **仅登记** | 见 claim `DESIGN-GAP-LAND-001` §4；**订正（2026-09-20，V5 分片 3）**：其中 **DG-B-10 属 D-03**（§9.67 定案 5：本期不加堆叠平滑项 + 实验义务）；**DG-B-11/PH-05/PH-06 的 D-04/D-05 已裁决**（§9.49 定案 1 / §9.50 定案 3 / §9.51 Q-C / §9.52），按裁决落地 —— 原「4 条…属 D-03/D-04/D-05 **待裁决**，按硬约束不动」作废 |
...
### 4.2 已裁决（**订正（2026-09-20，V5 分片 3）**：原「按硬约束**不动**（待负责人裁决 D-01..D-10，共 10 条）」作废）—— 仍待负责人裁决：**仅 D-08**

`DG-B-10`（D-03）= §9.67 定案 5（`:2584-2588` 本期**不加**堆叠平滑项 + 必须做实验验证）；`DG-B-11`（D-04）= §9.49 定案 1（检测改**星表引导**；`sdet` 去留 = 前台按 §9.50 授权实测评估）；`DG-B-S2-PH-05`/`PH-06`（D-05）= §9.51 Q-C / §9.52（口径 = PSF 域；孔径默认值/`mode` 去留 = 前台按 §9.50 授权定案）。另有 D-01/02/06/07/09/10 的**附加决策点**已分别由 §9.49 定案 2 + §9.62（D-01/D-10）、§9.50 定案 7 + §9.60 + §9.67 定案 4（D-02）、§9.67「前台须同时执行」4（D-06）、§9.55 S5（D-07）、§9.50 补传 + §9.67 定案 7（D-09）裁决。**仍待负责人裁决 = D-08**（`uncertainty_available` / W2 协方差 API；§9.66 B 同族）。
