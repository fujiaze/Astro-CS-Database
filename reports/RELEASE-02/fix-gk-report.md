...
> ⚠ **已取代/订正横幅（2026-09-20）**：本报告 §0 的**施加式**（`corrected_k(x) = raw_k(x) − δ_k(x)`，保留公共参考面 `B_ref`，多退少补）**与 2026-09-20 负责人 §9.67 定案 1 一致**，且已成为现行生产默认（`additive_mode` 默认 `"delta"`，`module_adapters.cpp:6129-6130`；`sky_plane.enabled` 缺省 = `delta_wanted`）。**但须同时知悉两点**：① §10.2 所述「生产公式是 `raw − C_k − δ_k`」是**改动前**的历史状态（双扣除已废）；② 本报告一度被 §9.54 裁决 1 / §9.55 S8（前台裁决：**保留 C、去掉 δ** ⇒ `raw − C`）取代，而**该前台裁决已被更晚的 §9.67 定案 1 推翻**（§9.54 的接缝判据经复核**退化**：`工程控制/RELEASE-02/GAP_AUDIT.md:2551-2554`）⇒ 现行口径回到本报告的「保留 `B_ref`、只扣 `δ_k`」。**本报告作为历史审计轨迹保留，不物理删除。**
...
2. **`C_k` 与 `B_ref` 的关系**：生产公式是 `raw − C_k − δ_k`。裁决文字只写了 `raw − δ_k`。本次按「`p2_upm_calibrate_block` 语义不动」保留 `−C_k`。若负责人意图是连 `C_k` 一并去掉，请明确（那会改变 W2 UPM 校正语义，超出本任务）。 **订正（2026-09-20）**：本问**已被回答** —— §9.67 定案 1（`工程控制/RELEASE-02/GAP_AUDIT.md:2548-2555`）：**保留公共面 `B_ref`、去掉 `C_k` 全减** ⇒ `calibrated_k = raw_k − δ_k`，`raw − C_k` 不再是默认；现行实现 `additive_mode` 默认 `"delta"`（`module_adapters.cpp:6129-6130`，2026-09-20 复核）。
