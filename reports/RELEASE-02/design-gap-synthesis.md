...
- **需变更 claim**：**是**　|　**优先级**：**P0**　|　**状态**：② 待负责人裁决（是否切 sdet：7–12 人日 + 全链重验收，见 D-04）
...
- **需变更 claim**：**是**　|　**优先级**：**P0**　|　**状态**：① 已裁决待落地（A4/A6 定案后转录）
...
- **需变更 claim**：**是**　|　**优先级**：**P1**　|　**状态**：② 待负责人裁决（口径定案，见 D-05）
...
- **需变更 claim**：**是**　|　**优先级**：**P1**　|　**状态**：② 待负责人裁决（见 D-05）
...
### D-01 「帧间一致性」到底是语义目标还是门禁判据？（claim §3.6 vs §9.49 定案 2）
...
### D-02 `F_ref` 是否改用「固定星等」定义（完全帧间独立）？
...
- **选项 B**：保留「组内公共 `F_ref`」⇒ 需先修 Phase1 写侧使组内取同一 `F_ref`，且「组」概念与定案 2 存在张力。
- **后果**：A 一次性消除「参考星选择」假信号（实测跨度 2.14×）并去掉「组」；B 需改实现且保留一个与帧间独立原则冲突的概念。

### D-03 UPM 拟合目标是否本期就加「堆叠平滑项」？
...
### D-04 是否把生产检测器切到 `sdet`（§9.48 ② 方案 A）？
...
- **选项 A（推荐）**：**先实现星表引导，再评估是否切 sdet**。星表引导下两检测器的差异只体现在「星表位置上的拟合质量/完备性」，可比性与决策依据都更强，且避免重复全链重验收。
...
### D-05 A6 `F_instr` 口径定案 + PH-05/PH-06 的孔径与 `mode` 默认
...
- **选项 B**：立项 **D2 PSF 加权最优提取**（更严拟合质量剔除 / 去混叠）以冲 ≤0.03 mag。
- **后果**：A 立即可落地且诚实；B 增加工期，但不保证在拥挤场可达（报告已注明「两种口径都未达」）。

### D-06 `p1_snr.json` 的 `frame_snr` 深度容器是否改名？
...
| 类别 | 条数 | 说明 |
|---|---|---|
| **② 待负责人裁决**（**D 节，跨条目的决策项**） | **10** | D-01..D-10；其中台账条目 `status` 直接标 ② 的为 4 条（`DG-B-10`、`DG-B-11`、`DG-B-S2-PH-05`、`DG-B-S2-PH-06`），其余 6 项（D-01/02/03/04/06/07/08/09/10 中除上述覆盖者）落在标 ①/③ 的条目上作为**附加决策点** |
| **③ 审计发现但未定案**（**E 节 11 条**） | **41** | 台账条目 `status` 标 ③ 的条数；E 节为其中的**决策级归纳**（11 项） |
| **① 已裁决待落地** | **35** | 台账条目 `status` 标 ① |
| **④ 已订正/已裁决/已撤销（仅登记）** | **6** | `DG-B-III-01`..`06` |
| **合计** | **86** | 35 + 4 + 41 + 6 = 86（`status` 口径） |
