...
> 订正（2026-09-20）：第 3 项已裁决（负责人裁决「全部验收通过后再改」，该行自述；DC-718 / 控制包 P0-15）⇒ 从本节「需要上呈」移出；第 1、2、4 项保留（不在已裁决清单）。
...
3. **§12 版本纪律 vs `ci/check_version.py` 的版本一致性门** —— 已裁决（订正 2026-09-20；旧文 =「需裁决版本纪律的生效时点与 CI 检查方向」）：负责人裁决「全部验收通过后再改」（DC-718；控制包已登记 P0-15）⇒ 生效时点 = 全部验收通过后；CI 检查方向按此执行，不再是待上呈项。
