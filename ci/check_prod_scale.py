...
判据（fail-closed）：
  S1 台账 ci/ledgers/prod_scale_params.json#params 声明关键参数的生产尺度
...
豁免唯一途径：ci/ledgers/prod_scale_params.json#entries 的
'prod_scale_exception:<param>:<file>' 条目（kind/reason/owner/exit_condition 缺字段即 rc=2）。

用法：
  python3 ci/check_prod_scale.py [--repo ROOT] [--json-out F] [--self-test]
...
LEDGER = "ci/ledgers/prod_scale_params.json"
