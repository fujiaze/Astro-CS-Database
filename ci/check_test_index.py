#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ci/check_test_index.py — Python 测试聚合判据门 (RELEASE-02 CI-HYGIENE)。
...
退出码:
  ...
  **本门只判"聚合标注是否诚实/自洽", 不重复判套件执行面** —— 套件执行由
  ci/checks.json 的 CHK-UNIT UT-* 步骤负责; 两者互补: UT-* 判红执行, 本门
  保证聚合面不会把红标成绿。若聚合 verdict=FAIL/ENV_FAIL, 摘要会显式给出。

用法:
  python3 ci/check_test_index.py
  python3 ci/check_test_index.py --csv tests/test_index.csv
  python3 ci/check_test_index.py --self-test      # tempfile 夹具正/负例
...
def run(csv_path: str) -> int:
    if not os.path.isfile(csv_path):
        print(json.dumps({"tool": "ci/check_test_index.py", "csv": csv_path,
