...
判据（登记表 = ci/spec_named_impls.json；每条含 spec/named/kind/owner/exit_condition）
...
已登记缺口（ci/ledgers/spec_named_impl_gaps.json，非豁免）
  ...
  ledger_stale 判红（豁免必须存活，只减不增）。本门禁**不写** ci/exemptions.json。

用法
  python3 ci/check_spec_named_impl.py [--repo ROOT] [--manifest F] [--ledger F]
...
MANIFEST = "ci/spec_named_impls.json"
LEDGER = "ci/ledgers/spec_named_impl_gaps.json"
