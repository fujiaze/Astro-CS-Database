#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ci/run_checks.py — 新规范机器检查入口（控制包任务 CI-001）。

权威依据
  - docs/ci/01_CHECKS.md §1（ci/checks.json 唯一注册表；ci/ 提供确定性执行器）、
...
本入口的合同（与 ci/run.py 并存：工作流暂仍调用 ci/run.py，QA-001 之后再改绑定）
  1. 选择：--all（注册表全量，受 --profile 限定；默认 fast，与 ci/run.py 的
  ...
  4. 机器可读 JSON（--json-out，原子写）：含 run 元数据、注册表 sha256、
     ...
     <run_root>/checks/<step-id>.json（与 ci/run.py 同布局，供聚合型
...
用法
  ...
  python3 ci/run_checks.py --all --json-out run/PROJECT-GOVERNANCE-01/CI-001/logs/ci_result.json
  python3 ci/run_checks.py --check CHK-MODULE-MANIFEST --check UT-API
  python3 ci/run_checks.py --all --profile linux-main --plan-only
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ci/run_checks.py",
        description="AstroCS 新规范机器检查入口（CI-001）：确定性执行 ci/checks.json。")
    ...
    p.add_argument("--registry", default=None, help="注册表路径覆盖（默认 ci/checks.json）")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.run_root:
            ...
            # 被 ci/run.py 派发（聚合项 command = run_checks.py --check <ID>）时继承
        ...
        print(f"ci/run_checks.py: {exc}", file=sys.stderr)
