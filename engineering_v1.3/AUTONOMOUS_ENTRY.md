...
2. 查找最新且结构有效的 `AstroCS_Recovery_Development_Pack*.zip`，解压并将包根目录保留为 `engineering_v1.3/`。
3. 运行：

   `python engineering_v1.3/tools/migrate_from_v12.py --repo . --new-root engineering_v1.3`

   该步骤必须迁移 `engineering_v1.2/` 的任务状态、证据与检查点，不得把已完成任务重置为 TODO。若 P11-004 为 DEFERRED/BLOCKED/FAILED，则恢复为待执行修复任务。
4. 运行：

   `python engineering_v1.3/tools/validate_pack.py engineering_v1.3`
