...
> doc_id: STD-REG-001
> 文档活动分类：以 `docs/DOCUMENT_INDEX.yaml` 登记为准（由 `tools/doccheck/check_doc_index.py` 现场校验；本表不自证状态，依 `ASTROCS_DESIGN.md` §0.2/§11）
...
2. **禁止按实现反推标准**（原 05 号 findings 登记册 `STD-F6`「国际标准冻结注册表缺失」处置条款；
   该登记册已随旧控制包世代清理删除，其跨域治理偏差定义自 2026-09-16 起迁入本注册表 §3.2）：
...
> owner 裁决 WCS-003（2026-09-09，原载 `docs/archive/HANDOVER.md:63`，已提回活动登记面）：
> 恢复 `1e-4 px` 冻结门，实码以 41×41/81×81 + 迭代反演达成（本机实测 3.3e-10~2.9e-9 px），
> 不再宣称数学不可达。原归档行原文见 `docs/archive/HANDOVER.md:63`（WCS-002 F1 选 B：
> AP/BP 布局扩展 + 消费方迭代反演，保留高畸变 fixture）。
...
- 偏差闭环（修复完成）后：删除基线/豁免条目、把清单行状态升为 `CONFORMANT`、
  本表行状态改为 `CLOSED`（不得直接删行，保留追溯）；旧 05 号登记册已删除，
  同步面改指 §3.2（跨域治理）与域偏差表。
...
| 偏差 ID | 严重度 | 指针 | 处置归属 |
|---|---|---|---|
| STD-F6 | 已闭环（治理级/P1） | docs/standards/STANDARDS_REGISTRY.md（本注册表 §1.2/§3.1）；工程控制/AstroCS_CONSTITUTION_ALIGNMENT_CONTROL_V1_20260909/05_FINDINGS_REGISTER_20260911.md（已删除，仅历史溯源） | STD-REG-001（本注册表建立即闭环；依据 = 原 05 号 findings 登记册 §STD-F6「国际标准冻结注册表缺失」处置面，2026-09-16 由 git 历史逐字取证迁移） |
...
- 旧 05 号 findings 登记册 `STD-F6`（「国际标准冻结注册表缺失」）的处置面已由本文件落地；
  其跨域治理偏差**定义**迁入 §3.2（2026-09-16，CI-003-B34；依据 = `git show a861d8f6^:<旧登记册>` :71-82 原文）。
- ROOT-008 迁移后引用刷新（2026-09-16，CI-003-B34）：D.fits 的证据指针
  `modules/services/io/include/astrocs/io/fits_stream_v1.h` → `lib/infrastructure/aio/io/include/astrocs/io/fits_stream_v1.h`
  （依据 = 该迁移的 staged rename 记录；旧路径工作树已不存在，刷新后指针重新可解析）。
...
## 附：状态字段口径（DOC-204，2026-09-20）

> 依据：`ASTROCS_DESIGN.md` §0.2（登记表/映射表不得写状态字段）、§11（状态必须现场计算）。

- 本表**不写**交付状态阶梯（§11.4 的 `CONTRACT_READY`/`IMPLEMENTED`/…）——那是模块/交付物的状态，由 `tools/quality/check_module_map.py` 现场计算；原文的 `doc_status` 自证行已删除，文档活动分类一律以 `docs/DOCUMENT_INDEX.yaml` + `tools/doccheck/check_doc_index.py` 为准。
...
- 若后续要求连这两列也改为「检查器现场计算」，须先改 `docs/standards/checks/check_standards_registry.py` 的 C4/C6 判据并同步 `ci/checks.json` 的 STD-REG 项——属门禁改造，不在本任务文件域内。
