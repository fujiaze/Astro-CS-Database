# Astro Celestial Sphere Database（ACSD） C ABI Standard

> 上游：ASTROCS_DESIGN.md §8.4（模块与 ABI）

- 异常保留在 C 边界函数内的 try/catch 全包裹中（如 p2_upm_open）。
- 输出在失败时重置：`*out_model = nullptr` 语义由实现保证；调用方的依据 =
  已完整初始化的对象。
...
- 错误返回：0=success，非 0=hard error；recoverable science status 经
  输出状态字段表达；rc 语义单一（0=success，非 0=hard error）。
