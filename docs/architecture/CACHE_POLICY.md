...
| Drizzle geometry cache | 8192 (LRU, lib/algorithms/drizzle/healpix_drizzle/spherical_overlap.h:303 TargetGeomCache(capacity=8192) 默认, 侵入式双向链表 LRU + unordered_map 索引, 命中路径 O(1)) | target ipix (TargetPixelGeometry {center,boundary4}) | 每次 drizzleTiled run 起始 clear()，随模型/NSIDE 重建 | 线程私有 (thread_local 仅本线程, 命中/未命中 hits()/misses() 可观测) |
