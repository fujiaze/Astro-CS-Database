...
| Drizzle geometry cache | 8192 (LRU, lib/algorithms/drizzle/healpix_drizzle/spherical_overlap.h:303 TargetGeomCache(capacity=8192) 默认, deque lru + unordered_map 实现) | target ipix (TargetPixelGeometry {center,boundary4}) | 每次 drizzleTiled run 起始 clear()，随模型/NSIDE 重建 | 线程私有 (thread_local 禁止跨线程共享, 命中/未命中 hits()/misses() 可观测) |
