...
- Phase2 sampler：`lib/phase2/src/sampler.cpp:604` 默认串行（`lib/phase2/CMakeLists.txt:18` `option(P2_ENABLE_OPENMP OFF)` hard-disable，未链 `libgomp`，code: `Phase2: OpenMP disabled (hotfix default, serial sampler)`）；需 `-DP2_ENABLE_OPENMP=ON` 显式开启后才 `parallel for` cells（tile 读 `critical(aio_read)` 串行化，其余 patch/统计/catalogue 查询并行；`rejected_*` atomic；`cells` 预分配按 `c*64+off` 确定性索引）。
