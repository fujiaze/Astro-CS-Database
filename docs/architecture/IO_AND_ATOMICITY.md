...
- ⚠ **待修缺口登记（R13，未闭合）——HiPS tile 非原子**：
  ...
  **闭合归属 = FIX-206**；**闭合判据** = tile 走「临时区 → 校验 → 哈希 → 原子改名」且负例可红。
...
## 非生产 / 诊断接口登记（DOC-202 R02，2026-09-20）

最高设计 §9 把 aio 定为**文件级唯一 I/O 边界**，并**穷举**允许的文件读写点。
下列符号是穷举点之外的「块 ↔ 文件」读写面，**已降级为非生产 / 诊断接口**
（代码侧归属声明见 `lib/infrastructure/aio/include/aio_pipeline.h:205-247`，FIX-201 降级登记）：
...
- 符号签名登记面 = `docs/architecture/api_inventory.csv`（该表**只登记函数签名**，不含生产性分级）；
  **生产性分级以本表为准**；
- **删除或降级 + 登记** 是 FIX-201 的验收项；本表是文档侧登记面（**不得**据本表推断它们可用于生产）。
