...
| ~~M42 T2 16 帧（叠加试标定 200 B/px，**未采纳**）~~ **该行已作废** | — | — | — | `t2_16f_after` 用的是**错通带曲线**（53 点 / 420–524 nm，整幅测光刻度错 ×2.457905）；"未采纳"的依据（与 `t2_16f_fixA` 的 8769/11820 DATASUM 差异）是**跨二进制+跨曲线**对照，已被 P1-CONCURRENCY-CALIB-01 证伪（§4.1） |
...
**裁决（P1-PARALLEL-AXIS-REDESIGN-01，2026-09-23）：`K = num_threads`（派生，不再硬钉 2）。**
