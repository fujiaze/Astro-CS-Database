...
| sparse_snr_layer | 帧内稀疏控制点上的**绝对** SNR（控制点值 = 该点的通量型信噪比 `F_ref/σ_F`，与帧级 SNR 同口径、同逐帧参考通量 `F_ref`，无量纲；Phase1 标准层，**默认稀疏路径要求默认产出**）。与 `frame_snr` **相互独立**：控制点值是绝对量本身，Phase2 由控制点**直接重建**为稠密 SNR 场（`SNR(x,y)` 由重建算子给出），**不得**乘/除帧级标量做还原；权重换算与帧级一致：`w(x,y)=SNR(x,y)²/F_ref²` | 帧内精细参考；Phase2 由它重建稠密 SNR |
