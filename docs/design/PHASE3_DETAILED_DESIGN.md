...
**足迹内有 tile 像素**（见 §8）；未覆盖处出现有限值只能是幻影数据。
...
必然出现非有限黑边，这是设计允许的几何（§8）。**禁用**「覆盖 > 0 ⇒ 必须有限」作为判红
条件：它把「画幅大于足迹」这一合法几何误判成缺陷。负例（判据判别力）：全画幅导出时非有限
...
内部空洞规则。**禁用** `finite_fraction != 1.0` 作为判红条件：它把 §8 允许的边界黑边判成缺陷，与 §8 冲突。
