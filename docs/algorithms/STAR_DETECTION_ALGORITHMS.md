...
- F1 合成高斯星场（已知中心/流量/FWHM/SNR）: 亚像素质心 |Δc|≤0.3 px
  ...
  **300 次/档**；`SNR_peak = A_fit/σ_bg`，σ_bg 为未平滑原图行差分背景噪声 RMS）：
  ...
  **两行的定义差别（不可互换）**：50% 过渡点 = 检出概率恰为 0.5 的信噪比；
  99% 召回阈 = 使实测召回达到 **300/300** 的最小信噪比格点。**召回域由 99% 行
  定义**，50% 行只定位过渡带中心。**样本量为何是 300**：零失败下
  Clopper-Pearson 95% 下界 = `0.05^(1/n)`，要 ≥0.99 须 `n ≥ ln0.05/ln0.99 =
  298.07`；`n = 64` 零失败只给 0.954 下界，**不足以确立 ≥99%**，其标定值系统性
  偏乐观（`σ_psf` = 1.5 / 2.5 px 各低 1 个信噪比单位，`σ_psf` = 1.0 px 低 8 个）。
  档间 `σ_psf` 取相邻两档中较严者（较大阈）；`σ_psf` 超出 `[1.0, 3.0] px`
  未标定，**不得**引用本表。
  **单 seed 稳定性（判据可复现性）**：以上表为域界，对 F1 召回场换 24 个噪声 seed
  重跑，域内召回逐个 seed 均为 100%（77/77），**判据判决翻转率 0/24**；以 64 次/档
  旧标定为域界时翻转率 3/16 = 18.75% ⇒ 样本量是该判据可复现的必要条件。

  **单参数闭式（趋势模型，非验收判据）**：`SNR_peak ≥ κ₉₉·(1 + σ_smooth²/σ_psf²)`，
  `σ_smooth = 2.0 px`（:1772-1774 常量）。**单参数 κ 拟合不了六档**：含
  `σ_psf = 1.0 px` 时 `κ₉₉ = 7.75 ± 1.56`，逐档残差
  −28.2% / +12.4% / +7.7% / −3.1% / +15.6% / +12.0%（最大 28.2%）；剔除该档后
  `κ₉₉ = 7.15 ± 0.51`（`σ_psf ∈ [1.27, 3.0] px` 五档），残差
  +3.6% / −0.8% / −10.7% / +6.5% / +3.2%（最大 10.7%）。幂次形式
  ...
  **适用域**：本表在 `σ_psf ∈ [1.0, 3.0] px` 且峰值对齐像素中心时成立。
  `σ_psf = 1.0 px` 档是**唯一不服从单参数标度**的档（`κ₉₉ = 10.8` vs 其余五档
  6.7–8.0），其过渡带最宽：检出概率在 `SNR_peak ∈ [48, 100]` 上始终 ≥0.99 但
  在 58–64 处仍只有 0.9933，300/300 仅在 `SNR_peak ≥ 54` 上出现 ⇒ 该档 99% 阈的
  不确定度最大，**必须**用表值、**禁止**用闭式内插，且引用该档阈时应同时报出
  其不确定度。`σ_psf < 0.8 px` 时离散采样使过渡区进一步展宽（实测 50% 点 90 vs
