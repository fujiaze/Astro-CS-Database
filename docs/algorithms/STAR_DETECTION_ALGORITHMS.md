...
- F1 合成高斯星场（已知中心/流量/FWHM/SNR）: 亚像素质心 |Δc|≤0.3 px
  ...
  **完整性（召回）门按 PSF 宽度分档冻结**，判据 = 逐档 99% 召回阈表（本仓实测，
  ...
  64 次/档；`SNR_peak = A_fit/σ_bg`，σ_bg 为未平滑原图行差分背景噪声 RMS）：
  ...
  **两行的定义差别（不可互换）**：50% 过渡点 = 检出概率恰为 0.5 的信噪比；
  99% 召回阈 = 使实测召回达到 1.00 的最小信噪比格点（64 次/档下 ≥0.99 与
  =1.00 等价）。**召回域由 99% 行定义**，50% 行只定位过渡带中心。档间 `σ_psf`
  取相邻两档中较严者（较大阈）；`σ_psf` 超出 `[1.0, 3.0] px` 未标定，
  **不得**引用本表。

  **单参数闭式（趋势模型，非验收判据）**：`SNR_peak ≥ κ₉₉·(1 + σ_smooth²/σ_psf²)`，
  `σ_smooth = 2.0 px`（:1772-1774 常量），`κ₉₉ = 7.33 ± 1.10`（由 99% 行反解，
  六档）。逐档残差（σ_psf = 1.0 / 1.27 / 1.5 / 2.0 / 2.5 / 3.0 px）：
  −20.4% / +6.2% / +7.1% / −8.4% / +20.1% / +5.8%，最大 20.4%；改幂次形式
  `SNR_peak ≥ 6.09·(1 + σ_smooth²/σ_psf²)^1.194` 后最大残差 12.9%，仍超 10%。
  **单参数闭式不足以定义召回域**，验收一律用上表；闭式仅作档间趋势判断，
  其内插结果**不得**低于相邻两档的较严者。50% 行的单参数式为
  `κ₅₀ = 6.22 ± 0.50`（逐档残差 ≤12.4%）。

  **适用域**：本表在 `σ_psf ∈ [1.0, 3.0] px` 且峰值对齐像素中心时成立。
  `σ_psf = 1.0 px` 档过渡带最宽（99% 阈 / 50% 点 = 1.48），单参数闭式在该档
  偏低 20.4%，**必须**用表值。`σ_psf < 0.8 px` 时离散采样使过渡区进一步展宽
  （实测 50% 点 90 vs 连续式给 36.3），**不适用**。亚像素相位未标定；
  随机相位输入**不得**直接引用本表。
  **判据非退化要求（AGENTS.md §5）**：F1 的召回场**必须**包含落在过渡带内的真星
  ——过渡带 = `10 ≤ SNR_peak < 99% 召回阈(σ_psf)`——且**必须**同时报告过渡带
  负例的召回率，该召回率**必须**低于 99%（证明召回度量非恒真）。过渡带真星
  **必须**覆盖六档中至少四档且总数 ≥8。仅当 `snr10` 档真星数与 `snr20` 档
  不同（即场中存在 `10 ≤ SNR_peak < 20` 的真星）时，该门才算行使了其声明域；
  在 `σ_psf = 1.0 px`、`SNR_peak = 10` 处召回实测 0%。
