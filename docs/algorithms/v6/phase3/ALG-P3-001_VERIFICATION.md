...
- 数值 Oracle（`eng/tools/alg_p3_oracle.py`）与合同门（`eng/tools/alg_p3_gate.py`）、规格门（`eng/tools/alg_p3_spec_check.py`）、
  文档一致性门（`eng/tools/alg_p3_doc_check.py`）均为审查侧独立构造，**不 import / 不链接 / 不执行任何生产实现**
