...
| `eng/tools/verify_alg_p1_001.py` | 独立 Oracle + 结构一致性检查 + 负向 mutation 门（可复跑） |
...
- 验证：`python3 eng/tools/verify_alg_p1_001.py --selftest` rc=0（38/38）；`--all-mutations` rc=0（39/39 CAUGHT）。
...
python3 eng/tools/verify_alg_p1_001.py --selftest        # rc=0, 38/38
python3 eng/tools/verify_alg_p1_001.py --all-mutations   # rc=0, 39/39 CAUGHT
python3 eng/tools/verify_alg_p1_001.py --mutation M-D1    # rc=2 (门红)
```
