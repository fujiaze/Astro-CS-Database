...
机器：`eng/contracts/proposals/v6/data/astrocs.v6.provenance.v1.schema.json`（`provenance.v1`）；正例 `examples/provenance.example.json`。
...
| `weight_mode_version` | 权重模式版本 | `FZ-FIELD-WEIGHTMODE` |（已按 §9.73 A44 作废：该概念不存在）
