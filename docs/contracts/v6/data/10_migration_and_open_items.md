...
上位锚：PROJECT_SPEC §11（迁移原则）；宪章 §1.2/§14.5/§16.2；CONTROLLER_LOG C-004.1..6；SCI-ADJ-001 §7/§8。
...
|---|---|---|---|---|---|
| MIG-WEIGHTMODE-LEGACY | `ASTROCS_WEIGHT_MODE` 整数 {0,1,2}；DATA_SEMANTICS §30.3；PUBLIC_API:1165；ACR {auto,ivar,equal,support_x_snr2} | 显式字符串枚举 {point_information,surface_gls,psfsw_robust} + 基线 {equal,pixel_ivar} | 整数 0 一律 REJECT；1→equal、2→pixel_ivar 仅作基线对照；ACR 旧枚举标 ARCHIVED | W6 | ADJ-S1；ADJ-AR-01；`FZ-FIELD-WEIGHTMODE` |
...
| MIG-PSFSW-VOCAB | SCI-PSFW 词表 `weight_kind/weight_units/normalization.scope` 与 SCI-P2 词表 `weight.kind/weight.units/group_normalized` | W6 归一为单一 schema（本提案两词表并存互映，不发明第三套） | 归一前自创第三套词表 → REJECT | W6 | ADJ-C004-03；ADJ-S1 |
...
| ID | 事项 | Owner | 为何不在本任务 |
|---|---|---|---|
| DI-01 | schema 词表单一化（两既有词表 → 单一） | SCHEMA-INTEGRATE-001（W6） | C-004.3 明确归 W6；本任务只给映射建议 |
...
| DI-07 | `weight_units` 字面量 `"1"`(冻结) vs `dimensionless_relative`(SCI-P2 R5) 的 W6 双射落定 | SCHEMA-INTEGRATE-001（W6） | C-004.3 归 W6；本设计只提供别名可表示与结构门 |
...
| 来源 | 词表 | 本设计的可表示性 |
|---|---|---|
| `docs/algorithms/v6/phase2-psfsw/PSFSW_ALGORITHM_SPEC.md` §7.4 | `weight_kind="relative_dimensionless"`、`weight_units="1"`、`normalization.scope="group"`+`median_target=1.0` | `psfsw.v1.weight.kind_alias_sci_psfw` / `units_alias_sci_psfw` / `normalization.scope`+`median_target` |
...
**唯一需 W6 落定的字面量差**：`weight_units` 的冻结值是 `"1"`（`FZ-UNIT-PSFSW`/`FZ-FIELD-PSFSW-UNIT`），而 `SCI-P2-001` 权重来源门 R5 的字面量是 `dimensionless_relative`。
本设计取 canonical `"1"` 并把 `dimensionless_relative` 放入 `units_alias_sci_psfw` 别名字段，使两套既有词表在 W6 归一前**互相可认**（`C25-dual-vocabulary` 结构门；不发明第三套）。
