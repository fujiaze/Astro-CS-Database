...
- 并发语义: 四函数纯函数无状态（0 处 thread/mutex/omp/全局可变
  ...
  §10）。roundtrip 冻结容差 <1e-8 px（SCI-P3-001 §7，禁放宽；FIX-406 由 1e-6 收紧，生产注册表 p3_wcs.cpp）。
  该紧门的**适用域** = `scale ≥ min_scale_arcsec = 0.9″/px`（低于下限时门不适用、报「超出适用域」而非判红，
  退回全域保守门 1e-6 px）；机器可读判定 = `p3_wcs_roundtrip_gate()`（GATES §3 G-P1-WCS-BRIDGE / -GLOBAL）。
