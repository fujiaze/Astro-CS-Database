...
- **反例串的去向（2026-09-23 订正）**：`"bader r"`/`"bader v"` 原登记为「最高设计 §3.3 滤镜名反例行」，该行已随 **RELEASE-04 根文档整体替换**（2026-09-21）**从设计文档删除**（现全篇不含 `bader`）⇒ 引用语义失效、**不存在可重锚的设计文档行**；现行唯一权威锚点 = 本文件 §10 负例表（`eng/packaging/config/filters.json#lookup.non_key_examples[].where` 已按 GATE-502 指向它）。两个串仍登记为 `lookup.non_key_examples`（`kind=design_doc_negative_example`、`resolution=ERROR`，值与判据未变）——它们不是合法库键，也不能靠归一化变成合法键（`bader` ≠ `Baader`；且库内不存在 Baader V 曲线）。`non_key_examples` 只把反例与合法值分开，**不**把反例升级为别名。正反例与门见 §10。CFG002-ANCHOR: item3-filter-name-policy → eng/packaging/config/config_registry.json
...
- 正反例由门 CFG002-04 在每个消费滤镜的 phase（normalize/mosaic）上**双向**复跑：反例必须被拒、正例必须通过；`non_key_examples` 的 `where` 锚点（现行 = 本文件 §10 负例表所在行；2026-09-23 订正：原引最高设计 §3.3 反例行，该行已随 RELEASE-04 根文档替换删除 ⇒ 引用语义失效，不重锚）必须真的含该串。
