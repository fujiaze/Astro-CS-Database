...
| ~~DATA-OBJ-PSFSW-ROBUST-WEIGHT-001~~ **已退役** | ~~psfsw_robust_weight~~：**对象已真删**（负责人 2026-09-20 裁决 B，14→13；GAP_AUDIT §4.5 C01；变更 claim `CHG-2026-09-20-PSFSW-RETIRE`）。canonical schema 与 example 已删除；旧产品若声明该对象 ⇒ **显式拒绝 + 迁移提示** | — | — | — | — | — | ~~unique（canonical owner = contracts/schemas/unified/psfsw_robust_weight.schema.json）~~ **文件已删除** | 不适用（退役留痕；不得据此实现或验收） |
