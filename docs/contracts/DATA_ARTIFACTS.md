...
| DATA-P1-PHOTPROV-001 | Phase1 测光 provenance sidecar(p1_phot.json; B2-A14 关闭伪造 PHOTAPPL) | 无标量(标签+比例) | n/a | 无量纲 | n/a | 缺文件=未应用测光(允许显式 ADU 降级); schema≠DATA-P1-PHOTPROV-001 / photscal 非有限或≤0 → drizzle DATA 拒绝; 禁硬编码 PHOTAPPL=1 | unique(p1_op_photometry 原子写) | JSON |
