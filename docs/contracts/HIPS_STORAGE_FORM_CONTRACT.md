...
| N8 | `.hips` / `.hips.zst` 中缀只用于 HiPS 产品；非 HiPS 产物（如 Phase3 平面 FITS）的落盘名取自其产品族命名 |
...
- **F0** 缺省 / 留空 ⇒ 解析结果必须是登记的默认值，**且**必须存在一条点名该键的 warn 事件；显式声明 ⇒ warn 面为空。两者缺一即判红（静默取默认与误报 warn 都是故障）。
...
- `archive_sha256` 的用途限于容器面；产品身份 = `tree_hash`（取解压后内容，§5）。
...
- `tree` 记录**解压后内容**的条目（与裸形态相同），`tree_hash` 与 §5 同口径；形态事实的落点 = 本段与产品级索引（HiPS `properties` 的字段面保持科学属性，§6.1）。
