...
| CHK-SPARSE-PUNCH-PROBE | 合同 | 上项的**可执行面**：把**生产头**编译成探针（被测代码 = 上线代码）后跑真实系统调用 —— 合成 FITS（NaN 边距 + 连续全零带 + DATASUM/CHECKSUM）与真实 Astro Celestial Sphere Database（ACSD） 瓦片（signal/support）打洞后：整文件 `sha256` 与 `st_size` 不变、`st_blocks` 下降、astropy（memmap/非 memmap）与 **cfitsio 交叉读器**逐 HDU header 卡片与数据区原始字节全等、DATASUM/CHECKSUM 自洽、跨洞边界 `pread` 全等、洞起点块对齐、**NaN 带不被任何洞覆盖**；**负例红**：对 NaN 区与对非零数据区强制打洞必须判 `VERIFY_MISMATCH`；**降级**：卷不支持（ramfs 实测 `EOPNOTSUPP` / 注入等价）⇒ `rc=UNSUPPORTED`、`released_bytes=0`、文件逐字节与分配均不变、不阻断 | `python3 eng/tools/quality/check_sparse_punch.py --probe-run --workdir run/ci/sparse-punch/probe --json-out run/ci/sparse-punch/probe.json` | P0 |
...
| CHK-ACTIONS-LOCK | 治理 | GitHub Actions 依赖锁：`eng/ci/actions.lock.json` 里每个 action tag 必须钉**完整 40 位 SHA**，且 `.github/workflows/*.yml` 中每条 `uses:` 引用必须与锁文件条目**精确匹配**（tag/branch 等可移动引用、锁外 SHA、workflow 面为空一律判红）。离线步做「锁文件结构校验 + workflow↔lock 交叉核对」（读真实产品，不触网）；**在线步真连 GitHub API 复验 tag→SHA**（`--allow-network-skip` 下网络不可达记 `SKIPPED(waivable)` 而非 PASS，CI 有网时必须给出真判定）。含 `--selftest` 6 例（tag 引用 / 锁外 SHA / 空 workflow 面 / 无 action 引用 / 正例×2） | `python3 eng/ci/verify_actions_lock.py --offline --workflow-dir .github/workflows` | P0 |
| CHK-WORKFLOW-GOVERNANCE | 治理 | workflow 面治理门（S2-A：登记与承载）：每个 `.github/workflows/*.yml` 必须**有注册承载**（被某注册项的 inputs / optional_inputs / command 读）、必须有顶层 `permissions`（不得全局写权限）、每个 job 必须有 `timeout-minutes`；`fatduck-admin.yml`（权限最重的执行通道：workflow_dispatch 输入在 self-hosted 节点以 pwsh 执行任意命令）额外要求只允许 `workflow_dispatch` 触发、`runs-on` 含 `self-hosted`+`fatduck-realdata`、`cmd` 输入 `required: true`、且不得声明任何写权限。扫描面为空判红；含 `--selftest` 7 例 | `python3 eng/ci/check_workflow_governance.py --json-out run/ci/workflow-governance/workflow_governance.json` | P0 |
...
| CHK-PRODUCT-CONTRACT | 合同 | 产品级 schema 符合性门（DATA-002 交换对象 + DATA-001 manifest + V6 幂次冻结）：T1 = 语料中每份交换对象过 `phase_product_exchange.schema.json` 且 role↔type_id↔schema_version 与 `artifact_types.registry.json`/`phase_product_exchange_matrix.json` 一致；T2 = `product_content.planes` 必须**显式声明语义面**（逐平面 plane_id/units/dtype/invalid_policy；落盘 science 平面必须逐个被声明，**未声明即红**）；T3 = 逐平面 units ⇒ pixel_area_power 必须等于 `FZ-BUNIT-SEMANTICS` 冻结默认（面亮度 signal −2 / variance −4 / ivar +4），且 variance = signal²、ivar = 1/variance；legacy 单标量 `pixel_area_power` 不得与任一已声明平面矛盾（**幂次不符即红**）。fail-closed；负例 = 删必需键 / 语义面声明改错 / 幂次不一致 / 落盘产品树缺交换对象文档 | `python3 eng/ci/check_product_contract.py --json-out run/ci/product-contract/report.json` | P0 |
| CHK-CITE-CLAIM-MARK-CONSISTENCY | 文档一致性 | 同一文献条目的「已核实类标记」（`[CR]`/`[DOI]`/`[ADS]`/`[arXiv]`/`[page]` 或已核对类判词）与「未打开原文／从未核实／未核到／待核对／`[UNVERIFIED]`」声明**不得并存**；文献键 = 首作者姓氏 + 年份（兼容「Da Costa」「Surname et al.」「A & B YYYY」）；归因按**子句 + 最近优先**，行内代码里的标记同样检出；判词必须带**两侧 `文件:行`**。豁免逐行：代码围栏内文本、声明侧本身；负面清单/未收录节的**标题不豁免其正文** | `python3 eng/ci/check_cite_claim_mark_consistency.py --json-out run/ci/cite-claim/report.json` | P0 |
| CHK-DOC-UNVERIFIED-CITE | 文档一致性 | 被**未核实清单**（`docs/science/**` 之外的「未打开原文／从未核实／未核到／待核对／`[UNVERIFIED]`」声明所点名的文献）不得出现在 `docs/science/**` 的**出处列**（表头命中 出处/来源/文献/参考/引用/一手 的列；无表格时取 参考文献/引用/出处/文献 节下的列表行）。判词必须带**两侧 `文件:行`**。豁免逐行：出处列同一单元自带未核实披露、引用落在非出处列；参考文献节的**标题不豁免其正文**。只判「未核实 ↔ 出处列」交叉，不判引用是否写错（DOI/卷页可对得上） | `python3 eng/ci/check_doc_unverified_cite.py --json-out run/ci/doc-unverified-cite/report.json` | P0 |
