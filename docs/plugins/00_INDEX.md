...
三个创新点各自是一个独立实验单元（最高设计 §12.3）：实验报告、固定 seed 的代码、结果与数据见 `实验/photometric-magnitude`（测光星等坐标系）、`实验/absolute-snr`（绝对 SNR 传递链）、`实验/additive-sky-seamless`（加性天光与无接缝）；公式与推导正本见 `docs/science/`、`docs/algorithms/`。
