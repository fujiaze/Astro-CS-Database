...
| **PSF Signal Weight（PSFSW）** | 综合**图像质量权重**：信号总量 × 信号集中度（mean flux，随 FWHM 变小而增大）/（稳健噪声 × 稳健平均背景 M*） | **权重**，额外含分辨率/FWHM 与背景梯度惩罚，不是信噪比 | 仅显式 `psfsw_robust` 口径时使用，四分量独立存储 （已按 §9.73 A44 作废：该概念不存在；权重是阶段二按该天球像素对应帧集合现场算出的派生量） |
...
- **逐帧 `F_ref,k`（不是组内公共常数）**：`F_ref,k = 10^(−0.4·(m_ref − ZP_k))` 由**该帧自己的** `ZP_k` 决定，`m_ref = 6.0`，`reference_flux_scope = frame_independent_fixed_magnitude`（§9.60 `FREF-BASELINE-001`）；公共锚 `F0 = 10^(−0.4·(m_ref − ZP_syn))` 与帧无关，严格恒等 `F_ref,k · k_photo,k = F0`（实测偏差 ≤ 4.3e-4）。
  **旧写法「`F_ref` 为组内公共常数」已作废**（§9.49 定案 2 帧间独立；§9.66 A 变更 claim `WEIGHT-FREF-PERFRAME-001`）：配对性定理只要求**同一帧内** SNR 与 `F_ref` 同源，**不要求跨帧相等**；不同指向/不同光学系统的帧**合法地**有不同 `F_ref,k`（旧的组间 `F_ref` 硬闸门已删除——它曾使纯逆方差权重 rc=2 （已按 §9.73 A44 作废：该概念不存在；权重是阶段二按该天球像素对应帧集合现场算出的派生量））。
...
- HiPS 是数据库：帧产品长期保存、可被任意多次、任意科学目标的叠加消费，因此入库的是客观的未加权 SNR（与具体集成无关的观测量），把"选哪种权重口径"留给 Phase2 （已按 §9.73 A44 作废：该概念不存在；权重是阶段二按该天球像素对应帧集合现场算出的派生量）；
- Phase2 默认 point_information 逆方差叠加；显式选择 `psfsw_robust` 口径时使用 `psfsw_robust` 复合权重（信号/集中度/稳健噪声/稳健背景四分量独立存储，口径显式选择，不自动切换）（已按 §9.73 A44 作废：该概念不存在；权重是阶段二按该天球像素对应帧集合现场算出的派生量）；
...
- **论文核心实验（判据 SP-0）**：同条件比较三条路径重建稠密 SNR 的精度；**不得预设稀疏一定最好**——「何时哪种最优」须由实验回答。（**2026-09-20 订正**：原文「实测同条件帧上帧级标量已最优到 **0.06%**（稀疏层**净亏 3.3%**）」**已作废**——§9.48 更正表判「方向对、数字不成立」：0.063% 是窗口产物（换 2048² 即 4.42%），物理真值下净亏 **0.175%**（小 19 倍）；SP-0 已降为**诊断量**、**不作默认开关**，其否定结论**只在共模场景成立**（§9.46）。）
