...
- 孔径测光：孔径定义、sky 环、误差传播——**仅作显式声明的诊断 / 交叉验证，不是生产口径**（全链只有一个 `flux` 口径 = PSF 拟合域 `flux = 2πA·sx·sy/3`；全链各模块共用这一个口径；PMM 正是孔径口径，**不照搬**）；
...
- **落地状态**：判据**尚未在代码中生效**——本模块须先产出 ① 逐星残差表（`star_id, F_instr, G_Gaia, F_syn, r_i, inlier`）、
  ...
  在 ①–④ 完成前，本模块**的测光一致性门表述 = 尚未生效**；`0.03 mag` **在实现/测试/文档中一律以误差预算推导值为准**。

### 4.2 外部参考实现的引用纪律（PMM / PhotometricMosaic）

- PhotometricMosaic（PMM）**本体的使用面 = 只读方法研究（保持原样）**：① **可**读源码做方法研究；② **代码只读、保持原样**；③ **引用面 = 方法学描述**；④ **分发面 = 无**；
