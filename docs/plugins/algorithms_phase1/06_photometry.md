...
- **职责**：对检测源做 **PSF 拟合域**测光（孔径测光**仅作显式声明的诊断/交叉验证**，非生产口径——**2026-09-20 订正**，依据 `ASTROCS_DESIGN.md:264`「全链只有一个 `flux` 口径」+ §9.51 Q-C），把本帧 signal 映射到统一线性通量尺度 `d = a_k F P + n`，给出 `a_k` 及其不确定度。
...
- 孔径测光：孔径定义、sky 环、误差传播——**仅作显式声明的诊断 / 交叉验证，不是生产口径**（**2026-09-20 订正**：全链只有一个 `flux` 口径 = PSF 拟合域 `flux = 2πA·sx·sy/3`，禁止任何模块另起盒和/振幅口径，依据 §9.51 Q-C / §9.52 + `ASTROCS_DESIGN.md:264`；§9.51 Q-C 明示「A6 不要照搬 PMM 的口径」——PMM 正是孔径口径）；
- PSF 测光：在 PSF 模型下最大似然通量 `F_hat = Q/W`，`Var(F_hat)=1/W`（与噪声/信息权重联动）；
- 光度响应 `a_k`：把本帧 ADU/e⁻ 映射到统一线性通量尺度；颜色项与适用域声明；
- 相对标度不足 → 标记"不可跨帧合并"，不静默降级。

### 4.1 测光一致性判据（**已从误差预算推导**；`0.03 mag` 已作废）
...
- **落地状态**：判据**尚未在代码中生效**——本模块须先产出 ① 逐星残差表（`star_id, F_instr, G_Gaia, F_syn, r_i, inlier`）、
  ② σ_psfsys(frame)（算法见 `run/RELEASE-02/parallel/scripts/07_real_psf_sys.py`）、
  ③ 冻结的误差预算表（9 项 + 出处，作**表**而非散落常数）、④ 能红能绿负例
  （伪造常数残差表 ⇒ 必须判红；把某帧 `F_instr` 乘随机因子 ⇒ 必须判红），并走**变更 claim**。
...
| 字段 | 默认 | 单位 | 说明 |
|---|---|---|---|
| `mode` | `psf` | —— | **生产只允许 `psf`**（PSF 拟合域口径）；`aperture` **仅诊断 / 交叉验证**（须显式声明，不得进生产链——**2026-09-20 订正**，依据 `ASTROCS_DESIGN.md:264` + §9.51 Q-C） |
| `aperture_radius` | ~~2×FWHM~~（**值待定**） | px | 孔径测光半径——**仅诊断 / 交叉验证**。（**2026-09-20 订正**：原默认「2×FWHM」与实现/README 的 `4.0px` / `6-10px` 二选一**冲突**，见 `reports/RELEASE-02/conformance/CONFORM-SWEEP-2.md:124-138`；且孔径不属生产口径，依据 `ASTROCS_DESIGN.md:264` + §9.51 Q-C；`config/config_registry.json` 亦登记为 `finding: "gap"`） |
| `sky_annulus` | —— | px | sky 环——**仅诊断 / 交叉验证**（同上；生产口径的局部背景由 PSF 拟合域承担——**2026-09-20 订正**） |
