...
2. **完整链路实测**：1374 采样点 / 4 帧（每帧 ≥4），星点掩膜覆盖最亮 0.1% 像素 100%，
   掩膜面积占比 1.19%，联合天光面 49 节点、rank 49 = n_nodes、χ²_red 0.771、δ_k 非零。
...
4. **真实数据**（M42 M1 T3 Red 4 帧）：生产天光面 rc=0、rank=49=n_nodes、χ²_red 1.004，
   但条件数 κ = **3.16e7**（近奇异，登记 FIX-3）；真实帧间背景乘性斜率中位 0.995
   （0.806–1.323，分块动态范围仅 ~10 ADU ⇒ 不确定度大）。
5. **收敛枚举缺口（FIX-2）**：生产 `p2_upm_convergence` 只能给 0/1，`max_iter` 与 `stalled`
   不可区分；规范要求的 0/1/2/3 语义未实现。绝对容差 `tolerance=1e-6` 在 ~300 e⁻ 尺度
   300 次迭代仍不收敛（**FIX-1**，见 `docs/science/PHASE2_UPM.md` §16.3）。
