...
- **SNR 层检测与重建（自动，非可选）**：
  - 自动检测输入 HiPS 是否有稀疏 SNR 层；
  - **有** → 实际 SNR = 帧级 × 帧内，参与科学运算；
  - **无** → 帧级 SNR 直接参与；
...
- 稀疏层存在 → 实际 SNR = 帧级 × 帧内（由稀疏控制点插值/重建为稠密，重建算子与误差入 manifest）；
- 无稀疏层 → 帧级 SNR 直接参与（等权重面）；
- 检测与重建是 Phase2 **标准行为**，不是可选开关；
...
- **SNR 层检测**：有稀疏层 / 无稀疏层两条路径输出正确；
- **SNR 重建**：稀疏→稠密重建与帧级铺满重建分别验证；帧级×帧内权重正确；
- 扩展源常量场、梯度、总通量、方差无偏；
