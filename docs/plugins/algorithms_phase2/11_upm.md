...
- **职责**：对重叠区域拟合每帧光度尺度修正（乘法 `g_k`）与**天光亮度平面**（加性 `b_k(x)`）的统一相对模型（UPM），并施加归一化。
- **不是**：不做排异；不做集成；`g_k`（乘法）与 `b_k(x)`（加性）**不得互相代替**；校准参数不确定度必须传播。
...
- **输出**：每帧 `g_k`、天光面参数（参考面系数 + 每帧梯度修正系数）及协方差、gauge 约束、秩/连通性/条件数、加权残差 RMS、拟合质量；施加归一化后的产品。
...
- `g_k` 乘法响应、`b_k(x)` 加性天光面，二者分开拟合、不得互相代替；
- `B_ref(x)`：全部帧联合构建的**参考天光面**；`δ_k(x)`：第 k 帧相对参考面的平缓梯度修正（低自由度）。
...
- gauge 约束（固定参考帧或和约束）固定加性/乘法自由度；报告秩、连通性、条件数、参数协方差。
...
- entrypoint：产品组+掩膜+控制点+天光采样点 → `g_k`、天光面样条系数、协方差、归一化产品；
...
- 构造已知 `g_k` 与已知 `b_k(x)`（常数、平面梯度、平滑曲面）场景 → 参数恢复满足精度，加权残差 RMS 达理论水平；
