...
| `smoothing_lambda` | `0.1`（生产常量 `P2_SMOOTHING_LAMBDA_AUTO`） | —— | **UPM 图平滑权重**（`upm.h:75` 逐字「图平滑权重（默认 0=关闭）」）。**2026-09-20 前台查明：仓库里有三个不同的东西，此前被混为一谈**——① **阻尼 `α≈0.5`** = 迭代阻尼（naive Gauss-Seidel `α=1` 在链式/二部覆盖图上特征值 −1 ⇒ 周期 2 振荡，`ASTROCS_DESIGN.md:459`）；② **本键 `smoothing_lambda`** = 对天光/δ 面**拟合的正则项**（既有机制，**保留**）；③ **堆叠平滑项** = 拟合目标里的**新项**（§9.67 定案 5 本期**不加**，做实验验证）。**三者互不相同、并存不冲突**；此前登记的「两条裁决冲突」**作废**。实现常量 `P2_SMOOTHING_LAMBDA_AUTO = 0.1`（`lib/algorithms/coverage/include/astro/phase2/stage2_common.h:24`）。|
