...
- `tools/docs_machine_consistency.py 9/9` (`config_weight_mode_ivar` （已按 §9.73 A44 作废：键不存在；权重是派生量）, `frame_id_contract_exact`, `error_taxonomy_exit_codes`, `integration_status_full_set`, `rejection_status_full_set`, `stage_ids_docs_vs_orchestrator`, `snr_constants`, `product_contracts`, `drizzle_variance_formula`) + `tools/config_consistency_check.py mismatches=[]`。
