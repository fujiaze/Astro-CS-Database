...
| 项 | 状态 | 依据（当前提交内可核） |
|---|---|---|
| Windows 发布工具链 preset 冻结（VS17 2022 x64 / v143 14.44.35207 / SDK 26100 / CMake 3.31.12 / ACR OFF） | PASS | `CMakePresets.json`（BLD-001）+ `eng/packaging/schemas/preset-contract.json` + `eng/cmake/toolchain/verify_toolchain.py`（合同冻结） |
| 唯一根 CMake 产品构建图（唯一 `project(acsd)`、唯一 `add_executable(acsd)`、无 GLOB） | PASS | `CMakeLists.txt`（BLD-002）静态可核 |
| DLL 边界合同冻结（module_dll_contract schema：instances/forbidden/negative） | PASS | `eng/contracts/config/module_dll_contract.schema.json`（ARC-001） || C ABI v1 冻结（4 纯 C 头：status_codes/host_api/module_api/artifact_api；`ACS_ABI_VERSION_V1=1u`） | PASS | `lib/include/astrocs/abi/*.h`（ABI-001）+ `lib/include/astrocs/common_abi_v1.h` |
| **科学模块安装面（Linux 技术预览：`modules/` + 产品清单 + 安全 loader）** | **`INSTALLED`** | `eng/cmake/install_layout.cmake`:104-105（五科学模块 SHARED 入安装树 + `$ORIGIN` RPATH）；`eng/packaging/astrocs.product.json` units=10；`eng/tests/abi/mod001_install_load_check.py` 本提交实测 **64/64 PASS**（含 5 类负向注入必败）+ `astrocs modules list/verify` units=10 PASS（MOD-001 `59fdeab3`） |
| **Windows DLL 化发布安装树（acsd.exe + runtime/io/模块/provider DLL）Windows 侧复验** | **`NOT_VERIFIED`** | 目标安装树定义于控制包 03 §4；Windows 侧未产出/复验 → 不宣称 Windows 已交付 |
...
| Linux 同源 `acsd` + `.so` 技术预览构建 | `IMPLEMENTED` | 本提交根 `ninja -C build` 实测 rc=0（全量 28 步）+ 安装树加载验证 64/64 PASS |
...
| 项 | 状态 | 依据（当前提交内可核） |
|---|---|---|
| 根 CMake `ASTROCS_ENABLE_ACR` 默认 OFF 且不 add_subdirectory(lib/infrastructure/acr) | PASS | `CMakeLists.txt`：option 默认 OFF；仅 `eng/tests/unit` add_subdirectory；message 显示 ACR 状态 |
...
| 项 | 状态 | 依据 |
|---|---|---|
| 唯一生产 Runtime（`astrocs_core`）：load_pipeline→run→cancel→inspect | PASS（源码在位） | `lib/include/astrocs/core/runtime.h`、`lib/infrastructure/scheduler/src/runtime.cpp`、`cli/runtime_client.cpp` 静态可核 |
...
| 契约 | 路径 | 状态 |
|---|---|---|
| 最高设计（根权威）+ 工程规范 | `ASTROCS_DESIGN.md`、`ENGINEERING_SPEC.md` | ACTIVE_NORMATIVE |
...
| 版本命名空间 | `docs/owner/RELEASE_STATUS.md` §2 | 见 `docs/DOCUMENT_INDEX.yaml` 登记 |
