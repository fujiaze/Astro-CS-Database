...
| 项 | 状态 | 依据（当前提交内可核） |
|---|---|---|
| Windows 发布工具链 preset 冻结（VS17 2022 x64 / v143 14.44.35207 / SDK 26100 / CMake 3.31.12 / ACR OFF） | PASS | `CMakePresets.json`（BLD-001）+ `packaging/schemas/preset-contract.json` + `eng/cmake/toolchain/verify_toolchain.py`（合同冻结） |
...
| **科学模块安装面（Linux 技术预览：`modules/` + 产品清单 + 安全 loader）** | **`INSTALLED`** | `eng/cmake/install_layout.cmake`:104-105（五科学模块 SHARED 入安装树 + `$ORIGIN` RPATH）；`packaging/astrocs.product.json` units=10；`tests/abi/mod001_install_load_check.py` 本提交实测 **64/64 PASS**（含 5 类负向注入必败）+ `astrocs modules list/verify` units=10 PASS（MOD-001 `59fdeab3`） |
...
| 项 | 状态 | 依据（当前提交内可核） |
|---|---|---|
| 根 CMake `ASTROCS_ENABLE_ACR` 默认 OFF 且不 add_subdirectory(lib/infrastructure/acr) | PASS | `CMakeLists.txt`：option 默认 OFF；仅 `tests/unit` add_subdirectory；message 显示 ACR 状态 |
...
| `lib/infrastructure/acr` 保留（源码 + 独立 CMake + tests/qualification/ci） | PASS | `lib/infrastructure/acr/` 目录与 CMakeLists 在位（SA-ACR-13 域） |
