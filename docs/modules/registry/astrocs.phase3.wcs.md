...
| 端口 | DATA | 必/可 | 单位 | 坐标 |
|---|---|---|---|---|
| `props` | `DATA-P3-PROPS` | 必 | `UnitId::DIMENSIONLESS` | `CoordinateFrame::HEALPIX` |
| `wcs_plan` | `DATA-P3-WCS` | 可 | `UnitId::DEGREE` | `CoordinateFrame::ICRS` |
...
`TEST-P3-WCS-001` 对应测试(逐任务 TASK_RESULT 证据); 容差=验收冻结。

## 已知限制

见 docs/KNOWN_LIMITATIONS.md 与 `ALG-P3-002` 合同边界。
