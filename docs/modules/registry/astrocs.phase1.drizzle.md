...
upstream: [SCI-P1-DRIZ-001, ALG-005, API-P1-007]
downstream: [TEST-P1-DRIZ-001]
---
...
由 `API-P1-007` 公共 API 定义(phase session extern "C"); 生命周期 create→validate→
run→inspect→destroy。

## Registry descriptor 与配置 schema

module_id=`astrocs.phase1.drizzle`; execution_class=`cpu_heavy`;
...
`TEST-P1-DRIZ-001` 对应测试(逐任务 TASK_RESULT 证据); 容差=验收冻结。

## 已知限制

见 docs/KNOWN_LIMITATIONS.md 与 `ALG-005` 合同边界。
