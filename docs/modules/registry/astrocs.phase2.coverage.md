...
upstream: [SCI-P2-COV-001, ALG-P2-COV-001, API-P2-001]
downstream: [TEST-P2-COV-001]
---

# 模块 astrocs.phase2.coverage
...
| 端口 | DATA | 必/可 | 单位 | 坐标 |
|---|---|---|---|---|
| `calibrated` | `DATA-P2-CAL` | 必 | `UnitId::ADU` | `CoordinateFrame::PIXEL` |
| `coverage` | `DATA-P2-COV` | 可 | `UnitId::DIMENSIONLESS` | `CoordinateFrame::PIXEL` |
...
module_id=`astrocs.phase2.coverage`; execution_class=`cpu_heavy`;
...
`TEST-P2-COV-001` 对应测试(逐任务 TASK_RESULT 证据); 容差=验收冻结。

## 已知限制

见 docs/KNOWN_LIMITATIONS.md 与 `ALG-P2-COV-001` 合同边界。
