...
upstream: [SCI-P1-COS-001, ALG-P1-COS-001, API-P1-002]
...
| `cleaned` | `DATA-P1-COSMETIC` | 可 | `UnitId::ADU` | `CoordinateFrame::PIXEL` |

invalid = NaN/coverage=0(按 DATA 合同)。

## 公共 header、核心 symbol 与生命周期

由 `API-P1-002` 公共 API 定义(phase session extern "C"); 生命周期 create→validate→
run→inspect→destroy。
...
`TEST-P1-COS-001` 对应测试(逐任务 TASK_RESULT 证据); 容差=验收冻结。

## 已知限制

见 docs/KNOWN_LIMITATIONS.md 与 `ALG-P1-COS-001` 合同边界。
