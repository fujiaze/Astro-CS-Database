...
- 本项目消费的是 XP 谱的**外部定标（绝对）谱辐照度**，单位 **W·m⁻²·nm⁻¹**，用它做**模型通带内的正向合成** `F_syn = ∫F_λ(λ)·T(λ)·Q(λ)·λ dλ`（单位 W·m⁻²·nm；公式权威在 `docs/science/PHOTOMETRY.md` §2a，claim `PHOT-FSYN-CANON-001`）。**`F_syn` 不含 `10^(−0.4·G)` 因子**：Gaia G 星等不进入参考通量（该写法会给逐星 `r_i` 注入 `+0.4·G_i` dex，单标量零点吸收不掉）。
  ...
  - **刻度实证（本仓）**：26211 颗真实 XPSD 星上，用官方 Gaia G 通带（Riello et al. 2021）+ GaiaXPy `Gaia_DR3_Vega` 零点 −26.4899 复算合成星等，`median(m_syn − magG) = −0.0037 mag`、MAD `0.0033 mag`（G∈[6,18] 且解码谱处处为正，n=11272）。证据 `run/SCI-PHOT-FORMULA-01/evidence/a1_xpsd_absolute_check.json`、`a2_bandpass_and_absolute.json`。
  - **区分两种表示**：XP 的**连续表示**（`xp_continuous_mean_spectrum`，基函数系数）是**内部系统**（单位 e⁻·s⁻¹，无物理刻度）；本仓消费的**采样表示**才是外部定标。两者不是同一产品。
  - **不转述、不重推** Gaia 的绝对定标推导本身（本包只做刻度性质的**实证锚定**与出处登记，不重做外定标）；
