...
> **ARCHIVED_NON_NORMATIVE** — GOV-001 归档（2026-09-16）：2026-09-11 时点的项目交接快照。其中「根 ASTROCS_PROJECT_CONSTITUTION.md（冻结宪章）＞ AGENTS.md」的权威声明已失效（该文件由 ROOT-007 删除；现行权威链见 `ASTROCS_DESIGN.md` §0），所引控制包路径亦已删除。保留作历史交接记录，不作为现行权威或必读入口。
