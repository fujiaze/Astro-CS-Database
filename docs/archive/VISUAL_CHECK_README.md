...
> **ARCHIVED_NON_NORMATIVE** — GOV-001 归档（2026-09-16）：HiPS 浏览器人工视觉验收 runbook。原文所引入口 `launch/start_browser.ps1` 与示例产品 `run/phase2/v9/geometry_truth.hips` 均已不存在（ASTROCS_DESIGN §1.3 HiPS Browser 不进产品 manifest、§10.1 Alpha 不含 GUI），故不再作为活动入口；保留内容作历史操作知识。现行入口：`docs/browser/HIPS_BROWSER.md`（若需恢复视觉验收，须另立任务重建 `launch/**` 并逐条 `test -e` 校验路径）。
