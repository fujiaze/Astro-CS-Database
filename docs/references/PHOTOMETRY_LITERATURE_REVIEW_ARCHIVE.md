...
- 标定因子的**绝对值无物理意义**：它把增益/口径/曝光等未知量全部吸收；产物通量以**星等/相对星等**表达，消除物理单位。仪器参数取自直接测量，物理闭合式（如 `k = g·h·c·1e9/(A·t)`）只作诊断对照。
...
- **Phase2 stage2 内部的 `local_snr` / `frame_snr` 是相对质量权重场**（`quality_weight = frame_quality_scalar × local_quality_proxy/median`），无量纲，仅供采样/加权；**不是**科学 SNR，其解释面限于采样/加权（`m_5` 与 `SNR_F` 有各自来源）。PSF 拟合质量代理（FWHM、残差尺度等）只作诊断，**不计入科学叠加权重**。
