...
并强制声明边界：psfsw_robust 的声明面 = "在指定验收数据上优于指定基线"，ivar/Fisher 不在该声明面内；
equal/pixel_ivar 仅为文档基线，科学最优性在其声明面之外；psf_snr_power 本包保持延迟。
...
1. 每个比较单元必须预注册（数据、基线、度量、效应量与 CI），训练/调参样本与验收样本各自独立。
