...
1. **入库量是通量型未加权原始 SNR**：帧级 `SNR=F_signal/σ_F`，`σ_F⁻²=ΣP_i²/σ_i²`，`σ_i²=(sky+dark+RN²+F·P_i)/g²`（Horne 1986, PASP 98, 609, DOI 10.1086/131801）。天光只进噪声项、不进信号项；MC 真值在 26+29 个扫描点上全部 ≤3σ（max|z|=2.68）。
