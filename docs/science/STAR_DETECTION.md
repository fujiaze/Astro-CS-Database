...
- completeness / false positive (synthetic fields): 完备性与虚警由合成星场
  ...
  **1000 次/档**；`SNR_peak = A_fit/σ_bg`，定义见 `GATES_AND_TOLERANCES.md` §2；
  `σ_smooth = 2.0 px` 是检测所用高斯平滑核宽度，`sdet_api.cpp:1772-1774`
  常量实参）：
  ...
  **两行不可互换**：50% 过渡点 = 检出概率恰为 0.5 的信噪比；99% 召回阈 = 使实测
  召回在 **1000 次/档**下达到 **1000/1000** 的最小信噪比格点。**召回域由 99% 行
  定义**，50% 行只定位过渡带中心。**样本量**：零失败下 Clopper-Pearson 95% 下界 =
  `0.05^(1/n)`，要 ≥0.99 须 `n ≥ ln0.05/ln0.99 = 298.07`；`n = 64` 零失败只给
  0.954 下界，**不足以确立 ≥99%**，其标定值系统性偏乐观。本表取 `n = 1000`
  （零失败下界 0.99701）。档间 `σ_psf` 取相邻两档中较严者。
  **⚠ σ_psf = 1.0 px 档只给区间 [52, 65]，不给点值**：1000 次/档复测确认其零失败
  性质**非单调**（零失败点 65 / 66 / 71 / 74 / 76 之间夹有 999/1000、998/1000、
  997/1000），「54.0」只是 300 次/档下首个恰好零失败的格点、不是稳定阈，已撤销。
  区间下端 52 = 使实测召回自该点起恒 ≥0.99 的最小信噪比；上端 65 = 首次出现零失败
  的格点。**引用该档阈必须同时报出区间**，**取值一律走区间标定**。
  **据此，`SNR_peak ≥ 10` 单独不构成召回域**：σ_psf = 1.0 px（FWHM 2.35 px）时
  `SNR_peak = 10` 的召回为 **0%**，该档 99% 阈在 **52–65** 之间。
  **判据可复现性**：以上表为域界换 24 个噪声 seed 重跑，域内召回逐个 seed 均
  100%，判决翻转率 **0/24**。
  **单参数闭式（趋势模型，非验收判据）**：`SNR_peak ≥ κ₉₉·(1 + σ_smooth²/σ_psf²)`；
  **单参数 κ 在六档上不成立**——含 σ_psf = 1.0 px 时 `κ₉₉ = 7.79 ± 1.34`（最大残差
  25.1%），剔除该档后 `κ₉₉ = 7.27 ± 0.45`（**最大残差 9.2%**，仅 `σ_psf ∈ [1.27,
  3.0] px` 可用）；幂次形式在六档上最大残差仍 >10% ⇒ **单参数闭式不足以定义召回
  域**，验收一律用上表。50% 行的单参数式为 `κ₅₀ = 6.21 ± 0.50`。推导与残差表见
  ALG-STARDET-001 §11.4 F1。
  **适用域**：本表在 `σ_psf ∈ [1.0, 3.0] px`（FWHM 2.35–7.06 px）且峰值对齐
  像素中心时成立；σ_psf = 1.0 px 档是唯一不服从单参数标度的档——其
  `σ_smooth = 2.0 px` 比该档点扩散尺度**还宽**，检出由平滑主导、不由点扩散主导，
  **本来就是另一个物理区制**；该档按区间标定、**取值一律走区间表**。
  `σ_psf < 0.8 px` 时离散采样使过渡区
  进一步展宽（实测 50% 点 `SNR_peak` = 90 vs 连续式给 36.3），**不适用**。
  亚像素相位未标定，本表的引用面 = 已标定相位的输入。虚警以纯噪声场（无注入星）
  计数，单位 1/千像素；实测 0.000/千像素（8 帧 256×256）——该 0 值是判据下界，
  判据的鉴别力由负例注入（§4 状态码负例 + 本页 §3 F2）与过渡带负例承担。
  **判据非退化要求**：召回场**必须**在**每一档**都有域内真星；**必须**包含落在
  过渡带内（`10 ≤ SNR_peak <` 99% 召回阈）的真星，总数 ≥8 且覆盖 ≥4 个 σ 档；
  **必须**同时报告过渡带负例的召回率，该召回率**必须**低于 99%。
...
- saturation / blend / edge: 饱和判定=3×3 邻域双条件
  ...
  **测法**：同帧生产盲检测输出取被标 `saturated` 的星；对每颗取 7×7 窗，
  ≥6 像元落在峰值 0.5% 内者判为真平台（n=36），平台电平取窗内平台像元的中位。
  **负对照**：94 颗被标饱和星中 58 颗为尖峰形（不满足平台判据）⇒ 平台判据有鉴别力；
  且 44.4% 的平台电平 > 65535 ⇒ 常数 65535 作饱和参考电平必须判红。
  ⇒ `saturated` 列在 float 定标帧上**必须**按「`A_fit > min(max(img),65535) − median(img)`」
  这一 Project-defined 判据消费，**与探测器饱和真值分属两个量**。
...
- **两套 `sigma_bg` 不可互换**：盲检测阈值用 `bgnoise`（行差分族）；
  ...
  **测法**：同一帧分别跑两套估计器（互不调用）。同帧实测：13.8148 vs 20.7384 ADU，
  比值 1.5012。**负对照**：两套 σ 若可互换，同帧比值必须为 1；实测 1.5012 ≠ 1
