...
- completeness / false positive (synthetic fields): 完备性与虚警由合成星场
  ...
  64 次/档；`SNR_peak = A_fit/σ_bg`，定义见 `GATES_AND_TOLERANCES.md` §2；
  ...
  | 50% 过渡点 `SNR_peak` | 31.1 | 21.4 | 17.7 | 14.2 | 9.4 | 8.3 |
  | **99% 召回阈 `SNR_peak`** | **46.0** | **24.0** | **19.0** | **16.0** | **10.0** | **10.0** |

  **两行不可互换**：50% 过渡点 = 检出概率恰为 0.5 的信噪比；99% 召回阈 = 使实测
  召回达到 1.00 的最小信噪比格点（64 次/档下 ≥0.99 与 =1.00 等价）。**召回域由
  99% 行定义**，50% 行只定位过渡带中心。档间 `σ_psf` 取相邻两档中较严者。
  **据此，`SNR_peak ≥ 10` 单独不构成召回域**：σ_psf = 1.0 px（FWHM 2.35 px）时
  `SNR_peak = 10` 的召回为 **0%**，该档 99% 阈高达 **46.0**。
  **单参数闭式（趋势模型，非验收判据）**：`SNR_peak ≥ κ₉₉·(1 + σ_smooth²/σ_psf²)`，
  `κ₉₉ = 7.33 ± 1.10`（由 99% 行反解六档），逐档残差最大 20.4%；幂次形式
  `6.09·(1 + σ_smooth²/σ_psf²)^1.194` 最大残差 12.9%，仍超 10% ⇒
  **单参数闭式不足以定义召回域**，验收一律用上表。50% 行的单参数式为
  `κ₅₀ = 6.22 ± 0.50`。推导与残差表见 ALG-STARDET-001 §11.4 F1。
  **适用域**：本表在 `σ_psf ∈ [1.0, 3.0] px`（FWHM 2.35–7.06 px）且峰值对齐
  像素中心时成立；`σ_psf < 0.8 px` 时离散采样使过渡区进一步展宽（实测 50% 点
  `SNR_peak` = 90 vs 连续式给 36.3），**不适用**。亚像素相位未标定，随机相位
  输入**不得**直接引用本表。虚警以纯噪声场（无注入星）计数，单位 1/千像素；
  实测 0.000/千像素（8 帧 256×256）——该 0 值是判据下界，判据的鉴别力由负例
  注入（§4 状态码负例 + 本页 §3 F2）与过渡带负例承担。
  **判据非退化要求**：召回场**必须**包含落在过渡带内（`10 ≤ SNR_peak <`
  99% 召回阈）的真星，覆盖六档中至少四档且总数 ≥8；**必须**同时报告过渡带负例
  的召回率，该召回率**必须**低于 99%。
