...
- completeness / false positive (synthetic fields): 完备性与虚警由合成星场
  ...
  **300 次/档**；`SNR_peak = A_fit/σ_bg`，定义见 `GATES_AND_TOLERANCES.md` §2；
  ...
  | **99% 召回阈 `SNR_peak`** | **54.0** | **24.0** | **20.0** | **16.0** | **11.0** | **10.0** |

  **两行不可互换**：50% 过渡点 = 检出概率恰为 0.5 的信噪比；99% 召回阈 = 使实测
  召回达到 **300/300** 的最小信噪比格点。**召回域由 99% 行定义**，50% 行只定位
  过渡带中心。**样本量为何是 300**：零失败下 Clopper-Pearson 95% 下界 =
  `0.05^(1/n)`，要 ≥0.99 须 `n ≥ ln0.05/ln0.99 = 298.07`；`n = 64` 零失败只给
  0.954 下界，**不足以确立 ≥99%**，其标定值系统性偏乐观。档间 `σ_psf` 取相邻
  两档中较严者。**据此，`SNR_peak ≥ 10` 单独不构成召回域**：σ_psf = 1.0 px
  （FWHM 2.35 px）时 `SNR_peak = 10` 的召回为 **0%**，该档 99% 阈高达 **54.0**。
  **单 seed 稳定性**：以上表为域界换 24 个噪声 seed 重跑，域内召回逐个 seed 均
  100%（77/77），判决翻转率 **0/24**。
  **单参数闭式（趋势模型，非验收判据）**：`SNR_peak ≥ κ₉₉·(1 + σ_smooth²/σ_psf²)`；
  **单参数 κ 拟合不了六档**——含 σ_psf = 1.0 px 时 `κ₉₉ = 7.75 ± 1.56`（最大残差
  28.2%），剔除该档后 `κ₉₉ = 7.15 ± 0.51`（最大残差 10.7%）；幂次形式在六档上
  最大残差仍 >10% ⇒ **单参数闭式不足以定义召回域**，验收一律用上表。50% 行的
  单参数式为 `κ₅₀ = 6.21 ± 0.50`。推导与残差表见 ALG-STARDET-001 §11.4 F1。
  **适用域**：本表在 `σ_psf ∈ [1.0, 3.0] px`（FWHM 2.35–7.06 px）且峰值对齐
  像素中心时成立；σ_psf = 1.0 px 档是唯一不服从单参数标度的档（`κ₉₉ = 10.8`），
  其检出概率在 `SNR_peak ∈ [48, 100]` 上始终 ≥0.99 但 58–64 处仍只有 0.9933，
  该档阈不确定度最大、**禁止**用闭式内插。`σ_psf < 0.8 px` 时离散采样使过渡区
