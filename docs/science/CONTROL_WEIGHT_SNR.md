...
- **权重所用的方差必须含源项（正向约束）**：§8c 定权式 `w = SNR²/F_ref² = 1/σ_F²` 中的
  ...
  **最优加权的唯一来源 = 含源光子散粒项的 `σ_F²`**。两个方差面的定义与构成见
...
- **链路接通（正向约束）**：阶段二**必须**由 PSF 信号 SNR 定权
  `w(x,y) = SNR(x,y)²/F_ref² = 1/σ_F(x,y)²`；该定权式的来源 = PSF 信号 SNR（空背景方差的倒数属另一面），
  权重来源面 = 该单一式（三条按数据可用性自动排序的路径属重建路径，§7 不可接受变化）。
...
- **质量权重场定位**：本文件 §4 的 `quality_weight`（`local_snr`/`frame_snr`）为**相对质量
  权重场**，**无量纲**，仅供采样与诊断（不构成积分权重）；其解释面限于质量权重本身（`m_5` 与 `SNR_F` 是另两个对象）。
- **Phase1 产品面帧级 SNR（同名不同物）**：Phase1 HiPS 文件头的 `frame_snr` 是**科学量**（点源 PSF 信号 SNR，纯信号/噪声，§0 注记），不属于本文件的相对质量权重场；本文件「帧级科学基准 = `m_5`」不改变其定义，二者各自具名、互不指代。
...
- `g` 的单位是 **e⁻/ADU**，换算 `ADU = N_e/g`（冻结约定见 `docs/science/NOISE_MODEL.md` §5c）。文献里同一关系的倒数形式 `I = O + g'·N_e + N_read` 中 `g' = 1/g`，两种写法各自具名（混用使源项差 `g²` 倍）。
...
- **「缓变」只能用于方差图（正向约束）**：缓变的是**方差图** `sigma_slow^2(x,y)`，**不是**噪声实现。噪声**实现**在空间上是白的（实测一阶差分 lag-1 自相关 −0.4985，白噪声解析值 −1/2），同一帧方差图的相对散布为 2.0%。规范里只允许写"**方差图缓变**"；**"噪声缓变"与"方差降为逐帧一个标量"均不属本规范措辞**（例如给噪声实现加相关核即属前者）。
- **有效域与诚实边界（正向约束）**：本模型的有效域由"源稀疏"先验划定。延展发射（星云本体）不在点源张成空间内——真实星云结构帧上源项只捕获 **14.2%**、逐像素相对误差中位 0.81 ⇒ 该域**必须**显式声明模型不适用；该域的成因声明 = 模型适用域边界（数值精度不构成解释）。端到端误差由**分子侧**（检测完备性与 PSF 尺度）主导：盲检测消融下逐像素相对误差中位 **0.376**（判红），而分母侧方差模型仍成立。
- **判据必须逐臂书写（正向约束）**：真值无源（`S_src ≡ 0`）的归零判据**必须**逐臂给出；判据书写面 = 逐臂（"各臂收敛到同一结果"不属判据）：
  ...
  "各臂必须收敛"类判据在两臂上恒真、在一臂上恒假，**不携带信息**；证据资格 = 空。
- **判据非退化（正向约束）**：归零判据**必须**配**有真值效应**的对照臂（同一判据在有源帧上实测 37.31 ≫ 0.05 判红），否则该判据无证据资格。
- **跨帧可比与定权（正向约束）**：逐像素 `SNR` 接入 §8c 的定权式 `w = SNR²/F_ref²` 时，`F_ref` **必须**与 `SNR` 处于**同一参考轮廓/孔径/背景域**（§2a），配对性只要求同一帧内同源；本节的量与 §4 的 `quality_weight`（相对质量权重场）各自具名。
...
3. **跨帧可比性硬约束**：逐帧 `F_ref,k`、同帧配对、`m_ref=6.0`（`m_ref` 为**记录参考
   ...
   归一互相抵消，变更 `m_ref` 只改记账口径不改权重。订正: A-P4-06）；`SNR_combined²=ΣSNR_k²` 相对偏差 2.2e-16，Q/W 信息量 `Var=1/ΣW` 实测 1275 vs 解析 1260（+1.2%）；逆方差组合严格优于等权与 `w∝SNR`。
