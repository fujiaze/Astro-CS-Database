...
- **稠密重建必须带亮度（正向约束）**：把稀疏控制点重建为稠密 SNR 场时，重建量随**源的亮度**
  变化（源项 ∝ `S`），故重建算子必须能表达"**方差图**缓变、信号强度各异"造成的**极不均匀** SNR 分布；
...
- 推导与文献锚：逐源最优提取 `SNR_F = F/σ_F` 见 Horne 1986, PASP 98, 609；帧级 5σ 点源深度见
  Tonry et al. 2012, ApJ 750, 99 / Huang et al. 2017, ApJ 838, 110 / Ivezić et al. 2019, ApJ 873, 111（同列于 §9）。

## 2b 逐像素绝对 SNR 的重建模型（正向规范）

本节是创新点二（`ASTROCS_DESIGN.md` §2.2）在**逐像素**粒度上的规范形式。它与 §2a 的逐源口径同源、粒度不同，二者**不得互相替代**。

**观测量分解（正向约束）**：校准后像素测量值（ADU 标度）**必须**按物理来源写成三项之和，**不得**再合并成"总亮度"项：

```text
I(x,y)            = S_src(x,y) + S_sky(x,y) + N_local(x,y)      [ADU]
sigma_slow^2(x,y) = Var[N_local] + S_sky(x,y)/g                 [ADU^2]  缓变面（方差图）
sigma_w^2(x,y)    = sigma_slow^2(x,y) + S_src(x,y)/g            [ADU^2]  加权面
SNR(x,y)          = S_src(x,y) / sigma_w(x,y)                   [1]      分子只有源
```

- `S_src`：源信号电平（空间**快变**，随 PSF 尺度与源亮度变化）；`S_sky`：加性天光电平（空间**缓变**）；`N_local`：本地噪声**实现**（读出/量化/暗流/偏置残差，零均值）。
- `g` 的单位是 **e⁻/ADU**，换算 `ADU = N_e/g`（冻结约定见 `docs/science/NOISE_MODEL.md` §5c）。文献里同一关系的倒数形式 `I = O + g'·N_e + N_read` 中 `g' = 1/g`，**禁止**混用（混用使源项差 `g²` 倍）。
- **两个方差面**：`sigma_slow^2` 是**背景方差面**（`docs/science/NOISE_MODEL.md` §5：空背景随机分量，含天光散粒、暗流散粒、读出、量化、偏置残差），`sigma_w^2` 是**加权方差面**（同文件 §5c）。两者量纲相同（ADU²）、用途不同，**不得互相替代**；本链**不得**再引入第三个方差面。
- **`sigma_slow^2` 的载体（正向约束）**：缓变面由**稀疏控制点 + 重建**给出、逐像素现场求值（生产 `snr_noise_model_v1` / `snr_noise_model_v1_fill`：8×8 patch 稳健方差 → 平面场；重建路径按 §8c 记录 `snr_path_effective`）。控制点**必须**先通过 `docs/science/NOISE_MODEL.md` §5d 的自校准有效性判据，被空间结构污染的 patch **不得**进入拟合。
- **`SNR` 的分子只有源（正向约束）**：天光**只**经其散粒噪声 `S_sky/g` 进入分母。**禁止**把 `S_sky` 或观测电平 `I` 放进分子；固定源通量下天光增大 ⇒ `SNR` 单调下降。
- **源的估计式（正向约束）**：`S_src_hat(x,y) = Σ_i F_hat_i·P_i(x,y)`，`F_hat_i` 为**星点测光**给出的源通量估计（生产 GLS 估计量 `astrocs::v6::p1psfw::w_info_solve`，FZ-FORMULA-WINFO/Q），`P_i` 为**生产 PSF 轮廓**（`snr_moffat4_profile_f64`，Moffat4）。**禁止**用"观测减去天光残差"（`I − S_sky_hat`）或任何残差相减式代替该式；`S_src/g` **必须**是 `sigma_w^2` 与 `sigma_slow^2` 的唯一区别项。
- **分离成立依赖三条支撑先验（正向约束）**：单帧只有均值与方差两个可观测量，源与天光在单帧上不可分解（秩与零空间论证见 `docs/science/NOISE_MODEL.md` §5b），故分离只能靠先验，三条**必须同时**成立，缺一即本模型不适用：
  ...
  3. **排异分层**：天光采样点**必须**落在源掩膜之外（生产 `p2_star_mask_caps` / `p2_star_mask_contains`、`p2_sky_patch_estimate`、`p2_reject_plan_resolve` / `p2_reject_stack`）。
- **「缓变」只能用于方差图（正向约束）**：缓变的是**方差图** `sigma_slow^2(x,y)`，**不是**噪声实现。噪声**实现**在空间上是白的（实测一阶差分 lag-1 自相关 −0.4985，白噪声解析值 −1/2），同一帧方差图的相对散布为 2.0%。规范里只允许写"**方差图缓变**"；**禁止**把"噪声缓变"写成建模前提（例如给噪声实现加相关核），也**禁止**由该措辞把方差降为逐帧一个标量。
- **有效域与诚实边界（正向约束）**：本模型的有效域由"源稀疏"先验划定。延展发射（星云本体）不在点源张成空间内——真实星云结构帧上源项只捕获 **14.2%**、逐像素相对误差中位 0.81 ⇒ 该域**必须**显式声明模型不适用，**不得**以数值精度不足解释。端到端误差由**分子侧**（检测完备性与 PSF 尺度）主导：盲检测消融下逐像素相对误差中位 **0.376**（判红），而分母侧方差模型仍成立。
- **判据必须逐臂书写（正向约束）**：真值无源（`S_src ≡ 0`）的归零判据**必须**逐臂给出，**不得**写成"各臂收敛到同一结果"：
  - 完整臂（`sigma^2 = sigma_slow^2 + S_src_hat/g`）与漏源项臂（`sigma^2 = sigma_slow^2`）在无源帧上**逐位恒等**（实测方差面相对差 0.0），且源项估计归零（实测 `median(|S_src_hat|/sigma_slow) = 0`）；
  - 天光双计臂（`sigma^2 = sigma_slow^2 + (S_src_hat + S_sky_hat)/g`）**不收敛**（实测方差比 1.36802）——该不收敛是**该臂确实双计天光散粒**的正例证据，**不是**归零失败的证据。
  "各臂必须收敛"类判据在两臂上恒真、在一臂上恒假，**不携带信息**，**不得**充当证据。
- **判据非退化（正向约束）**：归零判据**必须**配**有真值效应**的对照臂（同一判据在有源帧上实测 37.31 ≫ 0.05 判红），否则该判据无证据资格。
- **跨帧可比与定权（正向约束）**：逐像素 `SNR` 接入 §8c 的定权式 `w = SNR²/F_ref²` 时，`F_ref` **必须**与 `SNR` 处于**同一参考轮廓/孔径/背景域**（§2a），配对性只要求同一帧内同源；**禁止**把本节的量当作 §4 的 `quality_weight`（相对质量权重场）。
- 本节数值与判据的证据锚在 §8a/§8b（实验单元与结果文件路径由彼处给出）。
...
- **重建量分子纯度不变量**：`SNR` 的分子只含源（`S_src_hat = Σ_i F_hat_i·P_i`），天光只出现在分母；真值无源（`S_src ≡ 0`）时源项估计精确归零（§2b）。
...
- 把天光（`S_sky`）或观测电平（`I`）计入重建量的分子，或用残差相减式（`I − S_sky_hat`）代替 `Σ_i F_hat_i·P_i`（§2b）；
- 把"噪声缓变"当作建模前提（规范口径只允许"**方差图**缓变"，§2b），或由该措辞把方差降为逐帧一个标量；
- 在真值无源（`S_src ≡ 0`）的判据上写"各臂必须收敛"而不逐臂书写（§2b）；
- 把 §4 的 `quality_weight`（无量纲相对质量）当作 §2b 的绝对 `SNR`，或反过来。
