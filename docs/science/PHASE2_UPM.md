...
- **节点间距必须由输入自适应导出（不得是标定常数）**：节点间距是**表示能力的唯一决定量**（上一条），
  故它必须由输入几何**导出**，而不是取一个固定值。导出规则的正向约束：
  **节点间距 ≤ (该产品实际约束帧间改正量的最小尺度) / 2**——对「帧间加性天光差」这一目标量，
  该尺度是**重叠带宽度**与**指向间距**两者中的较小者（它们才是真正约束 `δ_k` 的量），
  由 manifest/输入几何逐产品算出。**禁止**在配置里留一个「默认节点间距」标定值。
- **节点间距必须进入自适应重试回路**：与 `roughness_penalty` **同级**——
  条件数或残差不达门时，除提高粗糙度惩罚外，**必须**允许细化节点重解。
  逐次尝试的节点间距与生效值必须写入 provenance。
- **节点间距与`roughness_penalty` 的分工（正向约束）**：节点间距负责**表示能力**（能不能表示），
  粗糙度惩罚负责**条件数**（解得稳不稳）。**不得**用强平滑掩盖不可表示分量——那会把接缝从「可见台阶」
  变成「被抹平的错误电平」，是更坏的失败形态。
- **κ 上限不得是两个不同的标定值**：同一份数据在不同求解器下被两个常数一放一拦（一个放过、一个拦下）
  说明门控口径本身没有物理依据。门控必须取**实际求解矩阵** `H_solve = H_red + λ·DᵀD` 的条件数，
  且判据按**矩阵谱自身**定（相对 `rank_rtol` 口径），**不得**保留互相矛盾的绝对常数。
- **自适应必须真的会被触发**：门控值若宽到让真实数据永远通过，则自适应等价于不存在。
  每条自适应路径都必须有「触发过」的实测记录，否则视为未实现。
