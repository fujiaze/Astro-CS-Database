...
| `S=0`（patch 内 ≥ 半数像素同值 ⇒ 稳健尺度 0） | **无尺度信息**：`control_ivar` 必须为 0 且 `control_variance` 标为无尺度信息（非有限）；发布面 = 无尺度信息（数值保护量的平方不属方差发布面） | ALG-P2-SMP-001 §5.4（发布口径权威）；`sampler.cpp:864-877` 为待整改点 |
...
1. **多退少补正确**：产品 `calibrated_k = raw_k − δ_k`（**保留 B_ref**）在覆盖子集突变处把
   ...
   全减背景臂（`raw − b_k`）产品中位 0.845 e⁻ ⇒ **退化**；无接缝证据面 = 保留 B_ref 的臂。
...
> 本节补齐 §9a 接缝指标门槛的**推导链**：观测量 → 零假设分布 → 虚警率 → 可检出下限 → 漏检面。
> **本节不改动门槛数值、判据式、默认参数与适用域**：它把既有门槛标定到实测噪声上，并给出它的性质。
> 证据与可复跑脚本：`run/SEAM-DERIV-01/`（报告 `REPORT.md`、证据 `evidence/*.json`、脚本 `tools/*.py`）。
...
- **中位数估计量的标准误**（渐近、正态样本）：`Var(median) = π σ²/(2N)` ⇒
  `se(median) = √(π/2)·σ_seam/√N_e`，**中位数的渐近标准误系数 = √(π/2) = 1.2533141…**。故
      ...
      σ_rel(e) = √(π/2) · MAD(seam)(e) / ( √N_e · bg(e) )        （MAD = 1.4826×中位绝对偏差）
...
| 实测跨边 2.814e-3：@50% / @95% | 0.997% / 1.471% | 0.844% / 1.219% | 0.717% / 1.023% |
  ...
  忽略多重性的**单边预测**（8% / 23% / 47% / 72% / 90%）**系统性偏低，是错误形式**（本报告登记该勘误）。
- **漏检面（必须按此判读；这是本门的适用域边界）**：
  ...
  4. **边界长度覆盖不足**：判据取**中位数**，边界上只有比例 `f` 的采样点承载台阶时 `f < 1/2` 恒有
     `median = 0` ⇒ 对**短于边界一半的局域接缝结构性失明**（判据量是「典型台阶」而非「最大台阶」）；
  5. **适用域之外**：M42 产品 196 条边界里 82 条（足迹外缘 / 贴数据边界）**不进判据**，那里没有无接缝证据。

### 17.4 实测标定与 `1e-2` 的性质

- M42 参考产品（适用域 114 条）实测：`max|rel_step| = 7.2561e-03`，中位 1.938e-3、p90 4.566e-3；
  全 196 条里 5 条超门，**全部**是 `not_interior`（被适用域排除）⇒ 适用域在**数值上**也是必要的：
  没有它，同一合格产品会有 5/196 条判红。

- **最差边 = 门的 72.6%，余量仅 1.378 倍**。三条独立标定一致给出 **`1e-2 ≈ 3.5 σ`**（σ = 实测跨边散布 2.814e-3）。
  ⇒ `1e-2` **不是**一个宽裕的工程裕度，而是「恰好压在一个合格产品的实测散布之上」。
...
- **负例（门的判红与推导预测一致，真实产品实测）**：按 §17.3 的临界值向 M42 产品的目标帧
  （`M42_M6_T2_flying_dutchman-20251212_043619-300S-Red`，2 条适用域边界，`bg_min = 5.0303e-4`，
  预测临界注入幅度 `1.0050e-2 × bg_min = 5.0556e-06`）注入台阶，9 档幅度逐档比对：
  **判决 PASS/FAIL 9/9 与预测一致**，翻转点落在 0.8–0.9 倍临界之间（实测 9.505e-3 判绿 → 1.040e-2 判红），
  `max|rel_step|` 的预测与实测最大相对差 4.5e-3（最低幅度档；来自被注入多边形的足迹与**别的帧边界相交**
  这一二阶效应），在翻转点附近为 1.4e-4。逐条证据见 `run/SEAM-DERIV-01/evidence/critical_injection.json`。

### 17.5 复现

```
python3 eng/tools/e2e/seam_footprint.py --self-test --json-out run/SEAM-DERIV-01/evidence/selftest.json
python3 run/SEAM-DERIV-01/tools/null_distribution.py      # A/B1/B2 三种零假设 + 虚警率
python3 run/SEAM-DERIV-01/tools/null_tail.py              # 超门计数、梯度/台阶判别、分母条件
python3 run/SEAM-DERIV-01/tools/critical_injection.py     # 真实产品上的临界值注入（负例）
python3 run/SEAM-DERIV-01/tools/synthetic_null_detect.py  # 合成零假设虚警率 + 检出率曲线
python3 run/SEAM-DERIV-01/tools/detection_floor.py        # 含多重性的可检出下限
```
