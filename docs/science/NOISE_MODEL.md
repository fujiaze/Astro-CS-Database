...
- **两个方差面（正向约束）**：本链承载**两个用途不同、量纲相同**的方差面，**各自独立、取值互不代用**：
...
- **增益与读出噪声必须进入加权方差面（正向约束）**：`gain`（e⁻/ADU）与 `read_noise_e` 是加权方差面
  的**输入量**，其来源、缺省行为与不可得时的显式降级声明按 §5c；可用状态的判据 = 数值路径实际读取该键，而非把"配置里有这个键、
  但数值路径不读它"当作可用状态。
...
控制点: 合格 patch（样本数 ≥ min_samples **且**通过 §5d① 自校准有效性判据）的 patch variance
空间场: 稳健非负平面拟合 var(x,y) = a + b·x + c·y（§5d②：相对误差加权 + 凸包内恒 ≥ 0）；**预测 ≤ 0 的像素 = 该处方差不可用
         ⇒ variance=0 ∧ ivar=0（§4a 显式不可用，**不得**由 clamp 产生、不得写 NaN）**；
         仅当预测为正且低于 variance_floor 时按 floor 夹逼（floor 是**数值保护**，见 §9）
几何退化/合格控制点不足: 近共线 (λlo/λhi < 1/16) 或样本不足 ⇒ has_spatial_field=0，fill 走全局常量场（**不得**用作平面缺陷的修复，§5d②）
...
ivar = 1 / max(variance, floor)   # 仅对**可用**方差（预测 > 0）：fill 阶段 max(a+b·x+c·y, floor)
                                  ...
                                  # control 点亦 max(patch_var, floor)（仅 §5d① **保留**的控制点）
```
...
**"慢变"的结论面 = 方差图平滑，不含逐帧标量降维**：天光梯度、DSNU、PRNU 的空间结构**都与源无关且可测**。
...
两种约定各自具名（混用会让源项差 `g²` 倍）。单帧只提供均值与方差
两个可观测量，待分离量多于两个 ⇒ **单帧上的"分解"主张不成立**；本链只做**归因**
...
- `S_src` 由**星点测光**给出的源通量与 PSF 轮廓导出；其导出面 = 星点测光（"从信号里减去噪声"不属导出路径）。
...
1. **本帧自估（首选，不依赖任何帧头关键字）**：对帧内像素做 `V = sigma_0^2 + S/g` 的
   ...
   必须显式判为不可得；有效标定的前提 = 杠杆臂足够（弱杠杆臂拟合值属不可得态）。回归前**必须**先扣除偏置，
   否则斜率与截距同时有偏（§5b）。
...
取值面 = 显式降级并具名登记（`g=0` 使源项无定义）；0 属返回值语义，可用状态的前提 = 数值路径实际读取该键。
...
⇒ 本面的设计面 = 自估路径（第 1 条）优先；"从帧头读 `GAIN`" 是可选交叉校验，自估路径是本链在正式数据面上的
**主路径**，其可辨识条件由实验给出。
...
- **控制点有效性（自校准）**：patch 的 MAD-σ 必须量的是**噪声**而不是**空间结构**。判据用 `R = σ_MAD / σ_white-equiv`，
  `σ_white-equiv` = 相邻像素一阶差分的稳健 σ 按 `1/√2` 折算（见 §5b）；**纯噪声的 `R` 理论值为 1**（恒等式，不是标定值）。
  `R` 显著大于 1 ⇒ 该 patch 的「方差」被空间结构污染（如延展星云）；进入平面拟合的 patch = `R` 不显著大于 1 者。判据取 `r = ln R` 的序统计量：
  取**最大**的 `k ∈ [min_keep, n−1]` 使跳变 `r_(k+1) − r_(k)` 超过保留主体自身极差 `r_(k) − r_(1)`，剔除 `R` 最大的 `n−k` 个；
  无此 `k` ⇒ **一个也不剔**；`min_keep = max(⌈n/2⌉, 天空预算 patch 数)`，`n < min_keep + 1` 时判据**不武装**（不剔除，只登记）；
  `R` 不可算（差分样本 < `min_samples`）的 patch **不**参与剔除并计数。阈值面 = 序统计量判据（形如 `R > 3` 的常数阈值不适用）。不变量：**纯噪声输入 ⇒ 不剔除任何 patch**。
- **拟合可行域（结构非负）**：拟合**必须**使平面在**控制点凸包内恒 ≥ 0**（线性函数在凸包上的最小值在顶点取到，
  凸包顶点是控制点子集 ⇒ 「对全部控制点 ≥ 0」⇔「凸包内恒 ≥ 0」）。实现口径 = **相对误差加权** `w_i = (v_med/v_i)²`
  （§5a 的误差预算是**相对**标准误 ⇒ Aitken 最优权重）+ 凸包非负**硬约束**；平面缺陷的处置 = fail-closed（「退回常数场」属几何退化/控制点不足的触发面）。
- **凸包外才是不可用**：仅当像素落在控制点凸包**外**（真正的边缘外推）且预测 ≤ 0 时，才按 §5/§9④
  编码为不可用态（`variance=0 ∧ ivar=0`）。凸包**内**出现大面积预测 ≤ 0 属**拟合缺陷**，
  必须按 §8 登记并 fail-closed；出片面 = fail-closed 登记。
- **可观测量（必须写入 provenance）**：控制点方差的动态范围、平面系数、凸包内预测 ≤ 0 的像素占比、
  被剔除 patch 数与 `R` 的分布。**缺任一项即视为该帧的方差面不可审计。**
- **判据与拟合的机器可检口径（正向约束）**：`R` 不可算与判据不武装都必须**计数登记**（不静默）；剔除后仍须满足 §5a 的天空预算；
  平面求解的路径由权重跨度决定（权重跨度 = 控制点方差动态范围的**平方**，正规方程的条件数是设计矩阵的平方：实测动态范围 1.2e6 的算例上
  正规方程解相对偏差达 40%），**必须**走正交化路径（Householder QR / 零空间参数化）；`hull_nonpositive_frac`（凸包内预测 ≤ 0 的面积占比）
  **必须严格等于 0** 才可审计——`0` 是「结构非负」约束的**定义值**，不是可调阈值；任何 ≠ 0 的取值（含正残差、NaN）一律判**不可审计**并按 §8 fail-closed。
...
- **产品 dtype 成对不变量**：产品面 `(variance, ivar)` 必须落在两态之一——**可用** `variance>0 ∧ isfinite(variance) ∧ ivar=1/variance`（输出 dtype 表示精度内）；**不可用** `variance=0 ∧ ivar=0`。生效 floor 在与产品相同的 dtype 中不可表示时（如按 α² 换算后 `1e-46` 在 float32 下溢为 0，而 `1/floor` 上溢为 `+inf`），该像素取不可用态——**该像素的发布对 = `(variance=0 ∧ ivar=0)`**（`(0, +inf)` 这类自相矛盾的对不在发布面）。消费侧按数组标度消费时，必须把 floor 一并按 α² 换算（§3 量纲；生产消费点 `module_adapters.cpp` 的 `nm_data_scale`）。
- **量纲一致**：`variance` [ADU²] → `ivar` [ADU⁻²] 倒数关系精确，`gain` 模型量纲 `max(signal,0)/gain` [ADU²] 无量纲混。
- **自适应判据不误伤不变量**：真值**纯噪声**输入 ⇒ `n_structure_rejected_patches == 0`（判据不动）；真值含**空间结构**（延展源/强梯度）⇒ 判据必须剔除且拟合改善——两个方向都必须可执行（**能红能绿**，负例见 §11）。
- **凸包内非负不变量**：`hull_nonpositive_frac == 0`（「结构非负」约束的定义值，不是阈值）；凸包**外**才允许预测 ≤ 0 并按 §5/§9④ 编码为不可用态。
- **常量场触发条件不变量**：全局常量场只由**几何退化**（控制点近共线）或**合格控制点不足**触发；平面缺陷（凸包内预测 ≤ 0）的处置 = fail-closed（§5d②）。
...
- **权重归一与适用域**：`ivar` 作为 Phase2 逐像素科学权重直接入加权（归一在消费侧 `Σw/Σ`），适用域=空背景随机分量；不含源泊松项/系统项/协方差（Drizzle 后相关见 UNCERTAINTY_AND_COVARIANCE.md）；`ivar=0` 显式表示不可用；该哨兵语义具名登记（§7 空 support 不传播）。
...
- PixInsight 官方 N* 稳健噪声（.pidoc 式[14][15]）：`N*_MAD=2.48308·MAD(R*)`、`N*_Sn=2.03636·S_n(R*)`（`S_n` 为 Rousseeuw & Croux 1993 尺度估计；常数用 10000 幅 4096² 高斯白噪声 bootstrap 标定）。**ACSD 未采用这两个常数**：本项目用标准正态 MAD 一致化 `1.482602218505602`（§9），与 PI 的 2.48308 定义域不同，**不可互换**。
