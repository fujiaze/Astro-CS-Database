...
> 引用：`工程控制/contracts/wcs_authoritative_pairs.schema.json` v1.0（P11-006 扩展 provenance）
...
| `工程控制/contracts/wcs_authoritative_pairs.schema.json` | properties | 新增可选 `provenance` 对象 | P11-006 |
...
### 规范文档
- `工程控制/docs/05_WCS_COORDINATE_CONVENTION_AND_CLOSURE_SPEC.md`（WCS 坐标约定与闭合性规范）
- `工程控制/docs/24_WCS_VALIDATION_V2_SPEC.md`（WCS 验证架构 v2，A/B/C 三层）
- `lib/plate_solve/IPV_PIPELINE.md`（管线说明，CD 无 1/cos(Dec)）
- `lib/plate_solve/cpp/ipv/SIRIL_COMPARISON.md`（与 Siril 对比）

### 契约
- `工程控制/contracts/wcs_authoritative_pairs.schema.json`（权威星对 schema v1.0，含 provenance 扩展）
- `工程控制/evidence/P11-001/COORDINATE_CONVENTION.md`（v1 FROZEN 前序文档）
