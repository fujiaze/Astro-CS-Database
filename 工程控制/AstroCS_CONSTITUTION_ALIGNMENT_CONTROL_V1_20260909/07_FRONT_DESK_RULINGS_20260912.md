...
## 6. rev6.2 追加裁决（2026-09-12，负责人指令）

> 负责人指令原文：「首先，禁用调 CP 框架；其次，维护工作树给所有子代理，前台负责合并。
> 会有热点文件的情况其他提取约束，或者合并为一个 agent 执行。」

### R-18｜停用 cprun（CP）框架作为调度通道

- **裁决**：不再使用 `cp register/dispatch/review/status` 调度任务。所有任务**直接以 SubAgent 派发**，
  前台以**简单台账**（`run/pack_state/WORK_LEDGER.json`）跟踪状态。
- **已在跑的 CP 子代理**：允许其把在制工作做完（避免丢弃真实劳动），但其产出统一由前台验收合并；
  不再对其执行 `cp review`。
- **控制包文档**（`control-pack.json`/`TASK_LEDGER.csv`）保留为**规格与任务定义的事实源**，
  但不再作为调度状态机。
- **依据**：负责人直接指令（高于工程层约定）。

### R-19｜共享工作树 + 前台统一合并

- **裁决**：全部 SubAgent 在**同一工作树** `/workspace/Astro CS Database` 直接作业，
  不建影子树/clone/worktree。**提交与 push 由前台统一执行**。
- SubAgent **禁止** git add / commit / push / stash / reset / clean / checkout。
- 前台义务：逐任务独立机器验收 → 精确暂存（`git add -- <该任务文件>`）→ 原子提交 → push →
  `fetch` 后核对三 SHA。**预存 dirty 永不收编**。
- **理由**：影子树会掩盖跨任务真实冲突；共享工作树 + 文件级写者约束 + 前台合并，
  才能在保证串行化正确性的同时获得真实并行度（宪章 §14.5 的约束对象是**文件写入**）。

### R-20｜热点文件三重处置规则

- **裁决**：
  1. **热点文件设单一写者**：`ci/checks.json`、`ci/checks.schema.json` 单写者=**前台**（R-15 沿用）；
     `.github/workflows/` 在当前阶段归**前台或单一合并代理**。
  2. **同域任务合并为一个 agent 串行执行**：当多个任务的写域落在**同一热点目录**时，
     合并为**一个 SubAgent** 按顺序做完（避免多人同写）。已按此合并 `CI-BACKEND-001` +
     `UT-CLI-MAINT` → 单一「CI 平台域合并代理」。
  3. **文件级单一写者**：其余并行任务必须把写域精确到**文件**（而非目录），
     由前台在派发消息中逐一点名，冲突时**以派发消息为准**（覆盖规格中的目录级白名单）。
- **依据**：负责人指令；宪章 §14.5；FD-R6-005/FD-R6-006。
- **实例**：`tests/unit/p1wcs/` 原同时归 `WCS-PATH-001` 与 `STD-F1-ADJ`，
  已裁定 `p1wcs_astropy_cross.py` 单归 WCS-PATH-001、其余归 STD-F1-ADJ。
