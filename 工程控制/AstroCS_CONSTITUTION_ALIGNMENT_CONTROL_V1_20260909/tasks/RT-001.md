# RT-001｜接入唯一Executor和实测资源门

## 目标
模块不自建计算线程池；Runtime 唯一 executor；按负责人冻结阈值基于实测 active threads/CPU/queue/progress fail-closed。

## 依赖
`P1-001`、`P2-002`、`P3-002`。执行时 BASE_SHA 必须为最新 main，若依赖提交后基线变化须重取。
