# CI-001｜收紧CI注册表与资源fail-closed

## 目标
WIN-BUILD/TEST/PACKAGE 与 P0/P1/manifest/hash/real-final 不可 waiver；候选缺失必须失败；监控必须调用 evaluate。

## 依赖
`CLI-001`。执行时 BASE_SHA 必须为最新 main，若依赖提交后基线变化须重取。
