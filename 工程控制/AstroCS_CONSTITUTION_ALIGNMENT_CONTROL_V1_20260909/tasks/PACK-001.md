# PACK-001｜生成并fresh-extract验证审核包

## 目标
按 03_AUDIT_PACKAGE_SPEC 白名单生成审核包：冻结宪章+活动 L0+final tracked manifest+任务台账+SCI→TEST 追踪+CI_RESULT+build provenance+real/fatduck 脱敏摘要+candidate manifest+findings；fresh extract 复验 SHA256。

## 依赖
`AUD-001`。BASE_SHA 取执行时最新 main（三 SHA 一致）。
