# P1-001｜实现Phase1真实节点与complete门

## 目标
每个 DAG node 必须唯一真实 operation/entrypoint/typed artifact，trace call_count=1；禁止子节点调用完整 phase_session_run。

## 依赖
`WCS-002`、`PSF-001`、`AIO-002`。执行时 BASE_SHA 必须为最新 main，若依赖提交后基线变化须重取。
