# CI-002｜验证同SHA Linux与Windows并生成候选

## 目标
完成“验证同SHA Linux与Windows并生成候选”，满足 00_READ_FIRST、02_GATES_AND_EXECUTION 和宪章冻结条款。

## 依赖
`CI-001`。执行时 BASE_SHA 必须为最新 main，若依赖提交后基线变化须重取。
