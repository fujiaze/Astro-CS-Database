# RT-001A｜模块私池拆除接入唯一Executor

## 目标
拆除 lib/phase2(sampler/upm)、lib/phase3_session(p3_session) 等处自建 std::thread 池，统一提交 Runtime Executor work-units（宪章 §10.4）；granted/active workers 记录进 trace。
...
- 扫描绿+2核加速≥1.60+利用率门达标+数值 bitwise/parity 不变。
