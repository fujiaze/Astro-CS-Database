...
| DG-B-12 | `06_photometry.md` 补 m(x,y)/应用点/F_instr | **未改**（m(x,y) 与落盘要求已在 §3.2:120/§3.6 落地）。**订正留痕（2026-09-20）**：旧文「F_instr 口径**待 D-05**」已作废 —— F_instr 口径 = **PSF 拟合域 `2πA·sxsy/3`**（A6：§9.39:1093-1098 + §9.51 Q-C:1647-1651 + §9.52:1705-1715 已定）；D-05 的 ≤0.03 mag 判据由 §9.50 定案 3 撤销、并由 §9.67 定案 6:2590-2594 判「**作废**，须从误差预算推导」。**残留项已消除（2026-09-20 代码复核）**：§9.54:1828 记「SNR 节点仍取盒和（`module_adapters.cpp:3754`）」**已不成立**（该行号现为注释）；`p1_op_noise` 现用 PSF 域逐源 `F/σ_F`（`module_adapters.cpp:3742-3758`）并落盘 `f_instr_domain="psf_analytic_flux_2pi_A_sx_sy_over_3"`（`:3733`）。⚠ 登记：报告 G15 的「残留：噪声节点仍取盒和」系引自 §9.54:1828 的**过期行号** |
| DG-B-15 | `UNIFIED_MODEL.md:42` 补红线判据指向 | **未改** |
| DG-B-10（D-03） | **已裁决**（§9.67 定案 5:2584-2588，负责人 2026-09-20）：**本期不加**堆叠平滑项；须做「(c) 排除自身 + 阻尼 α≈0.5 + 权重自洽 + 末端扣残差场」在**不加**平滑项下能否完美平滑的**非退化判据**实验（证明不能 ⇒ 附证据再议） | **未改**（代码面「本期不加」= 现状；实验项待做）。订正留痕（2026-09-20）：旧文与 B 报告 D15 记「D-03 无裁决 ⇒ 仍待」，**已被 §9.67 定案 5 推翻** |
| DG-B-11 / DG-B-S2-PH-05 / DG-B-S2-PH-06 | **已裁决**：D-04（检测范式）= §9.49 定案 1 **星表引导**（:1480-1485）；D-05（A6 口径）= **PSF 拟合域 `2πA·sxsy/3`**（§9.51 Q-C:1647-1651、§9.52:1705-1715），≤0.03 mag 硬门由 §9.50 定案 3 撤销、§9.67 定案 6:2590-2594 判**作废**（须从误差预算推导） | **未改**（按裁决落地）。订正留痕（2026-09-20）：旧文「**待负责人裁决 D-03/D-04/D-05**」+「按硬约束 D-01..D-10 只登记不动」**作废** |
