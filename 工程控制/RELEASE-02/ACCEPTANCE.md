...
| SD-19 | **F_ref 约定**：写侧逐帧 vs 读侧要求组内公共 ⇒ 混用，真实数据上 `weight_mode=2` fail-closed（`ASTROCS_REFERENCE_FLUX 逐帧不一致`） | WEIGHT-SCI 四方证据：**配对性定理** `SNR_f=a_f·F_ref/σ_f ⇒ SNR_f²/F_ref²=a_f²/σ_f²=w_f`，成立**当且仅当**分母 F_ref 与定义 SNR 用的是同一个；文档面（DESIGN §4.3:256/§3.4、07_noise_snr:59-63、UNIFIED_MODEL:42、aio_hips.h:302 等）高度一致；文献 Horne 1986 + Gauss–Markov `w=a²/σ²`；实测混用偏差 3.5× | `CONTROL_WEIGHT_SNR` vs `07_noise_snr` 的 UNRESOLVED 经查是**命名冲突**（同名 `frame_snr` 指两个对象），非公式冲突 ⇒ 走变更 claim 改名消歧 | ~~采纳裁决：**F_ref 用组内公共**~~ **已由 §9.66 A 取代**（订正留痕 2026-09-20；依据 §9.66:2439-2477 + §9.49 定案 2:1487-1495 + §9.67 定案 4:2570-2582）：`F_ref` **逐帧独立**（`F_ref,k` + 公共锚 `F0`）；组间散度**降级为报告字段**（`photscale_spread_gate="none (owner ruling 9.49: frame-independent)"`，`module_adapters.cpp:3700/:3727`；报告写 :3654/:3681，行已漂移；`reference_flux_spread_rel` 报告非门，§9.66:2475-2476） | ~~闸门**保持 fail-closed**（判为正确，不放宽）~~ **已作废**（同上依据）：硬失败分支已删；诚实登记：①`g_k` 乘性归一未接线 = §9.38 A2:1039-1043 裁决后的既定状态（`g_k ≡ 1` 本期不启用）②SNR 不含光度响应 `a_k`（**仍开**，B 报告 U-5） |
| SD-20 | **Phase2 并行度极低**：mosaic 整跑 1327.9s 仅 **0.71 核**，而 `active_workers` 恒 16；资源门自报 `avg_equivalent_cores 0.73 < 13.6` | 逐阶段（探针 wall × cpu_pct，END 语义）：upm_apply 347.7s/**0.86 核**、reject 214.1s/**0.68**、sample 160.8s/**0.69**、integrate 138.9s/**0.53**、write 23.1s/**0.72**；对比 Phase1 drizzle 可达 **12.82 核** ⇒ 框架并行能力可用 | 无 | **已定位**（订正留痕 2026-09-20；依据 `perf-p2-analysis.md` §2:123-149、§2.6:233-243）：`P2NodeModule::plan` 声明 `work_units = 1`（`module_adapters.cpp:8398`；报告写 :6407，行已漂移）+ 三个最大阶段是裸串行 `for` 循环 + `aio::cfitsio_io_mutex()` 进程级全局锁（`aio_hips_reader.cpp:126`）⇒ **待实施**（方案见 §3） | 裁决：**Phase2 是当前最大优化头寸**（1327.9s 若达 10 核 ⇒ 约 10×），优先级高于 Phase1 drizzle 算法重构（2.15×） |
| SD-21 | Phase2 `sky_plane build` 真实数据上 **rc=6 失败**：`reduced normal matrix not SPD`，显式回退 UPM C 场 | 前台在 L4 实测（mosaic 两次运行均复现） | 无 | **已定位并已裁决（SD-24）**（订正留痕 2026-09-20）：数值根因 = 尺度悬殊 + 惩罚舍入淹没数据（`fix-sky-report.md` §2.4:97-111），已修（§3.1:114-135）；处置 = 默认 fail-closed + 显式逃生阀 `sky_plane.allow_degraded`（默认 false）（本文件 :125 SD-24）。⚠ **落地待核**：代码现状仍为「显式回退 UPM C 场并继续」（`module_adapters.cpp:5903-5914`），`allow_degraded` 在 `lib/**`+`config/**` **grep 0 命中** | 裁决：**显式回退方向正确**（未静默），但须定位非正定的数值根因并正确处置 |

| SD-22 | **天光面归一化语义**：生产/参考路径均做 `corrected = raw − b_k`（`b_k = B_ref + δ_k`）⇒ **把整个背景面剪掉**：背景归零、46% 像素为负，且**不满足「各帧落在同一公共面」**（公共面也被减掉，帧间差异仍在 ⇒ 接缝） | 负责人裁决（选项 **B**，经 `ask_user_question` 确认）：**`corrected_k(x) = raw_k(x) − δ_k(x)`**，`δ_k = b_k − B_ref` ⇒ **只扣「相对公共面的偏差」，保留公共面 `B_ref`**，不除 `g_k`。负责人原话：「以前用的不是这个方法，而是把所有帧归一化到平滑的公共面」「为啥要剪掉？是通过不同帧拟合出了亮度，然后把所有帧都归一化到这个亮度上，多退少补」 | 设计文档 `docs/plugins/algorithms_phase2/11_upm.md:26-31` 支持：`b_k(x)=B_ref(x)+δ_k(x)`，`B_ref` 为**全部帧联合构建的参考天光面**，`δ_k` 为**第 k 帧相对参考面的平缓梯度修正**；输出是「归一化产品」(:18/:84/:85) | ~~采纳 B：改生产 `module_adapters.cpp` 与参考 `stage2.cpp:1068/:1439` 为 B 口径（已派 FIX-GK 改向）~~ **订正留痕（2026-09-20）**：本条为**负责人逐字裁决（选项 B，经 `ask_user_question`）**，**先被 §9.54 前台裁决 1:1792-1794（`raw − C`）取代、再被 §9.67 定案 1:2548-2555（负责人 2026-09-20 逐字裁决「多退少补到公共天光面」）恢复** ⇒ 现行 = `calibrated_k(x) = raw_k(x) − δ_k(x)`，**保留公共天光面 `B_ref`**；`raw − C_k`（全减）**不再是默认**（实现默认 `seam.additive_mode="delta"`：`module_adapters.cpp:5850-5855`；`docs/plugins/algorithms_phase2/11_upm.md:96-97,112`）。⚠ **登记冲突**：B 报告 G11 依 §9.54 写的「生产默认 `raw − C`」**已被 §9.67 推翻**；SD-22 的负责人原意经 §9.67 重新确认 ⇒ 原「建议回呈」**已消解**（负责人已逐字再裁决）；如前台仍要求按 AGENTS §9 第 1 类回呈，请照办。治理留痕：§9.54 曾以前台裁决覆盖负责人逐字裁决，§9.67 已纠正 | 背景**不归零** ⇒ 负值像素占比问题随之消失（与 §9.67 定案 1 一致）；`g_k` 保持不接线 |
| SD-23 | 天光面拟合判据：负责人要求「**不能用 RMS 之类的吗，然后把异常采样点剔除掉**」 | 设计文档 `11_upm.md:62` 原文即「最优解即对各采样点的**加权残差 RMS 最小**的天光面」；`:97`「加权残差 RMS 达理论水平」；`:100`「掩膜 + 粗糙度惩罚联合验证」 | 无 | **现状已查明**（订正留痕 2026-09-20；依据 `fix-gk-report.md` §8:140-155）：只有 **Huber 软降权**（`sky_plane.cpp:949`；报告写 :948，行已漂移），无硬剔除；最小改法见 `fix-gk-report.md` §8.1:151-155 —— **仍待批准**（科学默认行为变更，:155） | ~~与 SD-22 同批处理~~ **指针已失效**（订正留痕 2026-09-20）：SD-22 已由 §9.67 定案 1:2548-2555 处置；本条（硬剔除）**不在** §9.67 定案内 ⇒ 仍待批准（B 报告 U-2 仍开） |
| SD-24 | **结构性 rc（6/7/8）处置** —— 负责人授权「你自行决定」 | rc=6 原为每次必然触发（FIX-SKY 已修）；属**数值/结构缺陷**而非数据不足 | 无 | **决定：默认 fail-closed**，并提供**显式** `sky_plane.allow_degraded=true` 逃生阀（默认 false）用于确实退化的数据集；同时保留已落地的 `sky_plane_degraded` 顶层可见字段 | 理由：与 weight-chain rc=2 硬失败先例一致、符合 DATA-UNC-001 §30.1「禁止静默降级」；根因已修，正常不再触发，此为防止未来回归被静默接受 |

| SD-25 | **归一化必须正确传播信噪比**：负责人要求「保证信噪比在归一化里也能正确传播，**不能把信噪比高的帧归一化后等效权重降低**」 | 数学硬约束：归一化含乘性 `g_k` 时方差须同步变换 `var(corrected)=var(raw)/g_k²`，`w=1/σ²` 相应变换；若归一化引入拟合参数不确定度，须按 `C_out = C_stat + J_out C_θ J_outᵀ` 传播（`docs/plugins/algorithms_phase2/11_upm.md:6` 明确「校准参数不确定度必须传播」）；`upm.h` 亦规定「参数协方差 C_out = C_stat + J_out C_theta J_out^T，禁止由权重标量/诊断量反推 variance」 | 无 | 已派 UNC-PROP 专项审计归一化路径的不确定度传播，并并入 C-DELTA 的组合公式裁决 | 判据：高 SNR 帧经归一化后**有效权重不得降低**；需给数值证明 |
| SD-26 | **接缝根因新判断（待数值验证）**：帧间差异是**乘法**（SEAM 实测 1.56×、逐频段比 0.821–1.312），而 UPM C 场与天光面 δ_k **都是加性场** ⇒ 加性校正原理上消不掉乘性差 ⇒ 接缝残留；须接 `g_k`（乘性亮度归一） | 设计模型 `y_k=g_k·s+b_k`；`g_k` 已可拟合但生产未接线（WEIGHT-SCI/FIX-GK 独立登记）；方案 B 纯加性（实测帧间差逐位不变 1.4e-14）；C 场复活后接缝未改善（前台实测三边界量级相当） | 无 | 已交 C-DELTA 用 `y_A=a·y_B+c` 拟合与 `g_k` 反事实验证 | 若成立 ⇒ 修复 = 接 `g_k` + 明确 C/δ_k/g_k 三者分工 |
