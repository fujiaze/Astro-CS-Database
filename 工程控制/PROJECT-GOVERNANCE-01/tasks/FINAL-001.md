...
逐项复跑 GAP-001..021 对应的机器门与关键证据（含 U-01..U-07 的结论依据），核对每任务 diff 文件域/提交/状态/依赖，输出模块状态阶梯与遗留项，形成负责人审阅材料（不作发布决定）。

## 基线状态（编制时实测）

- 控制包编制时 3 项旧门为红（GAP_AUDIT §0），另有 3 项「rc=0 但不干净」的旧门（VERSION-CONSISTENCY / CLI-COMMAND-LAYER / L0-DOCS，见 GAP_AUDIT §2）；本任务必须独立复跑而非复用他人自述。
...
## 步骤
1. 逐条复跑 GAP-001..021 的机器门与关键证据（含 U-01..U-07 的结论依据），记录命令、退出码与输出摘要。
