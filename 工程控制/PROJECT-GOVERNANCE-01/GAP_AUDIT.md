...
**GAP-036　测试：`core_pipeline` 依赖的 fixtures 目录已被归档删除 ⇒ 该测试在任何 SHA 都红**

- **发现**：ARCH-001 全量 ctest 复查（上一轮 407 项 / 2 红）；前台登记。
- **事实**：`core_pipeline` 依赖 `工程控制/CONTROL_V6/…/fixtures`，该目录已被 `b7b2dea7`（GOV-002）归档删除 ⇒ 与 `lib/**` 迁移**无关**的既有红。
- **处置**：归 **QA-001**（测试/测试基建域）：要么把 fixtures 以**新位置**重建（若该测试仍代表活动契约），要么**显式退役该测试并登记能力去向**（不得静默红着）。
- **验收**：给出结论 + 复跑证据（`ctest -R core_pipeline` 的 rc 与输出）。
