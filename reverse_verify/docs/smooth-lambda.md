...
> 独立构建：`reverse_verify/experiments/smooth_lambda/build.sh`（g++ 直接编译，链接**真实** `lib/algorithms/coverage/src/upm.cpp`，非复刻）。
> 中间产物 `run/reverse_verify/smooth_lambda/`；大产物 `run/RELEASE-02/smooth-lambda/`。
> 判据**先于结果**登记在 `reverse_verify/experiments/smooth_lambda/CRITERIA.md`。
...
bash reverse_verify/experiments/smooth_lambda/build.sh                       # 编译 oracle（g++，链接真实 upm.cpp）
python3 reverse_verify/experiments/smooth_lambda/convert_real.py             # 真实 p2_samples.json -> UPMB
bash reverse_verify/experiments/smooth_lambda/gen_all.sh                     # 合成场景（物理噪声链）
SW_W=4 SW_MI=60 bash reverse_verify/experiments/smooth_lambda/sweep_all.sh real49 A_base A_vary B_prod
python3 reverse_verify/experiments/smooth_lambda/analyze.py --upmb <...> --prefix run/reverse_verify/smooth_lambda/sw_<name> \
        --truth run/reverse_verify/smooth_lambda/<name>.truth.npz --out run/reverse_verify/smooth_lambda/sw_<name>
python3 reverse_verify/experiments/smooth_lambda/report.py
``
