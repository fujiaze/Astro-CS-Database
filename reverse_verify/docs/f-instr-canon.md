...
> 工作项：**F-INSTR-SURVEY**（裁决 A6）。实验代码：`reverse_verify/experiments/f_instr/`。
> 逐条文献/开源软件记录：`reverse_verify/references/f-instr-survey.md`（本文件引用其编号 `[F-xx]`，**核对状态以该文件为准**）。
...
bash reverse_verify/experiments/f_instr/run_all.sh      # exp0-exp4, 约 2 min
python3 reverse_verify/experiments/f_instr/exp5_psf_shape.py   # 约 5 s
```
