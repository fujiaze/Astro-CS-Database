...
> 工作项：**FRAME-SNR-CANON**（RELEASE-02）。调研记录：`reverse_verify/references/frame-snr-survey.md`。
> 实验代码：`reverse_verify/experiments/frame_snr/`（**纯 Python，独立构建，不并入主线**）。
...
> 逐条记录见 `reverse_verify/references/frame-snr-survey.md`；此处只给归纳与判定。
...
> 全部代码：`reverse_verify/experiments/frame_snr/`；结果 JSON：`run/reverse_verify/frame_snr/`。
> 复跑：`bash reverse_verify/experiments/frame_snr/run_all.sh`。
...
> 复跑脚本：`reverse_verify/experiments/frame_snr/inventory_p1_snr.py` 与 `branch_discriminator.py`。
...
| `docs/design/UNIFIED_MODEL.md:42` | `… 天光散粒噪声计入 σ_n`（已符合定案） | 缺少**可复算判据**的指向 | 补一句"红线判据与数值见 `reverse_verify/docs/frame-snr-canon.md` §3；`B→∞ ⇒ SNR→0` 的解析断言见 §3.1" |
...
bash reverse_verify/experiments/frame_snr/run_all.sh
