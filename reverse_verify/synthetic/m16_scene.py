...
-----------------------------------------------
* 物理噪声链**复用** reverse_verify/synthetic/noise_model.py（NM.expose，唯一事实源）；
* PSF 核 / 星点叠加**复用** reverse_verify/synthetic/render.py（R.psf_kernel / R.stamp）；
* 本模块只补 M16 特有的三件事：
...
def load_real_base(rb: Dict[str, Any], shape: Tuple[int, int],
    ...
    src = MK.ROOT / "HST_M16" / MK.BANDS[band]["file"]
    if not src.exists():
