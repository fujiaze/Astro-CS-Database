...
| `../experiments/m16_sampling/run_reconstruct.py` | **重建驱动**：仿真采样帧 → `astrocs normalize` → `mosaic` → `export` → 与真值画布比对（标度/位置/结构/测光） |
...
python3 reverse_verify/synthetic/m16_sampling.py --selftest
python3 reverse_verify/synthetic/m16_sampling.py --list-scenes
python3 reverse_verify/synthetic/m16_sampling.py \
    --scene scenes/m16_sampling_mosaic_diff_pointing.json \
    --out ../../run/reverse_verify/m16_sampling/mosaic_diff_pointing
python3 reverse_verify/experiments/m16_sampling/run_reconstruct.py \
