...
上游数据（负责人上传，真实哈勃 HLSP Heritage 产品，各 268,917,120 B）::
    ...
    HST_M16/hlsp_heritage_hst_wfc3-uvis_m16_f502n_v1_drz.fits   [O III]  PHOTPLAM 5009.64 A
    HST_M16/hlsp_heritage_hst_wfc3-uvis_m16_f657n_v1_drz.fits   H-alpha  PHOTPLAM 6566.61 A
    HST_M16/hlsp_heritage_hst_wfc3-uvis_m16_f673n_v1_drz.fits   [S II]   PHOTPLAM 6765.92 A
...
* 掩膜**不入库**，落 run/reverse_verify/m16_scene/masks/；
  仓库内只登记索引与统计（reverse_verify/data/real/m16_scene_index.json）。

CLI
---
    export TMPDIR=/dev/shm/astrocs_m16
    python3 m16_mask.py --frames-dir HST_M16 \
...
def write_mask_products(valid: np.ndarray, flags: np.ndarray, meta: Dict[str, Any],
    ...
    hdr.add_history("built by reverse_verify/synthetic/m16_mask.py; NOT committed (run/ only)")
...
def main() -> int:
    ...
    ap.add_argument("--frames-dir", type=str, default="HST_M16")
