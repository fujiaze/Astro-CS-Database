...
1. `reverse_verify/synthetic/scenes/m16_*.json`（4 个）
   - `"stack_n": 1` → `"stack_n": 32`
   - `calibration.$comment` 追加 STACKN32-001 说明；新增 `calibration.stack_n_rule`
2. `reverse_verify/synthetic/m16_scene.py`
...
3. `reverse_verify/experiments/p7_noise/p7lib.py`
   - `det_of()` 的兜底默认 `scene.get("stack_n", 1)` → `..., 32)`（无行为影响：场景总是显式给值）
4. `reverse_verify/data/synthetic/generate.py`
   - **无 `stack_n` 硬编码**（已逐行核对：该文件从不读写 `stack_n`）；只在 docstring 追加 STACKN32-001 说明
5. `reverse_verify/data/synthetic/datasets.json`
   - 新增顶层 `$stack_n` 登记项（纯文档，无行为影响）
6. `reverse_verify/experiments/p7_noise/exp{1,3,4}_*.py`
