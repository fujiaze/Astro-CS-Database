...
CLI::
    ...
    python3 reverse_verify/experiments/m16_scene/exp_realbase_consistency.py --out run/RELEASE-02/paper/data/M16FIX/regression.json
"""
...
def main() -> int:
    ...
    for sid, band in SCENES:
        ...
        real = np.asarray(fits.getdata(
            str(MS.MK.ROOT / "HST_M16" / MS.MK.BANDS[band]["file"])), dtype=np.float64)
