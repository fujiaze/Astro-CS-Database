...
function New-AstroCSReviewPack {
    ...
    foreach ($fn in @("README.md","AGENTS.md","HANDOVER.md","eng/build/toolchain.ps1")) {
    ...
    $packReadme = @(
        ...
        "- docs/         README / AGENTS.md / HANDOVER.md / eng/build/toolchain.ps1",
        ...
        "",
        "## 生成命令",
        ".\eng/build/toolchain.ps1 review -Topic $Topic"
    )
...
if ($MyInvocation.InvocationName -ne ".") {
    switch ($Command.ToLower()) {
        ...
        "review" { if (-not $Topic) { Write-Error "用法: .\eng/build/toolchain.ps1 review -Topic <主题>"; exit 1 }
                   New-AstroCSReviewPack -Topic $Topic -Date $Date }
        default  { Write-Host "用法: .\eng/build/toolchain.ps1 {check|env|build|run|review}" }
    }
}
