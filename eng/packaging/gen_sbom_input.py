...
机器验收 (BLD-004 + W5-PKG-001):
  ...
  2. fresh configure 不读取机器绝对路径: 对 CMake 作用域 (CMakeLists.txt,
     CMakePresets.json, eng/cmake/**, eng/packaging/**, DEPENDENCIES.md) 扫描禁止
...
用法:
  python3 eng/packaging/gen_sbom_input.py --root <repo-root> [--json-out <p>]
  python3 eng/packaging/gen_sbom_input.py --root <repo-root> --self-test
...
def main():
    ...
    if not lock_path.exists():
        print("LOCK_MISSING eng/packaging/dependency-lock.json", file=sys.stderr)
        sys.exit(1)
    try:
