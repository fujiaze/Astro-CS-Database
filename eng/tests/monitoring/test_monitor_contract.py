#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""LOG-002 资源监控伴随器验收测试（eng/tests/monitoring 域，owner SA-LOG-08）。
...
方法：与 eng/tests/monitoring/test_log_contract.py 同款——纯 Python unittest，
...
class TestTraceObserverWiring(unittest.TestCase):
    ...
    def test_observer_never_reads_config(self):
        # observer 输入只有 trace 事件；把 eng/packaging/config/plan 伪装成事件也不冒充
        events = [
