/* CAT-GAIA-INT 模块本地集成测试 (eng/tests/unit/gaia_integration_test.c)
 *
 ...
 *      JSONL (astrocs.trace-event/v1 键序, lib/infrastructure/scheduler TraceEvent::to_jsonl 同型)
...
int main(int argc, char** argv) {
    ...
    for (int node_i = 0; node_i < 2; node_i++) {
        ...
        CHECK(destroy_calls == 1, "C15: node destroy 恰调用 1 次 (生命周期闭环)");
        free(out);
    }
