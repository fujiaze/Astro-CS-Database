/* P1-COS-INT 模块本地集成测试 (eng/tests/unit/cosmetic_integration_test.cpp)
 *
 * 对齐先例: eng/tests/unit/gaia_integration_test.c (CAT-GAIA-INT, 054af3a3) /
 *           eng/tests/unit/calibration_integration_test.cpp (P1-CAL-INT, 58d20223)。
 *
 ...
 *   B. typed ports 静态校验负面: p1.calibrated/p1.master_dark/p1.master_bias/
 *      p1.cleaned 绑定真实性 —— 缺 op/detail 101、词表外 op/detail 100、
 *      非 finite/detail 103、类型错/detail 102、词表外键/CONFIG_SCHEMA;
 *      manifest 通道负面: dtype 与 op 精度不一致/detail 111、平面字节数不符/
 *      detail 112、data_base64 缺失/detail 110。
 *   C. 调用计数 (每节点恰调用一次, 模板 INT §2): 两节点合成链
 *      (node1 correct_frame 带 master_dark/master_bias → node2 correct_frame
 *      仅 data, node1 输出 out_base64 作为 node2 data_base64 —— ArtifactStore
 *      真实传 handle 的本地最小替身) 各驱动一次完整协议 validate_config(1)
 *      →plan(1)→create(1)→execute 事务 (1 次 host 驱动 = 两阶段 strbuf 2 次
 *      API)→inspect(1)→destroy(1); host stub 观测 lease acquire/release 2/2
 *      平衡; inspect.exec_count==2 (两遍各计一次, 真实语义); 重复驱动可重入
 *      为 KI 登记 (v1 C ABI 事务完成后 state 回 CREATED, 模块不锁完成态),
 *      per-node 恰一次由调用方协议 + RT-006 runtime call counts / 重放违规
 *      检测保证。
 *   D. 真实 plan (禁预测值): plan JSON 键集 = DLL cos_plan 真实输出;
 *      work_units=width*height (数据事实) 独立算术交叉且 >0;
 *      work_unit/parallel_axis=pixel (ALG-COS-004 真实并行轴);
 *      memory_bytes_estimate=work_units*sizeof(dtype); io_bytes_estimate=0 为
 ...
 *
 * DLL 经 ASTROSCOS_DLL_PATH 加载 (cosmetic_adapter_test 同名); integration
 * descriptor JSON 经 ASTROSCOS_INTEGRATION_JSON; module.yaml 经
 * ASTROSCOS_MODULE_YAML; trace JSONL 输出经 ASTROSCOS_INT_TRACE_OUT
 * (CMake test properties 注入)。fixture: p1cos_fixtures.hpp FIX-COS-B 谱系
 * (splitmix64, 固定 seed, spike 幅度 ≥8σ 远离阈值边界)。
 */
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
/* ── SHA-256 (FIPS 180-4 §4.2.2 标准 K 表 64 项; 测试内独立 oracle, 计算 DLL
 * 文件内容 hash)。注意: 先例 (CAL-INT 58d20223/gaia) K 表缺 3 项错 2 项
 * (61 项, hash 自洽但非标准 SHA-256) —— 本测试以标准表 + 已知向量自检锚
 * ("abc" → ba7816bf..15ad) 证明 oracle 正确后才测 DLL; 先例缺陷另行登记。 ── */
typedef struct {
    uint32_t h[8];
    uint64_t len;
    uint8_t buf[64];
...
static void sha256_final(sha256_ctx* c, uint8_t out[32]) {
    ...
    uint8_t pad = 0x80;
    uint8_t z = 0;
    size_t pad_zeros = (c->buflen < 56) ? (56 - c->buflen - 1) : (120 - c->buflen - 1);
    sha256_update(c, &pad, 1);
    for (size_t i = 0; i < pad_zeros; i++) sha256_update(c, &z, 1);
    uint8_t lenb[8];
...
/* FIPS 180-4 已知向量自检锚: SHA256("abc") 的前 8 hex; oracle 先证己,
 * 后测 DLL (无此锚的 hash 断言不可信) */
static int sha256_selftest(void) {
    ...
    uint8_t d[32];
    sha256_final(&c, d);
    return d[0] == 0xba && d[1] == 0x78 && d[2] == 0x16 && d[3] == 0xbf &&
           d[4] == 0x8f && d[5] == 0x01 && d[6] == 0xcf && d[7] == 0xea &&
           d[28] == 0xf2 && d[29] == 0x00 && d[30] == 0x15 && d[31] == 0xad;
}
...
/* ── host stub (观测 executor 租借; make_host 同 cosmetic_adapter_test 口径) ── */
...
static int trace_open(trace_writer* tw, const char* path) {
    ...
    tw->f = fopen(path, "w");
    if (!tw->f) return -1;
    snprintf(tw->run_id, sizeof(tw->run_id), "p1cos-int-local");
    return 1;
}
/* RFC3339 UTC 观测时刻 */
...
/* ── fixture (FIX-COS-B 谱系; splitmix64 固定 seed) ── */
static const int FW = 8, FH = 6;
static const size_t FPX = (size_t)FW * FH;

/* FIX-COS-B: 常量基场 + 孤立单像素 hot spike (data=999 / dark=1000);
 * cold 域无注入 (bias 常量场)。spikes[] 为注入索引 (oracle 期望输入侧)。 */
static p1cos::FixCosB g_fx;
static std::string g_in1_b64;   /* node1 data_base64 (链式传递锚) */

static void build_fixtures(void) {
    g_fx = p1cos::fix_cos_b_spike_field(0x20260909u, (size_t)FW, (size_t)FH,
                                        /*gradient_base=*/false);
}

int main(void) {
    const char* dll_path = getenv("ASTROSCOS_DLL_PATH");
    const char* desc_path = getenv("ASTROSCOS_INTEGRATION_JSON");
    if (!desc_path || !desc_path[0]) desc_path = ASTROSCOS_INTEGRATION_JSON;
    const char* trace_out = getenv("ASTROSCOS_INT_TRACE_OUT");

    printf("== P1-COS-INT cosmetic_integration_test ==\n");
    ...
    if (!dll_path || !dll_path[0]) {
        printf("[FAIL] F-pre: ASTROSCOS_DLL_PATH 未注入\n");
        return 2;
    }
    ...
    if (dj) {
        ...
        CHECK(tjson_contains(dj, "\"name\": \"p1.calibrated\"") &&
                  tjson_contains(dj, "\"name\": \"p1.master_dark\"") &&
                  tjson_contains(dj, "\"name\": \"p1.master_bias\"") &&
                  tjson_contains(dj, "\"name\": \"p1.cleaned\""),
              "D12: typed ports 端口名 (module.yaml input/output 同名)");
        CHECK(tjson_contains(dj, "\"data_contract\": \"DATA-P1-COS\""),
              "D13: 端口 data_contract == DATA-P1-COS");
        {
        ...
        if (my) {
            ...
            CHECK(strstr(my, "  - p1.calibrated") && strstr(my, "  - p1.master_dark") &&
                      strstr(my, "  - p1.master_bias") && strstr(my, "  - p1.cleaned"),
    ...
    /* ════ B. typed ports 静态校验负面 (端口绑定真实性) ════ */
    {
        ...
        CHECK(rc == ACS_ERR_PARAM && err.detail_code == 101,
              "B1: 缺 op → PARAM + detail 101 (config 词表必填)");
        /* B2: 词表外 op (2 op 词表之外) → PARAM + 100 */
        err_init(&err);
        rc = api->validate_config(api,
              s_from("{\"op\":\"no_such_op\",\"width\":8,\"height\":6,"
                      "\"hot_sigma\":5.0,\"cold_sigma\":5.0,\"method\":\"median\","
                      "\"max_structure_size\":3}"), &err);
        CHECK(rc == ACS_ERR_PARAM && err.detail_code == 100,
              "B2: 词表外 op → PARAM + detail 100");
        /* B3: 非 finite 参数 (1e999 溢出 → inf; adapter_test 同口径) → PARAM + 103 */
        err_init(&err);
        rc = api->validate_config(api,
              s_from("{\"op\":\"correct_frame\",\"width\":8,\"height\":6,"
                      "\"hot_sigma\":1e999,\"cold_sigma\":5.0,\"method\":\"median\","
                      "\"max_structure_size\":3}"), &err);
        CHECK(rc == ACS_ERR_PARAM && err.detail_code == 103,
              "B3: 1e999 → PARAM + detail 103 (非 finite 参数拒绝)");
        /* B4: 类型错 (width 给字符串) → PARAM + 102 */
        err_init(&err);
        rc = api->validate_config(api,
              s_from("{\"op\":\"correct_frame\",\"width\":\"8\",\"height\":6,"
                      "\"hot_sigma\":5.0,\"cold_sigma\":5.0,\"method\":\"median\","
        ...
        /* B5: op 依赖必需键缺失 (缺 max_structure_size) → PARAM + 101 */
        err_init(&err);
        rc = api->validate_config(api,
              s_from("{\"op\":\"correct_frame\",\"width\":8,\"height\":6,"
                      "\"hot_sigma\":5.0,\"cold_sigma\":5.0,\"method\":\"median\"}"), &err);
        CHECK(rc == ACS_ERR_PARAM && err.detail_code == 101,
              "B5: 缺 max_structure_size → PARAM + detail 101 (op 依赖必需键)");
        /* B6: 词表外未知键 → PARAM + CONFIG_SCHEMA (9 §10; 8 键词表封闭) */
        err_init(&err);
        rc = api->validate_config(api,
              s_from("{\"op\":\"correct_frame\",\"width\":8,\"height\":6,"
                      "\"hot_sigma\":5.0,\"cold_sigma\":5.0,\"method\":\"median\","
                      "\"max_structure_size\":3,\"frames\":4}"), &err);
        CHECK(rc == ACS_ERR_PARAM &&
                  err.detail_code == ACS_DIAG_ECODE_CONFIG_SCHEMA,
              "B6: 词表外键 frames → PARAM + CONFIG_SCHEMA (8 键封闭词表)");
        /* B7: 合法 config (全必需参数) → OK */
        CHECK(api->validate_config(api,
                  s_from("{\"op\":\"correct_frame\",\"width\":8,\"height\":6,"
                          "\"hot_sigma\":5.0,\"cold_sigma\":5.0,\"method\":\"median\","
                          "\"max_structure_size\":3}"), &err) == ACS_OK,
              "B7: 合法 config (全参数) → OK");
    }

    /* ════ P. fixture 真值准备 (FIX-COS-B 谱系) ════ */
    CHECK(g_fx.w == (size_t)FW && g_fx.h == (size_t)FH,
          "P1: FIX-COS 谱系 fixture 生成 (8x6, 固定 seed)");
    CHECK(g_fx.data.size() == FPX && g_fx.dark.size() == FPX && g_fx.bias.size() == FPX,
          "P2: data/dark/bias 平面尺寸正确");
    CHECK(g_fx.spikes.size() > 0, "P3: hot spike 注入点非空 (hot 检测真实发生)");
    ...
    CHECK(sha256_selftest(),
          "T1a: SHA-256 oracle 自检锚 (FIPS 180-4 已知向量 abc)");
    ...
    /* 合成链: node1 cos_n1 (correct_frame, data+master_dark+master_bias,
     * FIX-COS-B spike 场) → node2 cos_n2 (correct_frame, 仅 data =
     * node1 out_base64, master 键缺席 = legacy NULL 恒等通道)。
     * 链式 artifact 传递是 ArtifactStore 真实传 handle 的本地最小替身。 */
    b64_encode((const uint8_t*)g_fx.data.data(), FPX * sizeof(float), g_in1_b64);
    std::string m1 = "{\"schema_version\":1,\"width\":8,\"height\":6,"
                     "\"dtype\":\"f32\",\"data_base64\":\"" + g_in1_b64 +
                     "\",\"master_dark\":\"";
    {
        std::string bd, bb;
        b64_encode((const uint8_t*)g_fx.dark.data(), FPX * sizeof(float), bd);
        b64_encode((const uint8_t*)g_fx.bias.data(), FPX * sizeof(float), bb);
        m1 += bd + "\",\"master_bias\":\"" + bb + "\"}";
    }
    const char* cfg1 =
        "{\"op\":\"correct_frame\",\"width\":8,\"height\":6,\"hot_sigma\":5.0,"
        "\"cold_sigma\":5.0,\"method\":\"median\",\"max_structure_size\":3}";
    /* node2 带 config.max_workers=2 → want=min(host 3, cfg 2)=2 (裁剪观测) */
    const char* cfg2 =
        "{\"op\":\"correct_frame\",\"width\":8,\"height\":6,\"hot_sigma\":5.0,"
        "\"cold_sigma\":5.0,\"method\":\"median\",\"max_structure_size\":3,"
        "\"max_workers\":2}";
    ...
    for (int node_i = 0; node_i < 2; node_i++) {
        ...
        if (rc == ACS_OK) {
            ...
            CHECK(pw == expect_wu && pw > 0,
                  "D6: plan.work_units == width*height 且 >0 (数据事实, 非预测)");
            CHECK(pu[0] && !strcmp(pu, "pixel") && !strcmp(pax, pu),
                  "D7: plan.work_unit/parallel_axis == pixel (ALG-COS-004 真实并行轴)");
            CHECK(pmn == 1, "D8: plan.min_workers == 1");
            CHECK(pmem == pw * 4 && pio == 0,
                  "D9: plan.memory=work_units*sizeof(f32), io=0 为 inline 数据面真实值");
            CHECK(!strstr(pb.data, "leased") && !strstr(pb.data, "max_workers"),
                  "D10: plan JSON 无观测词/无 max_workers 键 (禁观测冒充预测, 14 §4)");
        }

        /* B-ports: manifest 通道 typed ports 负面 (node1 驱动序列内) */
        if (node_i == 0) {
            /* P1: dtype 与 op 精度不一致 → PARAM + 111 (exec_run 首检,
        ...
        CHECK(ex.last_leased == (node_i == 0 ? 3 : 2),
              "C6: 租借观测 want=min(host max_workers, config.max_workers) (stub 真实观测)");
        art_sizes[node_i] = out ? (uint64_t)strlen(out) : 0;
        if (out) {
            long long sv = -1, hot = -1, cold = -1;
            char dt[16] = "", oop[64] = "";
            CHECK(tjson_ll(out, "schema_version", &sv) && sv == 1 &&
                      tjson_str(out, "dtype", dt, sizeof(dt)) && !strcmp(dt, "f32") &&
                      tjson_str(out, "op", oop, sizeof(oop)) && !strcmp(oop, "correct_frame"),
                  "C7: 输出 manifest 键序/词表 (schema=1, dtype=f32, op=correct_frame)");
            char ob[512] = "";
            CHECK(tjson_str(out, "out_base64", ob, sizeof(ob)) && ob[0],
                  "C8: 输出 out_base64 非空 (事务输出真实)");
            tjson_ll(out, "hot", &hot);
            tjson_ll(out, "cold", &cold);
            if (node_i == 0) {
                /* 真实检测观测: FIX-COS-B dark spike → hot == 注入点数;
                 * bias 常量场 → cold == 0 (oracle 期望输入侧独立推导) */
                CHECK(hot == (long long)g_fx.spikes.size(),
                      "C9: node1 hot == FIX-COS-B 注入点数 (检测计数真实观测)");
                CHECK(cold == 0,
                      "C10: node1 cold == 0 (bias 常量场无冷点)");
                snprintf(out1_b64, sizeof(out1_b64), "%s", ob);
            } else {
                /* NULL-master 恒等通道 (DISP-COS-009): 无检测 → hot/cold = 0;
                 * 输出平面 == 输入平面 (BITWISE, 链式 artifact 真实传递) */
                CHECK(hot == 0 && cold == 0,
                      "C11: node2 hot/cold == 0 (master 缺席恒等通道, DISP-COS-009)");
                CHECK(out1_b64[0] && !strcmp(ob, out1_b64),
                      "C12: 合成链 BITWISE: node2 输出 == node1 输出平面 "
                      "(NULL-master 恒等通道不改像素, 链式 artifact 真实传递)");
            }
        }
        ...
        CHECK(rc == ACS_OK && ib.data, "C13: node inspect 恰调用 1 次 → OK");
        if (rc == ACS_OK && ib.data) {
             ...
             * 断言用实现真实观测值, 不用理想化假值。 */
            CHECK(ec == 2, "C14: inspect.exec_count == 2 (一事务两遍各计一次)");
            CHECK(lop == (long long)ACS_OP_EXECUTE,
                  "C15: inspect.last_op == ACS_OP_EXECUTE (7, 实例级执行观测)");
            CHECK(!strcmp(st8, "created"),
                  "C16: inspect.state == created (v1 事务完成后回 CREATED, 如实观测)");
        }

        /* C: 重复驱动观测 (KI 登记, 与 gaia/CAL INT 同模式):
         ...
         * inspect 面可被 RT 巡检发现超限, 不伪造 STATE 断言。 */
        {
            ...
            CHECK(ec2 == 4,
                  "C18: 重复事务后 exec_count == 4 (2+2 观测递增, 供 RT 巡检超限)");
        }
    ...
    printf("== cosmetic_integration_test: %s (fail=%d) ==\n",
