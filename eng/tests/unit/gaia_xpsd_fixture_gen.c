 ...
 * oracle (eng/tests/oracle/gaia_oracle.py) 再用 Python 独立实现复算与比对。
...
static int fix_write_header(const char *path) {
    ...
    if (!f) return -1;
    fprintf(f,
        "/* 自动生成：eng/tests/unit/gaia_xpsd_fixture_gen.c --header（seed %llu）。\n"
        " * 参考真值 = 量化精确反演（独立公式，与被测实现同源 ALG s2）。\n"
        " * 值由 oracle (eng/tests/oracle/gaia_oracle.py) 二次独立复算。 */\n"
