// eng/tests/unit/p2002_unc_rej_prov_test.cpp — P2-002 §30.2 rejection 平面与
...
static void test_f_unc_003_no_plane_drift() {
  ...
  {
    const std::string s = repo_file("eng/contracts/data/phase_product_exchange.schema.json");
    CHECK(!s.empty());
  ...
  {
    const std::string s =
        repo_file("eng/contracts/data/phase2_uncertainty_rejection_provenance_v1.json");
    CHECK_MSG(!s.empty(),
