...
- `eng/tests/testkit/` **只含** harness、seed RNG、临时目录管理、artifact fixture、
...
# 模块测试按 label 分布（11 号文 §6 类型目录）：eng/tests/unit|properties|oracle|
...
- 测试注册源：`eng/tests/testkit/registry.json`（手工维护 + check_testkit 校验一致性）或
...
- `eng/tools/testkit/check_testkit.py` 对 `eng/tests/testkit/fixtures/` 下的示例测试做
...
- 校验 `eng/tests/testkit/schemas/test_metadata.schema.json` 存在且为合法 JSON Schema；
