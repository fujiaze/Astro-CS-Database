#!/usr/bin/env python3
"""SCHEMA-INTEGRATE-001 / W6 —— 生产数据字典（eng/contracts/data/v6_data_dictionary_v1.json）。

科学表（单位/权重模式/禁止项/provenance 最小集/条款锚）在本脚本中**独立手写**，
冻结表（clause 注册表/状态计数/签字项）从 W4 冻结表机械读取；两者由
eng/tests/contracts/v6/v6_oracle.py 独立对拍 —— 生产字典与冻结合同的任何漂移都会判红。
"""
...
CONCENTRATION_UNIT_AUTHORITY = {
    ...
    "legacy_proposal_example": "eng/contracts/proposals/v6/data/examples/psfsw.example.json 原写 'ADU/px'；生产正例 eng/contracts/data/examples/v6/psfsw.example.json 已按唯一权威修正为 'ADU/px^2'",
}

CANONICAL_OBJECT_RETIREMENT = {
    "note": "本文件属 v6 合同层，按 工程控制/RELEASE-03/GAP_AUDIT.md §4.1 Q2 裁决在位保留为「设计档案 / 产品族专用投影（非生产目标态）」，文件不删。统一对象 canonical psfsw_robust_weight retired（负责人 2026-09-20 裁决 B，14→13；变更编号 CHG-2026-09-20-PSFSW-RETIRE；依据 ASTROCS_DESIGN.md §2.1/§2.2/§2.3 与 工程控制/RELEASE-02/GAP_AUDIT.md §9.73 裁决 A44）：eng/contracts/schemas/unified/psfsw_robust_weight.schema.json 与其正例已删除，端口合同枚举不再接受该对象。本文件中的 psfsw_robust_weight 是 v6 提案/产品族内部标识（weight.kind 值、单位表符号、variance_from 禁止项 token），不是统一对象 canonical 引用；旧产品声明该对象 ⇒ 显式拒绝 + 迁移提示（eng/contracts/data/unified_object_compatibility_map_v1.json#retired_entries）。",
...
def build():
    ...
    for fn in SCHEMA_FILES:
        ...
        schema_index.append({
            "schema_id": d["$id"],
            "path": "eng/contracts/schemas/v6/" + fn,
            "version": 1,
    ...
    doc = {
        ...
        "examples_index": ["eng/contracts/data/examples/v6/" + e for e in examples],
        ...
        "weight_vocabulary_ref": "eng/contracts/data/v6_weight_vocabulary_v1.json",
        "migration_map_ref": "eng/contracts/data/v6_migration_map_v1.json",
