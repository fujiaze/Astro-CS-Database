...
class TestPhaseConfigFamily(unittest.TestCase):
    ...
    def test_filter_enum_equals_library_keys(self):
        ...
        for phase, rel in PHASE_SCHEMAS.items():
            ...
            self.assertEqual(lib, enum, "%s 的滤镜枚举与 eng/packaging/config/filters.json 不一致" % rel)
    ...
    def test_no_aggregate_second_definition(self):
        ...
        self.assertFalse(os.path.exists(os.path.join(C.REPO, "eng/contracts/schemas/phase_config.schema.json")))
...
class TestFiltersLibrary(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 定位符由 eng/packaging/config/filters.json 自己声明（仓库目录迁移后仍成立），内容由 sha256 冻结
...
class TestCpuProfileMigration(unittest.TestCase):
    ...
    def test_both_branches_validate(self):
        for rel in ("eng/tests/config/fixtures/positive/cpu_profile_v1_legacy.json",
                    "eng/tests/config/fixtures/positive/cpu_profile_v2.json"):
...
class TestRunManifest(unittest.TestCase):
    def test_positive_fixture_passes(self):
        errs = C.validate(C.load_json(MANIFEST_SCHEMA),
                          C.load_json("eng/tests/config/fixtures/positive/run_manifest.example.json"))
