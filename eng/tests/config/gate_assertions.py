...
用法: timeout 120 python3 eng/tests/config/gate_assertions.py
"""
import os
import sys
...
PHASE = {
    "normalize": ("eng/contracts/schemas/phase_config_normalize.schema.json", "eng/packaging/config/templates/normalize.phase_config.json"),
    "mosaic": ("eng/contracts/schemas/phase_config_mosaic.schema.json", "eng/packaging/config/templates/mosaic.phase_config.json"),
    "export": ("eng/contracts/schemas/phase_config_export.schema.json", "eng/packaging/config/templates/export.phase_config.json"),
}
NEGS = [
...
CPU = "eng/contracts/schemas/cpu_profile.schema.json"
MANIFEST = "eng/contracts/schemas/run_manifest.schema.json"
FILTERS = "eng/packaging/config/filters.json"
# 滤镜库/provenance 定位符由 eng/packaging/config/filters.json 声明（目录迁移后不失效）
...
for name, schema_key, fixture in NEGS:
    ...
    errs = C.validate(C.load_json(srel), C.load_json("eng/tests/config/fixtures/negative/" + fixture))
...
# 门 B：defaults 计数断言
ddoc = C.load_json("eng/packaging/config/defaults.json")
dfields = ddoc["fields"]
n_total = len(dfields)
