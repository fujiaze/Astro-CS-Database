...
> 机器核死入口：python3 eng/ci/validate_workflow_binding.py（exit 0 PASS / 1 FAIL）
> 运行期派发：python3 eng/ci/wf_step.py --step <STEP_ID>（声明唯一事实源：eng/ci/workflow_binding.json）

## 0. 结论

1. **workflow 只调用 eng/ci/run.py（不复制业务命令）**：两份 workflow 的 run: 体只剩四类
   调用——eng/ci/bootstrap.py / eng/ci/select_profile.py（CI 入口）、eng/ci/run.py（注册表执行，
   每份 workflow 恰一条）、eng/ci/wf_step.py（声明步派发：诊断 + 注册表侧门）。原先
   workflow 内联的 cmake 构建树、fixture 准备、候选 zip 校验、内联诊断 python 体
   全部迁入 eng/ci/ 并由注册表检查项执行或由声明派发。
...
3. **actions SHA lock 维持并复验**：eng/ci/actions.lock.json 4 条；offline 结构 PASS +
   online tag→SHA 逐条 PASS（eng/ci/verify_actions_lock.py）。
...
| 2 | Bootstrap locked toolchain | ci-entrypoint | eng/ci/bootstrap.py（工具链门） | eng/ci/ 入口 |
| 3 | Select profile | ci-entrypoint | eng/ci/select_profile.py；输出 profile ∈ {linux-main, linux-deep} ⊆ 注册表 profile | eng/ci/ 入口 |
...
| 6 | Run registered checks | **registry-runner** | `python3 eng/ci/run.py --profile "${{ steps.profile.outputs.profile }}"`；profile 逐值在注册表内 | 注册表（90+4 项） |
| 7 | Collect bootstrap diagnostics | infra-step | 声明 LINUX-BOOTSTRAP-DIAG → eng/ci/steps/collect_bootstrap_diag.py | eng/ci/ 声明体 |
...
| 3 | Bootstrap locked toolchain | ci-entrypoint | eng/ci/bootstrap.py | eng/ci/ 入口 |
| 4 | Run MSVC tests and package candidate | **registry-runner** | `python eng/ci/run.py --profile windows-main` | 注册表（63+3 项） |
| 5 | Upload candidate | artifact-upload | artifacts/candidate/，if: success()，if-no-files-found: error | 无 |
| 6 | Collect bootstrap diagnostics | infra-step | 声明 WINDOWS-BOOTSTRAP-DIAG → eng/ci/steps/collect_bootstrap_diag.py | eng/ci/ 声明体 |
...
## 3. 新增注册表检查项（eng/ci/checks.json 99 → 104）

| id | profile | 命令 | waivable | 说明 |
|---|---|---|---|---|
| LINUX-MAIN-FIXTURES | linux-main | `python3 eng/ci/wf_step.py --step LINUX-PREPARE-FIXTURES` | false | UT-BACKEND/UT-CLI 前置 fixture（原 workflow 步） |
| LINUX-MAIN-BUILD-TREE | linux-main | `python3 eng/ci/wf_step.py --step LINUX-BUILD-ROOT-GRAPH` | false | UT-BACKEND/UT-CLI 构建树前置（原 workflow 步） |
| WIN-CANDIDATE-VALIDATE | windows-main | `python3 eng/ci/wf_step.py --step WINDOWS-VALIDATE-CANDIDATE` | false | 候选 zip 结构/哈希/manifest 门（原 workflow 步） |
| WORKFLOW-REGISTRY-BINDING | fast/linux-main/windows-main | `python3 eng/ci/validate_workflow_binding.py ...` | false | 本审计的机器核死校验器 |
...
1. eng/ci/checks.json WIN-PACKAGE-CANDIDATE（waivable=false）outputs 登记
   `artifacts/candidate/AstroCS-candidate.zip`；run.py 对 exit 0 但产物缺失记
   V_MISSING_OUTPUT（FAIL）。
2. eng/ci/workflow_binding.json 的 WINDOWS-VALIDATE-CANDIDATE 声明
   require_outputs=[同 zip]、binds_check=WIN-PACKAGE-CANDIDATE、fail_closed=true；
   eng/ci/wf_step.py 执行前先核产物存在 → 缺失即 ::error:: + exit 1（绝不 warning 跳过）。
3. 该声明由检查项 WIN-CANDIDATE-VALIDATE（waivable=false）经 eng/ci/run.py 执行；
