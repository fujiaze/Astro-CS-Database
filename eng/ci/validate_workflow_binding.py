#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eng/ci/validate_workflow_binding.py — workflow 与 eng/ci/checks.json 联动核死校验器（CI-001B）。

用法:
    python3 eng/ci/validate_workflow_binding.py [--repo-root DIR] [--manifest P]
...
核死内容（任一违规 exit 1；全部通过 exit 0）:
  ...
  B4  pinned-action：必须有 uses、无 run、uses 锁到 eng/ci/actions.lock.json 的完整 SHA；
  ...
  B7  host-provisioning：体（去注释行后）不得出现仓库路径 token 或构建工具
      （eng/cmake/ctest/ninja/msbuild/gcc/g++/clang++/make）：business_command_in_workflow；
      且必须至少声明一个 provisioning 动词；
  B8  registry-runner：体必须逐字等于 <python|python3> eng/ci/run.py --profile <profile_expr>；
  ...
  B9  registry-bound-step / infra-step：run 体必须逐字等于
      <python|python3> eng/ci/wf_step.py --step <STEP_ID>；exec 非空、exec[0] 合法、
      exec 引用的 eng/ci/ 脚本必须存在（exec_target_missing / exec_not_under_ci）；
...
DISPATCHER = "eng/ci/wf_step.py"
RUNNER = "eng/ci/run.py"
CI_ENTRYPOINTS = ("eng/ci/bootstrap.py", "eng/ci/select_profile.py")
...
def validate(repo: Path, manifest_path: Path, registry_path: Path, lock_path: Path,
    ...
    for body in check_bodies:
        ...
        if not isinstance(argv, list) or not argv:
            ...
            if target is None:
                err("exec_target_missing", where, "exec 未引用 eng/ci/ 体文件")
            else:
                if not str(target).startswith("eng/ci/"):
    ...
    for wf_rel in scope_files:
        ...
        if not runners:
            err("registry_runner_missing", wf_rel,
                "workflow 必须恰有一条 eng/ci/run.py --profile 注册表执行步")
        elif len(runners) > 1:
        ...
        for idx, raw in enumerate(steps):
            ...
            if entry.get("pending_registration"):
                notice("pending_registration", where,
                       str(entry.get("pending_registration")) + "（挂账：需在 eng/ci/checks.json 注册为检查项）")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="eng/ci/validate_workflow_binding.py",
        description="workflow 与 eng/ci/checks.json 联动核死校验器")
    ...
    manifest = Path(args.manifest) if args.manifest else repo / "eng" / "ci" / DEFAULT_MANIFEST.name
    registry = Path(args.registry) if args.registry else repo / "eng" / "ci" / DEFAULT_REGISTRY.name
    lock = Path(args.lock) if args.lock else repo / "eng" / "ci" / DEFAULT_LOCK.name
