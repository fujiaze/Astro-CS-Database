...
- `mappings` 逐条计数（生成器实测，**与上表口径不同**）：合计 300 条 —— KEPT 148 / KEPT-PENDING-MERGE 3 / MERGED-INTO 149；其中在 `coverage.absorbed_entries` 内登记为 absorbed 的 142 条。
- 上表由各登记任务按 +N 维护，R13 只机器校验等式 `source_entry_count` == `len(mappings) − len(set(absorbed_entries))`（现为 158 == 300 − 142）；本文件**逐字引用**该块，不用逐条计数覆盖它。
- `absorbed_entries`：142 条登记（唯一 142 条），均为本波基线之外、由后续任务新增的执行单元。 注：`absorbed_note` = CHK-SECRET-HYGIENE 不属本波基线 146 项，来自 ROOT-006 注册文件（吸收并入）
