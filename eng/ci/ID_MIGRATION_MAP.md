...
- 目标语义 = `docs/ci/01_CHECKS.md §2` 表：`targets` 合计 110 个目标，其中 91 个的 `doc` 字段指向该表（含带登记批注的变体），19 个指向其它权威面（`docs/contracts/**`、`docs/science/**`、`docs/api/**`、`ASTROCS_DESIGN.md` 或扩展登记说明）；按 `kind` 计：doc 42 / extension 44 / ci 24。
...
- `mappings` 逐条计数（生成器实测，**与上表口径不同**）：合计 298 条 —— KEPT 146 / KEPT-PENDING-MERGE 3 / MERGED-INTO 149；其中在 `coverage.absorbed_entries` 内登记为 absorbed 的 140 条。
- 上表由各登记任务按 +N 维护，R13 只机器校验等式 `source_entry_count` == `len(mappings) − len(set(absorbed_entries))`（现为 158 == 298 − 140）；本文件**逐字引用**该块，不用逐条计数覆盖它。
- `absorbed_entries`：140 条登记（唯一 140 条），均为本波基线之外、由后续任务新增的执行单元。 注：`absorbed_note` = CHK-SECRET-HYGIENE 不属本波基线 146 项，来自 ROOT-006 注册文件（吸收并入）
...
| `CHK-SCHEMA` | doc | P0 | schema 校验 | `CON-CONFIG-CONTRACTS`、`LOG-CONTRACT-SELFCHECK` |
