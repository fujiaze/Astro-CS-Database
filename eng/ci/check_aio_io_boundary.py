...
────────────────────────────────────────────────────────────────────────────
  ...
  #4 的生产者见台账 eng/ci/ledgers/aio_io_boundary_inventory.json（逐条登记）。
...
INVENTORY（棘轮上限，fail-closed）
  N1 其余全部首方 C/C++ 源：每个命中文件必须在
     eng/ci/ledgers/aio_io_boundary_inventory.json 登记（逐条给理由 + 归属）。
  N2 未登记文件出现命中 ⇒ 红。
...
CLEAN-403（棘轮收口；fail-closed）
  ...
  C2 TEST-HARNESS 白名单与实际**一一对应**（文件 + 函数 + 理由）:
     ...
     · 条目必须给非空 functions（逐函数登记）;
     · 每个命中的所属函数必须已登记（未登记 ⇒ 红，含未登记文件）;
     · 登记函数必须确有命中（陈旧 ⇒ 红）。
...
INVENTORY_REL = "eng/ci/ledgers/aio_io_boundary_inventory.json"

# ── CLEAN-403: 测试桩白名单（文件 + 函数 + 理由；逐条一一对应） ──────────────
...
CATEGORY_RULES = (
    ...
    ("lib/infrastructure/scheduler/", "PRODUCTION-RESIDUAL",
     "生产面（scheduler）直读配置/产品文件；CLEAN-403 已收口 ⇒ 新增即红",
    ...
    ("lib/phase3_session/", "PRODUCTION-RESIDUAL",
     "生产面（阶段三会话）直建目录；CLEAN-403 已收口 ⇒ 新增即红",
     "CLEAN-403（已收口；新增越界须经 aio）"),
    ...
    ("lib/", "PRODUCTION-RESIDUAL",
     "未分类生产面残留 I/O；CLEAN-403 已收口 ⇒ 新增即红",
     "CLEAN-403（已收口；新增越界须经 aio）"),
)
...
def classify(rel):
    ...
    # PRODUCTION-RESIDUAL。
    if is_test_path(rel):
        return "TEST-HARNESS", TEST_HARNESS_REASON, TEST_HARNESS_OWNER
...
# ── CLEAN-403: 命中点的「所属函数」归属（白名单逐函数登记用） ────────────────
_FUNC_HEAD_RX = re.compile(r"([A-Za-z_~]\w*(?:::\w+)*)\s*$")
_CTRL_WORDS = {"if", "for", "while", "switch", "catch", "do", "else", "return",
               "sizeof", "namespace", "class", "struct", "union", "enum", "extern",
               "template", "try", "case", "default"}


def _name_from_signature(sig):
    """从作用域开启前的文本段抽名字（函数/类/命名空间）；控制语句与无名块返回 None。"""
    i = sig.find("(")
    head = sig[:i] if i >= 0 else sig
    m = _FUNC_HEAD_RX.search(head.strip())
    if not m:
        return None
    name = m.group(1)
    base = name.split("::")[-1].lstrip("~")
    if not base or base in _CTRL_WORDS:
        return None
    if base[0].isdigit():
        return None
    return name
...
def enclosing_functions(text):
    """逐行返回该行所属的**最内层具名作用域**（函数/方法/类/命名空间）。

    机制：单遍扫描（字符串/注释已掩码），在 `{` 处按「自上个 ;{} 以来的文本段」
    解析名字；无名块（if/for/lambda/初始化列表）不入栈具名层。取栈内最内层具名项；
    文件级（无具名作用域）记 "<file-scope>"。
    这是**确定性启发式**：台账由同一函数生成 ⇒ 登记值与判定值按构造一致；
    误判只会表现为「函数名粒度不同」，不会放宽任何判据（未登记即红）。
    """
    masked = mask_noncode(text)
    owners = []
    stack = []          # 每层作用域名（None = 无名块）
    seg = []

    def deepest():
        for nm in reversed(stack):
            if nm:
                return nm
        return None

    for line in masked.split("\n"):
        # 行归属 = 该行**处理过程中**出现过的最内层具名作用域（覆盖单行定义
        # `void f(){ ... }` 这类签名与命中同行的情形）。
        best = deepest()
        for ch in line:
            if ch == "{":
                stack.append(_name_from_signature("".join(seg)))
                seg = []
            elif ch == "}":
                if stack:
                    stack.pop()
            ...
            if cur:
                best = cur
        owners.append(best or "<file-scope>")
    return owners


def functions_for_hits(root, rel, found):
    """某文件命中行集合 → 去重排序的所属函数名列表。"""
    full = os.path.join(root, rel.replace("/", os.sep))
    try:
        with open(full, "r", encoding="utf-8", errors="replace") as fh:
            owners = enclosing_functions(fh.read())
    except OSError:
        return ["<unreadable>"]
    names = set()
    for ln, _rule, _snip in found:
        names.add(owners[ln - 1] if 0 < ln <= len(owners) else "<file-scope>")
    return sorted(names)
...
def build_inventory(root, hits):
    ...
    for rel in sorted(hits):
        ...
        # CLEAN-403: 逐函数登记（TEST-HARNESS 必须一一对应；其余类别同样登记
        # 以便复核，但不作一一对应判据）。
        entry["functions"] = functions_for_hits(root, rel, hits[rel])
...
def evaluate(root, inventory, strict_inventory):
    ...
    if inventory is None:
        return {
            ...
            "over": [], "stale": [], "diag": [], "a44": [],
            "residual": [], "misclassified": [], "fn_unregistered": [],
            "fn_stale": [],
    ...
    # CLEAN-403 门 1：PRODUCTION-RESIDUAL 必须为 0（不依赖是否已登记）。
    residual = sorted(rel for rel in inventory_hits
                      if classify(rel)[0] == "PRODUCTION-RESIDUAL")

    # CLEAN-403 门 2：TEST-HARNESS 白名单与实际一一对应。
    #   (a) 测试面文件不得登记为非 TEST-HARNESS（防「白名单掩盖生产路径」的反向
    #       滥用：把生产路径标成测试面）；
    #   (b) 测试面条目必须显式给非空 functions（文件 + 函数 + 理由）；
    #   (c) 每个命中的所属函数必须在该文件登记表内（未登记即红）；
    #   (d) 登记表内的函数必须确有命中（陈旧即红）。
    misclassified, fn_unregistered, fn_stale = [], [], []
    for rel in sorted(inventory_hits):
        entry = inventory.get(rel)
        cat = classify(rel)[0]
        if entry is None:
            if cat == "TEST-HARNESS":
                fn_unregistered.append((rel, "<file>", "未登记文件"))
            continue
        entry_cat = entry.get("category")
        if cat == "TEST-HARNESS" and entry_cat != "TEST-HARNESS":
            misclassified.append((rel, entry_cat, "测试面文件被登记为非 TEST-HARNESS"))
        if cat != "TEST-HARNESS" and entry_cat == "TEST-HARNESS":
            misclassified.append((rel, entry_cat, "非测试面文件被登记为 TEST-HARNESS"))
        if cat != "TEST-HARNESS":
            continue
        declared = entry.get("functions")
        if not isinstance(declared, list) or not declared:
            fn_unregistered.append((rel, "<file>", "缺 functions 登记（文件+函数+理由）"))
            continue
        actual = functions_for_hits(root, rel, inventory_hits[rel])
        for name in actual:
            if name not in declared:
                fn_unregistered.append((rel, name, "函数未登记"))
        for name in declared:
            if name not in actual:
                fn_stale.append((rel, name, "登记函数已无命中"))
...
def render(res, report, strict_inventory):
    ...
    print("  PRODUCTION-RESIDUAL (CLEAN-403 门 1: 生产残留必须为 0):")
    if res["residual"]:
        bad += len(res["residual"])
        for rel in res["residual"]:
            print("    VIOLATION %s (%d 处) — 生产路径直连 I/O，必须改经 aio"
                  % (rel, len(res["inventory_hits"].get(rel, []))))
    else:
        print("    PASS")
    print("  TEST-HARNESS 白名单 (CLEAN-403 门 2: 文件+函数+理由 一一对应):")
    th_total = sum(1 for rel in res["inventory_hits"]
                   if classify(rel)[0] == "TEST-HARNESS")
    th_bad = (len(res["misclassified"]) + len(res["fn_unregistered"])
              + len(res["fn_stale"]))
    if th_bad:
        bad += th_bad
        for rel, cat, why in res["misclassified"]:
            print("    VIOLATION %s — %s（登记类别=%s）" % (rel, why, cat))
        for rel, name, why in res["fn_unregistered"]:
            print("    VIOLATION %s :: %s — %s" % (rel, name, why))
        for rel, name, why in res["fn_stale"]:
            print("    VIOLATION %s :: %s — %s" % (rel, name, why))
    else:
        print("    PASS（测试面文件 %d 个，逐函数登记一致）" % th_total)
    if report:
        print("  ── 台账明细（按类别） ──")
    ...
    if bad:
        print("AIO-IO-BOUNDARY_FAIL: %d 项违规"
              "（HARD/A44/未登记/超限/PRODUCTION-RESIDUAL/白名单不一致）" % bad)
        return 1
    print("AIO-IO-BOUNDARY_PASS: HARD 层 = 0；A44 = 0；PRODUCTION-RESIDUAL = 0；"
          "TEST-HARNESS 白名单一一对应；台账无未登记/超限"
...
def self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="aio_io_boundary_selftest_") as td:
        ...
        _mk(os.path.join(tst, "stub_test.cpp"),
        ...
        pos_ok = (not res["hard"] and not res["unregistered"] and not res["over"]
                  ...
                  and not res["residual"] and not res["misclassified"]
                  and not res["fn_unregistered"] and not res["fn_stale"])
        ok = ok and pos_ok
        print("  self-test 正例(干净树 + aio 内部不误报 + 登记面 + 测试桩逐函数登记): [%s]"
        ...
        # 负例 9：**未登记**的测试文件直连 I/O ⇒ 必红（测试面不是豁免面）
        ...
        os.remove(os.path.join(tst, "sneaky_test.cpp"))

        # 负例 10：已登记测试文件出现**未登记函数**里的直连 I/O ⇒ 必红
        _mk(os.path.join(tst, "stub_test.cpp"),
            "void make_fixture(void){ std::ofstream f(\"fixture.bin\"); }\n"
        ...
        print("  self-test 负例10(测试桩新函数未登记): flagged=%s [%s]"
              % (n10, "PASS" if n10 else "FAIL"))
        _mk(os.path.join(tst, "stub_test.cpp"),
            "void make_fixture(void){ std::ofstream f(\"fixture.bin\"); }\n")

        # 负例 11：生产路径文件被登记成 TEST-HARNESS（白名单掩盖的反向滥用）⇒ 必红
        _mk(os.path.join(cli, "obs.cpp"),
            "void w(void){ std::ofstream f(\"resource_timeseries.csv\"); }\n")
        inv_bad = dict(inv)
        inv_bad["lib/infrastructure/cli/obs.cpp"] = {
            "path": "lib/infrastructure/cli/obs.cpp", "hits": 1,
            "category": "TEST-HARNESS", "functions": ["w"]}
        res = evaluate(td, inv_bad, strict_inventory=True)
        n11 = any(r == "lib/infrastructure/cli/obs.cpp"
                  for r, _c, _w in res["misclassified"])
        ok = ok and n11
        print("  self-test 负例11(生产文件登记为 TEST-HARNESS): flagged=%s [%s]"
              % (n11, "PASS" if n11 else "FAIL"))
        ...
        print("  self-test 负例12(登记函数陈旧): flagged=%s [%s]"
              % (n12, "PASS" if n12 else "FAIL"))

        # 负例 13：生产面（非测试路径）直连 I/O 且被登记 ⇒ PRODUCTION-RESIDUAL 判红
        _mk(os.path.join(alg, "residual.cpp"),
            "void r(void){ std::ofstream f(\"prod.bin\"); }\n")
        inv_res = dict(inv)
        inv_res["lib/algorithms/clean_mod/residual.cpp"] = {
            "path": "lib/algorithms/clean_mod/residual.cpp", "hits": 1,
            "category": "PRODUCTION-RESIDUAL", "functions": ["r"]}
        res = evaluate(td, inv_res, strict_inventory=True)
        n13 = "lib/algorithms/clean_mod/residual.cpp" in res["residual"]
        ok = ok and n13
        print("  self-test 负例13(生产残留 PRODUCTION-RESIDUAL 非零): flagged=%s [%s]"
