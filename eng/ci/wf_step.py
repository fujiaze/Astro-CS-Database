#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eng/ci/wf_step.py — workflow 声明步派发器（CI-001B 目标 1/2 的运行期执行面）。

用法:
    python3 eng/ci/wf_step.py --step <STEP_ID> [--dry-run] [--json]

设计（与 eng/ci/validate_workflow_binding.py 静态面配对）:
- workflow 的 run: 体不再承载任何业务命令，只允许出现本派发调用；
- 「这一步做什么」只声明在 eng/ci/workflow_binding.json，命令本体在 eng/ci/ 内；
- 声明与 eng/ci/checks.json 交叉核死：binds_check / serves_checks 必须在注册表内且
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="eng/ci/wf_step.py", description="workflow 声明步派发器")
    ap.add_argument("--step", required=True, help="eng/ci/workflow_binding.json 的 step_id")
