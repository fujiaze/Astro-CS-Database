# eng/ci/INVENTORY_REPORT.md — V8-CI-001 真实检查资产盘点报告
...
| ACR-DORMANT | `--forbid acr` 不存在；check_prod_reachability 需构建二进制 | 已对齐：以 eng/tools/check_legacy_exit.py 为 ACR dormant 门（PASS） |
| SYNTHETIC-SCIENCE | `eng/ci/run_synthetic.py` **不存在** | 未登记；留待执行器创建任务（V8-CI-002 后）按本 schema 入册 |
| LINUX-BUILD-TEST | `eng/ci/build_test.py` **不存在**；`--preset linux-ci-gcc` 不在 CMakePresets（仅 base-msvc/win-msvc-17.14.39-x64/linux-control） | 未登记；hosted 执行器任务负责 |
| LINUX-DEEP | `eng/ci/run_deep.py` **不存在** | 未登记 |
| WINDOWS-BUILD-PACKAGE | `eng/ci/build_test.py` **不存在** | 未登记；windows-main hosted |
| FATDUCK-PUBLISH-POLICY | 命令指向 runner 本机 `D:\AstroCSRunner\harness\verify-publish.exe`，非仓库入口 | 未登记；fatduck runner 部署任务负责 |

补充发现：`.github/` 目录不存在（无任何 workflow）——V8-CI-003+ 创建；`eng/ci/run.py` 统一执行器属 V8-CI-002。
