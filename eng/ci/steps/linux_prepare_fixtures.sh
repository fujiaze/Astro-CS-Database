#!/usr/bin/env bash
# eng/ci/steps/linux_prepare_fixtures.sh - linux-main 验证侧 fixture 准备步（CI-001B 迁出 workflow）。
#  来源：.github/workflows/ci-linux.yml 原 step "Prepare linux-main run/temp fixtures" 的
#  run: 体逐行迁移（CI-001B 目标 1：workflow 的 run: 体不再复制业务命令，统一由 eng/ci/ 声明体承担）。
#  绑定声明见 eng/ci/workflow_binding.json（step_id=LINUX-PREPARE-FIXTURES）。
#  该步自身注册为独立检查项的挂账见任务 commit message（eng/ci/checks.json 单写者冲突）。
set -euo pipefail
