#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eng/ci/steps/collect_bootstrap_diag.py — 失败后 bootstrap 诊断收集（CI-001B 迁出 workflow）。

来源：.github/workflows/ci-linux.yml 与 ci-windows.yml 原 step
「Collect bootstrap diagnostics」的内联 heredoc 体（两平台各一份，逐行等价）。
CI-001B 目标 1：workflow 的 run: 体不再内联业务命令，统一由 eng/ci/ 声明体承担
（声明见 eng/ci/workflow_binding.json，role=infra-step）。

语义与迁移前逐行等价：

- 仅在失败路径被调用（workflow 侧 if: failure()），不掩盖首要失败原因；
- 采集 eng/ci/bootstrap.py --json 的 rc / 报告 / 结构化 stderr FAIL 载荷；
...
def collect(platform: str, repo: Path, timeout: float = 120.0) -> dict:
    ...
    try:
        res = subprocess.run(
            [sys.executable, "eng/ci/bootstrap.py",
             "--policy", "eng/ci/toolchain.policy.json", "--platform", platform, "--json"],
