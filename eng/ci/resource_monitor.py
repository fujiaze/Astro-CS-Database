...
桥接 shim：
  ...
  * 无参数运行 → 打印能力说明 JSON 并 exit 0（被 run.py 存在性探测覆盖；
    也可用于 CI 冒烟确认监控层就绪）；
  * 带参数运行 → 透传给 eng/tools/monitoring/run_monitored.py
    （等价于 ``python3 eng/tools/monitoring/run_monitored.py <args>``），
    退出码透传（124=超时、128+signum=信号、否则为子进程退出码）。

不复制监控逻辑，单一事实源始终是 eng/tools/monitoring/run_monitored.py。
...
MONITORED = REPO / "eng" / "tools" / "monitoring" / "run_monitored.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not MONITORED.is_file():
        print(f"eng/ci/resource_monitor.py: 监控包装器缺失：{MONITORED}", file=sys.stderr)
        return 2
    # runpy 运行 run_monitored.py 的 __main__，SystemExit 透传其退出码。
    sys.argv = [str(MONITORED)] + argv
    # V8-CI-010 F-R2-01 修复：与直接执行 ``python3 eng/tools/monitoring/run_monitored.py``
    # 对齐 sys.path/模块解析语义。脚本模式下解释器会把被执行脚本所在目录置于
    # sys.path[0]；而 runpy.run_path 不做该注入——本 shim 自身被调用时
    # sys.path[0] 是本文件所在 eng/ci/ 目录，run_monitored 的同目录导入
