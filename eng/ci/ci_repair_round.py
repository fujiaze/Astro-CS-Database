#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eng/ci/ci_repair_round.py — CI 修复常驻线轮报器（CI-REPAIR-001）。
...
WHITELIST_PREFIXES = ("eng/ci/", "eng/tools/quality/", "run/ci_repair/")
...
# 写入白名单只有 run/ci_repair、eng/ci/、eng/tools/quality，凡根因落在这三域之外的
# 红灯一律 in_whitelist=false → 登记 finding 交域主，禁止就地放宽检查器。
ATTRIBUTION = {
    "AGENTS-GOV": {
        "domain": "GOV",
        "owner_node": "GOV-001|GOV-002",
        "root_cause": "AGENTS.md 瘦身（V5 治理要素迁入冻结宪章）后未被 eng/tools/check_agents_gov.py 的 10 项关键词断言同步",
    ...
    "THREAD-BUDGET": {
        ...
        "root_cause": "lib/{cosmetic,drizzle,calibration}/src/module_entry.cpp 的 *_OMP_SET 宏经 omp_set_num_threads 注入，未登记进 eng/tools/arch/check_thread_budget.py REGISTERED",
        "root_cause_path": "lib/cosmetic/src/module_entry.cpp",
        "in_whitelist": False,
        "minimal_patch": "三处宏是 host budget 注入形态（非硬编码字面量）：登记进 eng/tools/arch/check_thread_budget.py REGISTERED（键=module_entry.cpp 不可，须逐文件路径键），或按 ARCH-004 改为经 host callback 注入",
    },
    "CON-COMMENTS": {
        "domain": "CON",
        ...
        "in_whitelist": False,
        "minimal_patch": "在 4 个文件的对应不变量注释补 SCI/ALG ID 引用（如 HIPS_WRITER §9 不变量行补 SCI-HIPS-xxx），不得改 eng/tools/quality/contracts/check_comments.py",
    },
    "CON-FULL-INTEGRATION": {
        ...
        "root_cause_path": "eng/tools/quality/contracts/check_full_integration.py",
        "in_whitelist": False,
    ...
    "KNOWN-FAILURES-BASELINE-CHECK": {
        ...
        "root_cause": "全量测试失败集不满足「失败集 ⊆ 版本化基线」：出现不在 eng/ci/known_failures.json 的新失败、基线项 expected=fail 却已修复未删除、条目过期，或本次 run 缺结果来源（fail-closed）",
        "root_cause_path": "eng/ci/known_failures.json",
        "in_whitelist": True,
        "minimal_patch": "按 eng/tools/quality/known_failures_baseline.py --mode check 证据 JSON 处置：新失败交域主修复（不得登记进基线）、已修复项删除对应基线条目、过期项重登记或删除；禁止把永不允许豁免类别（SCI/ALG Oracle、ABI、生产路由、ACR dormant、heavy 单线程/低利用率、泄漏、崩溃、数据损坏、版本一致性、追踪断裂、安全凭据）写入基线",
    },
    "KNOWN-FAILURES-BASELINE-VERIFY": {
        "domain": "CI",
        "owner_node": "CI-BASELINE-001",
        "root_cause": "版本化基线文件自身非法：缺 owner/reason/首次登记 commit/reproducer/expiry/移除条件、首次登记 commit 不可达、unit 不在 eng/ci/checks.json（或 CTest 目标集）、类别越界或属永不允许豁免类别、条目过期",
        "root_cause_path": "eng/ci/known_failures.json",
        "in_whitelist": True,
        "minimal_patch": "按 eng/tools/quality/known_failures_baseline.py --mode verify 的逐条 V1..V8 错误订正 eng/ci/known_failures.json（补齐字段/修正 commit/删除越界或过期条目）；不得放宽校验规则",
    },
    "linux": {
        "domain": "CI-PLATFORM",
        "owner_node": "CI-REPAIR-001（本线：聚合与映射）",
        "root_cause": "job 级红灯 = linux-main profile 内检查项红灯聚合（无独立根因）",
        "root_cause_path": "eng/ci/run.py",
        "in_whitelist": False,
...
def attribute(name: str) -> dict:
    ...
    if entry is None:
        return {
            ...
            "in_whitelist": False,
            "minimal_patch": "在 eng/ci/ci_repair_round.py ATTRIBUTION 登记域名/承接节点/根因路径",
            "needs_mapping": True,
        }
    out = dict(entry)
...
def collect(args, token: str) -> dict:
    ...
    findings = {
        ...
        "notes": [
            "归因域为「根因所在域」，与检查器所在目录无必然关系；in_whitelist 判定按 CI-REPAIR-001 写入白名单（run/ci_repair、eng/ci/、eng/tools/quality）。",
