...
1. 注册表 conformance —— WIN-BUILD-RELEASE / WIN-TEST-UNIT /
   ...
   ``eng/ci/resource_monitor.py --timeout N --output run/ci/monitor/<ID>.json --``
...
_REG = json.loads((_REPO / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))
...
_spec = importlib.util.spec_from_file_location(
    "ci_windows_driver", _REPO / "eng" / "tools" / "quality" / "ci_windows_driver.py")
...
class TestRegistryConformance(unittest.TestCase):
    ...
    def test_windows_heavy_monitor_wrapper_prefix(self):
        for cid in _WIN_IDS:
            ...
            self.assertEqual(c["command"][:2],
                             ["python3", "eng/ci/resource_monitor.py"], cid)
            i = c["command"].index("--")
            self.assertEqual(c["command"][i + 1 : i + 3],
                             ["python3", "eng/tools/quality/ci_windows_driver.py"], cid)
    ...
    def test_registry_strict_pass(self):
        ...
        errors, n = VR.validate(_REPO / "eng" / "ci" / "checks.json", strict=True)
    ...
    def test_runner_skips_windows_checks_on_linux(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            shutil.copyfile(_REPO / "eng" / "ci" / "resource_monitor.py",
                            repo / "eng" / "ci" / "resource_monitor.py")
            shutil.copyfile(_REPO / "eng" / "tools" / "quality" / "ci_windows_driver.py",
                            repo / "eng" / "tools" / "quality" / "ci_windows_driver.py")
