...
``eng/ci/resource_monitor.py``（runpy.run_path 桥接 eng/tools/monitoring/run_monitored.py）
执行时 sys.path 断裂 —— shim 自身 sys.path[0]=eng/ci/，runpy 不做"目标脚本目录
...
本组测试锁定修复语义：
1. 包装执行与直接执行 ``python3 eng/tools/monitoring/run_monitored.py`` 的
...
def _load_shim():
    spec = importlib.util.spec_from_file_location(
        "resource_monitor_shim", _REPO / "eng" / "ci" / "resource_monitor.py")
...
SHIM = _load_shim()
MONITOR_DIR = _REPO / "eng" / "tools" / "monitoring"
...
class TestShimSysPathSemantics(unittest.TestCase):
    ...
    def test_no_args_capability_does_not_pollute_sys_path(self):
        ...
        self.assertEqual(code, 0)
        cap = json.loads(out)
        self.assertEqual(cap["script"], "eng/ci/resource_monitor.py")
