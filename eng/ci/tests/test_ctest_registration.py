...
1. 注册闭包（负向必败）—— 在内存 fixture CMake 源上故意加一个未注册的
   ...
   eng/ci/checks.json 的 ctest_targets 显式登记（且 command 携带）→ PASS；
...
_TOOL = _REPO / "eng" / "tools" / "quality" / "check_ctest_registration.py"
_spec = importlib.util.spec_from_file_location("check_ctest_registration", _TOOL)
TOOL = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(TOOL)

_REGISTRY = json.loads((_REPO / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))
_BASELINE = json.loads((_REPO / "eng" / "ci" / "ctest_baseline.json").read_text(encoding="utf-8"))
