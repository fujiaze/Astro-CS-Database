...
REPO = Path(__file__).resolve().parents[3]   # 主仓库根（只读引用）
...
def write_registry(repo: Path, checks: list[dict], *, with_schema: bool = False) -> Path:
    """向 fixture 仓库写入 eng/ci/checks.json（可选复制主仓库 registry schema 副本）。"""
    reg_dir = repo / "eng" / "ci"
    ...
    if with_schema:
        src = REPO / "eng" / "ci" / "checks.schema.json"
        if src.is_file():
...
def write_ci_result_schema(repo: Path) -> Path:
    ...
    dst.write_text((REPO / "eng" / "ci" / "ci_result.schema.json").read_text(encoding="utf-8"),
                   encoding="utf-8")
    return dst
