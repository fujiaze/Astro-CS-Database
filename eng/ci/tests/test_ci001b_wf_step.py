...
def run_wf_step(root: Path, *args: str, timeout: float = 120):
    return subprocess.run(
        [sys.executable, str(_REPO / "eng" / "ci" / "wf_step.py"),
         "--manifest", str(root / "eng" / "ci" / "workflow_binding.json"),
         "--registry", str(root / "eng" / "ci" / "checks.json"),
...
class TestDeclarationDrift(unittest.TestCase):
    ...
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.root = make_fixture(Path(self._tmp.name))
        self.manifest = json.loads((self.root / "eng" / "ci" / "workflow_binding.json").read_text(encoding="utf-8"))
        self.registry = json.loads((self.root / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))

    def tearDown(self):
        self._tmp.cleanup()

    def _write(self):
        (self.root / "eng" / "ci" / "workflow_binding.json").write_text(
            json.dumps(self.manifest, ensure_ascii=False, indent=2), encoding="utf-8")
        (self.root / "eng" / "ci" / "checks.json").write_text(
    ...
    def test_exec_target_missing(self):
        for step in self.manifest["steps"]:
            if step["step_id"] == "WINDOWS-VALIDATE-CANDIDATE":
                step["exec"] = ["python", "eng/ci/no_such_validator.py", _ZIP_REL]
        self._write()
        res = run_wf_step(self.root, "--step", "WINDOWS-VALIDATE-CANDIDATE", "--dry-run")
...
class TestTimeout(unittest.TestCase):
    ...
    def test_timeout_enforced(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            slow = root / "eng" / "ci" / "_slow.py"
            slow.write_text("import time\ntime.sleep(30)\n", encoding="utf-8")
            manifest = json.loads((root / "eng" / "ci" / "workflow_binding.json").read_text(encoding="utf-8"))
            manifest["steps"].append({
                "step_id": "TMP-SLOW", "workflow": ".github/workflows/ci-linux.yml",
                "job": "linux", "name": "TMP slow", "role": "infra-step",
                "exec": ["python3", "eng/ci/_slow.py"], "timeout_seconds": 1, "fail_closed": False,
            })
            (root / "eng" / "ci" / "workflow_binding.json").write_text(
