...
- GAP 补齐轮（前台裁决）：
  * GAP-G1（WRITE_LEASE 消费）→ **POLICY(控制面保障)**，不实现：WRITE_LEASE 是控制面
    状态，其校验由前台 dispatch 流程人工保障，不进 eng/ci/run.py（hosted runner 上无
    lease 文件）；对应用例为登记 POLICY 裁决的说明性测试（锚点断言 run.py 无
    WRITE_LEASE 消费，裁决改变时翻红提示更新矩阵）。
  * GAP-G2（硬编码线程）→ 已补齐：eng/ci/validate_registry.py 增 R9
    （hardcoded_core_in_command），registry 维度用例翻红为拒绝断言；
    执行链 env 直通维度登记 POLICY（命令参数通道已被 R9 封锁）。
  * GAP-G3（zip 路径逃逸）→ 已补齐：eng/ci/validate_candidate.py 增
    path_escape_in_zip 校验，用例翻红为拒绝断言。
- 类 6/7/12/13 的守卫位于 eng/ci/tests/test_fatduck_workflow.py / test_workflow_lock.py
...
LOCK_PATH = REPO / "eng" / "ci" / "actions.lock.json"
VALIDATOR = REPO / "eng" / "ci" / "validate_registry.py"
VERIFY_LOCK = REPO / "eng" / "ci" / "verify_actions_lock.py"
BOOTSTRAP = REPO / "eng" / "ci" / "bootstrap.py"
VALIDATE_CAND = REPO / "eng" / "ci" / "validate_candidate.py"
...
def _assert_fatduck_shape(doc: dict) -> None:
    """镜像 eng/ci/tests/test_fatduck_workflow.py 的形状守卫（只读，不改原测试）。
    ...
    assert "eng/ci/" not in script, "fatduck-validate 出现仓库相对路径"
...
class TestN01FakePassJson(unittest.TestCase):
    ...
    def test_invalid_json_registry_rejected_exit2(self):
        repo = _mk_repo()
        (repo / "eng" / "ci" / "checks.json").write_text("{oops", encoding="utf-8")
    ...
    def test_forged_pass_overwritten_by_execution(self):
        ...
        self.assertEqual(ci["generated_by"], "eng/ci/run.py (V8-CI-002)")
...
class TestN04DirtyWorkspaceAndLease(unittest.TestCase):
    ...
    - N04b（GAP-G1 → **POLICY(控制面保障)**，前台裁决不实现）：WRITE_LEASE 是
      控制面状态，其校验由前台 dispatch 流程人工保障，不进 eng/ci/run.py
    ...
    def test_write_lease_control_plane_policy__GAP_G1(self):
        ...
        dispatch 流程人工保障；hosted runner 上无 lease 文件，故 eng/ci/run.py
        不消费 WRITE_LEASE。本用例不做注入、不断言拒绝，仅锚定裁决现状。
        """
        src = (REPO / "eng" / "ci" / "run.py").read_text(encoding="utf-8")
    ...
    def test_unregistered_relative_script_rejected_by_r4(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reg.write_text(json.dumps({"schema_version": 1, "checks": [
                _check(id="N-UNREG", command=["python3", "eng/ci/nonexist.py"])]}),
                encoding="utf-8")
...
class TestN07TagAction(unittest.TestCase):
    ...
    def test_tampered_lock_short_sha_rejected(self):
        sys.path.insert(0, str(REPO / "eng" / "ci"))
        try:
...
class TestN08HardcodedCore(unittest.TestCase):
    ...
    GAP-G2 → **REJECTED(本轮补齐)**：eng/ci/validate_registry.py 增规则 R9
    （hardcoded_core_in_command）——command 全元素出现 -j<N>/--jobs/--threads/
    NPROC/*_NUM_THREADS 即 strict 拒绝；eng/ci/resource_monitor.py 前缀与 `--`
    ...
    def test_registry_j4_argument_rejected_r9(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reg.write_text(json.dumps({"schema_version": 1, "checks": [
                _check(id="N-J4",
                       command=["python3", "eng/ci/select_profile.py", "-j4"])]}),
                encoding="utf-8")
            proc = _registry_verdict(reg)
            self.assertEqual(proc.returncode, 1)
    ...
    def test_registry_threads_argument_rejected_r9(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reg.write_text(json.dumps({"schema_version": 1, "checks": [
                _check(id="N-THR",
                       command=["python3", "eng/ci/select_profile.py", "--threads=64"])]}),
                encoding="utf-8")
            proc = _registry_verdict(reg)
            self.assertEqual(proc.returncode, 1)
            self.assertIn("R9", proc.stdout)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reg.write_text(json.dumps({"schema_version": 1, "checks": [
                _check(id="N-MON", profiles=["linux-deep"], heavy=True,
                       ...
                       command=["python3", "eng/ci/resource_monitor.py",
                                "--timeout", "3500", "--output", "run/m.json",
                                "--", "python3", "eng/ci/select_profile.py"])]}),
                encoding="utf-8")
            proc = _registry_verdict(reg)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reg.write_text(json.dumps({"schema_version": 1, "checks": [
                _check(id="N-HEAVY", heavy=True, requires_monitor=False,
                       profiles=["linux-deep"],
                       command=["python3", "eng/ci/select_profile.py"])]}),
                encoding="utf-8")
            proc = _registry_verdict(reg)
            self.assertEqual(proc.returncode, 1)
...
class TestN12FatduckCheckout(unittest.TestCase):
    ...
    def test_repo_relative_ci_path_rejected(self):
        ...
        step = next(s for s in doc["jobs"]["fatduck-validate"]["steps"]
                    if "run" in s)
        step["run"] = step["run"] + "\npython3 eng/ci/validate_candidate.py"
        with self.assertRaises(AssertionError):
            _assert_fatduck_shape(doc)
