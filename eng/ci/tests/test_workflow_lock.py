...
_LOCK_PATH = _REPO / "eng" / "ci" / "actions.lock.json"
_POLICY_PATH = _REPO / "eng" / "ci" / "toolchain.policy.json"
...
VAL = _load_module("verify_actions_lock", "eng/ci/verify_actions_lock.py")
BS = _load_module("bootstrap", "eng/ci/bootstrap.py")
SP = _load_module("select_profile", "eng/ci/select_profile.py")
VC = _load_module("validate_candidate", "eng/ci/validate_candidate.py")
...
class TestActionsLockStructure(unittest.TestCase):
    """eng/ci/actions.lock.json 结构校验（VAL.load_lock）。"""
...
class TestWorkflowYaml(unittest.TestCase):
    ...
    def test_windows_candidate_gate_is_registry_bound(self):
        ...
        由 eng/ci/checks.json 的 WIN-CANDIDATE-VALIDATE（waivable=false）在
        eng/ci/run.py 内 FAIL，且 workflow 体内不再承载任何业务命令。
        """
        ...
        registry = json.loads((_REPO / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(gate[0]["command"],
                         ["python3", "eng/ci/wf_step.py", "--step", "WINDOWS-VALIDATE-CANDIDATE"])
        binding = json.loads((_REPO / "eng" / "ci" / "workflow_binding.json").read_text(encoding="utf-8"))
        ...
        self.assertTrue(entry[0]["exec"][1].endswith("eng/ci/validate_candidate.py"))
    ...
    def test_collect_bootstrap_diagnostics_step_on_failure(self):
        ...
        for name, doc in self.docs.items():
            ...
            # CI-001B：诊断体迁入 eng/ci/ 声明体（workflow 体只保留派发调用），
            # 证据路径断言随之绑定到声明 exec 指向的 eng/ci/ 脚本本体。
            ...
            self.assertIn("eng/ci/wf_step.py --step " + entry[0]["step_id"], body, name)
...
class TestVerifyActionsLockOffline(unittest.TestCase):
    ...
    def test_real_lock_offline_pass(self):
        res = run_script(_REPO / "eng" / "ci" / "verify_actions_lock.py",
                         "--offline", "--json")
...
class TestBootstrap(unittest.TestCase):
    ...
    def test_local_host_fail_exit2_structured(self):
        ...
        res = run_script(_REPO / "eng" / "ci" / "bootstrap.py",
                         "--policy", "eng/ci/toolchain.policy.json",
                         "--platform", "linux", "--json")
...
class TestSelectProfile(unittest.TestCase):
    ...
    SCRIPT = _REPO / "eng" / "ci" / "select_profile.py"
...
class TestValidateCandidate(unittest.TestCase):
    ...
    def test_minimal_valid_zip_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            res = run_script(_REPO / "eng" / "ci" / "validate_candidate.py",
...
class TestProfileCountsRegression(unittest.TestCase):
    ...
    def test_plan_only_counts_match_registry(self):
        ...
        for profile in ("fast", "linux-main", "linux-deep", "windows-main"):
            ...
            res = H.sh([sys.executable, str(_REPO / "eng" / "ci" / "run.py"),
    ...
    def test_registry_strict_still_passes(self):
        res = run_script(_REPO / "eng" / "ci" / "validate_registry.py",
                         "--registry", "eng/ci/checks.json", "--strict")
        self.assertEqual(res.returncode, 0, res.stderr)
