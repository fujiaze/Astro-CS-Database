# -*- coding: utf-8 -*-
"""eng/ci/tests/test_ci_repair_round.py — CI 修复常驻线轮报器回归（CI-REPAIR-001）。

测试设计（冻结宪章 §17 + 控制包 06 §3）：
  * 先红后绿：本文件先于 eng/ci/ci_repair_round.py 落地，模块缺失时全部 ERROR。
  * 负向注入必败（不得静默丢弃/不得假绿）：
...
TOOL = REPO / "eng" / "ci" / "ci_repair_round.py"
    ...
    def test_09_every_attribution_entry_is_complete(self):
        ...
        for key, entry in M.ATTRIBUTION.items():
            if entry["in_whitelist"]:
                ...
                self.assertTrue(rc.startswith(("eng/ci/", "eng/tools/quality/", "run/ci_repair/")),
