...
2. 注册表结构 —— validate_registry --strict 接受合法 prerequisite_tools、
   拒绝空数组/非字符串/非字符串数组元素；主仓库 eng/ci/checks.json（99 项）strict PASS。
3. profile 选择（只读主仓库 plan-only）——fast=59（不含新 deep 项；含
   ...
   全选，command 自含 eng/ci/resource_monitor.py 包裹前缀）。
...
def _declared(profile: str) -> list:
    """注册表里声明属于该 profile 的**顶层注册项**（eng/ci/run.py --plan-only 同口径）。
...
class TestRegistryStrict(unittest.TestCase):
    ...
    def test_main_registry_strict_pass_104(self):
        ...
        #  eng/ci/run_checks.py --check <ID>，真正的检查器在同名 step 里）。
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for c in data["checks"]:
                if c.get("command", [None, None])[1:2] == ["-c"]:
                    ...
                    c["command"] = ["python3", "eng/ci/validate_registry.py"]
...
class TestProfileSelection(unittest.TestCase):
    ...
    def test_heavy_commands_self_contain_monitor_prefix(self):
        ...
        # 项（command = eng/ci/run_checks.py --check <ID>）自身不带前缀，由 step 承载
        ...
        for c in entries:
            ...
            if c["command"][:2] == ["python3", "eng/ci/resource_monitor.py"]:
                direct += 1
                continue
            containers += 1
            ...
            for s in steps:
                self.assertEqual(s["command"][:2],
                                 ["python3", "eng/ci/resource_monitor.py"], s["id"])
...
class TestToolBehaviour(unittest.TestCase):
    ...
    def test_complexity_real_run_baseline_contract(self):
        ...
        proc = H.sh(["python3", str(REPO / "eng/tools/quality/check_complexity.py"),
    ...
    def test_complexity_excludes_acr_and_third_party(self):
        ...
        spec = spec_from_file_location(
            "cxc", REPO / "eng/tools/quality/check_complexity.py")
    ...
    def test_driver_rejects_outside_build_dir(self):
        proc = H.sh(["python3", str(REPO / "eng/tools/quality/deep_ci_driver.py"),
    ...
    def test_driver_step_timeout_returns_124(self):
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            "dcd", REPO / "eng/tools/quality/deep_ci_driver.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
    ...
    def test_driver_qa_sanitize_cache_flag_sanitizer_runtime(self):
        ...
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            "dcd_f1a", REPO / "eng/tools/quality/deep_ci_driver.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
    ...
    def test_driver_qa_sanitize_cache_flag_tsan_unchanged(self):
        ...
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            "dcd_f1b", REPO / "eng/tools/quality/deep_ci_driver.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
        captured: list[list[str]] = []
        with mock.patch.object(mod, "run_step",
    ...
    def test_coverage_runner_outside_output_dir_exit_2(self):
        proc = H.sh(["python3", str(REPO / "eng/tools/quality/ci_coverage_runner.py"),
    ...
    def test_coverage_runner_repo_relative_output_dir_hosted_shape(self):
        ...
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            "ccr_f6", REPO / "eng/tools/quality/ci_coverage_runner.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
    ...
    def _coverage_runner_with_fake_pytest(self, returncode: int,
        ...
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            f"ccr_{tmp_tag}", REPO / "eng/tools/quality/ci_coverage_runner.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
...
class TestCoverageCppDriver(unittest.TestCase):
    ...
    def _load_driver_mod(self):
        from importlib.util import spec_from_file_location, module_from_spec
        spec = spec_from_file_location(
            "dcd_covcpp", REPO / "eng/tools/quality/deep_ci_driver.py")
        mod = module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod
