# -*- coding: utf-8 -*-
"""CI-001B 单测：workflow ↔ 注册表联动核死（eng/ci/validate_workflow_binding.py）。
...
2. 负向注入必败（逐条独立 fixture 仓库，离线）：
   ...
   - 把注册表检查命令（或迁出的 eng/ci/ 体）抄回 workflow run: 体 →
...
_SUPPORT = ("eng/ci/checks.json", "eng/ci/actions.lock.json", "eng/ci/workflow_binding.json",
            "eng/ci/validate_candidate.py")
...
VB = _load("ci001b_validate_workflow_binding", "eng/ci/validate_workflow_binding.py")
...
def make_fixture(root: Path) -> Path:
    ...
    shutil.copytree(_REPO / "eng" / "ci" / "steps", root / "eng" / "ci" / "steps")
...
def validate(root: Path):
    return VB.validate(root, root / "eng" / "ci" / "workflow_binding.json",
                       root / "eng" / "ci" / "checks.json", root / "eng" / "ci" / "actions.lock.json",
                       ".github/workflows")
...
class TestRealRepoBinding(unittest.TestCase):
    ...
    def test_live_repo_binding_pass(self):
        ...
        self.assertEqual(report["registry"]["checks"], len(json.loads(
            (_REPO / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))["checks"]))
    ...
    def test_live_workflows_carry_no_business_commands(self):
        ...
        for rel in _WORKFLOWS:
            ...
            for token in ("cmake", "ctest", "eng/ci/validate_candidate.py",
                          "eng/ci/prepare_linux_fixtures.py", "eng/tools/quality/",
                          "eng/ci/steps/"):
                self.assertNotIn(token, code, rel + " 命令体含 " + token)
            self.assertIn("eng/ci/run.py --profile", code, rel)
...
class TestNegativeInjections(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.manifest = self.root / "eng" / "ci" / "workflow_binding.json"
        self.registry = self.root / "eng" / "ci" / "checks.json"
    ...
    def test_unbound_step_fails(self):
        ...
        text = read(self.linux).replace(
            ...
            "      - name: Sneaky business step\n        run: python3 eng/ci/run.py --profile linux-main\n"
    ...
    def test_registry_command_copied_back_fails(self):
        ...
        text = mutate_step(read(self.linux), "Collect bootstrap diagnostics",
                           "python3 eng/ci/wf_step.py --step LINUX-BOOTSTRAP-DIAG",
                           "python3 eng/tools/quality/check_ctest_registration.py --output x.json")
        write(self.linux, text)
        errors, _n, _r = validate(self.root)
    ...
    def test_warning_skip_branch_on_registered_output_fails(self):
        ...
        old_body = ("\n" + indent).join([
            ...
            "  python eng/ci/validate_candidate.py artifacts/candidate/AstroCS-candidate.zip",
            "} else {",
        ...
        text = mutate_step(read(self.windows), "Collect bootstrap diagnostics",
                           "python eng/ci/wf_step.py --step WINDOWS-BOOTSTRAP-DIAG", old_body)
    ...
    def test_check_command_drift_fails(self):
        ...
        for check in reg["checks"]:
            if check["id"] == "LINUX-MAIN-FIXTURES":
                check["command"] = ["python3", "eng/ci/prepare_linux_fixtures.py"]
        write(self.registry, json.dumps(reg, ensure_ascii=False, indent=2))
        errors, _n, _r = validate(self.root)
    ...
    def test_business_command_in_provisioning_fails(self):
        ...
        text = mutate_step(
            ...
            "          timeout 240 sudo apt-get update -qq\n"
            "          python3 eng/ci/prepare_linux_fixtures.py")
        write(self.linux, text)
        errors, _n, _r = validate(self.root)
        self.assertIn("business_command_in_workflow", codes(errors))
        self.assertIn("registry_command_copied_into_workflow", codes(errors))
    ...
    def test_missing_exec_target_fails(self):
        ...
        for step in data["steps"]:
            if step["step_id"] == "LINUX-PREPARE-FIXTURES":
                step["exec"] = ["bash", "eng/ci/steps/no_such_step.sh"]
        write(self.manifest, json.dumps(data, ensure_ascii=False, indent=2))
        errors, _n, _r = validate(self.root)
