...
2. 监控必须调用 evaluate 的 CI 落地面（F-CI-002-04/06 owner 裁决原则
   ...
   eng/ci/run.py 合同收窄为"命令请求了判定就必须兑现判定证据"
...
MONITOR_CMD_TEMPLATE = [
    "python3", "eng/ci/resource_monitor.py", "--timeout", "30",
    "--output", MON_JSON,
]
...
def _load_registry() -> list[dict]:
    return json.loads((_REPO / "eng" / "ci" / "checks.json").read_text(encoding="utf-8"))["checks"]
...
def _runner_mon_check(repo: Path, out_root: Path, check: dict) -> subprocess.CompletedProcess:
    ...
    # probe_prerequisite 要求统一监控包装器存在（repo/eng/ci/resource_monitor.py）；
    ...
    shutil.copyfile(_REPO / "eng" / "tools" / "monitoring" / "resource_probe.py",
    ...
    **非重计算面**，冻结阈值语义针对重计算区间；**重计算面必须显式请求
    ...
    def test_registry_recompute_surface_requests_gate(self):
        ...
        for c in gate_checks:
            cmd = c["command"]
            self.assertIn("eng/ci/resource_monitor.py", cmd,
                          f"{c['id']} 请求判定必须经 eng/ci/resource_monitor.py 并落判定证据")
    ...
    def test_registry_non_recompute_surfaces_do_not_request_gate(self):
        ...
        reg = _units()
        # 聚合项（eng/ci/run_checks.py 派发）自身不带监控包装；判定旗标只允许出现在叶子单元
        leaves = [c for c in reg.values() if c.get("requires_monitor")
                  and "eng/ci/run_checks.py" not in c.get("command", [])]
        ...
        for c in leaves:
            ...
            self.assertNotIn("--gate-workers", cmd,
                             f"{c['id']} 不得经 --gate-workers 请求判定（容量取自 host_probe）")
        for c in reg.values():
            if "eng/ci/run_checks.py" in c.get("command", []) and "--" in c["command"]:
    ...
    def test_win_checks_excluded_from_gate_by_owner_ruling(self):
        ...
        **CI-003 更新**：原注「CI 注册表当前零 --gate-required」已不成立——按
        ...
        for cid in _WIN_IDS:
            ...
            self.assertIn("eng/ci/resource_monitor.py", c["command"],
                          f"{cid} 仍保留监控包装（采样留证）")
    ...
    def test_linux_heavy_checks_keep_monitor_sampling(self):
        ...
        for cid in ("BUILD-GCC-RELEASE", "DEEP-CLANG-BUILD", "DEEP-SAN-ASAN",
            ...
            self.assertIn("eng/ci/resource_monitor.py", c["command"], cid)

    def test_ut_quality_registration_conformance(self):
        """UT-QUALITY 登记矛盾修正：命令无监控包装 → heavy/requires_monitor 均 false。"""
        reg = _units()
        c = reg["UT-QUALITY"]
        self.assertNotIn("eng/ci/resource_monitor.py", c["command"],
...
class TestRunnerGateEvidenceContract(unittest.TestCase):
    ...
    def _gate_cmd(cls, writer: str) -> list[str]:
        ...
        return (["python3", "eng/ci/resource_monitor.py", "--timeout", "30",
