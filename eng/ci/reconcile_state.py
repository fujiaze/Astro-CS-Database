#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eng/ci/reconcile_state.py — V81-ADOPT-003 任务与证据对账验收脚本（最小实现）。

用法：
    python3 eng/ci/reconcile_state.py --current-first --strict
...
REPO_CANDIDATES = [
    Path(__file__).resolve().parent.parent.parent,          # <repo>/ci/reconcile_state.py
    Path.cwd(),
]
...
def report(checks, args, root) -> int:
    ...
    ok = not failed
    summary = {
        "script": "eng/ci/reconcile_state.py",
