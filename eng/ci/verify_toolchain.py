...
用法:
    python3 eng/ci/verify_toolchain.py --policy eng/ci/toolchain.policy.json \
        --actual eng/ci/toolchain.lock.json --scope agent-host
...
校验（任一失败 exit 非零）:
  ...
  3) eng/cmake/python/git 条目存在且信息为实测: present 者版本非空、非占位且能被实测
...
def main():
    ...
    # ── ③/⑤ eng/cmake/python/git 实测版本（非空、非占位、正则复匹配）──────
