...
已登记差异（registry）
  eng/tools/fixtures/config_consistency_known_divergences.json 承载**已审计且未修**的差异，
...
用法
  python3 eng/tools/config_consistency_check.py [--root .] [--json-out F] [--strict]
  python3 eng/tools/config_consistency_check.py --self-test
exit 0 = 一致（或差异已登记）；1 = 判据违规；2 = 输入/台账不可用（fail-closed）。

只读；仅 stdlib；stdout 末段 JSON 保留 legacy 键 checked_keys/mismatches/pass
（eng/tools/api_doc_consistency.py 消费）。
"""
...
REGISTRY_REL = "eng/tools/fixtures/config_consistency_known_divergences.json"
