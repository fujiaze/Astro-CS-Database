...
两部分职责（互不影响，legacy 行为零改动）：
  ...
  B. CI 机器化门（CI-BASELINE-001，控制包依据 05_FINDINGS_REGISTER_20260911 §STD-F9
     ...
     仓库内 eng/ci/known_failures.json（每项含 owner / reason / 首次登记 commit /
...
用法：
  python3 eng/tools/quality/known_failures_baseline.py                       # legacy
  python3 eng/tools/quality/known_failures_baseline.py --mode verify --output run/ci/known-failures/known_failures_baseline.json
  python3 eng/tools/quality/known_failures_baseline.py --mode check --ctest-junit run/ci/build-gcc-release/ctest-full.junit.xml
  python3 eng/tools/quality/known_failures_baseline.py --selftest
...
BASELINE_REL = "eng/ci/known_failures.json"
REGISTRY_REL = "eng/ci/checks.json"
CTEST_BASELINE_REL = "eng/ci/ctest_baseline.json"
...
# 检查面失败集判定值域：硬失败 + KNOWN_FAIL。KNOWN_FAIL 是 eng/ci/run.py 对
...
def load_baseline(path: pathlib.Path) -> tuple[list[dict], list[str]]:
    ...
    顶层接受 {"failures": [...]}（07 合同 / eng/ci/run.py 消费形态）或裸数组。
    """
...
def registry_units(repo: pathlib.Path) -> tuple[set[str], list[str]]:
    """eng/ci/checks.json 已登记检查 id 集合。"""
    ...
    for idx, entry in enumerate(entries):
        ...
        if str(entry["check_id"]) != unit:
            errors.append("V2 %s check_id(%r) 必须等于 unit(%r)"
                          "（check_id 为 eng/ci/run.py 07 合同消费键）" % (where, entry["check_id"], unit))
...
def parse_ci_result(path: pathlib.Path) -> tuple[dict, list[str]]:
    """解析 eng/ci/run.py 的 CI_RESULT.json → {check_id: verdict}。"""
    path = pathlib.Path(path)
    if not path.is_file():
...
def parse_checks_dir(path: pathlib.Path) -> tuple[dict, list[str]]:
    """解析 eng/ci/run.py 的 per-check 结果目录（<out_root>/checks/*.json）→ {id: verdict}。

    CI 面主用来源：KNOWN-FAILURES-BASELINE-CHECK 在 linux-main 末位执行，此时
    eng/ci/run.py 已把上游每项检查的 per-check JSON 原子落盘（write_check_result），
...
def run_legacy(argv: list[str] | None = None) -> int:
    ...
    check_traceability = read(root, "eng/tools/check_traceability.py")
    ...
    add("F-026", "P1", len(set(ast_headers)) <= 3,
        "grep -oE '[\\w/]+\\.h' eng/tools/check_ast_api.py",
    ...
    add("F-027", "P1", vacuous_trace,
        "grep -n 'trace\\|output\\|PASS' eng/tools/check_pipeline_trace.py",
    ...
    add("F-028", "P1", old_trace,
        "grep -n 'prerelease_v5\\|66' eng/tools/check_traceability.py",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--baseline", default=BASELINE_REL,
                    help="版本化基线路径（默认 eng/ci/known_failures.json）")
    ...
    ap.add_argument("--ci-result", default=None, dest="ci_result",
                    help="check 模式：eng/ci/run.py 的 CI_RESULT.json（汇总形态；run 结束后可得）")
    ap.add_argument("--ci-checks-dir", default=None, dest="ci_checks_dir",
                    help="check 模式：eng/ci/run.py 的 per-check 结果目录（<out_root>/checks）。"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "verify":
        ...
        if checks_dir is None and ci_result is None:
            # CI 面缺省：eng/ci/run.py 注入 ASTROCS_CI_OUT_ROOT（本次 run 证据根），
