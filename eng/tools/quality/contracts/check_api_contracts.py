...
def main():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("extract_mod", str(repo / "eng/tools/quality/extract_cpp_api.py"))
        mod = importlib.util.module_from_spec(spec)
        # Instead directly run extractor via subprocess to get json
        import subprocess, json as js
        out = subprocess.check_output([sys.executable, str(repo / "eng/tools/quality/extract_cpp_api.py"), "--repo", str(repo)], text=True)
        ast = js.loads(out)
