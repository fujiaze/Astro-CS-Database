...
负面 fixture 在 eng/tools/quality/fixtures/ledger/ 下，由 selftest 模式逐一验证必须失败。

用法:
  python3 eng/tools/quality/validate_task_ledger.py LEDGER.csv [--baseline-sha256 HEX] [--structure-only] [--output out.json]
  python3 eng/tools/quality/validate_task_ledger.py --selftest   # 运行全部负例
"""
