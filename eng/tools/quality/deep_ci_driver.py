...
复杂度基线测量（DEEP-COMPLEXITY）由 eng/tools/quality/check_complexity.py 承担
（单一事实源，本驱动不重复实现）。
...
def cmd_coverage_cpp(args: argparse.Namespace) -> int:
    ...
    步（直接调 eng/cmake/qa_coverage_report.sh，与 ccov 目标第 4 步同参），
    ...
    steps.append({"name": "coverage-merge-report", "timeout": 300,
                  "argv": ["/bin/sh", str(REPO / "eng" / "cmake" / "qa_coverage_report.sh"),
...
def _ctest_argv(base: list[str], junit: str | None) -> list[str]:
    ...
    与版本化基线 eng/ci/known_failures.json 比对（失败集 ⊆ 基线）；ctest 文本日志
    非稳定解析面，故按 ctest 3.21+ 原生 --output-junit 落 JUnit XML
    （每 testcase 一条，含 name/failure/skipped，解析实现见
    eng/tools/quality/known_failures_baseline.py parse_ctest_junit）。
