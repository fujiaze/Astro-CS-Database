...
run_one() {
  ...
  if [ -f "$LOGS/$tag.events.jsonl" ] && [ -f "$LOGS/$tag.resource_timeseries.csv" ]; then
    python3 eng/tools/l4_rebuild/stage_profile.py "$LOGS/$tag.events.jsonl" "$LOGS/$tag.resource_timeseries.csv" \
...
echo '--- 闸门：产品数必须 == 49 ---'
python3 eng/tools/l4_rebuild/merge_products.py || { echo 'ABORT: product count gate failed'; exit 2; }
...
echo '--- 汇总热点 ---'
python3 eng/tools/l4_rebuild/hotspots.py || true
