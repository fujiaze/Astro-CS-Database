#!/usr/bin/env python3
# tests/integration/v6_p3/p3_v6_export_oracle.py — P3-INTEGRATE-001 独立磁盘重开 Oracle
#
...
def main(argv):
    ...
    with open(os.path.join(root, "contracts/schemas/v6/astrocs.v6.provenance.v1.schema.json")) as f:
        pschema = json.load(f)
    required = pschema["required"]
    allof = pschema.get("allOf", [])
    with open(os.path.join(root, "contracts/data/v6_data_dictionary_v1.json")) as f:
