...
| `tests/integration/v6_p3/CMakeLists.txt` | 自注册测试目标（可独立 configure，也可被根 `add_subdirectory`） |
| `tests/integration/v6_p3/p3_v6_export_e2e_test.cpp` | 端点测试：三模式正例 + 16 条负向注入（独立稠密/手算 Oracle） |
| `tests/integration/v6_p3/p3_v6_export_oracle.py` | 独立磁盘重开 Oracle：纯 stdlib FITS 解析 + CHECKSUM 重算 + 生产 provenance schema 门 + 可选 astropy 交叉复核 |
...
timeout 300 cmake -S tests/integration/v6_p3 -B run/v6/p3-intg/build -DCMAKE_BUILD_TYPE=Release   # rc=0
...
1. **生产 schema 与 bare-ADU 通量的交叉张力**（REVIEW_REQUIRED）：
   `contracts/schemas/v6/astrocs.v6.provenance.v1.schema.json#allOf` 规定 `units.bunit="ADU"`
