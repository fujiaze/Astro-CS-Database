...
不链接被测 C++ 实现：只读磁盘产物，用生产 schema（contracts/schemas/v6/）与
W6 最小校验器（tests/contracts/v6/jsonschema_min.py，只读导入）逐条验证：
...
def main():
    ...
    try:
        ...
        fail("cannot import jsonschema_min from tests/contracts/v6: %s" % exc)
