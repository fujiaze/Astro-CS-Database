...
判据（**不新造第二份键名清单**：键宇宙从唯一声明现场派生）：
  ① 键宇宙 U = parser.cpp session_keys() ∪ parser.cpp block_keys()
     （机器读取器 = tests/config/test_cfg004_unified_contract.py，本文件只引用）；
...
跑法：python3 -B -m unittest discover -s tests/cli -t tests/cli -p "test_fix210_*.py"
"""
import json
import os
import shutil
...
# 登记依据：contracts/schemas/phase_config_normalize.schema.json x-astrocs-notes
