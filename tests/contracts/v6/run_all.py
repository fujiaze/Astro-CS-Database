...
步骤（每步 rc 落 evidence/rc_summary.json 与 evidence/logs/）：
  ...
  4. CI 同款 python3 -B -m unittest discover -s tests/contracts -t tests/contracts（UT-CONTRACTS）；
...
def step_meta_schema():
    ...
    try:
        ...
        bad = []
        for p in sorted((REPO / "contracts/schemas/v6").glob("*.schema.json")):
...
def main():
    ...
    ut = subprocess.run([sys.executable, "-B", "-m", "unittest", "discover",
                         "-s", "tests/contracts", "-t", "tests/contracts"],
