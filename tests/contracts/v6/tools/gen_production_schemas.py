...
本生成器只做**确定性结构迁移**，不改变任何冻结语义（单位/公式/枚举/容差）：
  ...
  5. 词表归一：删除 SCI-PSFW/SCI-P2 双词表的"别名"桥接字段，单一权威词表落
     contracts/data/v6_weight_vocabulary_v1.json，双向映射落
     contracts/data/v6_migration_map_v1.json；schema 只认 canonical。
...
用法（仓库根）: python3 tests/contracts/v6/tools/gen_production_schemas.py
...
HEAD = ("[生产 schema | SCHEMA-INTEGRATE-001/W6] 由 contracts/proposals/v6/data/{f} 机械迁移；"
        ...
        "legacy 别名与双向映射见 contracts/data/v6_weight_vocabulary_v1.json 与 v6_migration_map_v1.json。"
...
def production_block(fname):
    return {
        ...
        "source_proposal": "contracts/proposals/v6/data/" + fname,
...
def migrate_units(d):
    return d  # 单位表值落 contracts/data/v6_data_dictionary_v1.json；schema 只约束结构
...
def migrate_weight_mode(d):
    ...
    d["x-astrocs-legacy-vocabulary"] = {
        ...
        "mapping_ref": "contracts/data/v6_weight_vocabulary_v1.json",
        "migration_ref": "contracts/data/v6_migration_map_v1.json",
    }
    d["allOf"] = [
