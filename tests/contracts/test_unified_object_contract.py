...
运行：python3 -m unittest discover -s tests/contracts -t tests/contracts
      或 python3 tests/contracts/test_unified_object_contract.py
"""
import json
import pathlib
import re
...
class TestCanonicalObjectSchemas(unittest.TestCase):
    ...
    def test_port_contract_schema_is_canonical_and_loadable(self):
        ...
        self.assertEqual("contracts/schemas/unified/port_contract.schema.json", ref["canonical_schema_file"])
    ...
    def test_every_object_schema_is_in_the_ownership_registry(self):
        ...
        for name, cls in declared.items():
            ...
            self.assertEqual("contracts/schemas/unified/%s.schema.json" % name, cls["canonical_schema_file"])
            self.assertTrue((REPO / cls["canonical_schema_file"]).is_file())
        # 每条 contracts/schemas/** 下的 schema 都必须被登记为某个对象类的 canonical / projection / other（U-02 口径）
        owned = set()
        ...
        for rel in ("contracts/schemas/phase_config_normalize.schema.json",
...
class TestRegistryIndex(unittest.TestCase):
    ...
    def test_contracts_data_is_compatibility_only(self):
        """contracts/data/** 只能是兼容期映射：每条登记必须指向 canonical schema，且不得自称为对象定义。"""
        m = load(REPO / "contracts/data/unified_object_compatibility_map_v1.json")
        reg = load(REPO / REGISTRY_REL)
    ...
    def test_legacy_id_map_written_into_index_and_compat_map(self):
        """映射必须同时落在 docs/contracts/INDEX.yaml 与 contracts/data/unified_object_compatibility_map_v1.json。"""
        reg = load(REPO / REGISTRY_REL)
