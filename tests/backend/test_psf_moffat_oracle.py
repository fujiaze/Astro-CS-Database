...
两种运行形态（缺一不可）:
  A) 独立脚本（PSF.md §11 复算口径 + 极性证据）:
       python3 tests/backend/test_psf_moffat_oracle.py                      -> rc=0, 打印 PSF MOFFAT ORACLE PASS
       python3 tests/backend/test_psf_moffat_oracle.py --negative-injection -> rc=0, 打印 NEGATIVE INJECTION DETECTED
       PSF_ORACLE_INJECT=1 python3 tests/backend/test_psf_moffat_oracle.py  -> 同上
  B) unittest 形态（必须：CHK-UNIT::UT-BACKEND 用
     "python3 -m unittest discover -s tests/backend -t tests/backend" 采集；
     非 TestCase 的纯 main() 脚本不被采集 ⇒ 门形同虚设。
     见 工程控制/PROJECT-GOVERNANCE-01/tasks/SCI-FIX-PSF.md CI-003-SCI-FIX-PSF-4）:
       python3 -m unittest discover -s tests/backend -t tests/backend -p "test_psf_moffat_oracle.py" -v
...
依赖: numpy + scipy（仓内既有依赖，见 tests/unit/v6_p2_upm/oracle/upm_ma_oracle.py 等 3 处）；
