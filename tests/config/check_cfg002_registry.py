...
单项职责（每项都能红能绿，负例入口 = --self-test）：
  CFG002-01  docs/plugins/** 配置表 <-> config/config_registry.json 一一对应（缺登记/多登记/默认值漂移）
  ...
  CFG002-07  索引归属唯一（config/** vs contracts/config/**；DOCUMENT_INDEX；tests/test_index.csv）
  CFG002-08  docs/contracts/CONFIG_CONTRACT.md 的 CFG002-ANCHOR 标记行存活（文档承诺与登记一致）

用法：
  python3 tests/config/check_cfg002_registry.py                # 跑真实仓库，rc=0 全绿
  python3 tests/config/check_cfg002_registry.py --self-test     # 真实仓库全绿 + 每类故障注入必红
  python3 tests/config/check_cfg002_registry.py --json-out X    # 机器输出
...
CPU_NEG = "tests/config/fixtures/negative/cpu_profile_v2_bad_os_abi.json"
CONTRACT_DOC = "docs/contracts/CONFIG_CONTRACT.md"
TEST_INDEX = "tests/test_index.csv"
...
def _validator(repo):
    ...
    if not os.path.isfile(path):
        raise Fail("missing validator: tests/contracts/v6/jsonschema_min.py")
...
def check_05_os_abi_domain(repo):
    ...
    pos = load_json(repo, "tests/config/fixtures/positive/cpu_profile_v2.json")
...
def _tests_dirs_with_sources(repo):
    ...
    if not out:
        raise Fail("tests/ 下未发现任何测试目录（锚失效）")
    return out


def check_07_index_ownership(repo):
    """config/** vs contracts/config/** 归属唯一 + 三处索引登记归属。"""
    import csv
    problems = []
    cfg_files = []
    for root, dirs, files in os.walk(os.path.join(repo, "config")):
        ...
        for f in files:
            ...
            if f.endswith(".schema.json"):
                problems.append("config/** 出现 schema 文件（第二事实源）: %s" % rel)
    cc_dir = os.path.join(repo, "contracts", "config")
    if not os.path.isdir(cc_dir):
        raise Fail("contracts/config/ 不存在（锚失效）")
    cc_files = sorted(os.listdir(cc_dir))
    for f in cc_files:
        if not f.endswith(".schema.json"):
            problems.append("contracts/config/** 出现非 schema 文件: %s" % f)
    ...
    # tests/test_index.csv：登记 tests/config + 已知未登记清单闭合
    ...
    if "tests/config" not in paths:
        problems.append("%s 未登记 tests/config" % TEST_INDEX)
    ...
    if not set(missing) <= known:
        problems.append("tests/** 未登记目录 %r 不在已登记缺口清单 %r 内（新增目录必须先登记或登记为缺口）"
...
def build_sandbox(src, dst):
    ...
    if not os.path.isfile(v):
        raise Fail("sandbox source missing: tests/contracts/v6/jsonschema_min.py")
    ...
    ("test_index_row_removed", "CFG002-07",
     lambda root: _edit_text(root, TEST_INDEX,
                             lambda t: "".join(ln for ln in t.splitlines(True)
                                               if not ln.startswith("tests/config,")))),
    ("config_schema_second_source", "CFG002-07",
     lambda root: _write_text(root, "config/injected.schema.json", "{}\n")),
