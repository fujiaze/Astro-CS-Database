...
本模块锁 schema 面（CLI 运行面见 tests/cli/test_multiblock_normalize.py）：
  ① 多块形态通过（config/templates/normalize.phase_config.json 与正例夹具）
  ② 平铺单块简写仍通过（向后兼容）
...
SCHEMA = "contracts/schemas/phase_config_normalize.schema.json"
TEMPLATE = "config/templates/normalize.phase_config.json"
POS_BLOCKS = "tests/config/fixtures/positive/normalize_blocks.phase_config.json"
POS_FLAT = "tests/config/fixtures/positive/normalize_flat.phase_config.json"
...
class TestMultiBlockForm(unittest.TestCase):
    ...
    def test_08_filter_passband_keeps_library_enum_gate(self):
        ...
        lib = list(C.load_json("config/filters.json")["filters"].keys())
