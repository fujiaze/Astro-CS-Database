...
本模块锁 schema/模板面（CLI 运行面见 tests/cli/test_unified_blocks_templates.py）：
...
POS = "tests/config/fixtures/positive/"
...
# ② 从 normalize_block 移除并改指 config_registry.json 的两条 sparse_snr_layer 登记点（config/**）。
...
class TestBlockKeySetSingleSource(unittest.TestCase):
    ...
    def test_04_a44_keys_excluded_from_new_branches(self):
        ...
        config/defaults.json#weight.default_mode 组删除、phase_config_mosaic 旧合同的
