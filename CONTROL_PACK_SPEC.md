# Astro Celestial Sphere Database（ACSD） 控制包规范（Control Pack Specification）

> 控制包把"最高设计与实际代码的偏差"变成可执行、可验收的工作单元。**制作控制包**与**执行控制包**是两条独立流程：制作任务从负责人给出的方向出发、Agent 派大量 SubAgent 找偏差并写成控制包；执行任务按已做好的控制包推进，Agent 派 SubAgent 干活、前台独立验证并统一提交。
...
控制包可自带 `docs/` 目录，放置经负责人核准的最新版根文档（最高设计、AGENTS、工程规范、验收规范、本规范），执行的第一步是用包内文档替换仓库对应文档并提交，后续任务以新版文档为权威。
...
├── PROMPT.md               新会话最简工作包提示词（可选）
├── docs/                   随包携带的最新版根文档（可选，执行首步替换）
├── tasks/
│   ├── <ID>-001.md         每个任务一个文件（见 §5 模板）
│   └── ...
...
控制包 ID 规则：`<阶段>-<批次>`，如 `RELEASE-04`、`NORMALIZE-CAL-01`、`MOSAIC-UPM-02`、`EXPORT-PROJ-01`。
...
## 禁止
- 不修改科学公式/默认容差（ENGINEERING_SPEC §3）
- 不在 main 外开分支
```
...
- 若包内带 `docs/`，执行首步替换根文档并单独提交，再做其余任务。
...
- 机器门红灯 ⇒ 任务状态为 FAIL（本规范的 waiver 面积为零）；
- 失败上报带可复现命令与原始日志（工具/环境原因同样给复现）；
- "验收过"的判据 = 断言/测试实跑 rc=0（"能编译"只证明构建通过）。

---

## 8. 统一提交

- 所有 PASS 任务由**前台统一按序提交**（一个 commit = 一个任务，或按批次提交，遵循 ENGINEERING_SPEC §6）；
- SubAgent 不参与 git 操作；
