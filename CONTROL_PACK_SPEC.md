...
控制包可自带 `docs/` 目录，放置经负责人确认的最新版根文档（最高设计、AGENTS、工程规范、验收规范、本规范），执行的第一步是用包内文档替换仓库对应文档并提交，后续任务以新版文档为权威。
...
|---|---|---|---|---|
| XXX-001 | PASS | ctest 全过 | run/.../logs/ | 复跑一致 |

### 7.3 禁止事项

- 不用 waiver 掩盖红灯；
- 不以"工具问题/环境问题"掩盖失败而不给复现；
- 不把"能编译"当"验收过"——必须跑断言/测试。
