...
规则 (RT-008 拆分后实现移至 cli/parser.cpp + cli/commands.cpp; main.cpp 仅 71 行薄壳):
1. parser.cpp 用 CmdRule kRules[] 表驱动分发 (无 argv 硬编码 if 链)。
...
def main():
    ...
    if "const auto& r : kRules" not in PARSER:
        errors.append("no kRules-driven dispatch")
    # 2) cmd_* 统一签名
    cmds = re.findall(r"int (cmd_[a-z_]+)\(const Parsed&", CMDS)
    if len(cmds) < 4:
        errors.append(f"too few cmd_* functions: {len(cmds)}")
    # 3) 稳定出口
    for token in ("--version", "--help"):
        if token not in PARSER and token not in CMDS:
            errors.append(f"stable exit missing: {token}")
    # exit_codes 引用
    if "exit_codes.h" not in PARSER and "exit_codes" not in CMDS:
