#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CHK-IMPACT-MAP | ci/impact_map.json 判据一致性门（changed-path -> checks 映射）。
...
权威依据
  - ENGINEERING_SPEC.md §8「机器一致性检查」：每项检查有正例与负例（能红能绿）、
    **可执行负例面**（--self-test 或 --fault-inject）、**fail-closed**（输入缺失/
    ...
    文件/目录必须存在）、**注册表双向一致**；
  - ci/impact_map.json 自述契约（spec_ref = tasks/02_CI_TASKS.md :: V8-CI-004）：
    「所有引用 id 均存在于 ci/checks.json 且为 fast profile 候选，由
    ci/tests/test_impact_map.py 机器校验」。

为什么另立本门（W4-A3 根因）
  旧 ci/tests/test_impact_map.py 只按**顶层 id** 索引注册表（`c["id"] for c in
...
判据（任一 R 违规 ⇒ exit 1）
  ...
  R4 路径域覆盖   PROBE_DOMAINS 声明的每个**现行**路径域：(a) 其锚路径在当前树中必须
                  ...
                  paths 命中（用 ci/run.py 的 _match_prefix 语义，与生产选择器同源）；
...
用法
  python3 tools/quality/check_impact_map.py                  # 校验（CI 检查面）
  python3 tools/quality/check_impact_map.py --json-out F     # 同时落证据 JSON
  python3 tools/quality/check_impact_map.py --self-test      # 负例自检（N0..N7）
...
MAP_REL = "ci/impact_map.json"
DOC_REL = "docs/ci/01_CHECKS.md"
RUNNER_REL = "ci/run.py"
...
PROBE_DOMAINS = [
    ...
    ("cmake/**", "cmake/install_layout.cmake", "cmake/install_layout.cmake"),
    ("ci/**", "ci/run.py", "ci/run.py"),
    ...
    ("include/**", "include/astrocs/common_abi_v1.h", "include/astrocs/common_abi_v1.h"),
    ("contracts/**", "contracts/schemas/run_manifest.schema.json", "contracts/schemas/run_manifest.schema.json"),
    ("third_party/**", "third_party/nlohmann", "third_party/nlohmann/json.hpp"),
    ("tests/**", "tests/testkit/registry.json", "tests/testkit/registry.json"),
    ("testdata/**", "testdata/index.json", "testdata/index.json"),
    ("tools/**", "tools/quality/check_module_map.py", "tools/quality/check_module_map.py"),
    ...
    ("packaging/**", "packaging", "packaging/astrocs.product.json"),
...
def index_registry(registry: dict) -> dict:
    ...
    step 未显式声明 profiles 时继承父项（与 ci/run_checks.py 的继承语义一致）。
    """
    top: set[str] = set()
    steps: set[str] = set()
    fast: set[str] = set()
...
def _load_match_prefix(repo: pathlib.Path):
    """复用 ci/run.py 的 _match_prefix（与生产选择器同源，避免两套 glob 语义）。"""
...
def evaluate(impact: dict, registry: dict, *, match_prefix, root: pathlib.Path,
    ...
    return {
        "tool": "tools/quality/check_impact_map.py",
        "rule": "CHK-IMPACT-MAP / V8-CI-004 (ci/impact_map.json 判据一致性)",
...
def _mp(probe: str, pat: str) -> bool:
    """自测用的最小 _match_prefix（与 ci/run.py 同语义：dir/** 前缀命中）。"""
    if pat.endswith("/**"):
        base = pat[:-3].rstrip("/")
...
def _refuse(code: str, detail: str) -> int:
    ...
    print(json.dumps({"tool": "tools/quality/check_impact_map.py",
