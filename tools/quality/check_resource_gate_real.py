...
- **正例**：`ci/resource_monitor.py --gate-required --gate-compute-interval <S>` 监控
...
用法:
  python3 tools/quality/check_resource_gate_real.py                 # 正例 + 负例都跑
  python3 tools/quality/check_resource_gate_real.py --fault-inject serial
  python3 tools/quality/check_resource_gate_real.py --self-test     # 等价于默认（自证两态）
...
MONITOR_REL = "ci/resource_monitor.py"
PROBE_REL = "tools/quality/resource_gate_probe.py"
