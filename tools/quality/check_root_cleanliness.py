...
权威：
  ...
  * ASTROCS_DESIGN.md §6.3 —— 运行产物只落配置 output_dir，不得以进程 CWD
    作隐式缺省写出；
  * ci/root_manifest.json —— §7 白名单的机器可读落盘（逐条对照，禁止更宽）。
...
DEFAULT_MANIFEST = REPO / "ci" / "root_manifest.json"
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--manifest", default=str(DEFAULT_MANIFEST), help="ci/root_manifest.json 路径")
