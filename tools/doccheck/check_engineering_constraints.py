...
用法：
  python3 tools/doccheck/check_engineering_constraints.py [--root <repo>]
      [--json-out <file>] [--quiet]
  python3 tools/doccheck/check_engineering_constraints.py --self-test
...
def relocated(entry: dict, root: str):
    ...
    替换成同名异内容"；彻底关闭需要登记面携带 sha256，而 `ci/root_manifest.json`
    不在本任务文件域。
...
def run_check(root: str):
    ...
    for rel in ("README.md", "AGENTS.md", "ENGINEERING_SPEC.md", "CONTROL_PACK_SPEC.md",
                "memory.md", "ASTROCS_DESIGN.md", "tools/check_agents_gov.py",
...
def self_test() -> int:
    ...
    try:
        ...
        report = {"tool": "tools/doccheck/check_engineering_constraints.py", "mode": "self-test",
