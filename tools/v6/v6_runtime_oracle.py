...
覆盖（每项 = 一条硬规则，任一违反 → rc=1）：
  ...
  R6  SO-05：资源判据在 SO-05 未签字前恒为 record_only + PENDING_OWNER_SIGNOFF，
      且不存在把该判定升级为硬失败的契约入口。
  R7  ci/checks.json 登记 V6 检查项（有 timeout / outputs / ctest_targets 或等价依赖）。
  R8  确定性契约键存在（FZ-RUNTIME-DETERMINISM）。

用法:
  python3 tools/v6/v6_runtime_oracle.py [--repo <root>] [--json-out <path>]
...
def rule_r7_ci_registration(ctx: Ctx):
    """ci/checks.json 登记 V6 检查项。"""
    src = ctx.read("ci/checks.json")
    if src is None:
        return
    try:
        doc = json.loads(src)
