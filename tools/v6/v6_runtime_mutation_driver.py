...
做法（不修改工作树）：
  ...
  2. 先在未注入的临时树上跑 tools/v6/v6_runtime_oracle.py —— 必须 PASS（防 Oracle 假红）；
  3. 逐条注入违规，重跑 Oracle —— 每条必须 FAIL（rc != 0），否则本驱动 FAIL。

用法:
  python3 tools/v6/v6_runtime_mutation_driver.py [--repo <root>] [--json-out <path>]
...
COPY_SET = [
    ...
    "cli/commands.cpp",
    "cli/monitor.h",
    "cli/resource_recorder.h",
    "runtime/v6_budget.py",
    "ci/checks.json",
...
MUTATIONS = [
    ("deferred-mode-enters-production", "cli/v6_runtime_contract.h",
     "FZ-MODE-DEFERRED: psf_snr_power is DEFERRED",
     "FZ-MODE-PRODUCTION: psf_snr_power is DEFERRED"),
    ("second-private-budget-source", "cli/v6_runtime_contract.h",
     "if (has_source_) return false;              // 第二来源：拒绝",
     "if (false) return false;                    // 第二来源被偷偷放行"),
    ("legacy-weight-mode-0-allowed", "cli/v6_runtime_contract.h",
