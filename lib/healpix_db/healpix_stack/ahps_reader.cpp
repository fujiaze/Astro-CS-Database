...
bool AhpsReader::open(const std::string& path) {
    ...
    if (m_headerCompSize > 0) {
        ...
        size_t got = aio_decompress(rawJson.data(), readSize, &m_headerJson[0], m_headerSize, 1/*ZSTD*/);
        if (got != m_headerSize) {
...
std::vector<uint8_t> AhpsReader::readChunk(const ChunkIndex& chunk) {
    ...
    if (chunk.codec == (uint8_t)Codec::NONE) {
        ...
        for (int a = 0; a < 8; a++) {
            result.resize(est);
            got = aio_decompress(compData.data(), compData.size(), result.data(), est, 1/*ZSTD*/);
            if (got > 0) break;
            est *= 4;
        }
        ...
        for (int a = 0; a < 8; a++) {
            result.resize(est);
            got = aio_decompress(compData.data(), compData.size(), result.data(), est, 2/*LZ4*/);
            if (got > 0) break;
            est *= 4;
        }
        if (got == 0) { result.clear(); return result; }
