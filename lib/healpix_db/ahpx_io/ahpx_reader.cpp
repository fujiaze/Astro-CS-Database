...
bool AhpxReader::open(const std::string& path) {
    ...
    if (!m_fp) {
        fprintf(stderr, "[ahpx][reader] 无法打开文件: %s\n", path.c_str());
        return false;
    }
    ...
    if (std::fread(fixedHeader, 1, HEADER_FIXED_SIZE, m_fp) != HEADER_FIXED_SIZE) {
        fprintf(stderr, "[ahpx][reader] 读取固定头失败 (文件过短): %s\n", path.c_str());
        close();
        return false;
    }
    ...
    if (std::memcmp(fixedHeader, MAGIC, 4) != 0) {
        fprintf(stderr, "[ahpx][reader] Magic 校验失败: %s\n", path.c_str());
        close();
        return false;
    }
    ...
    if (m_version != VERSION) {
        fprintf(stderr, "[ahpx][reader] 版本不兼容 (文件=%u, 支持=%u): %s\n",
    ...
    if (std::fread(rawJson.data(), 1, readSize, m_fp) != readSize) {
        fprintf(stderr, "[ahpx][reader] 读取 JSON 头失败: %s\n", path.c_str());
        close();
        return false;
    }
    ...
    if (m_headerCompSize > 0) {
        ...
        size_t decompressed = decompressZstd(rawJson.data(), readSize,
                                             &m_headerJson[0], m_headerSize);
        if (decompressed != m_headerSize) {
            fprintf(stderr, "[ahpx][reader] JSON 头解压失败: 期望 %u, 实际 %zu\n",
    ...
    // -------- 解析 JSON 头 --------
    if (!parseHeader()) {
        fprintf(stderr, "[ahpx][reader] JSON 头解析失败: %s\n", path.c_str());
        close();
        return false;
    }

    fprintf(stderr, "[ahpx][reader] 成功打开: %s (blocks=%zu)\n",
...
bool AhpxReader::parseHeader() {
    ...
    if (pos == std::string::npos) {
        fprintf(stderr, "[ahpx][reader] JSON 头中未找到 blocks 字段\n");
        // blocks 可能为空, 不算错误
        return true;
    }
    ...
    if (pos >= m_headerJson.size() || m_headerJson[pos] != '[') {
        fprintf(stderr, "[ahpx][reader] blocks 不是数组\n");
        return true;
    }
...
bool AhpxReader::readRaw(uint64_t offset, uint64_t size, void* dst) {
    ...
    if (std::fseek(m_fp, (long)offset, SEEK_SET) != 0) {
        fprintf(stderr, "[ahpx][reader] fseek 失败 (offset=%llu)\n",
    ...
    if (bytesRead != size) {
        fprintf(stderr, "[ahpx][reader] fread 不完整 (期望 %llu, 实际 %zu)\n",
...
std::vector<uint8_t> AhpxReader::readBlockByIndex(const BlockIndex* blk) {
    ...
    if (blk->codec == (uint8_t)Codec::NONE) {
        ...
        for (int attempt = 0; attempt < 8; attempt++) {
            ...
            decompSize = decompressZstd(compData.data(), compData.size(),
                                        result.data(), estSize);
            if (decompSize > 0) break;
            estSize *= 4;  // 扩大 4 倍重试
        }
        if (decompSize == 0) {
            fprintf(stderr, "[ahpx][reader] 块 '%s' ZSTD 解压失败\n", blk->id);
            result.clear();
            return result;
        }
        ...
        for (int attempt = 0; attempt < 8; attempt++) {
            result.resize(estSize);
            decompSize = decompressLz4(compData.data(), compData.size(),
                                       result.data(), estSize);
            if (decompSize > 0) break;
            estSize *= 4;
        }
        if (decompSize == 0) {
            fprintf(stderr, "[ahpx][reader] 块 '%s' LZ4 解压失败\n", blk->id);
            result.clear();
            return result;
        }
        result.resize(decompSize);
    } else {
        fprintf(stderr, "[ahpx][reader] 块 '%s' 未知 codec=%u\n", blk->id, blk->codec);
        result.clear();
    }
...
std::vector<uint8_t> AhpxReader::readBlock(const std::string& id) {
    ...
    if (!blk) {
        fprintf(stderr, "[ahpx][reader] 块 '%s' 未找到\n", id.c_str());
...
std::vector<float> AhpxReader::readPixels() {
    ...
    if (!getImageInfo(&w, &h, &c)) {
        fprintf(stderr, "[ahpx][reader] readPixels: 无法获取图像几何信息\n");
        return result;
    }
    if (w <= 0 || h <= 0 || c <= 0) {
        fprintf(stderr, "[ahpx][reader] readPixels: 无效几何 (w=%d h=%d c=%d)\n", w, h, c);
        return result;
    }
    ...
    if (rawData.empty()) {
        fprintf(stderr, "[ahpx][reader] readPixels: pixel 块为空\n");
        return result;
    }
    ...
    if (rawData.size() != expectedSize) {
        fprintf(stderr, "[ahpx][reader] readPixels: 大小不匹配 (期望 %zu, 实际 %zu)\n",
...
std::vector<float> AhpxReader::readSnr() {
    ...
    if (rawData.empty()) {
        fprintf(stderr, "[ahpx][reader] readSnr: snr 块为空\n");
        return result;
    }
...
std::vector<float> AhpxReader::readWeight(WeightMode* outMode, uint16_t* outGw, uint16_t* outGh) {
    ...
    if (rawData.empty()) {
        fprintf(stderr, "[ahpx][reader] readWeight: weight 块为空\n");
        return result;
    }
