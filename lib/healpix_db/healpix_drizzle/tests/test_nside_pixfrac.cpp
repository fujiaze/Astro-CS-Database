...
// ============================================================================
// 测试 A6: 9×9 网格采样验证 (构造 SIP 畸变在边缘的情况)
//
...
//   3. 9×9 网格采样能捕捉到边缘的 SIP 畸变 (通过比较 NSIDE 差异)
// ============================================================================
static void test_auto_nside_9x9_grid_sampling() {
    const char* name = "A6: 9x9 网格采样捕捉边缘 SIP 畸变";
    ...
    ASSERT_TRUE(nside_with_sip >= nside_no_sip,
                "A6b: 有 SIP 时 NSIDE >= 无 SIP (9x9 捕捉边缘畸变)",
                "有 SIP 时 NSIDE < 无 SIP, 9x9 网格未捕捉到边缘 SIP 畸变");

    // 验证 3: 9x9 网格采样覆盖整个图像 (通过构造畸变在边缘中间位置验证)
...
int main() {
    ...
    test_auto_nside_9x9_grid_sampling();
    printf("\n");
