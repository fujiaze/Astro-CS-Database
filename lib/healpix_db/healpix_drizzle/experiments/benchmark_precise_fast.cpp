...
// 辅助: 构造 DrizzleConfig
// ============================================================================
static DrizzleConfig make_config(int nside, double pixfrac, DrizzleMode mode) {
    DrizzleConfig cfg;
    cfg.nside = nside;
    cfg.nested = true;
    cfg.pixfrac = pixfrac;
        ...
        for (const auto& pf : kPixfracs) {
            for (const auto& rg : kRegions) {
                for (int pi = 0; pi < 2; ++pi) {
                    ...
                    DrizzleConfig cfg_precise = make_config(sc.nside, pf.value, DrizzleMode::PRECISE);
                    DrizzleConfig cfg_fast    = make_config(sc.nside, pf.value, DrizzleMode::FAST);

                    // 每个模式跑 kReps 次
                    RunResult precise_runs[kReps];
                    RunResult fast_runs[kReps];
                    for (int r = 0; r < kReps; ++r) {
                        precise_runs[r] = run_drizzle(img, cfg_precise);
                    }
                    for (int r = 0; r < kReps; ++r) {
                        fast_runs[r] = run_drizzle(img, cfg_fast);
                    }

                    // 比较 (取最后一次运行; 累加器内容是确定性的)
                    const Comparison cmp = compute_comparison(precise_runs[kReps - 1],
                                                               fast_runs[kReps - 1]);
                    ...
                    if (cmp.valid && cmp.candidate_misses > 0) ++cases_with_misses;
                    if (cmp.valid) {
                    ...
                    const bool has_overlap = cmp.valid;
                    const int64_t overlap_pairs = cmp.overlap_pairs;
                    ...
                    for (int r = 0; r < kReps; ++r) {
                        ...
                        emit_jsonl(case_id, "PRECISE",
                                   ...
                                   in_flux, of, fre,
                                   /*has_cmp=*/cmp.valid, cmp,
                                   rr.ok ? 0 : 1, r, /*warmup=*/(r == 0));
                    }
                    // 输出 FAST 行 (比较字段为 null; overlap_pairs 仍给出)
                    for (int r = 0; r < kReps; ++r) {
