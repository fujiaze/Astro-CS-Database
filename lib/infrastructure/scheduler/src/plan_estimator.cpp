...
//   P2: coverage target_order = min(max leaf order)（lib/algorithms/coverage coverage.cpp）;
//     block plan 峰值公式（p2_block_plan 先例）;
//   P1: 帧级行带（calibration/star-psf/wcs/photometry/noise-snr/drizzle）
//   kernel ID 权威清单 = lib/infrastructure/benchmark/backend_host/backend_table.inc（12 kernel）
...
// π（与 lib/algorithms/shared/healpix/healpix_core.cpp kPi 一致，禁止二义常量）
