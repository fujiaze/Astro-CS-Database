...
**归档操作**:
- 创建 lib/infrastructure/aio/healpix_db/archive/ 目录
...
**关键代码位置**: `lib/infrastructure/hips_browser/healpix_browser_qt/core/gl_renderer.cpp`（need_rebuild_mesh / FOV 下限 / MAX_SHIFT）、`core/stf_engine.cpp`（auto_stretch 百分位）、`app/main.cpp`（移除手动插件路径，Qt 自动查找 platforms/）。
