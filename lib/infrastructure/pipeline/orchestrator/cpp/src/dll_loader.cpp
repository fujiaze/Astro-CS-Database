...
std::string DllLoader::get_default_path(ModuleId id, const std::string& lib_base_dir) const {
    std::string sub;
    switch (id) {
        case ModuleId::AIO:             sub = "lib/infrastructure/aio/";                          break;
        case ModuleId::CALIBRATE:       sub = "lib/algorithms/calibration/";                            break;
        case ModuleId::PLATESOLVE:      sub = "lib/algorithms/platesolve/cpp/ipv/";                    break;
        case ModuleId::PSF:             sub = "lib/algorithms/psf/";                            break;
        case ModuleId::PHOTOMETRIC:     sub = "lib/algorithms/photometry/cpp/";                  break;
        case ModuleId::SNR:             sub = "lib/algorithms/noise_snr/cpp/";                      break;
        case ModuleId::DRIZZLE:         sub = "lib/algorithms/drizzle/healpix_drizzle/";             break;
        default:                        sub = "";
    }
...
bool DllLoader::load_all(const std::string& lib_base_dir) {
    ...
    // (Windows DLL 搜索路径问题的双保险)
    {
        std::string aio_dir;
        if (lib_base_dir.empty()) {
            aio_dir = "lib/infrastructure/aio/";
        } else if (lib_base_dir.back() == '/' || lib_base_dir.back() == '\\') {
            aio_dir = lib_base_dir + "lib/infrastructure/aio/";
        } else {
            aio_dir = lib_base_dir + "/lib/infrastructure/aio/";
        }
    ...
    // 预加载到进程地址空间, 避免传递依赖解析失败 (code=126)
    {
        std::string gaia_dir;
        if (lib_base_dir.empty()) {
            gaia_dir = "lib/algorithms/photometry/cpp/";
        } else if (lib_base_dir.back() == '/' || lib_base_dir.back() == '\\') {
            gaia_dir = lib_base_dir + "lib/algorithms/photometry/cpp/";
        } else {
            gaia_dir = lib_base_dir + "/lib/algorithms/photometry/cpp/";
        }
