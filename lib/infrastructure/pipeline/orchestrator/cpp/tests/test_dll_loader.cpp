...
//
// 编译:
// g++ -O2 -std=c++17 -Wall -o eng/tests/test_dll_loader.exe
// eng/tests/test_dll_loader.cpp src/dll_loader.cpp -Iinclude -static
//
// 运行 (在项目根目录):
...
// 注: lib_base_dir=".." 因为 cpp 目录在 lib/infrastructure/pipeline/orchestrator/cpp/ 下, 上一级是项目根
// ============================================================================
void test_load_all_modules() {
    TEST_SECTION("测试 2: 加载所有 5 个模块 (lib_base_dir=\"..\")");

    DllLoader loader;
    // cpp 目录在 lib/infrastructure/pipeline/orchestrator/cpp/, 项目根在 3 级之上
