...
- **残留检查**:
  - Grep 确认 lib/infrastructure/pipeline/orchestrator/cpp/ 下仅剩 6 处归档说明注释（无代码引用）
- **文档同步**:
  ...
  - `lib/algorithms/photometry/memory.md`: 追加"## 2026-07-18 GRADIENT_2D 模块归档"章节
...
- **stage handler 填充** (commit df06ef1):
  ...
  - Makefile: 8 个 -I 路径添加 (astro_image_io/plate_solve/dynamic_psf/photometric_calib/algorithms/noise_snr/healpix_drizzle/healpix_stack)
...
- **修改文件 (3个)**:
  - `lib/infrastructure/pipeline/orchestrator/cpp/include/orchestrator.h`: 新增 PLATESOLVE 环境资源成员 (project_root_dir_/gaia_client_dll_handle_/star_detector_dll_handle_/gaia_client_handle_/sdet_handle_/ipv_solver_handle_/platesolve_env_ready_) + init_platesolve_env/cleanup_platesolve_env 方法声明
  ...
  - `lib/infrastructure/pipeline/orchestrator/cpp/Makefile`: INCLUDES 新增 -I../../gaia_xpsd_client/src + -I../../star_detector/include
...
- **关键设计**:
  ...
  - PLATESOLVE 日志目录: <project_root>/lib/algorithms/platesolve/logs (通过 IpvParams.log_dir 设置)
...
**目标**: 在 lib/infrastructure/pipeline/orchestrator/cpp/ 下创建 DllLoader 类，运行时通过 LoadLibrary/GetProcAddress 加载 5 个模块 DLL（CALIBRATE/PLATESOLVE/PSF/PHOTOMETRIC/DRIZZLE），并提供统一的函数指针获取、状态查询、版本获取、线程数下发接口。

**新增文件 (3个)**:
- `lib/infrastructure/pipeline/orchestrator/cpp/include/dll_loader.h` - 动态加载器头文件
...
- `lib/infrastructure/pipeline/orchestrator/cpp/src/dll_loader.cpp` - 加载器实现
...
2. **DLL 依赖跨目录问题 (错误码 126)**: healpix_drizzle.dll 依赖 astro_image_io.dll（在 lib/infrastructure/aio/，与 healpix_drizzle 不在同一目录）。LoadLibraryExA + LOAD_WITH_ALTERED_SEARCH_PATH 只能找同目录的依赖。解决方案: load_all 中预加载 astro_image_io.dll 到进程地址空间，后续加载的 healpix_drizzle.dll 在解析依赖时直接复用已加载的 astro_image_io.dll，无需再次查找。
...
**目标**: 在 lib/infrastructure/pipeline/orchestrator/cpp/ 下创建 CheckpointManager 类，每个管线阶段 (CALIBRATE/PLATESOLVE/PHOTOMETRIC/DRIZZLE) 完成后将进度以 JSON 文件持久化到 <output_dir>/.checkpoint/<frame>.json，恢复时读取 JSON 跳过已完成阶段。

**新增文件 (3个)**:
- `lib/infrastructure/pipeline/orchestrator/cpp/include/checkpoint.h` - 检查点管理器头文件
...
- `lib/infrastructure/pipeline/orchestrator/cpp/src/checkpoint.cpp` - 检查点管理器实现
...
- `lib/infrastructure/pipeline/orchestrator/cpp/src/cli_command.cpp` - run 子命令增加 --fresh 解析, cmd_run/cmd_run_batch 调用 orch.set_fresh_start(true), print_usage 增加 --fresh 说明
- `lib/infrastructure/pipeline/orchestrator/cpp/include/cli_repl.h` - 注释增加 checkpoint list/clear 子命令说明
- `lib/infrastructure/pipeline/orchestrator/cpp/src/cli_repl.cpp` - handle_checkpoint 重构支持子命令 (list 列出+resume 点/clear 清除+计数/默认保存查询), print_help 增加 checkpoint list/clear 说明
...
**目标**: 在 lib/infrastructure/pipeline/orchestrator/cpp/ 下创建 Logger 单例类，支持 DEBUG/INFO/WARN/ERROR 四级别日志输出到文件 + stderr，文件按日期命名 (orchestrator_YYYY-MM-DD.log)，支持 --log-level 命令行参数。

**新增文件 (3个)**:
- `lib/infrastructure/pipeline/orchestrator/cpp/include/logger.h` - 日志系统头文件
...
- `lib/infrastructure/pipeline/orchestrator/cpp/src/logger.cpp` - 日志系统实现
...
- `lib/infrastructure/pipeline/orchestrator/cpp/src/cli_command.cpp` - 解析 --log-level 参数, 调用 Logger::instance().set_level()
- `lib/infrastructure/pipeline/orchestrator/cpp/include/cli_repl.h` - 新增 handle_log 方法
- `lib/infrastructure/pipeline/orchestrator/cpp/src/cli_repl.cpp` - 实现 log level/path 子命令, print_help 增加 log 命令说明
...
**修改文件 (2个)**:
- `lib/infrastructure/pipeline/orchestrator/cpp/src/cli_command.cpp` - 在 cmd_run_batch 中增加目录存在检查
...
- `lib/infrastructure/pipeline/orchestrator/cpp/Makefile` - 新增 test_orchestrator_cli 和 run_integration_test 目标
...
**关键发现**:
- 当前环境 `lib/infrastructure/aio/healpix_db/ahpx_io/` 下只有 `ahpx_io.dll` + `__pycache__/ahpx_io.cpython-310.pyc`，无 `ahpx_io.py` 源文件
...
- 日志: `lib/infrastructure/pipeline/orchestrator/logs/validate/validate_drizzle_output_YYYYMMDD_HHMMSS.log`
...
**修改文件 (1个)**:
- `lib/infrastructure/pipeline/orchestrator/cpp/src/orchestrator.cpp` 的 `run_stage_snr` 函数（原行 2334-2435，现 2504-2666）
