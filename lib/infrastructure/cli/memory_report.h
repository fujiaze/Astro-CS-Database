...
//
// 消费口径: ProcSample(lib/infrastructure/cli/monitor.h 同域) —— 不改外域 schema, 拒外域字段。
...
// 而实现算的是 MiB/s —— 口径差 4.858%（R-4 E6）。超预警线(未达失败线)记 Growing。
inline constexpr double kAllocGrowthUnboundedMbPerS =
    resource_gate_contract::kMemoryGrowthLimitMibPerS;
inline constexpr double kAllocGrowthWarnMbPerS = 2.0;
// 回落判定仅长 run(active window >10s; 契约 memory.allocation_reclaim_min_wall_seconds);
// 短 run 显式 ShortRunSkipped。
inline constexpr double kAllocMinReclaimWallSeconds =
    resource_gate_contract::kAllocationReclaimMinWallSeconds;
inline constexpr double kAllocMinReclaimFrac =
    resource_gate_contract::kAllocationReclaimMinFraction;       // (peak-last)/peak 下限
inline constexpr uint64_t kAllocReclaimResidualTolBytes =
    resource_gate_contract::kReclaimResidualToleranceBytes;
