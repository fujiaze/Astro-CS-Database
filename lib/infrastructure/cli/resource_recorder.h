// lib/infrastructure/cli/resource_recorder.h — MON-001 (G3) 资源记录器
// 每个 heavy node 自动生成 resource_timeseries.csv / resource_summary.json / worker_balance.csv
...
public:
    ...
    //  out_dir/resource_timeseries.csv, out_dir/resource_summary.json, out_dir/worker_balance.csv
...
inline bool ResourceRecorder::write_all(const std::string& out_dir, double wall_total,
    ...
    // ci/root_manifest.json、tests/cli/cli_test_hygiene.py 的既有机器判据一致；
    // ci/** 侧的同步登记在 CI-003）。
    {
