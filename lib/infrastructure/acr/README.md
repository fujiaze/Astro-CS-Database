...
机器守卫（`lib/infrastructure/acr/CMakeLists.txt` 顶部 guard + `lib/infrastructure/acr/ci/` 校验器）：

1. **生产构建不引入**：根 CMakeLists.txt 从不 `add_subdirectory(lib/infrastructure/acr)`；
   若任何生产构建尝试引入，lib/infrastructure/acr CMakeLists 顶部 guard 检测到
...
cmake -S lib/infrastructure/acr -B build/acr -DCMAKE_BUILD_TYPE=Release -DACR_BUILD_CUDA=OFF
cmake --build build/acr -j

# 负例（机器负测 NEG-ACR-001）：从产品根尝试 add_subdirectory 引入会被
# lib/infrastructure/acr/CMakeLists.txt FATAL guard 拒绝 —— 本树不可并入产品构建图。
```

```powershell
# CPU-only（默认，不依赖 GPU SDK）—— Windows 实验验证
cmake -S lib/infrastructure/acr -B build/acr -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release
cmake --build build/acr -j

# 启用 CUDA（本机 RTX 3060 Ti 真实验证；仅实验树，生产不含 CUDA）
cmake -S lib/infrastructure/acr -B build/acr-cuda -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release -DACR_BUILD_CUDA=ON
