...
$Violations = git diff --name-only HEAD -- . ':(exclude)lib/infrastructure/acr/' ':(exclude)工程控制/tasks/acr/' ':(exclude)工程控制/evidence/acr/' ':(exclude)tools/_*'
$Untracked = git ls-files --others --exclude-standard -- . ':(exclude)lib/infrastructure/acr/' ':(exclude)工程控制/tasks/acr/' ':(exclude)工程控制/evidence/acr/' ':(exclude)tools/_*'
