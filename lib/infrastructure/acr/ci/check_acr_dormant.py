...
Static checks (source-tree level; no configure/build required):
  ...
  ACK-ACR-001  production CMake graph never add_subdirectory(lib/infrastructure/acr)
                (root CMakeLists.txt has no add_subdirectory(lib/infrastructure/acr)).
  ACK-ACR-002  lib/infrastructure/acr/CMakeLists.txt carries the standalone-only guard
  ...
  ACK-ACR-006  registry/module loader rejects ACR modules in production
                (lib/infrastructure/scheduler/src/module.cpp refuses astrocs.acr.*).
...
ACR_HEADER_MARKERS = [
    ...
    "cuda_bridge_api.hpp",
    "acr_cuda_bridge.h",
    "lib/infrastructure/acr/backends/cuda/bridge",
]
...
def failures_for(repo: pathlib.Path) -> list[str]:
    ...
    # ACK-ACR-001: root CMake never add_subdirectory(lib/infrastructure/acr)
    root_cmake = repo / "CMakeLists.txt"
    root_txt = read_text(root_cmake)
    if re.search(r"add_subdirectory\s*\(\s*lib/infrastructure/acr\b", root_txt) or \
       re.search(r"add_subdirectory\s*\(\s*\$\{CMAKE_CURRENT_SOURCE_DIR\}/lib/infrastructure/acr\b", root_txt):
        fails.append("ACK-ACR-001: root CMakeLists.txt add_subdirectory(lib/infrastructure/acr) — ACR in product graph")

    # ACK-ACR-002: lib/infrastructure/acr guard present
    acr_cmake = repo / "lib/infrastructure/acr/CMakeLists.txt"
    acr_txt = read_text(acr_cmake)
    if "CMAKE_SOURCE_DIR STREQUAL CMAKE_CURRENT_SOURCE_DIR" not in acr_txt:
        fails.append("ACK-ACR-002: lib/infrastructure/acr/CMakeLists.txt standalone-only guard missing")
    if "FATAL_ERROR" not in acr_txt:
        fails.append("ACK-ACR-002: lib/infrastructure/acr/CMakeLists.txt guard must FATAL_ERROR on non-standalone")
    ...
    module_cpp = repo / "lib/infrastructure/scheduler/src/module.cpp"
...
def selftest() -> int:
    ...
    def mkrepo(violation: str) -> pathlib.Path:
        ...
        (td / "lib/infrastructure/acr/CMakeLists.txt").write_text(
            "cmake_minimum_required(VERSION 3.24)\n", encoding="utf-8")
        (td / "lib/infrastructure/scheduler").mkdir(parents=True)
        (td / "lib/infrastructure/scheduler/src").mkdir()
        (td / "lib/infrastructure/scheduler/src/module.cpp").write_text(
            "// rejects astrocs.acr.*\n", encoding="utf-8")
        if violation == "add_subdir":
            (td / "CMakeLists.txt").write_text(
                "option(ASTROCS_ENABLE_ACR \"dormant\" OFF)\n"
                "add_subdirectory(lib/infrastructure/acr)\n", encoding="utf-8")
        elif violation == "no_guard":
            pass  # lib/infrastructure/acr/CMakeLists.txt without guard (default above)
