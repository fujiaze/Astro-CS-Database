...
//
// 2026-07-16 按 architecture-refactor spec G1 从 lib/infrastructure/aio/healpix_db/healpix_io/ 合并入 aio
// 原 healpix_io.cpp 已归档到 lib/infrastructure/aio/healpix_db/healpix_io/archive/
...
// P2 (R9-A): HCSD JSON 头长度上限 —— uncompLen/compLen 来自文件 (uint32,
// 不可信), 此前恶意头直接 jsonStr.resize(4GB-1) 无界分配打爆内存。合法
// nside=64 库头 JSON 为百字节级, 64MB 余量充足 (与 aio_xisf 64MB XML 上限
// 同口径)。
static const uint32_t HCSD_MAX_JSON_BYTES = 64u * 1024 * 1024;
...
// P1 (R9-A): C 边界异常屏障专用错误码 (bughunt_p1_batchI)。错误码域
// -1..-7 为既有语义, -8 仅在 C ABI 边界拦截到 C++ 异常时返回。
static const int HIO_ERR_INTERNAL    = -8;
...
// aio_api.cpp P0-4 口径)。本文件 24 个 AIO_EXPORT 入口此前 0 个有 try 保护:
// hiss/hcsd reader-writer 内部 std::string/vector 分配 bad_alloc、
// length_error 及 JSON 解析链上的构造异常均可跨 C ABI 传播 (UB/terminate)。
// 统一口径: 指针返回型 -> nullptr; int 返回型 -> HIO_ERR_INTERNAL(-8, 新增,
// 不与既有 -1..-7 语义冲突); void -> 仅输出日志; aio_hio_free 家族虽仅
// free/delete 仍统一包壳保持家族一致。正常路径与修复前逐行等价。
// ============================================================================
AIO_EXPORT int aio_hiss_write(const char* path, uint32_t nside, int nested,
    ...
    try {
        ...
        fprintf(stderr, "[hio] exception: %s\n", e.what());
        return HIO_ERR_INTERNAL;
    } catch (...) {
        fprintf(stderr, "[hio] unknown exception\n");
        return HIO_ERR_INTERNAL;
    }
}
...
AIO_EXPORT int aio_hcsd_read(const char* path, uint32_t* nside, int* nested,
    ...
    try {
        ...
        *meta_json = nullptr;
        ...
        // P2 (R9-A): 头长度上限 (HCSD_MAX_JSON_BYTES), 防恶意头 4GB 无界分配。
        if (uncompLen > HCSD_MAX_JSON_BYTES || compLen > HCSD_MAX_JSON_BYTES) {
            fprintf(stderr, "[hio] hcsd_read: JSON 头长度超上限 (uncomp=%u comp=%u)\n",
                    uncompLen, compLen);
            std::fclose(fp);
        ...
        *n_pix = jsonNPix;
        ...
        // P2 (R9-A): n_pix 无界分配防护 —— n_pix 来自恶意 JSON 头, 此前直接
        // malloc(n_pix * 12) 巨量分配 (ipix+pixel)。HEALPix 全天硬上限
        // 12 * 2^30 像素 (nside<=2^15 本产品域), 此处取 12*2^35 为界 (32 倍
        // 余量), 超限按损坏拒绝 (HIO_ERR_BOUNDS=既有错误码)。完整文件读取
        // 仍要求实际字节存在, 短读走 HIO_ERR_FILE。
        if (jsonNPix > 12ULL * (1ULL << 35)) {
            fprintf(stderr, "[hio] hcsd_read: n_pix %llu 超上限 (损坏或恶意头)\n",
                    (unsigned long long)jsonNPix);
            std::fclose(fp);
            return HIO_ERR_BOUNDS;
        }
...
AIO_EXPORT int aio_hcsd_read_leaf(const char* path, uint64_t leaf_ipix_at_nside64,
    ...
    try {
        ...
        *pixel = nullptr;
        ...
        // P2 (R9-A): 头长度上限, 同 aio_hcsd_read (HCSD_MAX_JSON_BYTES)。
        if (uncompLen > HCSD_MAX_JSON_BYTES || compLen > HCSD_MAX_JSON_BYTES) {
            fprintf(stderr, "[hio] hcsd_read_leaf: JSON 头长度超上限 (uncomp=%u comp=%u)\n",
                    uncompLen, compLen);
            std::fclose(fp);
            return HIO_ERR_BOUNDS;
        }
        ...
        // P2 (R9-A): 索引项与 n_pix 语义校验 (均来自文件, 不可信)。
        // 1) totalNPix 上限同 aio_hcsd_read (12*2^35, HEALPix 全天硬上限余量)。
        // 2) entry.data_offset/data_length 巨值此前直接进 malloc / (long) 转换:
        //    data_offset=2^32 在 LLP64 下 (long) 截断为 0 → seek 到 ipix 数组
        //    起始 → 头区数据当像素读出仍返回 HIO_OK (offset 错位读)。
        // 3) data_offset 必须落在 totalNPix 的 ipix 数组域内且 8 字节对齐;
        //    data_length 不得越过数组末端。
        const uint64_t kMaxHCsdNPix = 12ULL * (1ULL << 35);
        if (totalNPix > kMaxHCsdNPix) {
            fprintf(stderr, "[hio] hcsd_read_leaf: n_pix %llu 超上限 (损坏或恶意头)\n",
                    (unsigned long long)totalNPix);
            std::fclose(fp);
            return HIO_ERR_BOUNDS;
        }
        if (entry.data_length > 0) {
            const uint64_t ipixArrayBytes = totalNPix * sizeof(uint64_t);
            if (entry.data_offset > ipixArrayBytes ||
                (entry.data_offset % sizeof(uint64_t)) != 0 ||
                entry.data_length > totalNPix - entry.data_offset / sizeof(uint64_t)) {
                fprintf(stderr, "[hio] hcsd_read_leaf: 索引项越界 (offset=%llu length=%llu n_pix=%llu)\n",
                        (unsigned long long)entry.data_offset,
                        (unsigned long long)entry.data_length,
                        (unsigned long long)totalNPix);
                std::fclose(fp);
...
AIO_EXPORT int aio_hiss_read_snr_model(const char* path, uint32_t* nside, int* nested,
    ...
    try {
        ...
        *n_pix = jsonNPix;
        ...
        // P2 (R9-A): n_pix/snr_n_points 无界分配防护, 同 aio_hcsd_read 口径
        // (12*2^35 HEALPix 全天硬上限余量; n_points 上限 1e8 — 稀疏控制点
        // 实际量级 <1e6)。恶意头此前直接 malloc 巨量分配打爆内存。
        if (jsonNPix > 12ULL * (1ULL << 35)) {
            fprintf(stderr, "[hio] hiss_read_snr_model: n_pix %llu 超上限 (损坏或恶意头)\n",
                    (unsigned long long)jsonNPix);
            std::fclose(fp);
            return HIO_ERR_BOUNDS;
        }
        if (jsonSnrNPoints > 100000000u) {
            fprintf(stderr, "[hio] hiss_read_snr_model: snr_n_points %u 超上限 (损坏或恶意头)\n",
                    jsonSnrNPoints);
            std::fclose(fp);
            return HIO_ERR_BOUNDS;
        }
