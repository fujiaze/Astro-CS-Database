...
- **目标**：主线 aio 直接读取 BASS DR3 等 fpack 压缩 FITS（science/weight/od）。索引与工具现位于 `testdata/BASS_DR3/`（2026-09-21 ROOT-CONSOLIDATION 由根目录 `BASS DR3/` 迁入）。
- **修改**（`src/aio_fits.cpp`）：
...
- **验证**：临时 DLL 冒烟（`testdata/BASS_DR3/tools/test_aio_fz.py`，BASS 实测样本；
  2026-09-21 ROOT-CONSOLIDATION 由 `BASS DR3/tools/test_aio_fz.py` 迁入）：
...
- `lib/algorithms/drizzle/healpix_drizzle/` - drizzle_engine.cpp/h, hp_drizzle_api.cpp, Makefile
- `lib/infrastructure/aio/healpix_db/healpix_stack/` - hp_stack_api.cpp, hp_stack_hiss.cpp, gradient/gradient_sampler.cpp, build.ps1
