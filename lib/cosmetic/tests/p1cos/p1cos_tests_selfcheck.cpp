// P1-COS-TEST · 故障注入自检 (selfcheck): 验证注入机制必败 + 基线对照
//
...
//   structure_filter_oracle / idw_center_bitwise / determinism_bitwise /
//   nan_dark_no_detection / f64_parity_f32 / ...
//   (P1COS_SELFCHECK_FAULT 可覆盖注入名; 默认 const_field_bitwise)
#include <sys/wait.h>
#include <unistd.h>
...
int run_selfcheck() {
    ...
    {
        char arg0[] = "p1cos_selfcheck";
        char arg1[] = "units";
        char* argv[] = {arg0, arg1, nullptr};
        const int rc = p1cos_run_core_groups(2, argv);
        if (rc != 0) {
    ...
    std::fprintf(stdout,
                 ...
                 name.c_str(), child_rc);
    std::fprintf(stdout, "P1COS SELFCHECK PASS (baseline + fault-injection both verified)\n");
    return 0;
}
