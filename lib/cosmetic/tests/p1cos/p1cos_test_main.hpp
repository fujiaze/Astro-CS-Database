...
#define P1COS_CHECK_EQ(cs, got, want)                                      \
    do {                                                                   \
...
inline int run_all_groups(const p1cos::TestGroup* groups, std::size_t n, int argc, char** argv) {
    ...
    if (const char* f = std::getenv("ASTROCS_P1COS_FAULT")) {
        ...
        while (pos < s.size()) {
            ...
            if (!tok.empty()) p1cos::FaultRegistry::instance().active.push_back(tok);
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        std::fprintf(stdout, "[p1cos] group %s ...\n", groups[i].name);
        std::fflush(stdout);
        const int rc = groups[i].fn();
        if (rc != 0) {
            std::fprintf(stderr, "[p1cos] group %s FAIL rc=%d\n", groups[i].name, rc);
            total_fail += 1;
        } else {
            std::fprintf(stdout, "[p1cos] group %s PASS\n", groups[i].name);
        }
        std::fflush(stdout);
    }
    if (total_fail == 0) {
        std::fprintf(stdout, "P1COS TESTS PASS (group=%s)\n", group.c_str());
        return 0;
    }
    std::fprintf(stderr, "P1COS TESTS FAIL (%d group(s) failed, group=%s)\n", total_fail, group.c_str());
    return 1;
}

}  // namespace p1cos

#endif  // P1COS_TEST_MAIN_HPP
