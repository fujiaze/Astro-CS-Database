    ...
    bool use_ratio)
{
    ...
    if (fov_diag > 0.0) {
        ...
        // use_ratio=true 时存储距离比值 d[k]/d_ref, d_ref=d_1 (抗宽 FOV 畸变)
        double d_ref = 1e-6;
        ...
        // use_ratio=true 时存储距离比值 d[k]/d_ref, d_ref=d_1
        double d_ref = 1e-6;
    ...
    bool use_ratio)
{
    ...
    // 距离比模式 - 遍历 W, 用 d_ref_w × ratio 恢复绝对距离验证
    ...
    bool use_ratio)
{
    ...
    // 距离比模式 - 计算 candidate_w 的最近邻星距离 d_ref_w
    double d_ref_w = 0.0;
    if (use_ratio) {
        d_ref_w = 1e18;
    ...
    double sigma_d)
{
    ...
    // 4. 统计最大票数
    // VoteMap value 改为 double (支持 angle bonus), max_v 同步改为 double
    double max_v = 0.0;
    ...
    bool use_ratio)
{
    ...
    for (int p = 0; p < n_pivot; ++p) {
        ...
        // use_ratio=true 时输出距离比值 (抗宽 FOV 畸变 fallback)
        ...
        if (n_valid_k5 >= min_match1) {
            ...
            // VoteMap value 改为 double, max_v 同步改为 double
            double max_v = 0.0;
            for (const auto& kv_pair : result.votes) {
                if (kv_pair.second > max_v) max_v = kv_pair.second;
            }
    ...
    VoteMap& votes)
{
    ...
    // 宽 FOV (>3°) 时 geo_n_max 降到 50, 减少 pairwise 组合 (C(100,2)=4950 -> C(50,2)=1225)
    ...
    // 保持固定 9.0" 是经验最优值。
    // 宽 FOV 时 tol 降到 6.0" (2 像素), 减少错误距离匹配
...
//
// 设计要点 ( 改进):
// 原 每个 u 只取 top-1 w, 当 top-1 是错误配对时真实匹配被丢弃,
// 导致 PROSAC 无法找到正确变换 (NGC55_T3_01 max_vote=26 但仅 2 内点)。
// 每个 u 取 top-3 w, 增加 candidates 多样性, 让 PROSAC 有更大采样空间。
    ...
    const IPVSolverParams& params)
{
    ...
    // 每个 u 取 top-K_w 个 w 候选
    const int K_w = 3;

    // 按图像星索引 i 聚合 (w, vote)
    // vote 类型从 int 改为 double (支持 angle bonus 累加)
    ...
    for (int i = 0; i < N_U; ++i) {
        ...
        // top-2/top-3 加入条件: vote >= max(vote_threshold, max_vote/2)
        // 避免引入票数过低的噪声候选
        // max_vote 现为 double, secondary_threshold 同步改为 double
    ...
    // 按 vote 降序排序候选列表
    // vote 改为 double, 用 < 比较避免浮点相等的边界问题
