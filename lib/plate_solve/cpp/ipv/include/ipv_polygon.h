...
// use_ratio=true 时输出距离比值 [d_k/d_ref], d_ref=d_1 (抗宽 FOV 畸变)
HexDescriptor build_hex_descriptor(
    ...
    bool use_ratio = false            // true=输出距离比值, false=输出绝对距离
);

// 收集星表候选星
// 对 HexDescriptor 的前 K 个距离 d[k], k-vector 查询 [d[k]-3σ_d, d[k]+3σ_d]
// 收集至少出现在 >=min_occur 个查询中的星表星
// use_ratio=true 时需传入 W, 对每个候选 w 用 d_ref_w × ratio 恢复绝对距离查询
std::set<int> collect_candidates(
    ...
    const std::vector<StarPoint>* W = nullptr,  // 距离比模式需要星表侧
    bool use_ratio = false            // true=距离比模式
);

// 验证候选星的多边形完整性
// 检查 candidate 的前 K 个邻星是否与 hex 的前 K 个邻星距离匹配
// 返回匹配的 (u_neighbor_idx, w_neighbor_idx) 对列表
// use_ratio=true 时用 d_ref_w × ratio 恢复绝对距离查询
...
// use_ratio=true 时启用距离比描述符 (抗宽 FOV 畸变 fallback)
PolygonMatchResult polygon_match_adaptive(
    ...
    bool use_ratio = false            // true=距离比模式 fallback
);
