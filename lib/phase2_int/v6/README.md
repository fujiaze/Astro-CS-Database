...
- `lib/phase1/v6`（P1-INTEGRATE-001）：`open_phase1_product`、`consume_phase1_group_for_psfsw`
- `lib/phase2/src/upm.cpp`（IMPL-P2-UPM-001）：`p2_upm_ma_*`、`p2_upm_control_variance`
- `lib/phase2/src/rejection.cpp`（IMPL-P2-REJ-001）：`p2_reject_classify`、`p2_reject_plan_resolve`、
  `p2_reject_plan_thresholds_inherited`、`p2_rejection_weight_surface_guard`
- `lib/phase2/src/{sampler,coverage}.cpp`（IMPL-P2-SAMP-001）：`p2_spatial_model_eval|summary`、
...
- `lib/astro_image_io/v6`（IMPL-AIO-001）：流式 FITS + DATASUM/CHECKSUM + 原子发布 + provenance + BUNIT
...
```bash
cmake -S lib/phase2_int/v6 -B <build> && cmake --build <build>
...
- 未改 `lib/phase2/src/{upm,rejection,sampler,coverage}.cpp` 及其头、`lib/phase1/**`、
  `lib/astro_image_io/**`、`contracts/**`、`docs/**`、根/公共 CMakeLists。
- `lib/phase2/src/integrate.cpp`/`block.cpp` 的 legacy 生产 API 未修改：V6 组合核心落在本目录，
