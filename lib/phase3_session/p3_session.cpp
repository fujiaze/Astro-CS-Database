...
static acs_status parse_request(SessionState* s, const acs_span_u8 req, json* out) {
    ...
    if (std::fabs(dec) > 85.0) { s->last_error = "abs(center.dec_deg) must be <= 85 deg (request-domain narrowing; TAN diverges only at 90 deg from CRVAL)"; return ACS_ERR_PARAM; }
