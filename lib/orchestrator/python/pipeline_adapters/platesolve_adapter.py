...
本文件从 lib/plate_solve/python/pipeline_adapter.py 迁移而来。
日志目录已更新为 orchestrator/logs/。
...
# 日志目录: orchestrator/python/pipeline_adapters/ -> ../../logs = orchestrator/logs
...
def _init_logger() -> logging.Logger:
    ...
    log_file = os.path.join(
        _LOG_DIR,
        "platesolve_adapter_" + datetime.now().strftime("%Y%m%d_%H%M%S") + ".log",
    )
    formatter = logging.Formatter(_LOG_FORMAT, datefmt=_DATE_FORMAT)

    lg = logging.getLogger("platesolve_adapter")
