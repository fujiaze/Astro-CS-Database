...
本文件从 lib/astro_image_io/python/orchestrator.py 迁移而来，适配器路径已更新为
本地 pipeline_adapters/ 目录下的副本。
"""
...
# 添加模块路径（dynamic_psf + 本地 pipeline_adapters）
_HERE = os.path.dirname(os.path.abspath(__file__))
_LIB_DIR = os.path.normpath(os.path.join(_HERE, "..", ".."))
_PIPELINE_ADAPTERS_DIR = os.path.join(_HERE, "pipeline_adapters")
if _PIPELINE_ADAPTERS_DIR not in sys.path:
    sys.path.insert(0, _PIPELINE_ADAPTERS_DIR)
...
from psf_adapter import make_psf_fit_handler
...
class Orchestrator:
    ...
    def __init__(self, calib_params=None, solve_params=None,
        ...
        参数:
            ...
            gaia_data_dir: Gaia DR3SP 数据目录路径 (None=不创建 gaia_client);
                           当 photo_params.gaia_client 为 None 且此参数提供时,
                           Orchestrator 创建 GaiaClientPy 实例传给 photo_params;
                           若 solve_params.env 已有 gaia_client 则优先复用
        """
        ...
        self.gaia_data_dir = gaia_data_dir
        self._gaia_client_owned = False  # 标记 gaia_client 是否由 Orchestrator 创建
        ...
        # ---- gaia_client 生命周期管理 ----
        # 为 photo_params 注入 gaia_client (用于 pc_calibrate_simple_with_gaia 新模式)
        # 优先级: 1. photo_params.gaia_client 已设置 2. solve_params.env[0] 复用 3. gaia_data_dir 新建
        if photo_params is not None and photo_params.gaia_client is None:
            if solve_params is not None and solve_params.env is not None:
                # 复用 PLATESOLVE 阶段的 gaia_client
                photo_params.gaia_client = solve_params.env[0]
                logger.info("复用 solve_params.env 的 gaia_client 给 photo_params")
            elif gaia_data_dir is not None:
                # Orchestrator 创建新的 gaia_client
                photo_params.gaia_client = self._create_gaia_client(gaia_data_dir)
                self._gaia_client_owned = True
        ...
        # 适配器文件位于本地 pipeline_adapters/ 目录
        if calib_params is not None:
            self._calib_handler = self._extract_handler(
                "orchestrator/python/pipeline_adapters/calibrate_adapter.py",
                "_orch_calib_adapter",
        ...
        if solve_params is not None:
            self._solve_handler = self._extract_handler(
                "orchestrator/python/pipeline_adapters/platesolve_adapter.py",
                "_orch_solve_adapter",
        ...
        if photo_params is not None:
            self._photo_handler = self._extract_handler(
                "orchestrator/python/pipeline_adapters/photometric_adapter.py",
                "_orch_photo_adapter",
        ...
        if drizzle_params is not None:
            self._drizzle_handler = self._extract_handler(
                "orchestrator/python/pipeline_adapters/drizzle_adapter.py",
    ...
    def _create_gaia_client(self, gaia_data_dir):
        """创建 GaiaClientPy 实例 (DR3SP 模式, db_type=2)

        用于 photometric_calib 的 pc_calibrate_simple_with_gaia 新模式。
        创建的 gaia_client 由 Orchestrator 持有, 需在结束时调用 close() 释放。

        参数:
            gaia_data_dir: Gaia DR3SP 数据目录路径

        返回:
            GaiaClientPy 实例
        """
        vmm_path = os.path.join(self._lib_dir, "gaia_xpsd_client", "python")
        if vmm_path not in sys.path:
            sys.path.insert(0, vmm_path)
        from vector_match_v2 import GaiaClientPy
        gaia_client = GaiaClientPy(gaia_data_dir, db_type=2)
        logger.info("Orchestrator 创建 GaiaClientPy: data_dir=%s, db_type=2 (DR3SP)",
                    gaia_data_dir)
        return gaia_client

    def close(self):
        """释放 Orchestrator 持有的资源 (如自建的 gaia_client)

        若 gaia_client 由 Orchestrator 创建 (_gaia_client_owned=True), 调用其 close() 释放;
        复用 solve_params.env 的 gaia_client 不在此释放 (由 platesolve 侧管理)。
        """
        if self._gaia_client_owned and self.photo_params is not None:
            gaia_client = getattr(self.photo_params, "gaia_client", None)
            if gaia_client is not None:
                try:
                    gaia_client.close()
                    logger.info("Orchestrator 自建的 gaia_client 已释放")
                except Exception as e:
                    logger.warning("释放 gaia_client 失败: %s", e)
                finally:
                    self.photo_params.gaia_client = None
                    self._gaia_client_owned = False
