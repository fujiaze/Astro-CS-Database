...
// [P27-DEAD-PARAMS] 生产路径**全部字段不可达**: 唯一生产调用点
// (ipv_solver.cpp::solve_post_select) 传字面量 `RobustRefineParams{}`（默认构造），
// 因此本结构体所有字段改变都不影响生产行为也不报错。
// 机器锁: ctest ipv_dead_params_lock (frozen_literals 校验该字面量调用点)。
// 详见 run/perf-fix/P27-dead-params/REPORT.md。不改变任何默认值/公式/容差。
// ---------------------------------------------------------------------------
// 鲁棒精化参数 (默认值对齐 spec)
// ---------------------------------------------------------------------------
