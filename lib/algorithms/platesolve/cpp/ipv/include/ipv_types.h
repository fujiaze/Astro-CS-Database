...
// 星点 (原点在图像中心; 单位随向量而定: U=像素, W=角秒)
struct StarPoint {
    double x;          // X坐标 (U 向量: 像素 / W 向量: 角秒)
    double y;          // Y坐标 (U 向量: 像素 / W 向量: 角秒)
    double flux;       // 流量
    bool   saturated;  // 是否饱和
};
...
struct SIPCoeffs {
    ...
    int    order;      // 前向 SIP 阶数 (0=无 SIP, >0 为拟合阶数, 受 36 槽位约束)
    int    ap_order;   // 逆向 SIP 阶数 (0=无逆向 SIP, >0 为拟合阶数)
...
struct IPVSolverParams {
    ...
    // Gaia 客户端"每文件返回上限" (lib/infrastructure/gaia_xpsd_client/src/gaia_client.c MAX_STARS_RESULT,
