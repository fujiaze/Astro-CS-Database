#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ipv_abi_layout_lock.py — IpvParams / IpvWcsResult 布局一致性机器锁 (P18 / V2-N-01)

宪章 §8.6: 跨 DLL 边界使用版本化 C ABI; 结构体带 struct_size/abi_version。
本锁把"公共 C 头定义"与"Python ctypes 镜像"钉死:

  1) 运行 C 探针 ipv_abi_layout_probe (只 include ipv_api.h, Linux 可编译),
     读取其 sizeof / alignof / 逐字段 offsetof+sizeof 的 JSON;
  2) 与权威镜像 lib/algorithms/platesolve/tools/ipv_abi_mirror.py 逐字段比对;
  3) 字段名、字段顺序、字段 offset、字段 size、结构体 sizeof、alignof
     任一不一致 => 退出码 1 (ctest FAIL)。

任一字段偏移或大小漂移都会必然失败, 因此 V2-N-01 型"头改了、镜像没改"
不可能再悄悄通过。

用法:
  ipv_abi_layout_lock.py --probe <探针可执行文件> [--mirror-file <镜像.py>]
  ipv_abi_layout_lock.py --probe <...> --selfcheck    # 反向证明锁非恒真

--selfcheck: 用探针数据构造一个被故意破坏 (缺字段) 的镜像, 断言锁必然报错。
若锁对被破坏镜像仍然"通过", 说明门失效, 自检失败 (非零退出)。
"""
...
# HERE = lib/algorithms/platesolve/cpp/ipv/test -> ../../../tools = lib/algorithms/platesolve/tools
DEFAULT_MIRROR = os.path.normpath(
    os.path.join(HERE, "..", "..", "..", "tools", "ipv_abi_mirror.py"))


def load_module(path, name="ipv_abi_mirror_under_test"):
...
def make_mutated_mirror(probe_data):
    """用探针真实布局构造一个"漏同步"的假镜像 (删掉 m_lim_safety 字段),
    用于证明锁的报警不是恒真。返回 SimpleNamespace 形态的镜像命名空间。"""
    import types
    c_fields = probe_data["IpvParams"]["fields"]
    cls = type("MutatedIpvParams", (ctypes.Structure,), {})
    fields = []
    for f in c_fields:
        if f["name"] == "m_lim_safety":
            continue  # 故意漏字段
        if f["size"] == 256:
            fields.append((f["name"], ctypes.c_char * 256))
        else:
            fields.append((f["name"], _ctype_for_size(f["size"])))
    cls._fields_ = fields
    ns = types.SimpleNamespace()
    ns.MIRRORED_STRUCTS = {"IpvParams": cls}
    ns.IPV_PARAMS_ABI_VERSION = 999   # 版本也篡改
    ns.IPV_PARAMS_STRUCT_SIZE = -1    # 尺寸也篡改
    return ns
...
def check_all(probe_data, mirror_ns):
    ...
    # 宪章 §8.6 ABI 头部契约 + 常量一致性
    if "IpvParams" in probe_data:
        pf = probe_data["IpvParams"]["fields"]
        if not pf or pf[0]["name"] != "struct_size" or pf[0]["offset"] != 0:
            errors.append("宪章 §8.6: IpvParams 首字段必须是 struct_size@0")
        if len(pf) < 2 or pf[1]["name"] != "abi_version" or pf[1]["offset"] != 4:
            errors.append("宪章 §8.6: IpvParams 第二字段必须是 abi_version@4")
        abi_c = probe_data["IpvParams"].get("abi_version_value")
        abi_py = getattr(mirror_ns, "IPV_PARAMS_ABI_VERSION", None)
        if abi_c != abi_py:
            errors.append("IPV_PARAMS_ABI_VERSION: C=%s Python=%s" % (abi_c, abi_py))
        size_py = getattr(mirror_ns, "IPV_PARAMS_STRUCT_SIZE", None)
        if size_py != probe_data["IpvParams"]["sizeof"]:
            errors.append("IPV_PARAMS_STRUCT_SIZE: C=%d Python=%s"
                          % (probe_data["IpvParams"]["sizeof"], size_py))
...
def print_tables(tables):
    for name, probe_struct, rows in tables:
        ...
        print("  %-4s %-34s %-8s %-6s | %-34s %-8s %-6s %s"
              % ("#", "C field", "off", "size", "Python field", "off", "size", "OK"))
        for i, cn, co, cs, pn, po, ps, ok in rows:
            print("  %-4d %-34s %-8s %-6s | %-34s %-8s %-6s %s"
                  % (i, cn, co, cs, pn, po, ps, "OK" if ok else "MISMATCH"))


def main(argv=None):
    ap = argparse.ArgumentParser(description="IpvParams/IpvWcsResult 布局一致性锁")
    ap.add_argument("--probe", default=os.environ.get("IPV_LAYOUT_PROBE"),
                    help="C 探针可执行文件路径 (或设 IPV_LAYOUT_PROBE)")
    ...
    if not args.probe:
        sys.stderr.write("缺少 --probe (或环境变量 IPV_LAYOUT_PROBE)\n")
        return 2
    ...
    if args.selfcheck:
        ...
        print("[selfcheck] 对被破坏镜像的报错数: %d" % len(errors))
        for e in errors[:8]:
            print("[selfcheck]   " + e)
        if errors:
    ...
    print("C ABI_VERSION=%s 镜像 ABI_VERSION=%s"
          % (probe_data.get("IpvParams", {}).get("abi_version_value"),
             getattr(mirror_ns, "IPV_PARAMS_ABI_VERSION", None)))
    if errors:
        print("\nIPV_ABI_LAYOUT_LOCK FAIL (%d 处不一致):" % len(errors))
        for e in errors:
            print("  - " + e)
        return 1
    print("\nIPV_ABI_LAYOUT_LOCK PASS: C 与 ctypes 镜像逐字段一致")
    return 0
