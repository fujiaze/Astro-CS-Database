# lib/algorithms/platesolve — 模块合同（P1-WCS-DOC）
...
| legacy_paths | `lib/algorithms/platesolve`（生产实现 ipv_solver.dll 所在，本合同主落位）+ `lib/algorithms/platesolve/wrapper_phase1`（旧符号 WcsTan，计划迁移，见 §9） |
...
- **DISP-WCS-001（R1，核心）**：CD 矩阵退化静默坍缩——
  lib/algorithms/photometry/cpp/src/wcs_transform.cpp:39-49 det<1e-15 →
  cdInv 全零仅 stderr，输出坍缩 CRPIX−1 无错误通道；同族
  lib/algorithms/platesolve/wrapper_phase1/wcs_tan.cpp:48-51 det<1e-30 → 返回 CRPIX 零日志。
...
- dll_loader ModuleId::PLATESOLVE 加载名与路径吻合（lib/algorithms/platesolve/ 下
