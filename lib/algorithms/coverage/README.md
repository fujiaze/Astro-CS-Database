...
> lib/algorithms/coverage/src/coverage.cpp（239 行实测）与唯一权威签名头
> lib/algorithms/coverage/include/astro/phase2/coverage.h（59 行）位于本目录，故
...
- doc revision: README r1（P2-COV-DOC 2026-09-07）；源码零改动断言：
  本任务 lib/algorithms/coverage 生产源 diff=0（git status 白名单核验）。
...
- 本模块无独立 config schema：`p2_coverage_build` 无 config 参数
  ...
  coverage 相关键归 P2 session 域（lib/algorithms/coverage/tools/config_smoke.py）。
...
- 无 provider 轴（cpu_providers=[baseline]，单线程整数集合运算无
  ISA 分支）；AIO reader 为唯一外部依赖（lib/infrastructure/aio，只读
...
- 生产调用链：p2_session 阶段 1（p2_session.cpp:119-148，两阶段调用
  ...
  （lib/algorithms/coverage/tools/stage2.cpp:189-200）；UPM 经控制拓扑间接关联
