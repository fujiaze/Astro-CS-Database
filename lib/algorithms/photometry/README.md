# lib/algorithms/photometry — 模块合同（P1-PHOT-DOC）
...
| legacy_paths | `lib/algorithms/photometry`（生产实现 C ABI DLL，本合同主落位）+ `lib/algorithms/photometry/wrapper_phase1`（aperture 测光旧符号，计划迁移，见 §9） |
...
- 构建：lib/algorithms/photometry/cpp/Makefile（:11 TARGET=photometric_calib.dll，
  ...
  集成归 P1-PHOT-IMPL。
- DLL 装载：lib/infrastructure/pipeline/orchestrator/cpp/src/dll_loader.cpp:40（ModuleId::PHOTOMETRIC
  → "photometric_calib.dll"）、:54（子目录 lib/algorithms/photometry/cpp/）；
