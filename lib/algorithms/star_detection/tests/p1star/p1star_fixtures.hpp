...
// sdet_create(nullptr), 256² 单星居中, 峰值对齐像素中心, **1000 次/档**)。
//   THR50 : 检出概率 0.5 的信噪比 (50% 过渡点)
//   THR99 : 使实测召回达到 **1000/1000** 的最小信噪比格点。
//           零失败下 Clopper-Pearson 95% 下界 = 0.05^(1/n): n=64 → 0.95427 (不足以
//           确立 ≥99%), n=300 → 0.99006, n=1000 → 0.99701。最小样本量
//           ln(0.05)/ln(0.99) = 298.07。本表用 n=1000。
//   σ_psf = 1.0 px 档**不给点值**: 1000 次/档复测确认其零失败性质**非单调**
//   (零失败点 65/66/71/74/76 之间夹有 999/1000、998/1000、997/1000), 故记区间
//   [LO, HI] = [52, 65] —— 下端 = 使实测召回自该点起恒 ≥0.99 的最小信噪比,
//   上端 = 首次出现零失败的格点。分类用下端 (POS 集 = 阶梯上 ≥52 的点),
//   但**引用该档阈必须同时报区间**。其余五档在 n=1000 下自阈起连续 ≥4 个格点
//   零失败, 点值稳定。
...
inline constexpr double F1TB_THR99[6] = {52.0, 24.0, 20.0, 16.0, 12.0, 10.0};
inline constexpr double F1TB_THR99_HI[6] = {65.0, 24.0, 20.0, 16.0, 12.0, 10.0};
inline constexpr bool F1TB_THR99_IS_INTERVAL[6] = {true, false, false, false, false, false};
inline constexpr double F1TB_KAPPA50 = 6.21;       // 50% 点单参数拟合 (趋势模型)
inline constexpr double F1TB_KAPPA99 = 7.27;       // 99% 阈单参数拟合, σ>=1.27 五档
                                                   // (残差 ≤9.2%; 六档含 σ=1.0 时
                                                   //  7.79 ± 1.34, 残差 25.1%)
...
// 该档 99% 阈为区间 [52, 65] (高于空档上端), 不补则该档只提供负例、域内召回
// 不被行使。阶梯上 ≥65 的三级 (65/75/85) 使该档即使按区间上端读法也被行使。
