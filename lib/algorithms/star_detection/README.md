...
`lib/algorithms/star_detection/wrapper_phase1`（P1-003 桥接层独立 sigma-clip 背景+阈值实现，§10 如实差距）。
...
- 无取消检查点；无 checkpoint；日志 sdet_log 默认阈值（lib/algorithms/star_detection/logs/）。
