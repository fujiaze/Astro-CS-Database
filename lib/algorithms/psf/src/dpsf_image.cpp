...
inline float mirror_fetch(const float* src, int w, int h, int x, int y) {
    if (x < 0) x = -x - 1;
    if (x >= w) x = 2 * w - x - 1;
    if (y < 0) y = -y - 1;
    if (y >= h) y = 2 * h - y - 1;
...
void gaussian_filter_separable(const float* src, float* dst, int w, int h, double sigma) {
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            ...
            for (int k = -radius; k <= radius; k++) {
                val += (double)mirror_fetch(src, w, h, x + k, y) * kernel[k + radius];
            }
            tmp[y * w + x] = (float)val;
        }
    }
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            double val = 0.0;
            for (int k = -radius; k <= radius; k++) {
                val += (double)mirror_fetch(tmp.data(), w, h, x, y + k) * kernel[k + radius];
            }
            dst[y * w + x] = (float)val;
        }
    }
        ...
        for (int x = 0; x < w; x++) {
            ...
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    p[idx++] = mirror_fetch(src, w, h, x + dx, y + dy);
                }
            }
        ...
        for (int x = 0; x < w; x++) {
            ...
            for (int dy = -2; dy <= 2; dy++) {
                for (int dx = -2; dx <= 2; dx++) {
                    buf[idx++] = mirror_fetch(src, w, h, x + dx, y + dy);
                }
            }
...
void median_filter(const float* src, float* dst, int w, int h, int radius) {
    if (radius == 1) {
        median_filter_3x3(src, dst, w, h);
        return;
    }
    if (radius == 2) {
        median_filter_5x5(src, dst, w, h);
        return;
    }
...
void morph_op_box(const float* src, float* dst, int w, int h, int radius, bool is_dilate) {
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            float val = is_dilate ? -1e30f : 1e30f;
            for (int dy = -radius; dy <= radius; dy++) {
                for (int dx = -radius; dx <= radius; dx++) {
                    float v = mirror_fetch(src, w, h, x + dx, y + dy);
...
void morph_op_circle(const float* src, float* dst, int w, int h, int radius, bool is_dilate) {
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            ...
            for (auto& off : offsets) {
                float v = mirror_fetch(src, w, h, x + off.first, y + off.second);
                if (is_dilate) val = std::max(val, v);
                else val = std::min(val, v);
            }
            dst[y * w + x] = val;
...
float robust_mad(const float* data, int n) {
    if (n <= 0) return 0.0f;
    float med = robust_median(data, n);
...
void atrous_b3v_filter(const float* src, float* dst, int w, int h, int scale) {
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            ...
            for (int k = -2; k <= 2; k++) {
                int sx = x + k * step;
                val += (double)mirror_fetch(src, w, h, sx, y) * b3v[k + 2];
            }
            tmp[y * w + x] = (float)val;
        }
    }
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            ...
            for (int k = -2; k <= 2; k++) {
                int sy = y + k * step;
                val += (double)mirror_fetch(tmp.data(), w, h, x, sy) * b3v[k + 2];
            }
            dst[y * w + x] = (float)val;
        }
    }
...
void iterative_sigma_clip(const float* data, int n,
    ...
    float prev_med = robust_median(buf.data(), (int)buf.size());
    float prev_mad = robust_mad(buf.data(), (int)buf.size());

    dpsf_log(LOG_DEBUG, "SIGMA_CLIP", "Round 0: med=%.6f mad=%.6f pixels=%d", prev_med, prev_mad, (int)buf.size());
    ...
    for (int round = 1; round <= max_rounds; round++) {
        ...
        if ((int)clipped.size() < 100) {
            dpsf_log(LOG_DEBUG, "SIGMA_CLIP", "Round %d: only %d pixels left (< 100), stopping", round, (int)clipped.size());
            *out_med = prev_med;
            *out_mad = prev_mad;
            return;
        }

        float med = robust_median(clipped.data(), (int)clipped.size());
        float mad = robust_mad(clipped.data(), (int)clipped.size());

        dpsf_log(LOG_DEBUG, "SIGMA_CLIP", "Round %d: med=%.6f mad=%.6f clipped=%d/%d (%.1f%%)",
        ...
        if (med_change < 0.001f && mad_change < 0.001f) {
            dpsf_log(LOG_DEBUG, "SIGMA_CLIP", "Converged at round %d: med_change=%.4f%% mad_change=%.4f%%",
    ...
    dpsf_log(LOG_DEBUG, "SIGMA_CLIP", "Reached max_rounds=%d: med=%.6f mad=%.6f", max_rounds, prev_med, prev_mad);
    *out_med = prev_med;
    *out_mad = prev_mad;
}
...
void extract_lowfreq_atrous(const float* src, float* dst, int w, int h, int downsample_factor, int n_scales) {
    ...
    downsample(src, w, h, small.data(), dw, dh);

    atrous_b3v_filter(small.data(), c1.data(), dw, dh, 0);

    if (n_scales >= 2) {
        atrous_b3v_filter(c1.data(), c2.data(), dw, dh, 1);
        upsample_bilinear(c2.data(), dw, dh, dst, w, h);
    } else {
        upsample_bilinear(c1.data(), dw, dh, dst, w, h);
    }
}
