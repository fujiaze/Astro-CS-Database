 ...
 *           weight_value}
 *   psfsw canonical = kind="psfsw_robust_weight" / units="1" /
...
struct Phase1Units {
  std::string signal_sb = "ADU/px^2";
  std::string pixel_variance_in = "ADU^2";
  std::string sb_variance_out = "ADU^2/px^4";
  std::string sb_ivar_out = "px^4/ADU^2";
...
/* ── Phase2 消费面：仅由磁盘重开的产品组成帧组，产组内归一 PSFSW 权重 ──
 * 组内 median=1 归 Phase2（OI-01）；返回的 w_psfsw 由 Wave 5 实现计算，
 * 并用 validate_psfsw_record 复核（组级记录门）。 */
