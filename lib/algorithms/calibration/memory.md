...
- **路径**: `lib/algorithms/calibration/python/master_generator.py`
...
- **日志**: `lib/algorithms/calibration/logs/master_generator_YYYYMMDD_HHMMSS.log`，UTF-8，文件+控制台
- **验证**: 6/6 合成测试通过（离群值剔除/median合并/none+mean/flat归一化/常量区域/参数校验）

## calibrator.py 详情
- **路径**: `lib/algorithms/calibration/python/calibrator.py`
- **IO**: 统一使用 astro_image_io（ImageReader 读 FITS/XISF，FITSWriter 写 float32 FITS）
- **核心算法**:
...
- **日志**: `lib/algorithms/calibration/logs/calibrator_YYYYMMDD_HHMMSS.log`，UTF-8，文件+控制台
...
- **路径**: `lib/algorithms/calibration/python/cosmetic_corrector.py`
- **IO**: 统一使用 astro_image_io（ImageReader 读 FITS/XISF，FITSWriter 写 float32 FITS）
...
- **C++ DLL绑定**: 优先调用C++ DLL（OpenMP并行），失败时fallback到Python scipy
  - `_load_cpp_dll()`: 模块级缓存加载 `lib/algorithms/calibration/cosmetic_corrector.dll`，设置4个函数签名（cc_correct_median/cc_detect_hot/cc_detect_cold/cc_last_error）
...
- **日志**: `lib/algorithms/calibration/logs/cosmetic_corrector_YYYYMMDD_HHMMSS.log`，UTF-8，文件+控制台
...
- **路径**: `lib/algorithms/calibration/python/calibrate_fits.py`
...
- **设计要点**:
  ...
  - 日志: `lib/algorithms/calibration/logs/calibrate_fits_YYYYMMDD_HHMMSS.log`，UTF-8
