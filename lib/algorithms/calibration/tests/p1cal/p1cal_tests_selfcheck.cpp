...
int run_selfcheck() {
    ...
    // 阶段 2: 子进程注入 → 必 FAIL（逐名验证；BIAS-001 增加 bias/K 参与名）
    const char* env_fault = std::getenv("P1CAL_SELFCHECK_FAULT");
    std::vector<std::string> names;
    if (env_fault != nullptr && *env_fault != '\0') {
        names.push_back(env_fault);
    } else {
        names = {"const_field_bitwise", "gradient_std_oracle", "gradient_darkopt_oracle",
                 "bias_participation_gate", "bias_influence_standard", "bias_only_influence",
                 "std_k_applied"};
    }
    ...
    std::fprintf(stdout, "P1CAL SELFCHECK PASS (baseline + %zu fault-injections verified)\n",
                 names.size());
    return 0;
}
