...
运行:
    cd "f:\Astro dev\Astro CS Normalization Database\lib\astro_image_io\python\tests"
...
变更说明 (2026-07-12):
    - 适配新版 handler:
      - photometric 不再生成 grad_map 块 (简化版 C++ DLL)
      - photometric 生成 photo_stats KV 块 (含 N_MATCHED, SCALE_FACTOR)
      - plate_solve 无临时文件 (ipv_solve_from_memory 内存接口)
      - drizzle 无临时文件 (hp_drizzle_run 命名块直通)
    - 新增验证项:
      - 无临时 FITS 文件生成 (检查临时目录)
      - .ahpx 输出正确且非空
      - header 含 WCS 字段 (CD1_1 等)
      - star_det/gaia_cat 块存在
      - psf 块存在
      - data 块被 photometric 校准后替换
      - photo_stats KV 块有 SCALE_FACTOR
      - 各阶段耗时记录
"""
...
def _load_params():
    ...
    try:
        mod = _load_module(
            "_test_calib_adapter",
            os.path.join(LIB_DIR, "calibration", "python", "pipeline_adapter.py"),
        )
    ...
    try:
        mod = _load_module(
            "_test_solve_adapter",
            os.path.join(LIB_DIR, "plate_solve", "python", "pipeline_adapter.py"),
        )
    ...
    try:
        mod = _load_module(
            "_test_photo_adapter",
            os.path.join(LIB_DIR, "photometric_calib", "flux_calibrator",
                         "python", "pipeline_adapter.py"),
        )
    ...
    try:
        mod = _load_module(
            ...
            os.path.join(LIB_DIR, "healpix_db", "healpix_drizzle", "pipeline_adapter.py"),
        )
