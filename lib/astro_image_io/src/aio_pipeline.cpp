...
AIO_EXPORT int aio_frame_add_block(PipelineFrame* frame,
    ...
    if (count < 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "add_block: count < 0 (%lld)", (long long)count);
        return 2;
    }

    /* 若同名块已存在，先移除 (释放旧 data) */
    int idx = find_block_index(frame, name);
    if (idx >= 0) {
        if (frame->blocks[idx].data) {
            std::free(frame->blocks[idx].data);
            frame->blocks[idx].data = nullptr;
        }
    } else {
        /* 新增块 */
        if (ensure_capacity(frame) != 0) return 3;
        idx = frame->n_blocks++;
    }

    AioBlock* blk = &frame->blocks[idx];
    init_block_fields(blk, name, type, count, dims, n_dims, description);

    /* 分配并拷贝数据 */
    size_t elem_size = block_type_elem_size(type);
    if (elem_size == 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "add_block: unknown type %d", (int)type);
        blk->data = nullptr;
        blk->count = 0;
        return 4;
    }
    ...
    blk->data = std::malloc(total_bytes);
    if (!blk->data) {
    ...
    if (data) {
        std::memcpy(blk->data, data, total_bytes);
    } else {
        std::memset(blk->data, 0, total_bytes);
    }
...
AIO_EXPORT int aio_frame_add_block_move(PipelineFrame* frame,
    ...
    if (count < 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "add_block_move: count < 0");
        return 2;
    }
...
static char* read_str_with_len(FILE* fp, char* buf, size_t buf_size) {
    int32_t len = 0;
    if (read_bin(fp, len) != 0) return nullptr;
    if (len < 0) return nullptr;
    if ((size_t)len >= buf_size) {
        /* 截断 */
        if (buf_size > 0) {
            if (read_bytes(fp, buf, buf_size - 1) != 0) return nullptr;
            buf[buf_size - 1] = '\0';
            /* 跳过剩余 */
            std::fseek(fp, (long)(len - (buf_size - 1)), SEEK_CUR);
            return buf;
        }
        /* 缓冲区为 0，只跳过 */
    ...
    if (read_bytes(fp, buf, (size_t)len) != 0) return nullptr;
    buf[len] = '\0';
    return buf;
}
...
/* 内部: 反序列化单个块的数据 (KV 块特殊处理) */
static int deserialize_block_data(FILE* fp, AioBlock* blk) {
    int64_t data_size = blk->count;
    /* data_size 对 KV 块是条目数，对其他类型是元素数 */

    if (blk->type == AIO_BLOCK_KV) {
        /* 分配 KV 数组 (预留容量) */
        int64_t cap = data_size > 0 ? data_size * 2 : 8;
        ...
        for (int64_t i = 0; i < data_size; ++i) {
            char key[256];
            char value[512];
            if (!read_str_with_len(fp, key, sizeof(key))) return -1;
            if (!read_str_with_len(fp, value, sizeof(value))) return -1;
            copy_str_to_fixed(entries[i].key, sizeof(entries[i].key), key);
            copy_str_to_fixed(entries[i].value, sizeof(entries[i].value), value);
        }
        blk->data = entries;
        blk->dims[0] = (int)cap;
        blk->n_dims = 1;
        ...
        size_t elem_size = block_type_elem_size(blk->type);
        if (elem_size == 0) return -1;
        size_t total = (size_t)data_size * elem_size;
        if (total == 0) {
            blk->data = nullptr;
            return 0;
        }
...
AIO_EXPORT int aio_frame_load_cache(PipelineFrame* frame, const char* path) {
    ...
    if (!failed && read_bin(fp, version) != 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: read version failed");
        failed = true;
    }
    if (!failed && version != AIO_CACHE_VERSION) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: version mismatch (got %d, expected %d)",
                version, AIO_CACHE_VERSION);
        failed = true;
    }
    if (!failed && read_bin(fp, n_blocks) != 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: read n_blocks failed");
        failed = true;
    }
    if (!failed && read_bin(fp, stages) != 0) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: read stages failed");
        failed = true;
    }
    ...
    /* 预分配容量 */
    init_cap = n_blocks > 0 ? n_blocks : 8;
    new_blocks = (AioBlock*)std::malloc((size_t)init_cap * sizeof(AioBlock));
    if (!new_blocks) {
        std::fclose(fp);
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: malloc failed");
        return 3;
    }
    frame->blocks = new_blocks;
    frame->blocks_capacity = init_cap;
    std::memset(frame->blocks, 0, (size_t)init_cap * sizeof(AioBlock));

    /* 逐块反序列化 */
    for (int i = 0; i < n_blocks && !failed; ++i) {
        AioBlock* blk = &frame->blocks[i];
        char name[256];
        if (!read_str_with_len(fp, name, sizeof(name))) { failed = true; break; }
        int32_t type_val = 0;
        ...
        if (deserialize_block_data(fp, blk) != 0) { failed = true; break; }

        char desc[256];
        if (!read_str_with_len(fp, desc, sizeof(desc))) { failed = true; break; }
    ...
    std::fclose(fp);
    if (failed) {
        aio_log(AIO_LOG_ERROR, "PIPELINE", "load_cache: read failed");
        return 3;
    }
    aio_log(AIO_LOG_INFO, "PIPELINE", "load_cache: ok <- %s (%d blocks)",
            path, frame->n_blocks);
    return 0;
}
