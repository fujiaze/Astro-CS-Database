...
权威顺序**只有**本节图中所列的一处。**禁止**任何下级文档声明自己的权威顺序，**禁止**自称「唯一权威」。
架构问题的唯一权威是 §7；科学公式与算法推导的唯一权威是 `docs/science/` 与 `docs/algorithms/`；与本设计冲突时一律以本设计为准。

### 0.2 详细文档层的地位

`docs/architecture/**`、`docs/interfaces/**`、`docs/standards/**`、`docs/modules/**`、`docs/contracts/**` 等**详细文档层是由本设计推出**的合法层，具体工作按它做。它们：

- **不另立权威链**；
- **不得**与本设计相反；
- **不得**放宽本设计的约束；
- 每一份文档、每一个机制**必须能追溯到本设计的一条要点**；追溯不到 ⇒ 要么补要点、要么该机制无授权；
- **登记表与映射表不得写状态字段**（状态由检查器现场计算，防「表内自证绿」）。
## 1. 项目定位与非目标
...
- 三个命令是**平级独立命令**，各自独立启动、**独立重跑**（**无断点续算**：重跑 = 新运行目录 + 新 manifest）、独立验收；**禁止**把三阶段隐式串接为一次运行的入口。
...
- **数据对象禁止互相冒充**：signal / variance / ivar / source_snr / depth_m5 / frame_snr / point_information / sparse_snr_layer / support / coverage / validity / rejection / provenance 各是各。
...
### 2.1 总纲：**全程只有 SNR，不存在「权重模式」这个概念**（负责人 2026-09-20 裁决）

- HiPS 里**存**的只有两样：**帧级 SNR** 与**稀疏的相对 SNR 比值**；
- **权重不是被存的东西**：只有阶段二在集成时，**按某个天球像素对应的那组输入帧现场算**出来（**派生量**）；
- **阶段一、阶段三不产生、也不消费任何权重**；
- **禁止**任何「权重模式 / 权重档位 / 默认权重模式 / mode0·mode1·mode2」的键名、枚举、配置项或产物；
- 变量名直接叫 `weight`（叫什么只是变量名，但**不得**再引入「模式」语义）。

### 2.2 数据对象唯一性

每个数据对象（signal / variance / ivar / frame_snr / support / coverage / validity / rejection / provenance …）**只有一份正本定义**，全仓只有一个编号；其它 schema 只能是从它派生的投影，**禁止**再写一份等价定义。字段名**禁止**用裸名（`weight` / `value` / `mask` / `snr`）承载多义，必须带对象全名。模块的输入端口**只接同一种对象**，接错**必须直接判红**，不允许自动转换。

### 2.3 质量代理不进科学权重

PSF 拟合质量代理（`q_psf`、残差尺度）与测光质量（`sigma_cal`）**只作诊断**，**禁止**计入阶段二科学叠加权重。

### 2.4 星表查询零网络

星表只解析**本地**星表文件（**离线、零网络**）；**禁止**引入网络超时/重试类错误码；科学身份不受网络影响。

### 2.5 每个科学量必须写清五件事

单位、坐标系、归一化、精度要求、有效有限域；默认科学计算用双精度；epsilon **必须**说明物理/数值来源，**禁止**裸常数。**全链只有一套球面索引（HEALPix NESTED）与一套帧标识哈希实现**，**禁止**任何模块自带第二套。
...
#### 三命令通用输入合同（**三个命令的配置是同一个形状**）

顶层一份「块」的列表；**每块三件东西——输出名称、运行参数、一组输入帧**。
...
```jsonc
{
  ...
  "blocks": [
    {
      "name": "red",                    // 可选块标签 → 日志与 run manifest 的 block.name
      "output_dir": "path/to/out/red",  // 块级必填非空、块间唯一、禁静默缺省
      "upm": { }, "reject": { },        // 运行参数：块内平铺，键名一律以命令行实际认的键为准
      "hips_paths": ["path/to/p1_hips_a", "path/to/p1_hips_b"]   // 第三件：一组输入帧
    }
  ]
}
```

- 顶层 `{"schema_version":"1","blocks":[…]}` 与「**平铺单块简写**」**二选一**（**互斥**；同时出现 → 报错，不静默取一）；
- 块内**必填** `output_dir`（块间不得重复、禁静默缺省）+ 可选块标签 `name`；
- 块内运行参数**平铺**在块内（**取消 `config` 子对象**）；**键名一律以命令行实际认的键为准**，模板与帮助由**同一份键表**生成——**禁止**第二份键名清单；
- 第三件 = **一组输入帧**：阶段一 `input_lights`（一组原始帧）、阶段二 `hips_paths`（一组 HiPS）、阶段三 `source`（一个输入产品）；
- **禁止**在配置里再写阶段判别键（如 `phase_name`）——**命令名就是阶段身份**；
- **禁止**逐帧重复写校准帧/滤镜：一块 = 一组亮场 + 一套母版 + 一个波段 + 块级运行参数；
- **退役形态必须显式拒绝**：出现旧的「一帧一个对象」写法 ⇒ **明确报错 + 迁移提示**。

**母版标度四条红线（fail-closed）**：① 提供暗场母版时**必须**显式声明「是否已减 bias」；② 亮场中位数 >1 ADU 而母版 ≤1 且未声明归一化 ⇒ **拒绝**；③ 平场中位数不在冻结区间且未声明按中位数归一 ⇒ **拒绝**；④ 声明归一化**必须**带正的换算因子且 bias/dark 声明域一致。

**drizzle 默认值**：`pixfrac` 默认 **1.0**（通量严格守恒端点）；`pixfrac<1` 时**必须**在 provenance 记守恒因子 `pixfrac²`。
**计算精度键**：阶段一 = `drizzle.precision_mode`（0=FP32 / 1=FP64，**必须显式**，缺失即拒绝）；阶段二/三 = 位深键（−32/−64）。
...
#### 预检 = ① 打印有没有报错 + ② 详细预估（负责人 2026-09-20 裁决）

- 三级结果不变：🟢 correct / 🟠 warn（**不阻塞、不是判据**）/ 🔴 error（**阻塞且必须报出原因**）；
- `-y` / `-yes` 跳过确认；`-force` 跳过全部检查、直接进入运行——**三者是负责人设计的正式接口，必须保留**；
- **不需要**对标外部软件的交互式提示窗（此前理解有误，**作废**）；
- **详细预估**含**资源与磁盘**预估（输出规模、临时空间需求、当前可用空间）；
- **资源门只管磁盘**：运行前磁盘余量不足 ⇒ 报 **warn**（不阻断）；运行中**写盘失败/磁盘满 ⇒ 报错**（fail-closed）；**内存 / CPU / 线程不设门**。
### 3.6 硬约束
...
- **测光标定语义**：目标 = 真实测光坐标系（星等）；手段 = 星点光通量积分 + Gaia + CCD QE + 滤镜透过率；**消除物理单位、只使用星等**，`k_photo`/`a_k`/`scale` 绝对值**无物理意义**；**禁止**任何物理闭合式（如 `k = g·h·c·1e9/(A·t)`）反推仪器参数或论证标定因子合理性；**禁止**为标定因子设绝对窗口。有意义的判据只有**一条**（尺度无关）：**测光一致性**（施加后星点星等与 Gaia 的残差散度/MAD 小）。
  ...
  **禁止**拍脑袋阈值；**禁止**把只在 `reports/` 里活着的阈值当作门禁依据。
  - ✅ **判据已推导（定案 6 落地，2026-09-20；完整推导 + 逐项出处 + 复算脚本见 `run/RELEASE-02/parallel/06.md`）**：
    判据 = **双边界、单帧自算、尺度无关**（门仍只有一个 = 单帧标定是否可信，与其它帧无关）：
    ```text
    A6-GATE(frame):  n = 本帧匹配星数（Gaia 匹配 ∧ 有效域 ∧ IRLS inlier）
      σ_obs     = 2.5·MAD(r_inliers)/0.6744897501960817                     [mag]
      σ_floor   = (1 − 3·1.166/√n)·σ_fit(白; 本帧匹配星通量分布)             [物理下限]
      σ_ceiling = (1 + 3·1.166/√n)·sqrt(Σ 预算项²; 本帧)                     [预算上限]
      PASS ⟺ σ_floor ≤ σ_obs ≤ σ_ceiling；两个比值与逐项分解**无论 PASS/FAIL 都必须报出**
    ```
    `1.166 = SD(MAD)/MAD` 正态渐近常数；`3×` = 3σ 抽样允差（本判据**唯一**的约定性选择，须显式标注）。
  - **逐项预算（M42 T2 Red 300 s，匹配样本中位通量 F=9644 ADU，n=800；单位 mag，全部实测/推导）**：
    | 项 | 值 | 单帧自算方式（出处） |
    |---|---|---|
    | σ_fit 光子+天光+读出+PSF 拟合自由度（白噪声） | **0.0140**（F=1.77e6 时 0.0008） | 生产 `dpsf_fit_batch_d` 在合成 Moffat4 场重复拟合；噪声前向模型含源泊松项 |
    | σ_fit 含天光面结构 | 0.0204 | 同上，σ_pix 取生产 8×8 patch 稳健值（22.686 ADU） |
    | σ_psfsys PSF 域通量方法系统误差 | **0.0307** | 本帧「生产拟合器 vs 独立孔径(r=10px)」比值稳健散度（噪声预期仅 0.0032 ⇒ 系统主导） |
    | σ_color 通带失配（QE 未建模） | 0.0054 | 本帧恒星 Gaia XP 谱 × 滤光片曲线 × 相机 QE 曲线（生产取 `Q(λ)≡1`） |
    | σ_skyres 天光扣除残差 | 0.0045 | 背景常数项 `σ_pix/√N_used` 经 N_eff 折算（结构项不重复计） |
    | σ_Gaia / σ_flat,hf / σ_q（1/12 量化） | 0.002 / 0.0007 / 0.0005 | Gaia DR3 形式误差（假设值）；平场自身噪声；`V_q=q_adu²/12` |
    ⇒ **σ_floor = 0.0122 mag；σ_ceiling = 0.0422 mag**（逐帧 0.0418–0.0586）。
    **实测对照（L4 49 帧，PSF 域口径）：σ_mag 中位 0.0611 / 最坏 0.2428 ⇒ PASS 1/49**；
    未消系统项 `√(0.0611²−0.0422²)=0.0442 mag`，最可能位于**参考侧**（`F_syn` 通带未建模项 +
    XP 谱误差 + M42 差分消光）——该侧当前**无任何不确定度传播**。
    ⇒ **本判据当前会判红 48/49 帧，这是如实结论；不得为「让测试变绿」放宽任何一项。**
  - **判不了（须登记，不得编数）**：① 平场**大尺度**残差（缺 repeat-flat / sky-flat 对照）；
    ② Gaia XP 合成通量 `F_syn` 的定标误差（本仓 GaiaDR3 `*.xpsd` 的 `magBP/magRP` 实测恒为 0，
    且 photometry 层不产出逐星 `F_syn` 误差）；③ 望远镜光学 / 大气 / 差分消光项（无仓内曲线）。
    未测项按**不加**处理（预算只含已测项 ⇒ 上限偏严 ⇒ fail-closed）。
  - **落地状态**：判据**尚未在代码中生效**（`photometry` 节点未产出 σ_psfsys(frame) 与逐星残差表）。
    在产出补齐与「能红能绿负例」完成前，**不得声称存在通过/不通过的测光一致性门**；`0.03 mag` **不得**复活。
...
- **检测与重建逻辑**是 Phase2 的标准行为，不是可选开关；SNR 重建三口径由配置显式指定（**不是**权重模式——本设计不存在「权重模式」，见 §2），**不得静默降级**（见 §3.4/§4.3）。
...
```mermaid
flowchart TD
    IN["输入 HiPS + 配置 SNR 重建口径"] --> DET{"SNR 重建口径（JSON 显式指定，默认 sparse_reconstruct）"}
...
- Phase2 **默认消费**稀疏 SNR 层（`snr_path` 默认 `sparse_reconstruct`）：实际 SNR = **帧级 × 帧内**（`SNR(p) = SNR_frame · rho(p)`，`rho` 为 SNR 相对因子；进叠加时 `w(p) = w_frame · rho(p)²`）。三种 **SNR 重建口径** `dense` / `sparse_reconstruct`（默认）/ `frame_reconstruct` 由 JSON 显式指定（**这不是权重模式/权重档位**——本设计**不存在「权重模式」**：HiPS 只存**帧级 SNR** 与**稀疏的相对 SNR 比值**，权重一律由阶段二按该天球像素对应的帧集合**现场算出**，见 §2）；**不得静默降级**，须记录 `snr_path_effective`。负责人 2026-09-19 裁决（`GAP_AUDIT §9.46`）。
...
- **稠密权重是数学表示，工程上按需计算**：
  ...
  - ⚠ **判据 SP-0 的已知退化与适用域（EXP-205 定案，如实登记；判据冻结 sha256 `81244053…`）**：帧级臂的 `RMSE_frame` **在定义上恒等于** `s_field` ⇒ 「帧级精度门」**恒真**、无鉴别力，**不得**据该门声称「帧级最优」；SP-0 的精度比较只对**稠密/稀疏**两臂构成有效判据。实测适用域：① 地面视宁度受限 + σ 场平滑可分辨 ⇒ **稀疏最优**（testdata M42 主切片 sparse 0.093/0.228/0.099 vs 帧级 0.111/0.384/0.115）；② 高分辨率高对比结构（HST M16）⇒ **帧级最好**（0.112 vs 稀疏 0.261、稠密 0.198）；③ **稠密口径在任何面上都不最准**且 67 MB/帧超存储门 64×；④ 稀疏失效边界 **Δ*/ℓ ≈ 1**（面 A 0.98 / C 0.50 / B 0.35），机制 = cell 稳健 MAD 被 cell 内未分辨结构抬偏（**不是**插值误差）。
...
**天光亮度平面**（UPM 的加性背景 `b_k(x)`）采用**稀疏表示**：星点掩膜外每帧取稀疏背景采样点（采样点带 **`control_ivar`** 权重：`w_ki = control_ivar_ki = N_retained/(k_corr·(π/2)·σ_bg²)`——EXP-201 定案；`w = 1/σ² = SNR²/F_ref² ∝ SNR²` 的 `∝` 以固定参考通量为前提，天光控制点被估量本身在变 ⇒ `SNR²` **不是**有效逆方差代理），全部帧联合构建参考天光面，各帧以稀疏样条/插值把平缓梯度校准到参考面；面的栅格值按需现场求值，不构建稠密背景栅格。细节见 `docs/plugins/algorithms_phase2/10_sampling.md` 与 `11_upm.md`。
...
2. **按该输出像素的 `n` 自适应选择排异算法**。**最终档位表（负责人 2026-09-19 裁决，冻结）**：
   ...
   > ⚠ **本节下方这张四档表已作废**（2026-09-20）：负责人 §9.71 裁决 3 改以 **WBPP 实测表**为准（档界 6 / 16），
   > 全文见本文件下方「**逐像素排异：按该像素的输入集数量 N 自动选择**（负责人 2026-09-20 裁决）」一节。
   > 其中 `n ≤ 3` 档「不排异」的取舍**由实验定案**（RELEASE-03 / EXP-204），实验结论落地前**不得**按本表实现。
   > 保留原文仅为留痕（`ENGINEERING_SPEC.md §3` 变更 claim 的可追溯要求）。
...
#### 逐像素排异：**按该像素的输入集数量 N 自动选择**（负责人 2026-09-20 裁决，一手证据定案）

**路由**：排异算法**逐像素**、按该像素**输入集数量 N** 自动选择（**不是**全局单算法）；N = 该像素的**几何可贡献帧数**（一次解析；**禁止**用整组帧数或掩膜后存活数代替）。

**映射表以负责人上传的 WBPP 脚本为准**（一手实测 `BPP-FrameGroup.js:1304-1312`）：

| N（该像素输入集大小） | 算法 |
|---|---|
| **N < 6** | percentile clipping |
| **6 ≤ N ≤ 15**（或 BIAS/DARK 类帧） | winsorized sigma clipping |
| **N > 15** | linear fit clipping |

**禁止使用 min/max**：WBPP 脚本明文拒绝（「Min/Max rejection should not be used for production work」，`BPP-FrameGroup.js:1239-1240`），且其算法清单内**不含** min/max（`BPP-engine.js:2695-2719`）⇒ **维持禁用**；`NoRejection` 与已废弃的 CCD clip 同样**不得**使用。

**显式指定的合法性窗口**（一手实测 `BPP-FrameGroup.js:1229-1293`）：percentile 仅 ≤8 帧；winsorized / linear fit / ESD 需 ≥8 帧（linear fit 建议 ≥20、ESD 建议 ≥20–25）；RCR 需 ≥15 帧；averaged sigma 仅 8–10 帧；sigma clip 8–15 帧。**不合适只告警、不硬阻断**；**禁止**静默改算法、**禁止**静默降级。

**provenance**：实际使用的方法、参数与 N **必须**写入排异 provenance，可追溯。

> ⚠ **原冻结四档表（`n≤3` / 4–7 / 8–15 / ≥16）作废**——与 WBPP 实测表（档界 6 / 16）不一致；原「显式指定单一算法 + 用户表达式 / 分段映射」表述**删除**。
## 5. export（Phase3）：投影导出

### 5.1 使命

把任意合同兼容 HiPS 按用户指定 WCS 导出为测量意义明确的平面 FITS。**输出不需要带权重——上游已完成叠加，这里只投影到平面并直接计算生成对应 WCS。** **输出帧必须重算点源信息量**（用输出的有效 PSF 与完整协方差）；**禁止**把输入帧的量直接重采样搬过去。
...
- **内置多种投影算法**，首批冻结 **TAN / SIN / CAR / AIT / STG / MOL / CEA / ZEA**（每种声明适用域、奇点、经度 wrap、轴手性、CRPIX/CRVAL/CD/PC/CDELT/CTYPE）。**已实现并可作为产品声明的以实际注册表为准**；**未实现的必须显式报「不支持」，禁止声称支持**（当前登记：**仅 TAN 已实现**）。
- 新增投影经 projection registry 注册并附独立往返 Oracle；
- 用户通过 `projection` 字段选择，缺省 TAN；
- **输入语义守卫**：导出阶段**只接受面亮度语义**的输入；方差/逆方差按显式消费并**传播**；其余语义（通量/权重等）**显式拒绝**，不做静默丢弃。
- 输出模式显式：`surface_brightness` / `point_source_flux` / `visualization`；缺所选模式所需信息 → 拒绝或明确 unavailable。`visualization` **必须声明「不可测量」**；**禁止**把方差/逆方差/点源信息当测量层写出去；要做通量换算而无逐像素立体角 ⇒ **拒绝**。
...
三个命令是**平级独立命令**（**三者共享同一套运行前预检合同，见 §3.5**），由**唯一可执行入口**提供，薄入口：职责限定为命令解析、配置预检、运行控制、机器输出、取消与退出码；科学公式实现在 `lib/algorithms/` 各模块，FITS/HiPS 读写由 `infrastructure/aio` 承担，线程池统一由 `scheduler`（+ `pipeline`）管理。
...
- 退出码（唯一源 `lib/infrastructure/cli/exit_codes.h`）：
...
| 8 | 输出完整性验证失败 |
| 9 | 用户取消或超时 |
| 10 | 磁盘写满 / 写盘失败（**一般性资源超限门已取消**：内存/CPU/线程不设门） |
...
└── infrastructure/             # 不定义科学公式的工程基建
    ...
    ├── gaia_xpsd_client/       本地星表解析（**离线、零网络**）；缓存与坐标语义
...
- **模块名只有一份**：顶层结构 = §7.1 所列——`scheduler`（注册、资源预算、执行、取消、checkpoint）与 `pipeline`（typed DAG、artifact、内存/数据管线）；**禁止**再用第二个名字（如 `runtime`）指同一职责；
- **依赖方向是硬约束**：命令行 → 运行时 → 注册表 → 模块 → 计算后端；**禁止**反向依赖；**禁止**用目录通配隐式塞源文件；
- **单一执行平面**：一个进程只有一个全局执行顺序与资源预算源；科学模块**禁止**读全局配置、**禁止**建无预算线程池、**禁止**直接结束进程、**禁止**写未声明文件、**禁止**绕过产物仓库、**禁止**直接选指令集路径；
- **加载前必须过固定检查，且只认清单授权的绝对路径**：CPU 特征 / OS 可安全执行状态 / 进程可用 CPU / manifest 要求 / 文件哈希 / ABI 版本**全过才执行**；**禁止**从当前目录或环境变量路径发现动态库；失败**绝不回退**，只报错；
- **能力探测用「OS 可安全执行平面」**：**不能**用 CPUID 硬件位单独判定；探测自身**永不触发非法指令**；
- **旧可执行程序不是入口**：入口只能是唯一命令行的三个子命令；
- **生产调度面必须覆盖强制节点**：阶段一的注册面**必须**含两轮 WCS 与「测光归一化落到像素」这一步；**禁止**用更粗的节点粒度代替设计流程；
- **C ABI 附加纪律**：跨边界**禁止**抛异常 / 传 C++ 容器；结构体带大小与版本；错误字符串**只当日志**、**禁止**用来判断状态；模块**禁止**调用进程退出；**第三方库隔离在动态库内部**；
- **FITS 支持域**：只支持基本图像 HDU 与固定位深；扩展头 / 随机群 / 表 / 压缩**一律显式拒绝**；读 FITS **不隐式应用缩放关键字**；**HiPS 读取不得重造 FITS 解析**；**句柄不共享**；
- **注释与版本字样**：生产代码里**禁止**出现内部版本号、控制包名、审计轮次、计划编号、「本次修复」「以后再做」等字样。
### 7.2 架构图

```mermaid
flowchart TD
    EXE["ACSD Cli 唯一入口（normalize/mosaic/export 子命令）"]
    EXE --> RT["infrastructure/scheduler + pipeline<br/>typed DAG · 调度 · 统一线程预算 · 资源监控"]
...
- ISA 支持：amd64 baseline、AVX2/FMA、AVX-512 子集；**「支持」= 能力上限，不是发货清单**——发货档由实测台账决定（当前仅 AVX2+FMA 的 2 个热点；**AVX-512 实测不发货**）；baseline 禁止泄漏 `/arch:AVX*`；高级 ISA 只作用于对应 provider。
...
- **重计算负载资源门（G-RES-01）**：**已收窄为「磁盘门」**——内存/CPU/线程**不设门**（实测消耗几百 MB ~ 数百 GB，内存不是约束）；判定域、已分配容量分母、record/enforce 划分、exit 10（磁盘写满/写盘失败）条件见 `docs/plugins/infrastructure/21_observability.md` §8；**阈值唯一源 = `contracts/resource_gate_v1.json`**（C++/Python/外挂 judge 共读，实现侧只引用契约常量）。
...
- **线程预算唯一来源**：可用 CPU = **亲和性 ∩ cgroup ∩ Job Object** 的交集（**不是**机器总核数）；任何时刻活动 worker 之和**不得超过**预算；预算快照写进 manifest；
- **worker 数不得由「硬件并发数」默认值决定**：**只能**来自 benchmark 生成的机器 profile 与全局预算对象；**禁止**任何绕过 profile 的默认值；
- **线程角色固定**：串行 I/O 与控制面恒定单线程；异步 I/O 恒定一个专用线程；后台服务各一个线程且用**独立小预算**；
- **线程创建必须登记**：全仓**禁止**硬编码线程数；模块内并行线程数由宿主按预算注入（**禁止**自行设定）；未登记即判红；
- **异步只有两类**：I/O 管线与后台服务；**科学计算里禁止** async/future；**禁止**嵌套并行；
- **确定性合同**：浮点归约顺序**必须冻结**；**并行开关不得改变科学数值**（ON/OFF 数值恒等）；输出**不得**依赖线程调度；
- **加速按 kernel 逐个选择**：**不存在**「整个程序切到某个 ISA 模式」；只为实测热点做变体，非热点回落 baseline；**禁止**复制 / 重写科学实现；
- **变体容差**：变体与 baseline **同式同序**，只允许浮点舍入差（相对容差 **2e-4**，冻结于实现之前）；每个变体**必须**过同一套 Oracle；1 worker 与 N worker 逐位一致；
- **编译隔离**：主程序与 baseline 不得含任何高级指令，变体只在自身编译单元开旗标，CI **必须双向**反汇编证明；
- **计算中后端失败必须安全中止整个阶段**：**禁止**悄悄换一个后端把两种结果混进同一产品；
- **缓存四要素**：任何缓存**必须**有容量、身份、失效、线程模型；过期缓存**必须显式拒绝**；
- **热路径禁令**：逐像素**禁止**分配 / 日志 / 文件系统 / 时钟调用；每条快路径**必须**配参考实现 + 等价性 Oracle + 失败回落；
- **有界异步 I/O**：队列**必须**有界并带背压；取消要能唤醒阻塞者并传播错误；共享 I/O 句柄**不得**并发读；
- **生产构建不得链入 ACR/CUDA**：**禁止**生产目标编译或链接 ACR 的任何源文件；**禁止**任何 GPU 路由开关与第二个可执行入口。
## 9. I/O 与原子产品

- `aio` 是唯一 FITS/HiPS/manifest 读写边界；Phase1/2/3 复用同一套 AIO。
- 所有产品（**含 HiPS tile**）：临时文件/目录 + 校验 + fsync + 原子 rename 提交；失败/取消时清理临时产物，正式产品目录只出现完整产品。
  ⚠ **本期例外（如实登记，未修完前不得声称「原子发布已全覆盖」）**：HiPS tile 当前仍是「先删后建、失败不回滚、不清理已写文件」的非原子路径；阶段二马赛克当前直写输出目录、无暂存区。
...
- manifest 至少记录：产品类型/schema 版本、软件来源、run ID、输入产品标识、科学配置、像素/采样语义、算法 ID、模块 build ID、实际 provider、生成时间。
  **单位 / 坐标 / 平面 / 无效值策略不写进清单**（清单 schema 无该字段且禁止额外字段）——它们由**产品内容证据**块显式声明，缺失即不许消费。
...
**aio 是文件级唯一 I/O 边界**：任何文件读写**必须**经 aio；**不得**有第二处 I/O 实现。
**全部 I/O 点（穷举，此外无）**：阶段一读**原始 FITS** + 读**校准帧**（fits 系列 `.fit` / `.fts` / `.fits` 等）→ 写 **HiPS**；阶段二读 **HiPS** → 写**天球 HiPS**；阶段三读**天球 HiPS** → 写 **FITS**。
**机器判据**：全仓文件打开 / 流式读写 / 文件系统写操作，除 aio 内部外**应为 0**。⇒ 现存违规（HiPS 发布实现、FITS 输出里的直接文件打开、块↔文件导出/缓存接口）**须登记并整改**。

- **块↔文件的导出/缓存接口不是生产接口**：aio 合同里「把内存块写成 FITS/XML」「把整帧存成缓存文件再读回」这类接口**必须**删除或明确降级为非生产/诊断并登记；**禁止**任何阶段内节点用它们搬运数据；
- **跨阶段交换的是一份自包含说明**：这是什么角色、登记成哪种类型、从哪来、完整清单、以及**科学内容证据**（单位 / 坐标 / 平面 / 无效值策略）。**缺说明、缺其中任一块 ⇒ 拒收**；
- **不得猜语义**：产品的含义只能从说明与清单读；**禁止**从文件名 / 目录名 / 存储地址尾段猜；**不要求上游运行编号匹配**；
- **成功对象的定义**：产物先写进本次运行私有的临时区 → 校验 → 算哈希 → 原子改名发布 → **最后落完成清单**；**没有完成清单就不算成功对象**；同一标识**只能有一个生产者**；**绕过存储层直接写出的文件永远不被承认**；
- **运行目录隔离与路径词法**：每次运行有自己的目录树；产物路径逐段合法（禁绝对路径、父目录穿越、空段、反斜杠、非法字符）；已存在的路径段是符号链接 ⇒ **拒绝**；
- **产物落点**：产品**只落块级 `output_dir`**；`run/` 只放**临时产物与日志**；**禁止**以进程当前目录作隐式缺省；**禁止**把 `run/` 说成「唯一运行输出目录」；
- **取消语义**：取消即无产物（协作取消 → 关写入器 → 写 incomplete manifest → 删/隔离临时产物 → exit 9）；取消后预算立即回收；**没有断点续算**——重跑 = 新运行目录 + 新 manifest；
- **观测真实性**：重计算 / 长运行**必须**有资源监控伴随器，否则该次运行判失败；监控原始数据**必须**不可事后编造；provider / 模块 / 活动 worker 等**只能**来自真实观测；缺失值写空串，**绝不填 0 冒充**；
- **产品无效值语义（唯一口径）**：无覆盖 / 无数据 = **NaN**，且与「支撑度 ≤ 0」一致；**禁止**用 0 或 ±Inf 冒充无效；NaN 在重采样中的处置规则**必须唯一**并写进产品内容证据；
- **缺 tile 就是缺**：请求某层 tile 而它不存在时**必须**如实报「缺失」，**绝不**返回父层内容冒充；不提供隐式层级回退；
- **兼容策略**：科学正确性优先于透明兼容；含糊 / 损坏的旧文件**必须显式拒绝**；读取旧格式**必须**强制校验；
- **磁盘上还有别的固定产物**：天光面模型与稠密缓存、诊断文件、控制点接受记录等也落盘，**必须**逐个登记名字 / 生产者 / 原子性归属；**禁止**未登记产物出现。
### 10.1 平台与形态
...
- **官方 Windows 工具链只有一个**：**MSVC**（与工程规范一致）；详细层里「正式工具链 = MinGW64」的说法**须订正**。交付形态（`.dll` 由谁编译）**必须**与之一致。
### 10.3 开发顺序（主路线）
...
#### 11.1.1 科学模块的**实验佐证**要求（负责人 2026-09-20 令，**强制**）

> 负责人逐字：「重新推导 **SNR 链条、科学门禁、测光校准**这几个重要科学模块，**必须有严格的实验佐证**。
> 可以用纯合成数据，基于哈勃真实信号、科学生产噪声/梯度的合成数据，`testdata` 里的真实数据等等验证。
> 相关实验材料和结果落在**实验**文件夹里……里面**按照实验单元划分**，要有**实验代码、实验报告**等等。
> ……我需要确保**工作流和门禁的严格科学性**。**必须有详细的实验和论文，开源库代码佐证**。」

**适用范围（强制）**：SNR 链条、科学门禁、测光校准，以及**任何**改动会影响科学结论的模块。

**1. 一个科学结论 = 一个实验单元。** 每个单元必须自包含：

```text
实验/Exx-<主题>/
  README.md          ← 实验报告（强制要素见下）
  code/              ← 实验代码（可复跑、固定 seed、依赖与版本声明）
  results/           ← 结果（图/表/JSON；生成命令写在 README）
  data/              ← 数据（或指向共享数据的说明）
```

**2. 实验报告的强制要素（缺一即不合格）**：
① **假说**（可证伪）；② **方法**（公式/推导/判据）；③ **数据**（来源与生成方式）；
④ **结果**（**真实数字** + 不确定度 + 对照）；⑤ **结论**（**可判定**：成立/不成立/证据不足，不许含糊）；
⑥ **诚实边界**（适用范围、已知反例、未验证部分）；⑦ **复现命令**；⑧ **佐证来源**。

**3. 佐证来源必须是独立的一手证据**（三者齐备，缺一不可）：
- **论文/标准**：给 **DOI 或 arXiv 号**，并**核验过可解析**（Crossref/arXiv API/DataCite/直接 HTTP 之一）；
  **禁止**凭记忆写引用；**禁止**引用未经核验的条目；
- **开源库代码**：给**项目 + 版本 + 文件:行**（如 SExtractor / photutils / SEP / DrizzlePac / Astropy / HEALPix）；
  开源实现**同样可能有错**，须回到其引用的文献与标准；
- **仓内实测**：给**复现命令 + 实测输出**。

**4. 判据必须非退化**：判据在「真值无效应」时必须**归零或报警**；
否则该实验**无鉴别力**，**不得作为证据**。反例（已实测）：
- 「接缝残留百分比」单独作判据 —— `raw − C` 把背景减掉后各帧都 ≈0，两帧相减自然 ≈0，
  **接缝小是因为背景没了**，不是因为对齐做好了；
- 「纯加性天光」作合成数据 —— 真值无效应时度量恰为 **0.0000%**，什么都测不出。

**5. 门禁阈值只接受误差预算推导**（见 §3.6 与 §11.1）；
**禁止**拍脑袋常数；**禁止**阈值只活在 `reports/` 而未进入权威文档链。

**6. 结论冲突的处理**：实验结论与 `docs/science/**` / `docs/algorithms/**` 冲突时，按 `AGENTS.md §8` 查证；
**以独立证据为准**，既不盲从文档也不盲从现有代码；查证记录（文献号/章节、开源项目与版本、复算过程）
入变更 claim 或任务回执。

**7. 独立审稿**：每个科学单元的结论须经**独立子代理审稿 ≥5 轮**，轮次记录留档；
审稿者不得是该结论的作者。

**8. 未满足本条的模块**：**不得**声称科学正确；其结论**不得**用作其它模块的输入或门禁依据，
须在最高设计与插件文档中**如实登记为未验证**。


#### 验证体系的强制条款（要点）

- **数值比较容差事前冻结**：容差在写测试之前冻结，**禁止**跑完再调；比较两个结果时，**NaN/Inf/缺失出现在哪里也必须完全一致**（**禁止**只比「有限的那几个像素」）；元数据 / 掩码 / 计数 / 索引 / 选择结果精确一致；容差调整**必须**是独立提交并附失败分布与推导；
- **受影响验证范围由机器从改动集算出**，**只许扩大不许缩小**；Agent **禁止**手工缩小；
- **三层错误语义**：成功 / 可恢复的科学状态（走状态字段）/ 硬错误（显式错误码）；**禁止**「返回成功但状态无效」的双语义；**禁止**用告警代替错误状态；**禁止**默认吞错、**禁止**静默截断或置零；
- **测试纪律**：每个科学条目至少一个独立测试且追踪表不断链；测试**必须**走公共生产接口；固定随机种子；浮点断言带容差并说明来源；不可用就**跳过而不是伪造通过**；改动过的模块**必须**重跑相关门；
- **状态必须现场计算**：登记表 / 映射表**禁止**写状态字段；「设计冻结 VERIFIED」只表示测试**设计**已冻结；登记声明的路径**必须真实存在**，**禁止**用「本目录为空」掩盖已存在的实现；**门禁字面量如实**——工程基础门只有全过才写 PASS，真实数据验证未做就写 PENDING。
### 11.2 验证层级
...
| L2 合成性能 | 规模化合成负载 | 磁盘门零违约 + 资源观测如实（内存/CPU **不设门**） |
...
- **文档 / 合同的内部修订号不进入程序、代码与产物**；但**禁止**用版本号作为生效或退役条件（改用日期或变更编号）。
- **发行包内容有白名单**（README / 报告 / 证据 / 自审 / 首方源码包 + 清单 / 文档快照）；**禁止**打包构建产物、第三方、数据目录；源码包只含首方代码。
## 附录 A. 术语
