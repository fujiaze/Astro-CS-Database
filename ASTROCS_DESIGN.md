...
- **不存在第二条权威链**：任何文档不得声明自己的权威顺序或"唯一口径"；同一主题只有一份正本文档，新版本替换旧版本，旧版本不留在库内。
...
AstroCS 是**天文 CCD/CMOS 图像校准与标准化数据库系统**：把单帧观测转换为可独立消费、带不确定度与来源链的球面科学产品（HiPS），再按明确科学目标合成马赛克、导出测量意义明确的 WCS FITS。
...
- 对外只有一个 CLI 入口（`astrocs`），它按命令拉起对应阶段的调度器（§8.1）。
...
AstroCS 的科学核心是一条链：**先把每帧校准到统一的测光星等坐标系，再在这个坐标系上测量不受天光影响的绝对信噪比，最后用加性方式去除天光、建立帧间连续的绝对信号平面，使叠加结果天然无接缝。**

```mermaid
flowchart LR
    A["① 测光校准到星等坐标系<br/>Gaia XP 光谱 × QE × 透过率<br/>整帧消除物理单位"] --> B["② 跨帧绝对信噪比<br/>帧级 SNR + 稀疏绝对 SNR<br/>Phase2 重建为稠密"]
    B --> C["③ 加性天光与无接缝叠加<br/>联合建立连续绝对天光平面<br/>各帧多退少补，叠加即连续"]
```

三个创新点互为前提：①让帧间信号处于同一尺度，②的 SNR 才跨帧可比；①让真实信号连续后，③的阶跃才只剩可加性去除的天光；②为③的天光平面拟合与叠加提供逐像素权重。三者各自是一个独立实验单元（见 §12.3），alpha 发布前必须全部经实验证实并跑通。

全天 HEALPix 产品还要求一条**产品级几何分配路径**（§2.4）：它不属于上面三个科学创新点的链条，但决定全天产品的面积口径、权重分母与逐 leaf 通量闭合门，因此单列为创新点四。

### 2.1 创新点一：测光校准到测光星等坐标系
...
- 传统信噪比把天光信号计入信号，被天光抬高而失真。AstroCS 的帧级 SNR 只计**真实源信号能量与噪声**：信号经独立局部背景估计与扣除，天光只通过其散粒噪声进入噪声项；天光越亮 SNR 越低，趋于零。
- 方法学对标 PixInsight 公开文档中的 PSFSNR（功率比信噪比）；PixInsight 的 PSF Signal Weight 是叠加权重、不是信噪比，AstroCS 入库的是信噪比本身，权重在 Phase2 消费时才计算。
...
- Phase2 消费时用重建算子把稀疏**绝对 SNR 控制点重建为稠密 SNR**（每个像素都有值），供信号层拟合与叠加加权使用；工程上按像素现场求值，不整体驻留内存。
...
### 2.4 创新点四：面向全天 HEALPix 产品的精确面积交叠分配
...
- **创新边界（只有以下五条，其余均不主张）**：
  ...
  4. **首次落地到天文 HEALPix drizzle 场景**：把该分配方式用于源像素 drop × NESTED leaf 的权重累加与 weight/variance 链（同类几何此前只见于地球科学栅格重采样）。
  5. **任意 WCS footprint 与权重/variance 链的整合**：drop 由任意 WCS（含 SIP 畸变）的源像素足迹给出，交叠面积直接进入 w_jp = a_jp / A_pixel 与 variance 传播。
- **奇点条款**：chart 映射在 10 个 face 角点与面内 |z| = 2/3 折线处存在分支结构，drop 靠近这些位置时解析延拓可能跳变。这些**奇点被精确定位、精确检测、有处理方式，全天正确性由此保证**；本方案不主张"不存在奇点"，也不主张"任意输入都无需回退"。
...
- **帧级 SNR 与稀疏 SNR 层是两个独立对象**：帧级 SNR 是"整帧一个值"的参考电平，稀疏层（`sparse_snr_layer`）存控制点处的**绝对** SNR；稀疏层不以帧级 SNR 为尺度基准，消费时也**不得**乘/除帧级 SNR 做还原——绝对控制点直接重建为稠密 SNR 场。两者共用同一物理定义与同一逐帧参考通量 `F_ref`，权重换算 `w = SNR²/F_ref²` 对两者一致。若未来要求两者一致，方向**只能**是由绝对控制点导出帧级摘要（`frame_snr := median_p(SNR_c)`）；**禁止**反向把稀疏场归一化到帧级标量。
- **权重只能来自纯净信号与噪声之比**：即由 SNR 换算的逆方差。要求**跨帧可用**——**不基于参考帧**、不依赖帧内相对基准，而是**绝对标定**；任何使偏差随帧而变的量（含 PSF 拟合质量代理）都不得进入科学叠加权重。
...
- **星表引导检测**：用**本帧已解出的权威 WCS**把 Gaia 星表逆投影到像素域，只对星表位置做质心/PSF 拟合；拟合成功即星点，拟合失败丢弃（不计虚警、不报错）；按亮度取 top 2–5 万颗为上限，极限星等按焦距、画幅、曝光时间派生估计（宁多勿少）。**取向先验取自解算产物**（含取向与镜像的完整线性 WCS），不要求调用方预设像面取向；配置给出的显式近似 WCS 是可选覆盖。先验不可得时按声明模式显式降级或 fail-closed，**不得**以「北向上/东向左」假设冒充权威取向。
...
测光输出语义：产物通量处于**测光星等坐标系**——即带**逐帧相对测光零点**的**线性**面亮度（`I_photo = k_photo·I_cal`；量纲/单位见 `docs/contracts/DATA_SEMANTICS.md` §31.1a），星等按 `m = ZP_k − 2.5·log10 F` 现场派生而不作为数据形态落盘；标定系数的绝对值无物理意义，禁止由它反解增益、口径、曝光时间等仪器参数，也不设绝对数值窗口；测光正确性的判据是单帧测光一致性（星点星等对 Gaia 的残差），门限由光子噪声、PSF 拟合不确定度、平场/天光残余、星等定标误差的误差预算逐项推导，推导过程与逐项数值见 `docs/science/PHOTOMETRY.md`。
...
- 全部命令由唯一可执行程序提供：Windows 为 `astrocs.exe`，Linux 为 `astrocs`；其余能力以 DLL/SO 形式存在或在可执行程序内部实现，不设子入口。
...
- 任何模块、阶段或子系统运行出问题，都必须以稳定错误码上行并在 CLI 收敛为退出码；**禁止**吞错
  （空 catch、忽略返回码）、**禁止**只写日志不返回错误、**禁止**把故障降级为"警告后继续"。
- 模块不自行决定退出码、不自行终止进程。退出码唯一源 = `lib/infrastructure/cli/exit_codes.h`
  （§7.2 表）；域→码映射唯一 = `docs/contracts/LOG_AND_ERROR_CONTRACT.md` §5，禁止第二套数值表。
...
- 每次运行（含失败与取消）在块级 `output_dir` 下产出完整运行日志，日志目录默认
  `<output_dir>/logs`；**禁止**落进程 CWD、源码树目录、`run/`、安装目录或用户家目录。
...
- 一个进程只有一个资源预算源（§9），调度器不允许模块私建线程池；科学计算内核不嵌套并行。
...
| 平台 | 入口 | 交付形态 |
|---|---|---|
| Windows 10+ amd64 | `astrocs.exe` | 解压目录：唯一 exe + 各 .dll + schemas + manifest |
| Linux amd64 | `astrocs` | 解压目录：唯一 ELF + 各 .so + schemas + manifest |
...
三个科学创新点各立一个自包含实验单元，是本项目最核心的算法验证：
...
创新点四（§2.4）另立一个**产品级实验单元**，验证全天几何分配路径：
...
三个科学创新点实验单元是 alpha 发布门；创新点四的产品级实验单元随 Phase1 drizzle 模块一并交付与复核。
...
| L4 真实视觉验收 | M42 + Galaxy Center 全量（本版仅 R 通道） | 整马赛克→平面 FITS→拉伸 PNG→切块目检：无黑洞、无亮斑、无接缝、星点正常、全局观感正常，负责人确认 |
