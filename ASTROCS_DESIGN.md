...
- 拟合结果应用到整帧像素，把图像校准到**测光星等坐标系**，产物通量以星等/相对星等表达，**消除物理单位**：吸收、增益、口径、曝光时间等未知量不需要、也不可被分离反推。
...
flowchart TD
    ...
    BN --> D["star_detection 星表引导检测<br/>Gaia 逆映射定位星点"]
    D --> P["psf PSF 建模"]
    P --> A["platesolve WCS 解算<br/>星表匹配 + 稳健迭代精化"]
    A --> PH["photometry 星点测光与 Gaia 拟合<br/>并在同一步内把归一化施加到像素"]
...
测光输出语义：产物通量以星等/相对星等表达；标定系数的绝对值无物理意义，禁止由它反解增益、口径、曝光时间等仪器参数，也不设绝对数值窗口；测光正确性的判据是单帧测光一致性（星点星等对 Gaia 的残差），门限由光子噪声、PSF 拟合不确定度、平场/天光残余、星等定标误差的误差预算逐项推导，推导过程与逐项数值见 `docs/science/PHOTOMETRY.md`。
