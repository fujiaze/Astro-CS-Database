...
```jsonc
{
  "config": {                            // 处理配置（必要参数）
    ...
    "sparse_snr_spacing_px": 64,        // 稀疏层控制点间隔 Δ（px）；复用 Phase2 UPM 8×8/tile 控制网格，见 §3.4
...
1. **HiPS 文件**（含帧级 SNR）：
   ...
   - `F_ref` 是**参考通量**，必须**显式落盘**其口径与作用域（`reference_flux_scope`）；单位 ADU；
...
- **判据只有两条（均尺度无关）**：① **测光一致性**（施加后星点星等 vs Gaia 残差）；② **帧间一致性**（同组帧间 `k` 峰峰）；
...
- **测光标定语义**：目标 = 真实测光坐标系（星等）；手段 = 星点光通量积分 + Gaia + CCD QE + 滤镜透过率；**消除物理单位、只使用星等**，`k_photo`/`a_k`/`scale` 绝对值**无物理意义**；**禁止**任何物理闭合式（如 `k = g·h·c·1e9/(A·t)`）反推仪器参数或论证标定因子合理性；**禁止**为标定因子设绝对窗口。有意义的判据只有两个（均**尺度无关**）：测光一致性（施加后星点星等与 Gaia 的残差散度/MAD 小）与帧间同一测光体系（**语义目标，不是门禁**，见 §3.5）。
...
**公共面语义：只扣「多退少补」的偏差，不剪掉整个背景（负责人 2026-09-19 裁决）**：

Phase1 归一化是把各帧**「多退少补」拉到公共亮度面**上，Phase2 的 `raw − C_k` 中：
- **`C_k` 只承载「本帧相对公共面的偏差 `δ_k`」**（即该帧比公共面亮/暗多少），**不是**该帧天光的全部；
- **公共背景面必须保留** —— 叠加后的马赛克**仍带背景**（天光是真实测量的一部分，且其散粒噪声已计入 `σ_F`）；
- **禁止**把整个背景剪掉（`raw − C_full`）—— 历史实现正是**剪错了**，导致 **46% 像素变成负值**；
- 判据：叠加输出**不得出现大面积负值**；公共面的绝对零点由 §4.3 的 gauge 约定承载，**不得**用「把背景减到零」来定义。
...
- **合成测试必须模拟真实物理噪声过程（强制方法论）**：源/天光/暗流各自 **Poisson（电子域）**、读出 **Gaussian（电子域）**、电子→ADU 的**增益/饱和/量化**、平场/空间响应 `m(x,y)`、天空梯度/加性天光面，按需宇宙线/坏点/PSF 变化。**严禁纯加性天光**：天光对 SNR 的影响必须通过**散粒噪声**体现（`B↑ ⇒ σ² ⊃ B/g ↑ ⇒ σ_F ↑ ⇒ SNR↓`），**不得**用「算术加常数」代替「加天光」——实测纯加性天光下真值无效应时度量恰为 **0.0000%**（什么都测不出），而光子域同样天光给出 `−7.81%/−20.33%/−37.19%`。优先用真实数据作底（本仓 M16 F657N/F673N 带 `PHOTFLAM`，可作合法绝对标定）。凡已有合成测试不符合本条者须**重做或标注失效**；
