...
权威顺序只有本节图中所列这一处。架构问题的唯一权威是 §8；科学公式与算法推导的唯一权威是 `docs/science/` 与 `docs/algorithms/`。下级文档陈述与本设计一致的细化内容，不另立权威链。
...
- `docs/DOCUMENT_INDEX.yaml` 是全文档集的唯一索引地图。
...
```mermaid
flowchart LR
    A["① 测光校准到星等坐标系<br/>Gaia XP 光谱 × QE × 透过率<br/>整帧消除物理单位"] --> B["② 跨帧绝对信噪比<br/>帧级 SNR + 稀疏相对 SNR<br/>Phase2 插值重建为稠密"]
...
- 每帧产出两层 SNR：
  1. **一个帧级 SNR 标量**（点源 PSF 信号口径），写入 HiPS 文件头，跨帧可比；
  2. **稀疏控制点上的区域相对 SNR**（无量纲相对场，中位归一），表达帧内 SNR 的空间变化，精度高于"整帧一个值"。
- Phase2 消费时用插值把稀疏相对 SNR **重建为稠密 SNR**（每个像素都有值），供信号层拟合与叠加加权使用；工程上按像素现场求值，不整体驻留内存。
...
- **全程只有 SNR，没有"权重模式"这个概念**：HiPS 里存的是帧级 SNR 与稀疏相对 SNR；权重是 Phase2 集成时按天球像素对应的输入帧集合现场计算的派生量；Phase1 与 Phase3 不产生、不消费权重。
- PSF 拟合质量代理（FWHM、残差尺度等）只作诊断，不计入科学叠加权重。
...
- 每个科学量写清五件事：单位、坐标系、归一化、精度要求、有效有限域；默认科学计算用双精度；epsilon 说明物理或数值来源。
...
flowchart TD
    ...
    BN --> A0["platesolve 第一轮：盲检测粗解 WCS"]
    A0 --> D["star_detection 星表引导检测<br/>Gaia 逆映射定位星点"]
    D --> P["psf PSF 建模"]
    P --> A["platesolve 第二轮：引导星表精化 WCS"]
    A --> PH["photometry 星点测光与 Gaia 拟合"]
    PH --> AP["apply photometry 测光归一化应用到像素"]
    AP --> S["noise_snr 噪声与帧级/稀疏 SNR"]
    S --> R["drizzle 球面重采样"]
    R --> V["产品验证"]
    V --> O["原子发布 HiPS + JSON"]
```
...
1. **HiPS 文件**（含帧级 SNR）：
   ...
   - 可选稀疏控制点层（`sparse_snr_layer`，默认开启）：作为标准层插入 HiPS，存无量纲相对场（中位归一）与控制点间隔，重建算子返回预测方差；不启用时只输出帧级 SNR。
...
- 实际 SNR = 帧级 × 帧内相对因子；叠加权重由 SNR 现场换算为逆方差 `w = 1/σ² = SNR²/F_ref²`，这是 point information 最优集成（Zackay & Ofek），不是直接用 SNR 加权。
...
- 全部命令由唯一可执行程序提供：Windows 为 `ACSD Cli.exe`，Linux 为 `acsd_cli`；其余能力以 DLL/SO 形式存在或在可执行程序内部实现，不设子入口。
...
- 运行目录隔离，产物路径逐段合法，产品只落块级 `output_dir`，`run/` 只放临时产物与日志。
...
| **SCI-B 绝对 SNR 传递链** | 帧级 SNR 不含天光信号、稀疏相对 SNR + 稠密重建、逆方差集成最优 | 已知噪声仿真下 SNR 与真值一致、天光升高 SNR 单调趋零；三口径适用域图谱；`SNR_combined² = ΣSNR_k²`；与 PixInsight PSFSNR 方法学对标 |
...
- 容差在写测试前冻结，NaN/Inf/缺失出现的位置也必须逐位一致；受影响验证范围由机器从改动集算出，只扩大不缩小。
