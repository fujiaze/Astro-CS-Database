...
**判据的名词口径一律照最高设计的冻结语句，本节不复述、不改写**（「口径唯一、不在下层复述」）：① 星等属**表达层**——`k_photo` 是线性乘性标度、施加后帧像素与 HiPS signal **仍是线性面亮度**，星等只在派生/展示时换算、**不作为数据形态落盘**（最高设计 §2.1「数据形态（架构口径，勿读成"数据变成星等"）」）；② **必须保持线性**——Phase2 做加性天光校正与逆方差加权求和，星等是对数量不可相加，阶段二/三消费的始终是线性面亮度、链内零星等换算（最高设计 §2.1「必须保持线性」）；③ 稀疏控制点层（`sparse_snr_layer`）存的是**绝对 SNR**（最高设计 §2.2「帧内稀疏控制点层」与 §4.4「可选稀疏控制点层」）。本节的判据若与上述冻结语句**反义**（把星等当数据形态、或把稀疏控制点层写成相对 SNR），**以最高设计为准并立即订正判据**；该反义由机器门 `eng/tools/doccheck/check_spec_polarity_consistency.py`（`--self-test` 内含「反义必判红」负例）对**验收判据 ↔ 设计冻结**逐条比对，出现反义即红。
...
| 物理单位消除 | **数据形态 = 带逐帧相对测光零点的线性面亮度**；星等属表达层、只按 `m = ZP_k − 2.5·log10 F` 现场派生；不存在由标定系数反解增益/口径/曝光的闭合路径；无绝对数值窗口（正本见下方口径锚） |
| 帧间独立 | 各帧独立标定到同一坐标系；不同透明度帧系数不同但残差分布一致；跨帧 k 散度不构成门禁 |
| 星表引导检测 | 检测定义域上限由配置键 `star_detection.max_stars` 承载（正本见下方口径锚）；相对盲检的匹配率提升与算力节省有量化对照；拟合失败丢弃、不计虚警 |
...
验证最高设计 §2.2：不受天光影响的帧级 SNR + 稀疏**绝对** SNR，Phase2 插值重建稠密并用于拟合与叠加。**「绝对 SNR」是定义名而非可选修饰**：稀疏控制点层（`sparse_snr_layer`）存**控制点处的绝对 SNR**，与帧级 SNR 同物理口径、同一参考通量 `F_ref`（最高设计 §2.2「目标产品」「命名」两条与 §4.4 第 1 条已冻结）。该层的定义面就是绝对 SNR 本身：写成「相对 SNR」或写成以帧级 SNR 为尺度基准的归一化场，属与冻结语句反义的表述，出现即由机器门判红并**以最高设计为准订正判据**。
...
| 稀疏层与重建 | 稀疏**绝对**场（控制点层存**绝对 SNR**，与帧级同口径同 `F_ref`；不以帧级 SNR 为尺度基准）+ 插值重建的稠密 SNR 在平滑噪声区精度达标；重建算子返回预测方差；给出 dense/sparse/frame 三口径适用域图谱（地面视宁度、HST 高对比两类场景） |
