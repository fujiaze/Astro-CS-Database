...
| 层 | 目标 | 数据 | 主要执行方 |
|---|---|---|---|
| L1 | 科学性 | 确定性合成数据（真值已知）——**必须按最高设计 §11.1 物理噪声过程生成；纯加性天光合成数据无效**（**2026-09-20 订正**，§9.41；`ASTROCS_DESIGN.md:679`） | 机器（ctest/检查器） |
...
**物理噪声过程强制（§9.41，2026-09-20 订正；`ASTROCS_DESIGN.md:649-650,679`）**：合成数据**必须模拟真实物理实现**，至少覆盖——源 Poisson（电子域）、天光 Poisson（电子域）、暗流 Poisson（电子域）、读出 Gaussian（电子域）、增益/饱和/量化、平场乘性响应 `m(x,y)`、天空梯度。**严禁纯加性天光**：天光对 SNR 的影响**必须通过散粒噪声体现**，不得以加性常数项代替。凡已有合成测试不符合本条者，须**重做或标注失效**。**优先用真实数据作底**（本仓 `HST_M16` 带 `PHOTFLAM`，可作合法绝对标定；用途 = **真实信号模板 + 仿真采样帧重建**，**不得**用作多帧叠加/排异/接缝的数据源——§9.67 定案 7）。
...
| 判据判别力 | **每个度量必须有「真值无效应 ⇒ 归零」的负例**：注入后该度量必须归零/判红，**禁止恒真门**（**2026-09-20 补**，§9.41 + §9.66 A「负例反转、判别力保持」） |
