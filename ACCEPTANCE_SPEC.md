...
| 内存 | 分配/RSS 稳健斜率低于无界增长阈值且运行结束可解释回落；峰值内存随数据规模合理（记录每 GB 输入的峰值占用） |
...
发布门的版本纪律见最高设计 §12：验证全部通过时 `--version` 输出 `0.1alpha`，此前代码与产物中无任何版本信息。
