#!/usr/bin/env python3
"""Validate that the V3 control package itself is complete and internally strict."""
...
def main() -> int:
    ...
    if ledger.is_file():
        ...
        known = set(ids) | {f"CP{i}" for i in range(9)}
        for row in rows:
            ...
            if not re.fullmatch(r"CP[0-8]", row["checkpoint"]):
