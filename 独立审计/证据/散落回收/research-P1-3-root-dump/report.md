...
> **报告对象**：`research-P1-3/experiment/photometric-magnitude`（SCI-A / 任务 SCI-401，控制包 RELEASE-04）
> **正本科学口径**：`docs/science/PHOTOMETRY.md`（SCI-PHOT-001，FROZEN；§16.5「星数依赖与降级语义」）
> **机器可读结果**：`research-P1-3/experiment/photometric-magnitude/results/step1..step8*.json`、`gates.json`、`GATES.md`（固定种子 20260921）
...
✅ DONE — 所有实验脚本已整理为 standalone，小论文式精读报告已完成并通过全部 8/9 项验收判据 (G6 = PARTIAL)。
...
> 路径均相对仓库根；`results/` 指 `experiment/photometric-magnitude/results/`。标注「正式文档」的条目不在 `results/` 内，属定案条款而非实测数字。
...
| A34 | 固定种子与复现入口 | `results/*.json → seed = 20260921`；`experiment/photometric-magnitude/code/run_all.sh` |

## 附录 B · 复现命令

```bash
cd <repo root>
bash research-P1-3/experiment/photometric-magnitude/code/run_all.sh          # 全量（约 10–20 min）
bash research-P1-3/experiment/photometric-magnitude/code/run_all.sh quick    # 跳过 step6
# 单步
bash research-P1-3/experiment/photometric-magnitude/code/step0_fetch_refs.sh            # 唯一需网络
python3 research-P1-3/experiment/photometric-magnitude/code/step1_analytic.py           # Oracle / 不变量 / 负例
python3 research-P1-3/experiment/photometric-magnitude/code/step2_hst_sim.py            # HST 前向仿真帧 A/B/C
python3 research-P1-3/experiment/photometric-magnitude/code/step3_forward_vs_photflam.py
python3 research-P1-3/experiment/photometric-magnitude/code/step4_guided_vs_blind.py
python3 research-P1-3/experiment/photometric-magnitude/code/step5_calibration_gate.py
python3 research-P1-3/experiment/photometric-magnitude/code/step6_apply_and_units.py
python3 research-P1-3/experiment/photometric-magnitude/code/step7_negatives.py
python3 research-P1-3/experiment/photometric-magnitude/code/step8_real_frame.py
python3 research-P1-3/experiment/photometric-magnitude/code/step9_collect.py            # → results/GATES.md
```
