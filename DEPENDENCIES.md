...
工具链的**取值落点** = `eng/packaging/windows/README.md`（组件与工具链取值登记）；**机器单一事实源** =
`eng/packaging/schemas/preset-contract.json`；机器校验入口
`eng/cmake/toolchain/verify_toolchain.py`。三者冲突时以机器源为准。

## Windows x64 发布工具链（冻结，BLD-001；发布版本号以仓库根 `VERSION` 为唯一事实源，本节不复制字面）
...
| VS 组件清单 | `eng/packaging/windows/.vsconfig`（7 组件） | 组件清单与取值落点见 `eng/packaging/windows/README.md`；禁 `--includeRecommended` |
...
正式 preset：`win-msvc-17.14.39-x64`（`CMakePresets.json`）——Windows 工具链取值以该 preset 的显式声明为来源，其机器合同 = `eng/packaging/schemas/preset-contract.json`。preset 边界之外的取值面（VS 2026/v144/v145、17.14 evergreen latest、CMake 4.x、Ninja 以外的 Windows generator、MinGW/MSYS2、`/MT`、**任何固定并行度**（最高设计 §9 禁硬编码线程数）、Windows 11-only API）不属正式工具链面；**禁面的取值集合以 `preset-contract.json` 的 `forbidden` 段为机器正本，本文件不复制**。
...
| Python | 3.13（控制节点实测） | 工具链/测试脚本；Windows 侧验证脚本所用 Python 版本无仓内可核依据，故不登记取值 |
...
- 优化档由 `CMAKE_BUILD_TYPE=Release` 的编译器默认决定（本仓不显式声明 `-O2`/`-O3`，全仓无该 flag）; 无未锁定 flag; 无全域 -w
...
`eng/build/toolchain.ps1` 是**现役** Windows 侧构建/自检脚本（ENGINEERING_SPEC §7 根固定条目），
不是遗留待清理对象；其依赖面只允许**仓内 vendored 依赖与系统工具链**——上方
`msys2_mingw: FORBIDDEN` 与 `machine_absolute_path: FORBIDDEN` 对它同样成立。
