...
2. **权重误差的代价是二阶的、且只由帧间散射决定**：
   ...
   **共模权重误差完全免费**（实测 ratio = 1.000000）——该定理对绝对与相对**两种表示同样成立**（绝对表示根本不产生该因子），
   因此**没有区分能力**：它只说明**共模退化场景下单帧加权效率不受影响**，不足以支持「稀疏 SNR 层只存相对值」，
   也管不住绝对电平与跨帧（见 §3.6 与 `EXP-05` §3.3/§3.5）。
...
6. **当前生产链的成品 σ 相对误差约 131%**，主导项不是统计噪声而是**帧标量 σ 的结构污染偏差**
   （EXP-3 Part A 实测：`p1_snr.json.frames[].variance` 隐含的整帧未裁剪 MAD 与 `p1_sources.json.frames[].noise_sigma`——帧级 SNR 实际使用的整帧裁剪 RMS——之比中位数 `1.3127`）；修好它 + 传播 `C_θ` 后降到 ~3.7%（良态）或 ~220%（病态）（EXP-5）。
   > **【复核后（SNR-EXP-AUDIT，§13）】原结论的数字有三处口径/算术错误，方向不变。**
   > ① **1.3127 是比值，不是相对误差**：把它当相对误差即得 131%，σ 的相对误差实为 **+31.27%**（方差高 76.8%）；
   > 文档 §5.4 的分项表本来就写 31.3%，与摘要/场景表的 131.3% **自相矛盾**。
   > **并且该比值不是 `frame_snr` 相对真值的偏差**：分子来自**另一条生产者**（逐像素噪声模型 A 的整帧未裁剪 MAD，
   > 承载 `p1_snr.json.frames[].variance`），分母才是 `frame_snr` 实际使用的 `noise_sigma`
   > （`StarDetector::estimate_background` 的整帧 2 轮裁剪 RMS）⇒ 它度量的是**同一产品内两个噪声生产者互相矛盾约 1.3×**，
   > 原「⇒ 帧级 SNR 偏低 23.8%」的归因**已删**。`frame_snr` 的真实电平偏差须**直接对真值测**：
   > `EXP-05` 表 A 在真实 M42 帧上实测**偏低 15% ~ 77%**（随面板结构含量变化；逐帧明细见 `results/EXP05_TABLES.md` 表 A2）。
...
- **差距量级（实测，EXP-3 Part A）**：同一帧内，`p1_snr.json.frames[].variance` 隐含的 σ（整帧 MAD）
  ...
  **该比值不是 `frame_snr` 相对真值的偏差**（分子来自另一条生产者：噪声模型 A 的整帧未裁剪 MAD）；`frame_snr` 的真实电平偏差须直接对真值测（`EXP-05` 表 A：真实 M42 帧偏低 15%~77%）。
...
**这是「共模退化场景下单帧加权效率不受影响」的定量依据**；该定理对绝对与相对**两种表示同样成立**（绝对表示根本不产生该因子）⇒ **无区分能力**，且**管不住**绝对电平与跨帧（`EXP-05` §3.3/§3.5）。**不得**据此支持「稀疏层只存相对值」。
...
> ① **"131.3%" 是比值被当成相对误差**：`σ_MAD/σ_clippedRMS = 1.3127` ⇒ σ 相对误差 **+31.27%**（方差 +76.8%）。
> **该比值不是 `frame_snr` 相对真值的偏差**（分子来自另一条生产者：噪声模型 A 的整帧未裁剪 MAD），原「SNR −23.8%」归因**已删**；`frame_snr` 的真实电平偏差实测**偏低 15%~77%**（`EXP-05` 表 A）。
...
| P1-2 | 帧标量 σ = **整帧未裁剪 MAD**（`wrapper_phase1/noise_model.cpp:164-190`） | σ_bg 来自**掩膜 patch** MAD | **同一产品内两个噪声生产者互差 1.3127×（方差 1.7676×）**（分子 = 本行整帧未裁剪 MAD，分母 = `frame_snr` 用的整帧裁剪 RMS）——是**生产者不一致**，**不是**本行 σ 相对真值的偏差（真实帧上帧级 σ 偏低 15%~77%，`EXP-05` 表 A） | 同 P1-1；`module_adapters.cpp:3568` 换实现 |
...
| 4 | 「生产成品 σ 相对误差 **131%**」 | 把比值 1.3127 当相对误差；且该比值本身是**两个噪声生产者互相矛盾**（噪声模型 A 的整帧未裁剪 MAD vs `frame_snr` 用的整帧裁剪 RMS），不是帧级 σ 相对真值的偏差 | **不成立（算术/口径/归因错）** | 「σ 相对误差 **+31.3%**（方差 +76.8%）；`frame_snr` 的真实电平偏差实测**偏低 15%~77%**（`EXP-05` 表 A）」 |
