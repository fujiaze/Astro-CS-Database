...
> **正本科学口径**：`docs/science/PHOTOMETRY.md`（SCI-PHOT-001，FROZEN；本轮新增 §16.5「生产适用域与降级」）
...
注意 `m` 是**归一化修正场**（乘性改正），不是仪器增益本身——该方向错误曾在审稿 R9 中被指出并修复（`results/REVIEW.md`）。未启用时产品必须写显式 `degraded_reason` 且 **fail-closed**；低星数帧（`n < 100`）按 §16.5 必须显式降级（如 `low_star_count`）并不承诺标定。
...
| 条件 | 边界 | 证据 |
|---|---|---|
| 星数 | **生产适用域 = 匹配星数 n ≥ 100**；`n < 100` 显式降级 `degraded_reason`、不承诺标定 | `PHOTOMETRY.md` §16.5 第 1–2 条（定案）；域内实测：帧 C `n=105` PASS、真实帧 `n=157` PASS |
...
5. **降级 fail-closed**：低星数帧（`n<100`）与未启用路径必须写 `degraded_reason` 并拒绝下游消费；**不得**把 NO_DATA 的占位 `k=1.0` 伪装成「已应用」。
...
2. **`n ≥ 100` 是定案阈值，不是本实验扫描出的临界值**：本实验落在域内的只有帧 C（选中 105）与真实帧（157）；且帧 C 的判据内样本 `budget.n = 96 < 100`（9 颗星未进入白噪声子样）⇒ 域阈值口径与判据内 `n` 口径尚未统一。
3. **下界域外失效**（N5）：`n ≲ 22` 时下界无判别力；若将来要在域外使用，须改为 `max(ρ_lo, 0)·σ_fit` 或加最小样本量硬门槛。
...
15. **落地缺口**：`PHOTOMETRY.md` §16.5 的 `n<100` → `low_star_count` 降级在 `lib/` 生产代码中**未实现**（生产硬门槛仍是 `kMinFitStars = 3`）。

### 5.5 与正式科学文档口径的关系与冲突

| # | 事项 | 判定 |
|---|---|---|
| 1 | §16.5「生产适用域 n≥100 + 低星数降级 `low_star_count`」 vs 生产代码硬门槛 `kMinFitStars = 3`（`frame_photometry_fit.h:70`；`module_adapters.cpp` 的 `P1_PHOT_MIN_FIT_STARS = 3`） | **规范已定案、机器未落地**（缺口登记；本报告不主张代码已实现域限制） |
| 2 | `06_photometry.md` §4.1「判据尚未在代码中生效」 vs §16.5 的适用域与降级 | **不冲突**：前者是模块落地状态，后者是适用域语义；但须注意**实验 PASS ≠ 生产已设门** |
...
| 5 | 帧 C「n=105」与判据内 `budget.n=96` | **口径未统一**（域阈值 vs 判据样本），登记为缺口 |
...
2. **成立条件明确**：生产适用域 = 可解析实拍帧、`n ≥ 100`；域外（`n ≲ 22`）下界失效（N5），**只有上界是硬约束**；判据只认证天光**噪声**预算，不认证天光**扣除**质量。
...
6. **遗留待办**：① `n<100` 降级语义落地到生产代码；② 判据下界在域外的形式修正；③ 天光扣除残差检查；④ 帧 C 域阈值与判据样本口径统一；⑤ `README` 与归档 JSON 的数字同步（墙钟时间）。
...
| A31 | 生产适用域 `n ≥ 100` 与低星数降级 `low_star_count` | **正式文档**：`docs/science/PHOTOMETRY.md` §16.5 第 1–2 条（定案；非 `results/` 实测） |
...
| A33 | 生产代码硬门槛 `kMinFitStars = 3`（与 A31 的落地缺口） | `lib/algorithms/photometry/cpp/src/frame_photometry_fit.h:70`；`lib/infrastructure/scheduler/src/module_adapters.cpp` `P1_PHOT_MIN_FIT_STARS` |
