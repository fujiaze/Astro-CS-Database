...
- 下游 P5 的天光定权 control_variance 所涉 k_corr 已由分歧台账 D-08 终裁改为 k_shape × k_geo 两因子几何查表（P3 单元承载），P4 接口仅须传递 N_retained 与几何元组，不承载该常数。

### 2.3 精度约定

- 节点复现自检门 1e-9（P-CST-12）是"精确插值"的浮点性质而非松容差：三路实测残差 1.8e-15 / 3.55e-15 / ≤9.3e-15，距门限六个数量级余量（语义按分歧台账 D-10 口径写明）。
