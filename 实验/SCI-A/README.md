...
13. **CI 登记缺口（需负责人处理，本轮不自处理）**：`flock /tmp/astrocs_ci.lock python3 ci/run_checks.py --all --profile fast`
   ...
   "其他固定目录"段），AGENTS.md §7 同样列出；而检查器 `tools/quality/check_root_cleanliness.py`
   只解析该代码块开头的"仓库根固定条目"清单（第 98–101 行）与 `ci/root_manifest.json` 的
   `allowed_dirs`，两者都不含 `实验` ⇒ 规范已认可、机器白名单未同步。
   本实验单元按任务书与 AGENTS.md §7
   落位 `实验/SCI-A/`，**未越权修改根级登记文件**（`ci/root_manifest.json` / `ENGINEERING_SPEC.md`），
...
**遗留需负责人处理**：`实验/` 根条目在 `ci/root_manifest.json` 未登记（见 §6.13）。
