...
用法:
  ...
  python3 packaging/verify_install_tree.py --repo <repo> --prefix <prefix>
  python3 packaging/verify_install_tree.py --self-test
...
def build_fixture(base: Path, repo: Path):
    """最小自洽夹具: repo_root(含 packaging/VERSION) + prefix(合同全集在位)。"""
    fx_repo = base / "repo"
...
# 复用旧 build dir 是合法工作流（tests/abi/mod001 就用既有 --build-dir 安装）,
