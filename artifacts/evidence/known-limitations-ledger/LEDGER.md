# 已知限制过程台账（KNOWN_LIMITATIONS Ledger）

> **性质**：过程台账，**不是**正式文档。依据 `ASTROCS_DESIGN.md §0.3`（正式文档只写现行设计要怎样）、
> `ENGINEERING_SPEC.md §8` 规则 2（正式文档不堆任务编号与日期）。
> 正式文档 = `docs/KNOWN_LIMITATIONS.md`（只保留现行有效的限制陈述：现象 / 规范依据 / 归属去向）。
> 本文件承载该页条目的**过程面**：编号注册表 · 严重度 · 状态 · 发现任务 · 裁决出处。

> **落点依据**：`ENGINEERING_SPEC.md §7`（`artifacts/` 含证据锚 `artifacts/evidence/**`）；
> `artifacts/evidence/README.md`「目录规则」（只有被正式文档引用为**证据锚**的内容才进本目录）。
> 不放 `工程控制/`：控制包收口即清理（`CONTROL_PACK_SPEC.md §9`），台账须长期可解析。

## 1. 编号注册表（ID 可解析性）

正式文档 `docs/KNOWN_LIMITATIONS.md` **保留原条目号**作为稳定 ID 锚点：仓库内以下活动引用按
「条目 N」「§E M-x」「§<节>」解析，迁移后**必须仍可解析**（判据见
`eng/tools/doccheck/check_doc_hygiene.py`）：

| 引用方 | 引用形式 | 解析目标 |
|---|---|---|
| `eng/tests/unit/mem_wire_test.cpp` | `docs/KNOWN_LIMITATIONS.md 条目 30` | 正式文档 §E 条目 30 |
| `eng/tests/unit/CMakeLists.txt` | `docs/KNOWN_LIMITATIONS.md 条目 30` | 正式文档 §E 条目 30 |
| `eng/tests/unit/mosaic_window_test.cpp` | `docs/KNOWN_LIMITATIONS.md 条目 35` | 正式文档 §E 条目 35 |
| `工程控制/RELEASE-05/OPEN_QUESTIONS.md` | `docs/KNOWN_LIMITATIONS.md §E M-4` | 正式文档 §E 条目 32（原发现编号 M-4） |
| `docs/RELEASE_STATUS.md` | `docs/KNOWN_LIMITATIONS.md §15` / `§C` | 正式文档 §B 条目 15 / §C |
| `docs/modules/registry/*.md`、`docs/owner/SCIENCE_OVERVIEW.md`、`README.md` 等 | `docs/KNOWN_LIMITATIONS.md`（整页） | 正式文档整页 |

**编号 ↔ 发现任务对照**（迁移前 §E 条目号 → 发现任务；本节第 2 部分逐字保留原 §E 全文）：

| 条目 | 发现任务 / 审核编号 | 严重度 | 状态 |
|---|---|---|---|
| 30 | ARCH-AUDIT-01 B-3 ＝ ARCH-AUDIT-02 F-04 | 阻塞 | 已修（MEM-WIRE-01，2026-09-22） |
| 31 | ARCH-AUDIT-01 M-2 | 重要 | 待修 |
| 32 | ARCH-AUDIT-01 M-4 | 重要 | 待修 |
| 33 | ARCH-AUDIT-01 M-5 | 重要 | 待修 |
| 34 | ARCH-AUDIT-02 F-03 | 阻塞 | 待修 |
| 35 | ARCH-AUDIT-02 F-07 | 阻塞 | 已修（MEM-WIRE-01，2026-09-22） |
| 36 | ARCH-AUDIT-02 F-13 | 次要 | 待裁决 |
| 37 | ARCH-AUDIT-03 P3-02 | 重要 | 待修 |
| 38 | ARCH-AUDIT-03 P3-03 | 重要 | 待修（文档订正） |
| 39 | ARCH-AUDIT-03 P3-04 | 重要 | 待修 |
| 40 | ARCH-AUDIT-03 P3-05 | 重要 | 待修（文档订正） |
| 41 | PROJ-AUDIT-01 P3 | 中低 | 待修 |
| 42 | PROJ-AUDIT-01 P4 | 低 | 待修（文档订正 + 可选判据增强） |
| 43 | PROJ-AUDIT-01 P2 | 中低 | 已修（文档表述，ARCH-DEBT-01 落地） |
| 44 | DRIZZLE-AUDIT-01 N1 | 中 | 待裁决 |
| 45 | DRIZZLE-AUDIT-01 N2 | 中低 | 待修 |
| 46 | DRIZZLE-AUDIT-01 §1.6/§1.7 | 次要 | 待修（文档订正） |
| 47 | 机器台账交叉引用（ARCH-AUDIT-01 B-5 / ARCH-AUDIT-03 P3-05/P3-07/P3-08 / ARCH-AUDIT-02 F-06/F-08） | —— | 已由机器台账承载 |

> **裁决出处**：条目 30/35 依负责人 2026-09-22 逐字裁决：「我是不设置上限，有多少资源吃多少资源。
> （内存最高吃掉空闲的 95% 避免卡死，且这个参数配置在 config 里面可调，默认 95）」；
> 条目 43 依前台裁决「改文档表述，保持 fail-closed 四角守卫不动」。
