...
`ENGINEERING_SPEC.md:117-120`（§8 机器一致性检查）：`eng/ci/checks.json 是唯一检查注册表`；**「每项检查有正例与负例（能红能绿）」**；`修改代码/测试后必须本地复跑对应检查项`。
...
| M7-F-201 | 逐处给数值容差 + dtype 参照（bilinear 常数场：FP64 权重下 `|Δ| ≤ 4·ULP(1)·|C|`，FP32 输出另加 `2^-24` 量级）；「构造保证」改「按构造等于 1 ± k·ULP，k=1（实测）」；补一条 CTest 使其可红 | 高 | RSMP §5c/:86、§8/:100、§9/:105；PHASE3_FITS_IMPL:325；PHASE2_REJECTION §11.4 | 验收面可证伪性、`eng/ci/checks.json`（若挂机器门） |
...
4. **测试/门**：`tests/unit/v6_p3_proj/p3_proj_wcs_oracle.py`（CASES + 不变量 + band 判据域）、`p3_proj_legacy_deviation.py`（xfail + 偏差表）、`tests/unit/p3_projection_test.cpp`（随 v1 退场）、`eng/ci/checks.json`（若新增容差门）。
