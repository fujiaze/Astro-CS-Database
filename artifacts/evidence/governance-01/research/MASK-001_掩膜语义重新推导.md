...
| SCI 合同 lint | `python3 eng/tools/science_contract_lint.py docs/science/NOISE_MODEL.md` | **SCIENCE_CONTRACT_LINT_PASS** 15 sections（EXIT=0） | **零**：只校验 15 节结构 + claim ID + 锚点，无数值/语义判据 |
| `eng/ci/checks.json` | 全表 grep | 与掩膜覆盖率/半径相关的检查项 **0 条** | **零** |
...
3. `eng/ci/checks.json` 注册 V-7 的新 oracle 用例（正例 + 负例各一）。
...
（`docs/**`、`lib/**`、`config/**`、`tests/**`、`eng/ci/**`、`问题扫描/**` 均未写）；
...
| 门禁-3 | SCI 合同 lint | `timeout 600 python3 eng/tools/science_contract_lint.py docs/science/NOISE_MODEL.md` | 0 | `logs/gate_science_contract_lint.log`（**PASS 15 sections**） |
