...
| 12 | `docs/algorithms/GATES_AND_TOLERANCES.md:39` | 锚 `:1637-1647 → :1782-1792`（阈值数值不动） | ALG-GATES 线 | `eng/tools/check_gates_and_tolerances.py` |
...
5. **A4 的假绿缺口**：在 `CFG002` 增设「登记点值 vs 文档默认值」一致性断言之前，任何 `conflict → none` 的操作都可能掩盖未落地的值修改；本条建议纳入 `eng/ci/checks.json` 的后续加固（CI 线）。
