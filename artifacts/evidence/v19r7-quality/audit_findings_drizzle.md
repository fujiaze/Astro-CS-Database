...
A1 `eng/tools/docs_machine_consistency.py` 未在本会话重跑，已独立审计；本文 `machine_consistency` 结论标记 **待 machine_consistency 复核**，不阻塞 B1 进入但 B5 前必须 0 broken 复核。
...
- machine_consistency：本次未重跑，标记 **待复核** (A1-01 `machine_consistency_before.json` 9 checks 0 broken 为初扫快照，B5 前需重跑 `eng/tools/docs_machine_consistency.py`)。
