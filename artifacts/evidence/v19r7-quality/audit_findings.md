...
> **冻结语义**: 以上 4 项在 B1/B4 修复并经 `eng/tools/docs_machine_consistency.py` 与 `candidate_oracle`/`synthetic_gate` 复核 0 broken 前，**禁止进入 B2 全量实现**；其余 60 项可在 B 阶段并行收口。
...
| 6 | **NO-04** | Noise | A1-02证据缺失 | `eng/tools/file_audit` 缺失致 873/~713+ 覆盖度为替代统计，noise 域 NUMERIC/COMMENT 违规未行级定位 | **A1-02补齐** |
...
| 桶 | 任务 | 承接发现 | 产出/门禁 |
|----|------|----------|-----------|
| **B1 科学层** | **B1-01** SCOPE 文档化 | SC-03 | `SCIENCE_SCOPE.md` 补 `SCI-SCOPE-*` 合同段 + `TRACEABILITY.csv` 增行 + 重跑 `eng/tools/docs_machine_consistency.py` |
...
|  | **A1-02补齐** | NO-04 | `eng/tools/file_audit` 找回 + `standards_violations.json` 行级证据闭环 |
...
**入 B 条件（G-QA A→B）**:
- P0 4 项修复并经 `eng/tools/docs_machine_consistency.py` + `candidate_oracle 9003例` + `synthetic_gate UPMW/PR` 复核通过；
