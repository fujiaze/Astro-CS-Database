...
- tool: eng/tools/docs_machine_consistency.py (已修复 Linux ROOT 自动推导)

## 通过条件
- `python3 eng/tools/docs_machine_consistency.py` 在 vm-bj Linux 上可运行（退出码 0/1 均可，JSON 已生成）
...
## 产出清单
- `eng/tools/docs_machine_consistency.py` — 修复 ROOT 自动推导（仅改 ROOT 计算，保留10项检查逻辑）
