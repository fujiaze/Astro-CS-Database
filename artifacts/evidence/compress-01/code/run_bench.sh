#!/bin/bash
# COMPRESS-01 战线一主基准: zstd 多档 × (数据段 / 整文件 / 对照) + FITS 原生压缩
set -uo pipefail
cd "$(dirname "$0")/.."
export COMPRESS01_TMP=/tmp
mkdir -p logs evidence
LEV="-7,-5,-3,-1,1,3,5,9,12,15,19,22"

echo "[1/5] zstd 数据段 (188 瓦片, shuffle=0)"
timeout 3600 ./bin/zstd_bench data/*.bin --levels "$LEV" --reps-c 5 --reps-d 9 --shuffle 0 \
  --csv evidence/zstd_tiles_shuf0.csv 2> logs/zstd_tiles_shuf0.err
echo "  rc=$? rows=$(wc -l < evidence/zstd_tiles_shuf0.csv)"

echo "[2/5] zstd 数据段 (shuffle=4, f32 字节转置)"
timeout 3600 ./bin/zstd_bench data/*.bin --levels "$LEV" --reps-c 5 --reps-d 9 --shuffle 4 \
...
timeout 3600 ./bin/zstd_bench $(cat files_real_fits.txt) --levels "$LEV" --reps-c 5 --reps-d 9 --shuffle 0 \
