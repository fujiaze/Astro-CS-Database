...
## S. 建议条款 S-1..S-5（V12 轴提出，**目的是收敛而非新增裁决**）

### S-1｜科学/门禁数值常数的**单一权威点**（建议进 `docs/contracts/DATA_SEMANTICS.md` 或 `docs/standards/`）
1. 凡进入科学量计算或门禁判定的数值常数，必须且只能有**一个可机器定位的定义点**：登记面 `docs/algorithms/NUMERIC_CONSTANTS.md`（值 / 单位 / 适用域 / 推导源 / `DISP` 号 / 退休口径），生产侧由一个常数头（建议 `include/astrocs/sci_constants.h`）承载具名 `constexpr`，二者由机器门**逐值**比对（**不得用子串在场判据**）。
2. **一个数值一个名字**：同一换算系数禁止以倒数式（`x/0.6745`）与正数式（`x*1.4826`）两种形式并存；需倒数时由权威点定义 `kInv = 1/k`，或同时登记两值并写明舍入方向与引入的相对偏差。
3. **位数即合同**：登记位数 = 代码位数 = 文档位数；文档允许截断展示时必须显式写「截断展示，权威值见 X」并给出**复算的**相对偏差（不是「≈」）；**截断串不得被任何机器门当作过门必要条件**（灭 `V12-N-02`）。
4. **量纲与适用域同行**：登记单位（无量纲 / 星等 / MB·s⁻¹ / B / px / nm / deg）与适用域（分布假设、样本量下限、FOV/半径上限、**边界开闭 `>` 与 `>=`**）（灭 `N-08(c)`、`N-09`、`N-10`）。
5. **退休即登记**：口径替换（α 1.3→0.2885、1.4826→15 位、0.7316728→全精度等）时退休文本标 `ARCHIVED`、替换落 `DISP-*`，且**登记锚必须是 git 跟踪的真实路径**（禁 `run/`、禁不存在路径）（灭 `N-06`，同 `V6-N-04` 族）。
6. **本条吸收同类裁决**：`A-12`、`M3-A-008`、`V6-N-08c`、`A-39`、`A-42` 一类"某常数位数/取义是否违规"的争议一律按 S-1 判——**无权威点即缺陷；有权威点则一切引用它**，不再逐值立案。
> **给负责人的一句话（V12 原案，我认同并采纳）**：**`A-12` 不再单独裁「`1.4826` 是否违规」，而是裁「是否接受 S-1」**；接受后 `A-12`、`M3-A-008`、`V6-N-08c` 与 `V12-N-01~04` 全部由 S-1 + S-4① 自动判定。**我已把 A-12 条目改为此表述。**

### S-2｜权威点样板仓内已有，不必新造风格
- 正例：`cli/memory_report.h:47-57`（六条 `inline constexpr` + 注释给单位与判据）与 `gaia_client.c:26-31`（`WL_COUNT` 派生 `STAR_STRIDE_SP`、`QUERY_CACHE_MAX_BYTES` 由宏派生）。
### S-3｜跨语言禁手抄
- `run_monitored.py:451-456` 与 `resource_gate.h:96-111` 的 85%/60%/10.0/2/2 必须同源（Python 侧读同一登记表/生成头，或两侧都读 `contracts/` 数值清单）⇒ 直接对应 `V12-N-08`。
### S-4｜三条机器门（做成 `ci/checks.json` 的真检查项，而非现在这个**未接线**的 `docs_machine_consistency.py`）
①`check_numeric_constants`：逐值比对 + 拒绝同语义多位数字面量（族扫描 `1.4826*`、`0.6745`、`0.73167*`、`1.23031*`）。②`check_comment_anchor_exists`：注释里的 `docs/**`、`contracts/**` 路径与 `DISP-*`/§ 锚必须存在（直接灭 `V12-N-06`，顺带覆盖 `M6b-E-002` 行锚面）。③`check_gate_threshold_parity`：C++/Python 门禁阈值逐条对齐、**比较号与边界开闭一致**（灭 `N-08(a)(c)`、`N-09`）。
### S-5｜反向钉死禁令
- 测试或门若钉住「字面串 / 文档措辞 / 诊断码名」，必须在同处注释给出该串的**权威值来源**；**禁止把合成 fixture 值（如 `N-12` 的 `step=1`）钉成期望值而不标注**。本轴四实例：`N-02` 截断串、`N-08(b)` 诊断码串、`N-12` 波长网格、`p1star_mad_check.cpp:84`「两常数不相等」。

### 附：V12 对我转述数字的两处订正（按簇 10 纪律入档，**改我引用的数**）
- `V1-N-09`/我报的「`1.230310` 相对差 1.6e-6」→ V12 复算 **`+1.907986e-06`**（解析值 `2√2·√(2^{1/4}−1)=1.2303076525901024`）。**以 V12 为准**。
- 旧「`0.7316728` 与代码差 2.74e-07」的 R-06 系说法**不采信**：三套实测为 代码↔解析 `4.13e-7`、docs↔解析 `4.04e-7`、16 位↔7 位 `9.67e-9`。
- 互证成立的两处（V12 独立复算与我给的一致）：`1/0.6745` 相对差 `1.5196e-5`（`V2-N-07(c)`）；σ/A 门阈值 `0.13489815189531906` vs `0.13489795003921634` 差 `1.496e-06`（`V2` §1.6-1）。
- V12 另记：**`40_OWNER_DECISIONS.md` 无 `OWNER-06`/`OWNER-10` 条目**，按「权威源缺失」记，**不判"不一致"**（此判法正确，采纳）。

### C-21｜注入式反向锁的三条合格判据（V15 实测后提出）
- **背景**：V15 按黄金口径（把缺陷改回去看门是否变红）实测本仓仅 **≈30 把**有效锁，而**被当作"有注入锁"的 11 TU / 12 个 ctest selfcheck 目标不构成缺陷回归锁**——其 `main.hpp:53-67` 的命中 CHECK 走 `++failures` 而 **`cond` 不参与求值**，且 25 个注入名里 **CI 实际逐一驱动的为零**。
- **判据（三条同时满足才算一把锁）**：①**CI 逐一驱动**（`ENVIRONMENT` 或等价机制显式设置每个注入名，且未驱动即红）；②**`cond` 真参与求值**（注入命中 ⇒ 由该条件本身导致失败，不得旁路计数）；③**生产侧注入名编译期剔除**（现 14 名里 13 名在生产源内常驻 `getenv` 分支，仅 `p3o_output.cpp:103-104` 有 `#ifdef`）。
- **前提数字按 V18 实测订正（原写 25 名 / 12 目标 / 2 处 ENVIRONMENT，全部作废）**：注入名全集 **501 个不同名 / 787 个 CHECK 站点**，其中 **226 站的注入名是变量而非字面量** ⇒ 静态枚举不可完备，门须按调用点判；CI 用 `ENVIRONMENT` 出现的有 **3 处**，但 `ASTROCS_P1STAR_FAULT=` **值为空是"清除继承注入位"** ⇒ **真驱动 2 处、各覆盖 1 名 ≈ 0.4%**；selfcheck ctest 目标 **13 个**（第 13 个 `ipv_abi_layout_lock_selfcheck`），且它是**全仓唯一合格的负例自检形态**（`make_mutated_mirror()` 破坏真镜像后调**同一个** `check_all()`）⇒ **`C-21` 的样板就取它，不必另造**。
- **`C-21①` 的可写性结论（V18 片2 实测，比"没做到"更强）**：501 名三态 = **CI 驱动 0 / 测试驱动 24 / 无人驱动 477**（`0+24+477=501`；B 态 24 名逐字清单在 `_verify/V18.md`，V15 记 14 是因只数 `*_FAULT` 变量驱动者，漏了 `p1drz` 手抄表与多注入点）。且 **`ci/checks.json` 全 130 条里 `ASTROCS_*` 出现次数 = 0**（`scripts_v18_checks.py` → `NONE`），其 15 个键（`changed_paths command ctest_targets dirty_ignore_exact dirty_ignore_prefixes heavy id mutates_workspace outputs platform prerequisite_tools profiles requires_monitor timeout_seconds waivable`）**没有 env/variant 维度** ⇒ **判据①在当前 CI 合同面上根本写不出来；先给 `checks.json` 加 `env` 维度是前置件，不是一条门**。另 `p3_output.cpp:103-104` 的 `#ifdef` 依赖 `-Dastrocs_hash_fail_inject`，而 checks.json 零引用该 define ⇒ **该剔除分支在 CI 配置下永不可达**（`C-21③` 现为 1/14 合规）。ENV 变量层全集 **32 个**（生产侧读点 8 / 纯测试侧 24）⇒ **"注入面主要住在测试侧"，故生产二进制的可编程行为面比看上去小，但 13/14 生产侧读点仍常驻无剔除。**
- **为什么必须成条**：这三条同时是"非豁免真门为何仍绿而事实红"的机制来源，与 `A-38`（0 用例/0 输出算不算绿）、`V15-N-16`（backend 缺 EXE 时真断言整体 error）、`V9` 全门清单的可信度扣减直接相关。**`V9` 输出的门清单将以本条为扣减依据。**

### A-44（V8-N-07）两族「负责人裁决 2026-09-14」在登记面零条目，请确认其真伪并补登记或撤下
- **①PSF-FAST-001**「只留 FAST + 精确路径保留」被 **12 处**代码/文档援引（`module_adapters.h:30`、`module_adapters.cpp:1599/1754/1791/6162`、`dynamic_psf/README.md:204/218`、`module.yaml:157`、`phase1_session/README.md:49`、`p1phot_fixgates.cpp:13/90`），**`CHANGELOG.md`/`memory.md`/本档全部 0 命中**。⇒ 该裁决若不存在，则"inactive 精确 PSF 路径按裁决保留"（`V8-N-01` 的虚构测试凭据即为其配套）不成立，**死码应清**。
- **②P9「帧头 WCS 未授权」**被 **6 处**援引，其中 `DATA_SEMANTICS.md:788/791-792` **带引号称"裁定原文"**，而该引文的唯一仓内宿主就是合同自己那一行 ⇒ **自指成环**。⇒ P9 的**实测数字**（906 帧 / 31 帧无 CRVAL / 最大偏差 0.88247°@idx561）经 V8 独立复算**全部为真**，所以本项争的不是数据而是**"谁授权改写冻结合同"**。
- **请择一**：**(a)** 确有其裁决 ⇒ 我在 `40_OWNER_DECISIONS.md` 补 A-44a/A-44b 两条登记，代码/合同行后加编号指针（配 `C-18` 生命周期）；**(b)** 无 ⇒ 撤下援引并回退相关冻结文本，我按 `§1.2`（Agent 无权新设裁决条款）立 P0。
- **注**：同批 **P5-SNR 授权**已在 `CHANGELOG.md:9/:11` 正确登记（负责人授权、`scientific_change=YES`）⇒ **本仓已有合规形态可照抄**，说明这不是流程不懂而是漏登。
