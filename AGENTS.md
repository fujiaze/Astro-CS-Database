...
### 1.1 铁律：**任何任务必须先到最高文档确定规范，再动手**

**做任何事之前——写代码、改文档、出控制包、写测试、改门禁、命名、定目录结构、定提交格式——必须先回到权威链（`ASTROCS_DESIGN.md` ① 起，按 §0 顺序）把"这件事的规范在哪、怎么规定的"查清楚。**

- **规范已存在** ⇒ **照它做**。不得凭印象、凭惯例、凭别的项目的样子自己发明一套；
- **规范不存在** ⇒ **先在最高设计（或它指向的那一层）把规范补上**，再按新规范做；**不得**先做出来再补规范；
- **规范在别的文档**（如控制包格式在 ④ `CONTROL_PACK_SPEC.md`、CI 在 ⑥ `docs/ci/`、插件在 ⑦ `docs/plugins/`）⇒ **去那一份读**，不只看最高设计；
- **自检**：交付前必须能回答「**我这件事的规范依据是哪一份文档的哪一条**」。答不出来 = 违规。
