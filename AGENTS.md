...
## AstroCS 开发协作规范（2026-08-21）

> 本节为 2026-08-21 新增协作总纲，统一为中文；原有章节（项目根目录结构、编译环境、统一工具链、审核包工作流等）完整保留在后文，不删除、不覆盖，重复处以“补充”标注。
...
### 开发
- 修改前明确：需求、影响范围、风险、验收标准。
- 遵循最小修改：禁止无关重构、格式化和扩大修改范围。
- 模块必须：职责明确、接口明确、数据含义明确。
- 核心算法修改必须同步更新：科学文档、软件文档、测试。

### 质量
- 关注：正确性、数值安全、内存生命周期、错误处理、并发安全、性能。
- 性能优化先分析：CPU、内存、IO、等待、重复计算和算法复杂度。
- 禁止无意义重复跑完整基准。

### 验证
- 修改后必须测试。
- 报告：修改内容、测试结果、限制、遗留问题。
- 未经验证不得声明完成。
...
### Linux环境（补充：原“编译环境 / 环境约束 / 统一工具链”保留，本节为协作补充）
- 开发环境：vm-bj Linux。
- 默认：bash + git + Linux 工具链。
- 所有长时间任务必须：有超时；有日志；可恢复。

### Windows验证（补充：原 MSYS2/MinGW 编译环境保留，本节为远程验证补充）
- Windows 仅作为远程验证节点。
- 需要时通过 SSH 调用 Windows PowerShell 进行：Windows 编译；MSVC 测试；GUI 验证。
- Windows 不可用时记录等待，不阻塞 Linux 开发。

### 工程包执行（补充：原“工程控制包合并规则 / 审核包工作流”保留，本节为协作补充）
- 收到工程包：读取规范和 memory；分析依赖；执行任务；完成测试；提交报告。
- 不要自行扩大任务范围。
